...
| **Nous IR — semantic code graph** | a change's identity is its *content*, not its name — definition-unit nodes, typed morphisms, a deterministic commute judge, an append-only ledger, a proof cache, and cryptographic authorship (see below) |
...
## Nous IR — code that composes itself

Underneath the agent is a small **semantic layer** for agent-native code collaboration, where a
change's *identity comes from its content, not its name*. It turns "did these two edits conflict?"
from a text-diff guess into a deterministic, content-addressed judgment — and records every accepted
change on a tamper-evident ledger.

- **Definition-unit nodes** — `context ingest-ts <file>` parses a TypeScript file into one
  content-addressed node per function/type. A **rename, a reformat, or a local-variable rename does
  not change a node's id** — identity is the normalized meaning, so two agents that write the same
  function converge to the same node.
- **Namespace** — `context ns-bind / ns-rename / ns-resolve / ns-log` keep name↔cid mappings as
  versioned first-class data. A rename is a *mapping event only*; the node is never touched.
- **Typed morphisms + a commute judge** — `context morph-diff` derives a typed change between two
  file versions; `context morph-judge` decides **deterministically** (a compiled judge, never an
  LLM) whether two changes commute or must escalate. "Conflict" means predicate/effect
  incompatibility over the *(nodes, names)* world — so two textually-disjoint edits that both bind
  the same name are caught, and a reformat-only edit composes with anything.
- **Ledger + promotion** — `context morph-compose` / `morph-pin` promote a judged-safe composition
  into an append-only, hash-linked ledger behind an owner ceremony; an *escalated* pair can never
  compose, structurally. `context ledger-log / ledger-state / ledger-checkpoint` inspect and bound
  the audit trail.
- **Proof cache + audit** — `context proof-verify` memoizes verification so identical bytes verify
  by *lookup* with zero recomputation; `context proof-audit` re-runs a seed-selected fraction and
  **slashes** a forged result; `context proof-reputation` shows signer standing.
- **Redaction** — `context redact` removes a secret / PII from the content store **without breaking
  the audit chain** (the chain hashes operations, not content, so the ledger stays byte-identical
  and verifiable); `context redaction-log` records *what class* was redacted, never the content.
- **Cryptographic identity** — `context identity` / `context attest` prove authorship with a
  hash-based (Lamport-on-sha256) signature. It is *not* a wallet and moves no funds.

Every one of these is a free READ except the owner-ceremony writes (`morph-compose`, `morph-pin`,
`ledger-checkpoint`, `proof-audit`, `redact`). Custody, funds, and chain-write stay hard-locked.
