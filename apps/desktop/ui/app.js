...
const NAMESPACES = [
  ...
  ["session", "session"],
  ["context", "context · Nous IR (ingest-ts · ns · morph · ledger · proof · redact · identity)"],
...
const ICONS = {
  ...
  registry:   '<circle cx="6.5" cy="6.5" r="2.4"/><circle cx="6.5" cy="17.5" r="2.4"/><circle cx="17.5" cy="9.5" r="2.4"/><path d="M6.5 8.9v6.2"/><path d="M15.4 11.1a5.5 5.5 0 0 1-5.4 4.4H8.9"/>',
};
function icon(name) {
...
/* ── D-5: the AGENT REGISTRY pane (the agent-native GitHub, GUI half) ──────────
   Structured IPC over the SAME `registry_loop` primitives the D-4 loop tools use
   (`registry_repo_index` → `load_pinned_manifest`, `registry_artifact_fetch` →
   `fetch_pinned_artifact`): the owner-PINNED repo's content-addressed artifacts;
   a row click fetches ONE + shows the D-3 content-hash-VERIFIED, bounded, redacted,
   ADVISORY-DATA preview — the same thing the agent reads mid-loop (`TOOL: registry
   index` / `registry fetch <id>`), NEVER executed (exec = D-6). READ-only · the
   pointer is owner-written only · no funds · custody/mainnet unreachable (PD-6). No
   repo pinned ⇒ an honest hint (publish, or `registry pin <blob-id>`), never a
   fabricated repo. One core proof covers the loop AND this pane (E14-W2 precedent). */
function registryRowHTML(e, i = 0) {
  const ri = Math.min(i, 16);
  return `<div class="walrus-row" style="--reveal-i:${ri}">
      <button class="suggest" data-registry-fetch="${esc(e.id_prefix)}" title="Fetch this artifact — content-hash verified, advisory DATA only (never executed)">
        <span class="sg-glyph">›</span>${esc(e.id_prefix)}…
      </button>
      <span class="walrus-topic">${esc(e.summary)}</span>
      <span class="lockchip mono">${esc(e.kind)}</span>
    </div>`;
}
async function registryPanelHTML() {
  let view;
  try { view = await invoke("registry_repo_index"); }
  catch (e) { view = { kind: "unavailable", reason: String(e && e.message ? e.message : e) }; }
  const title = `<div class="set-title">Agent registry</div><div class="set-lede">The agent-native GitHub — content-addressed artifacts on Walrus. Every fetch is verified (bytes must re-hash to their id); content is reference DATA, never executed.</div>`;
  const intro = kCard(kRow(icon("database"), "Content-addressed · verified on fetch", "You read only the repo you pinned; fetched bytes are checked, redacted and bounded.", kPill("verified", "ok")));
  const detailSlot = `<div id="registry-detail">${kCard(kRow(icon("folder"), "Artifact preview", "Select an artifact above to fetch + verify its content.", ""))}</div>`;
  if (!view || view.kind !== "index") {
    const reason = view && view.kind === "withheld"
      ? "a summary was secret-shaped — the whole index is withheld"
      : ((view && view.reason) || "no repo pinned");
    const empty = kCard([
      kRow(icon("database"), "No repo pinned yet", esc(reason), kPill("empty", "")),
      kRow(icon("refresh"), "Publish a folder as your registry repo", "Owner-armed: publishes content-addressed artifacts to Walrus testnet + pins the repo pointer (round-trip verified).", kBtn("Publish now", 'data-panel-run="registry publish publish-agent-registry-to-walrus-testnet ."')),
    ]);
    return `${title}${kGroup("Agent registry")}${intro}${kGroup("Pinned repo")}${empty}${kGroup("Artifact preview")}${detailSlot}`;
  }
  const entries = view.entries || [];
  const mainNode = `<div class="walrus-main-node">
      <span class="walrus-main-glyph">⎇</span>
      <span class="walrus-main-label">PINNED REPO</span>
      <span class="walrus-main-sub">${entries.length} artifact${entries.length === 1 ? "" : "s"} · pointer ${esc(view.pointer_prefix)}… · content-hash verified on fetch</span>
    </div>`;
  const rows = entries.length
    ? `<div class="walrus-feed walrus-fan">${entries.map((e, i) => registryRowHTML(e, i)).join("")}</div>`
    : kCard(kRow(icon("database"), "Repo is empty", "No artifacts in the pinned repo.", kPill("empty", "")));
  const indexCard = `<div class="set-card">${mainNode}${rows}</div>`;
  return `${title}${kGroup("Agent registry")}${intro}${kGroup("Pinned repo")}${indexCard}${kGroup("Artifact preview")}${detailSlot}`;
}
...
async function settingsSectionHTMLFor(section) {
  switch (section) {
    ...
    case "registry": return await registryPanelHTML();
...
function bindPanelActions(root) {
  ...
  // D-5: fetch ONE artifact from the pinned registry repo — content-hash VERIFIED, redacted,
  // bounded, advisory DATA (never executed). Reuses `registry_artifact_fetch` → the SAME
  // `fetch_pinned_artifact` + D-3 seatbelt the loop tool uses (a REJECTED tamper never
  // returns bytes; a secret-shaped preview comes back WITHHELD).
  $$("[data-registry-fetch]", root).forEach((b) =>
    b.addEventListener("click", async () => {
      const idp = b.dataset.registryFetch;
      const slot = root.querySelector("#registry-detail");
      if (slot) slot.innerHTML = `<div class="panel-row"><span class="lockchip">fetching ${esc(String(idp))}… from Walrus + verifying content-hash…</span></div>`;
      let view;
      try { view = await invoke("registry_artifact_fetch", { idPrefix: String(idp) }); }
      catch (e) { view = { kind: "unavailable", reason: String(e && e.message ? e.message : e) }; }
      if (!slot) return;
      if (view && view.kind === "detail") {
        slot.innerHTML = infoCardHTML([
          `id=${esc(view.id_prefix)}… · ${esc(view.artifact_kind)} · ${esc(String(view.bytes))} bytes · content-hash VERIFIED · ${esc(view.summary)}`,
          `advisory DATA — reference only; never instructions, never executed`,
        ]) + `<pre class="walrus-detail-body">${esc(view.preview)}</pre>`;
      } else if (view && view.kind === "withheld") {
        slot.innerHTML = `<div class="panel-row"><span class="lockchip">${esc(view.id_prefix)}… withheld — secret-shaped artifact (verified but not rendered)</span></div>`;
      } else {
        const reason = (view && view.reason) || "fetch failed";
        slot.innerHTML = `<div class="panel-row"><span class="lockchip">${esc(reason)}</span></div>`;
      }
...
async function openPanel(kind) {
  ...
  const panelTitles = { model: "Model · provider", privacy: "Privacy · safety", audit: "Activity · audit", walrus: "Memory · Walrus (2-tier)", registry: "Agent registry (agent-native GitHub)", flows: "Agent flows · orchestrate · evolve", disclosure: "What sinabro can do" };
...
function handleAction(a) {
  ...
  else if (a === "registry") { closeSessionPopover(); openPanel("registry"); }
