...
// ── D-5: the AGENT REGISTRY pane (the agent-native GitHub, GUI half) ──────────────────────
// Structured IPC (the R4.5 lesson — never emit-text parsing) over the SAME audited
// `registry_loop` primitives the D-4 loop tools use (`load_pinned_manifest` /
// `fetch_pinned_artifact`): pointer-pinned trust anchor (owner-written only), the D-3
// content-hash seatbelt on every fetch, the canonical redact() belt on every render,
// bounded preview, and NEVER execution (exec = D-6). One core proof covers the loop AND
// this pane (the E14-W2 walrus precedent). READ-only; no funds; custody/wallet/mainnet
// unreachable (PD-6). No pointer yet ⇒ an honest `unavailable` reason, never a fabricated
// repo. Threat model: ops/evidence/stage_g/agent_loop/AGENT_REGISTRY_LOOP_TOOL_THREAT_MODEL.md.

/// One pinned-repo artifact row for the GUI (id 16-char prefix + kind + summary — the
/// SAME projection the loop's `registry index` renders).
#[derive(serde::Serialize)]
...
/// The pinned repo's index for the GUI panel (honest `unavailable` when nothing is
/// pinned / the GET missed / the manifest failed the fail-closed decode; `withheld`
/// when a rendered summary is secret-shaped — the WHOLE index withholds, mirroring the
/// D-4 loop executor).
#[derive(serde::Serialize)]
#[serde(tag = "kind")]
enum RegistryIndexView {
    #[serde(rename = "index")]
    Index {
...
/// One seatbelt-VERIFIED artifact fetch for the GUI: bounded, redacted, advisory DATA —
/// never executed. `withheld` = verified but secret-shaped (never rendered);
/// `unavailable` = honest miss/reject reason (a REJECTED tamper never returns bytes).
#[derive(serde::Serialize)]
#[serde(tag = "kind")]
enum RegistryFetchView {
    #[serde(rename = "detail")]
    Detail {
        id_prefix: String,
    ...
    #[serde(rename = "withheld")]
    Withheld { id_prefix: String },
    #[serde(rename = "unavailable")]
    Unavailable { reason: String },
}

/// Char-safe byte cap for the GUI preview (the loop's `REGISTRY_PREVIEW_CAP_BYTES`
/// bound, applied at a char boundary — a multibyte char is never split).
fn registry_cap_chars(s: &str, cap: usize) -> String {
    if s.len() <= cap {
        return s.to_string();
    }
    let mut end = cap;
...
/// True iff the canonical redact() belt flags any fragment (fail-closed: a redaction
/// error ⇒ withhold) — the SAME belt the walrus pane + the D-4 loop executors run.
fn registry_belt_flags(fragments: &[&str]) -> bool {
    match sinabro::provider::redaction::redact(&sinabro::provider::redaction::RedactionRequest {
        fragments,
        candidate_memory_ids: &[],
        deleted_ids: &[],
        include_private_memory: false,
...
/// Read the OWNER-PINNED registry repo's index for the GUI panel (D-5). Reuses the SAME
/// `registry_loop::load_pinned_manifest` the D-4 loop tool uses (pointer → testnet GET →
/// try-AEAD-open-else-plaintext → fail-closed AGRX decode). The local store is OPTIONAL —
/// a PUBLIC repo needs no key (the D-3 try-open-else-raw idiom). Runs on a blocking-pool
/// thread (the GET must never block the UI thread).
#[tauri::command]
async fn registry_repo_index() -> Result<RegistryIndexView, String> {
    tauri::async_runtime::spawn_blocking(registry_repo_index_sync)
        .await
        .map_err(|e| format!("registry index task failed: {e}"))?
}

/// The blocking body of [`registry_repo_index`].
fn registry_repo_index_sync() -> Result<RegistryIndexView, String> {
    let store = sinabro::memory_store::PersistedStore::open_local().ok();
    match sinabro::registry_loop::load_pinned_manifest(store.as_ref()) {
        Ok((pointer, manifest)) => {
            let joined: String = manifest
                ...
                .collect::<Vec<_>>()
                .join("\n");
            if registry_belt_flags(&[joined.as_str()]) {
                return Ok(RegistryIndexView::Withheld);
            }
            let entries = manifest
                .entries
                .iter()
                .take(64)
                .map(|a| RegistryEntryView {
                    id_prefix: a.id.chars().take(16).collect(),
                    kind: a.kind.label().to_string(),
                    summary: a.summary.clone(),
                })
                .collect();
            Ok(RegistryIndexView::Index {
                pointer_prefix: pointer.chars().take(8).collect(),
                entries,
            })
        }
        Err(reason) => Ok(RegistryIndexView::Unavailable {
            reason: reason.to_string(),
        }),
    }
}

/// Fetch ONE artifact from the pinned repo for the GUI (D-5). Reuses the SAME
/// `registry_loop::fetch_pinned_artifact` the D-4 loop tool uses — resolve inside the
/// pinned manifest (full id / unique >=8-char prefix), GET, try-open-else-raw, and the
/// D-3 `registry_content_verified` seatbelt (failing bytes are REJECTED, never returned).
/// The preview is byte-BOUNDED + redact-belted and is DATA — never executed (D-6). Runs
/// on a blocking-pool thread.
#[tauri::command]
async fn registry_artifact_fetch(id_prefix: String) -> Result<RegistryFetchView, String> {
    tauri::async_runtime::spawn_blocking(move || registry_artifact_fetch_sync(&id_prefix))
        .await
        .map_err(|e| format!("registry fetch task failed: {e}"))?
}

/// The blocking body of [`registry_artifact_fetch`].
fn registry_artifact_fetch_sync(id_prefix: &str) -> Result<RegistryFetchView, String> {
    let store = sinabro::memory_store::PersistedStore::open_local().ok();
    match sinabro::registry_loop::fetch_pinned_artifact(store.as_ref(), id_prefix) {
        Ok(verified) => {
            let bytes = u32::try_from(verified.content.len()).unwrap_or(u32::MAX);
            let preview = registry_cap_chars(
                &String::from_utf8_lossy(&verified.content),
                sinabro::registry_loop::REGISTRY_PREVIEW_CAP_BYTES,
            );
            let short: String = verified.id.chars().take(16).collect();
            if registry_belt_flags(&[preview.as_str(), verified.summary.as_str()]) {
                return Ok(RegistryFetchView::Withheld { id_prefix: short });
            }
            Ok(RegistryFetchView::Detail {
                id_prefix: short,
                artifact_kind: verified.kind.to_string(),
                bytes,
                summary: verified.summary,
                preview,
            })
        }
        Err(reason) => Ok(RegistryFetchView::Unavailable { reason }),
    }
}
...
mod tests {
    ...
    // D-5 — the ONE GUI-specific pure helper beyond the shared `registry_loop` core
    // (`registry_repo_index_sync`/`registry_artifact_fetch_sync` call the SAME
    // `load_pinned_manifest`/`fetch_pinned_artifact` the D-4 loop proved LIVE). The
    // preview truncation must never split a multibyte char and must bound the length.
    #[test]
    fn registry_cap_chars_is_char_safe_and_bounded() {
        // ASCII under the cap ⇒ unchanged, no ellipsis.
        assert_eq!(registry_cap_chars("hello", 800), "hello");
        // A run of 4-byte emoji: capping at a byte that lands mid-char must back off to a
        // char boundary (never panic, never emit a broken code unit).
        let emoji = "😀".repeat(10); // 40 bytes
        let capped = registry_cap_chars(&emoji, 10); // 10 is mid-emoji (each is 4 bytes)
        assert!(capped.ends_with('…'));
        // the kept prefix (minus the ellipsis) is valid UTF-8 whole emoji ≤ 10 bytes
        let kept = capped.trim_end_matches('…');
        assert!(kept.len() <= 10);
        assert!(kept.chars().all(|c| c == '😀'));
        // exact-boundary cap keeps everything (len == cap ⇒ no truncation)
        assert_eq!(registry_cap_chars("abcd", 4), "abcd");
    }

    // D-5 — the redact belt is the SAME canonical gate the walrus pane + the D-4 loop
    // executors run: a clean fragment passes, a secret-shaped one flags (⇒ WITHHELD).
    #[test]
    fn registry_belt_flags_on_secret_shaped_only() {
        assert!(!registry_belt_flags(&["a benign strategy note about hedging"]));
        assert!(
            registry_belt_flags(&["-----BEGIN OPENSSH PRIVATE KEY-----"]),
            "a secret-shaped fragment must flag ⇒ the pane withholds"
        );
    }
}
