...
//! `check_capability` / `CapabilityError`. Phase 0 is the *ownership
//! gate* surface only: a fixed-width `#[repr(u8)] enum` discriminant
//! identifies which capability is being asserted, a `Capability` token
//! binds that discriminant to a Sui owner address, and
//! [`check_capability`] is a by-construction equality check that both
//! the required kind matches and the actor equals the bound owner.
//! There is no encryption, no key server and no threshold cryptography
//! on this path — those are custody-adjacent and land in Phase 1. The
//! guiding principle: Phase 0 performs a capability *check* only, and a
//! capability is a fixed-width enum tag.
//!
//! [`SuiAddress`](mnemos_d_move::SuiAddress) is reused byte-for-byte: a
//! `Capability.owner` and a `MemoryRoot.owner` are the same 32-byte Sui
//! account id at the type level. No parallel address shape is
//! introduced here.
//!
//! Test coverage: [`tests::f0_1_capability_granted_for_owner`],
//! [`tests::f0_1_denied_for_wrong_kind`],
//! [`tests::f0_1_denied_for_wrong_actor`].

use mnemos_d_move::SuiAddress;

/// Capability discriminant. Phase 0 carries exactly three kinds:
/// `ReadMemory = 1`, `WriteMemory = 2`, `AnchorChunk = 3`. The
/// `#[repr(u8)]` pin makes the in-memory width byte-exact (1 byte) and
/// stable across edits, so future BCS / wire-form serialisation
/// (Phase 1) can encode a `CapabilityKind` as a single byte without
/// ambiguity. Explicit discriminant values are written out so
/// reordering variants is a compile-time-visible byte-VALUE change,
/// not an implicit one (cross-language schema lock discipline).
///
...
pub enum CapabilityKind {
    ...
    /// `add_chunk` entry — paired with `assert_add_chunk_owner`).
    AnchorChunk = 3,
}
...
/// exactly one variant — `Denied { kind }`. The carried `kind` is the
/// *required* capability kind, not the kind stored on the token: a
/// future audit log entry then reads as "actor X was denied a
/// `<required>` capability" regardless of whether the denial was for
/// kind mismatch or for actor mismatch.
///
...
mod tests {
    ...
    /// Happy path: a `WriteMemory` token bound to ALPHA, asserted with
    /// required = `WriteMemory` and actor = ALPHA, returns `Ok(())`.
    #[test]
    ...
    /// Kind-mismatch denial: a `ReadMemory` token bound to ALPHA,
    /// asserted with required = `WriteMemory` and actor = ALPHA, returns
    ...
    /// Actor-mismatch denial: a `WriteMemory` token bound to ALPHA,
    /// asserted with required = `WriteMemory` and actor = BETA, returns
    ...
    /// canonical byte-width contract. This structural assertion catches
    /// any future variant reordering / repr drift at test time without
    /// depending on the `static_assertions` dev-dep.
    #[test]
    ...
    /// integer discriminants `ReadMemory = 1`, `WriteMemory = 2`,
    /// `AnchorChunk = 3`. A `#[repr(u8)]` enum can be cast to `u8` with
    /// `as`, so this is
