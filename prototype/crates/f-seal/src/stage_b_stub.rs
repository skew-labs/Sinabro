//! Seal stub capability types.
//!
//! Canonical surface — [`StageBSealStubPolicy`],
//! [`StageBSealStubEnvelope`], [`StageBSealStubError`]. This is the
//! Stage B *Seal boundary marker* surface and nothing more: there is
//! **no encryption, no key server, and no private-memory publish path**
//! here. The stub marks the boundary only — no encryption claim, no key
//! server, no private-memory publish; it is fixture-only with no live
//! egress, and secret-like inputs are denied fixtures with no debug, no
//! clone, and redaction in traces. The real Seal AEAD / threshold path
//! is custody-adjacent and deferred to Phase 1.
//!
//! Canonical inputs:
//! - [`PublishPayloadClass`](mnemos_c_walrus::PublishPayloadClass) — the
//!   home-of-record content classifier (`c-walrus` `publisher.rs`),
//!   reused verbatim. No second classifier is minted; the edge `f-seal ->
//!   c-walrus` is acyclic (c-walrus is a leaf crate).
//! - [`Capability`](crate::capability::Capability) — the ownership token
//!   binding a [`CapabilityKind`](crate::capability::CapabilityKind) to a
...
/// Seal stub policy. The single boolean gate decides whether real
...
/// field is `pub` to match the canonical declaration, but the default-deny
/// posture is the construction path callers are expected to use.
///
...
impl StageBSealStubPolicy {
    ...
    ///   is the one freely admissible class (only synthetic public fixtures
    ///   publish).
...
/// Seal stub error. Exactly three variants. `Copy` + no owned bytes so the
/// error channel cannot leak a body or a secret-like substring through
/// `Debug` (mirrors [`CapabilityError`](crate::capability::CapabilityError)).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[non_exhaustive]
pub enum StageBSealStubError {
    ...
    /// the wording guard; minted here so the Seal-stub error set is declared
    /// once and frozen `#[non_exhaustive]`.
...
/// Seal stub envelope — a *boundary marker* binding a publish content
...
///
/// The fields are `pub` to match the canonical declaration and to let the
...
impl StageBSealStubEnvelope {
    ...
    /// minted by [`stub`](Self::stub); exposed so callers can assert the stub
    /// invariant without reaching into the field.
    #[inline]
