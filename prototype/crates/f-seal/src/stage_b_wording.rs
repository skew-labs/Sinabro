//! Seal wording guard.
//!
//! Canonical surface — a guard against misleading encryption claims. Any
//! UX / log / doc phrase claiming real Seal encryption in Stage B is a
//! failure. Stage B has **no real encryption** (the Seal path is a
//! stub); any user-facing string, log line, or doc that says otherwise
//! is a safety failure because it would make a user trust a
//! confidentiality property that does not yet exist.
//!
...
//! companion runbook must contain it.
...
/// must never assert that bytes are encrypted. The companion runbook is
/// required to contain this phrase.
