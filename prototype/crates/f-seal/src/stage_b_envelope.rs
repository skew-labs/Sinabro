//! Content class to Seal stub envelope mapping.
//!
//! Canonical surface — the mapping from a publish content class to a
//! [`StageBSealStubEnvelope`]. The guiding rule: private / secret-like
//! classes stop before the Walrus planner; secret-like residue is a
//! denied fixture only, with no debug, no clone, and redaction. This
//! module is the *only* place that turns a content class into a stub
//! envelope; it reuses the [`StageBSealStubPolicy::admits`] decision and
//! layers the **owner signature** requirement on top for user-owned
//! public memory.
//!
//! The four test obligations are: private denied, secret denied,
//! synthetic envelope ok, and user-owned public requires owner
...
///   admitted — the one freely publishable class. Returns a
...
pub fn stage_b_seal_envelope(
    ...
    match class {
        ...
        // `#[non_exhaustive]` additions: default-deny via the stub policy.
        _ => policy
            .admits(class)
