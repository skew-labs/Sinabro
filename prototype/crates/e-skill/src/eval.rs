//! `mnemos-e-skill::eval` — the eval / tests digest
//! contract.
//!
//! ## Overview
//!
//! - [`SkillEvalScore`] — six per-axis scores (`rust`, `move`,
//!   `prover`, `gas`, `security`, `korean`) plus a 32-byte
//!   `reproducible_command_hash_32`. An eval score is meaningless unless it
//!   links to the reproducible commands that produced it; so a
...
//!   command strings. The hash changes whenever any command changes,
//!   so a forged "100%" score with a mismatched command set is
//!   catchable.
...
/// spoofed score and is rejected.
pub const MAX_EVAL_SCORE: u16 = 10_000;

// ===========================================================================
// 1. SkillEvalScore — six axes + reproducible command hash
// ===========================================================================

/// Per-axis eval score for a skill. Every axis is in basis points of
