//! `mnemos-e-skill::install_plan` — the user-approved
//! install plan.
//!
...
//! package verification, compatibility, capability approval, dry-run evidence,
//! and explicit user confirmation. A plan with any precondition unmet — or a
//! drifted digest — is [`InstallDecision::Blocked`].
...
pub struct InstallPreconditions {
    /// `verify_skill_package` returned a verified package.
    pub package_verified: bool,
    /// The compatibility check passed for the host environment.
    pub compatibility_pass: bool,
    /// The capability diff was shown and approved.
    pub capability_approved: bool,
    /// A try-before-use dry-run passed.
