//! `mnemos-e-skill::package` — `SkillPackageV1`
//! wrapper over the `SkillManifest`, plus the supporting content-digest
//! spine shared across the signed-package surface.
//!
//! ## Public API surface
//!
//! - [`SkillPackageV1`] — the signed-package aggregate. It **wraps** the
//!   [`SkillManifest`] verbatim; it does NOT replace
//!   it and never re-parses the base manifest fields. [`load_manifest`]
//!   stays the only parser for the base manifest (a reuse invariant).
...
//! - [`SkillSecurityState`] — a 5-variant `#[repr(u8)]` trust-state
//!   label `{Unknown=1, SandboxPass=2, AuditPass=3, Quarantined=4,
//!   Revoked=5}`. A bare signature never grants `AuditPass`/`SandboxPass`;
//!   those require the WASM-sandbox / audit surfaces. This constructor
//!   only ever derives `Unknown` for a freshly-verified package.
//! - [`SkillSupplyChainReceipt`] — supply-chain evidence: SBOM hash,
...
// ===========================================================================
// 4. SkillSecurityState — 5-variant trust-state label
// ===========================================================================

/// Security trust-state of a skill package. `#[repr(u8)]` 1-byte
/// discriminant. The 5-variant set is fixed (no `#[non_exhaustive]`
...
// ===========================================================================
// 5. SkillSupplyChainReceipt — supply-chain evidence
// ===========================================================================

/// Supply-chain evidence bound into a signed package. Every hash is
...
// ===========================================================================
// 6. SkillPackageV1 — the signed-package aggregate
// ===========================================================================

/// Signed skill package. Wraps the A [`SkillManifest`] and binds the
...
/// exactly 8 fields, no commerce field, no replacement
...
/// struct stays byte-exact while honouring the full signature coverage
/// list).
...
impl SkillPackageV1 {
    ...
    /// `SkillPackageV1` stays exactly the 8-field shape while the
    /// signature still covers compatibility + the no-commerce policy
    /// (the full signature coverage list).
    #[must_use]
...
mod tests {
    ...
    fn a_manifest_fixture_loads_unchanged_through_wrapper() {
        // Wrapper test: the wrapped A manifest is byte-identical to a
