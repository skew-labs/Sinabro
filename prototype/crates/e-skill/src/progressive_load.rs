//! `mnemos-e-skill::progressive_load` — atom #314 · D.4.3 — the progressive
//! disclosure loading policy.
//!
//! ## Canonical OUT (§314)
//!
//! Search loads only lightweight cards; the full manifest / README / fixtures /
//! WASM metadata / eval logs load only for a selected candidate (§314 광기).
//! [`progressive_search`] returns at most `top_n` [`SkillCardSummary`] (#298)
...
//! usable in a terminal (§314 criterion — top-20 search p95 ≤ 100ms at 10k:
//! search touches only the lightweight cards, no per-result manifest load).
//!
//! ## Reuse (#298 / #306)
//!
//! Cards are the #298 [`SkillCardSummary`] / [`SkillCardDetail`]; the source is
//! a #306 [`SignedCatalogCache`]. A signature-drifted cache serves nothing
...
//! Ranking is NOT done here — that is #303; this module only governs the
...
/// but never a signature-drifted one. `malicious_fixture_pass` is the #274 red-
/// team verdict for the skill.
