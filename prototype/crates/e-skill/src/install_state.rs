//! `mnemos-e-skill::install_state` — on-chain install-state model +
//! runtime-usable decision.
//!
//! [`InstallState`] is the lifecycle enum, byte-pinned to the Move
...
//! action. Complements [`crate::install_plan`]: the install plan decides
//! whether to install; this state decides whether an existing install may run.
//!
//! ## Local install state machine
//!
//! This module is also the home for the **local** install state machine over
//! the reused [`crate::rollback::LocalSkillState`] (NOT the on-chain
...
//! install, `update` (compat-gated), and `remove`/`revoke` (reusing
...
/// On-chain install lifecycle state, byte-pinned to the Move
...
/// A forward operation on the local skill lifecycle. Distinct from the
...
pub enum LocalSkillTransition {
    ...
    /// same state); requires a compatible decision.
    Update,
...
pub enum TransitionError {
    ...
    InvalidTransition,
    /// An `Update` was attempted without a compatible decision.
    Incompatible,
}
...
/// `Update` requires a compatible decision
