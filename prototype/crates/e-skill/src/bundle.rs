//! `mnemos-e-skill::bundle` — atom #249 · D.0.8 — the package bundle layout
...
//! relative path + a content hash + a size) and enforces the §249 광기:
//! extraction **cannot** path-traverse, overwrite an existing file, or hide
//! a duplicate artifact name.
//!
...
//!   wrapping package (§249 criterion: bundle digest = artifact digest
//!   tree), and is stable regardless of input entry order.
//!
//! The layout never parses bytes from disk and never decompresses — that
//! belongs to a later WP behind a sandbox. Here it is a pure, panic-free
...
impl BundleLayout {
    /// Validate the bundle's extraction-safety invariants (§249 광기).
