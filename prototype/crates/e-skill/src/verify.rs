...
//! every precheck passes: schema, no-commerce, manifest,
...
//!
//! The base manifest is parsed **only** by [`load_manifest`]: the verifier
//! re-serializes the `[manifest]` section and
//! hands it to `load_manifest`, never re-implementing manifest parsing.
//!
//! Verification is offline and allocation-bounded for small metadata — a
//! benchmark enforces the p95 budget.
...
/// Maximum package-metadata byte length the verifier accepts. A package
/// whose canonical TOML exceeds this is rejected
...
// ===========================================================================
// 1. VerifyError — the error taxonomy
// ===========================================================================
...
pub enum VerifyError {
    ...
    Encoding,
    /// The package carries a commerce-shaped field (no-commerce policy).
    Commerce,
...
pub struct VerifiedPackage {
    ...
    /// (sandbox/audit promotion happens later).
...
// ===========================================================================
// 3. verify_skill_package — the entry point
// ===========================================================================
...
pub fn verify_skill_package(package_bytes: &str) -> Result<VerifiedPackage, VerifyError> {
    // (0) size budget: reject oversized metadata before any work.
    ...
    //     the [manifest] section and feed it through, so this module never
    //     re-implements base-manifest parsing.
...
/// tests, the malicious-fixture tests, the property corpus,
/// and the verify benchmark so all four exercise one source-of-truth
