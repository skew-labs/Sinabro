//! `mnemos-e-skill::chain_bindings` — Rust chain bindings (BCS arg builders)
//! for the `mnemos_skill_registry` Move package.
//!
...
/// `SkillChainAction` — the on-chain action a registry call performs.
...
/// `InstallReceiptId` — the Sui object id of an install receipt.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(transparent)]
...
/// `SkillRegistryArgs` — args for `publish_skill` / `fork_skill`.
///
...
/// `InstallReceiptArgs` — args for `record_install`.
///
...
/// `InstallReceiptView` — a decoded on-chain install receipt.
///
...
/// Encode a [`ProvenanceNode`] to the canonical BCS byte layout (parity
