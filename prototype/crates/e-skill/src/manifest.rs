//! `mnemos-e-skill::manifest` — skill manifest
//! TOML parse + validate.
//!
//! Canonical shape:
//!
//! - [`SkillId`] — `#[repr(transparent)]` newtype over `u16`. Public
//!   inner field, mirroring the `ToolId` rationale: no invariant beyond
//!   "name newtype over u16", so the inner exposure is correct (same
//!   precedent as `TurnUsage`). 16-bit width pinned by
//!   [`_SKILL_ID_SIZE_IS_2`] below.
//! - [`SkillManifest`] — fixed-shape carrier with private fields +
//!   `pub const fn` accessors (the same invariant-protection precedent
//!   used by `TurnState` and `LazyToolSchema`). The name string
//!   itself is NEVER retained — only its byte length collapses into
//!   `name_len_u8: u8` at the parser boundary (the same `_lenN`
//!   collapse precedent used by `RuntimeConfig`). `tool_ids` is the
...
//!   (the same source-redaction spine `MnemosError` applies, here
//!   scoped to a local error channel) so a canary embedded in a manifest
//!   body cannot reach `Debug`, `Display`, or `source()` via this
//!   surface. Class labels are namespaced under `manifest.*` (the same
//!   pattern as `tool_schema.*`).
//! - [`load_manifest`] — the only public function. Signature:
//!   `(toml_text: &str) -> Result<SkillManifest,
...
//!   canonical allow-set for the Phase 0 manifest, matching the
//!   forward-commitment `Builtin{ReadFile=1, WriteFile=2,
//!   RunCommand=3}`. A future revision will promote this set to a
//!   `Builtin` `#[repr(u8)]` enum and SHOULD re-export this const so
//!   the discriminants stay one source-of-truth. Until that lands,
//!   the const is the authoritative whitelist.
//!
//! ## Why name is collapsed to a length-only u8
//!
//! `name_len_u8: u8` (not `name: String`) is intentional. The
//! token-saving design forbids the runtime from carrying redacted-able
...
//!
//! ## Why a local error channel (not `MnemosError`)
//!
//! [`ManifestError`] is a 4-variant local enum.
//! `MnemosError` carries source-redacted plumbing that is
//! correct for *runtime* failures (boot / agent / tool); the manifest
//! is a *configuration boundary* (the same precedent as
//! `RuntimeConfig`) that fails fast at load time with a fixed-shape
//! verdict. Folding into `MnemosError` here would create an asymmetric
//! surface — callers would handle 4 manifest failure modes via a
//! 6-class `ErrorCode` envelope, paying the dispatch cost without
//! gaining redaction (the manifest body is operator-controlled, not a
//! network response). Dispatch errors (from a future `Builtin` enum)
//! fold into `MnemosError` as `tool_denied`; manifest validation errors
//! stay local. The boundary is the load-time / run-time split.
//!
//! ## Design rationale (accepted trade-offs)
//!
//! 1. **`SkillManifest` fields private.** The canonical signature
//!    shows public fields (the prose-form declaration is
//!    `{ id: SkillId, name_len_u8: u8, ... }` without a `pub` modifier
//!    on each field). This module makes them private +
//!    `pub const fn` accessors, following the same `TurnState`
//!    precedent — the "name string dropped, only length retained"
//!    invariant requires construction-time validation, which a public
//!    field would let callers bypass. The const accessors preserve
//!    zero-cost read for downstream consumers.
//! 2. **`SkillId` keeps `pub u16` inner field.** Mirrors the
...
//!    registry.** The signature
//!    `load_manifest(toml_text: &str) → Result<...>` admits NO
//!    registry parameter — UnknownTool validation must use a
//!    compile-time set. The const matches the
//!    `Builtin{1, 2, 3}` forward-commitment; a future revision takes
//!    this over (either by re-using the const or by adding a
//!    compile-time assertion that the `Builtin` discriminants match).
//!    This module deliberately does NOT depend on that future
//!    revision — the const is self-contained and references the
//!    `Builtin` commitment only via this doc comment.
//! 4. **`ManifestError` is payload-less.** The canonical shape lists
//!    the 4 variants without payloads. `MnemosError`'s pattern
...
//! 8. **Token-cost is `u32`, declared not measured.** The field is
//!    `token_cost_estimate_u32: u32` — the manifest
//!    DECLARES an estimate; the actual measurement lives in
//!    `m-agent::token_bench` (per-call envelope) and
//!    `DailyTokenBudget`. The estimate has no enforced ceiling here —
//!    the Phase 0 trust boundary is operator-controlled.
...
///
/// A future revision promotes these discriminants to a `Builtin`
/// `#[repr(u8)]` enum; this const is the one source-of-truth until
/// then. Future changes that grow the whitelist MUST update this
/// slice AND the `Builtin` commitment simultaneously.
...
/// `u16`-shaped index (the same precedent as `ToolId`). 16-bit width
/// per the canonical commitment — up to 65 536 skills per agent
/// instance.
///
/// Inner `pub u16` per the `ToolId(pub u16)` rationale: "no
/// invariant beyond inner value, so public inner is correct" (same
/// precedent as `TurnUsage`).
...
/// (`name_len_u8`). This matches the `RuntimeConfig` `_lenN`
/// collapse precedent and the token-saving design.
///
...
///
/// Class labels namespaced under `manifest.*` (the same pattern as
/// `tool_schema.*`).
...
/// the canonical field names verbatim. `#[serde(deny_unknown_fields)]`
/// rejects unknown top-level keys at parse time (the same precedent
/// as `RuntimeConfig`).
///
...
// ===========================================================================
// 7. load_manifest — the only public function
// ===========================================================================
...
///
/// The signature deliberately admits no registry parameter,
...
// ===========================================================================
// 8. Inline unit tests line 1220 verbatim names
// ===========================================================================
...
mod tests {
    ...
    fn known_tool_ids_match_section_4e_line_711_commitment() {
        // Canonical commitment: `Builtin { ReadFile=1, WriteFile=2, RunCommand=3 }`.
        // This const and the canonical commitment must move together.
    ...
    // ---- line 1220 verbatim tests ------------------------------

    /// `e0_1_parses_valid_manifest` — verifies that a well-formed
    /// manifest with all four canonical fields parses successfully and
