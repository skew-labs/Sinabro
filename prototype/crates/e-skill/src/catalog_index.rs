//! `mnemos-e-skill::catalog_index` — atom #296 · D.3.0 — the catalog index entry.
//!
//! A [`SkillCatalogIndexEntry`] is a **denormalized, search-facing view** of an
//! already-signed, already-verified package (§4.4). It folds together the
//! signed package digest (#242/#252), the eval / security state (#245), the
//! evaluated compatibility decision (#251), the capability diff (#244), and the
//! content-addressed provenance (#246), plus three weak/strong popularity
//! counters.
//!
//! The 광기 invariant: a catalog entry is **never** the source of executable
...
//! capability approval, dry-run, and the #271 install plan — the index only
...
/// A denormalized, search-facing view of one verified skill package (§4.4).
///
...
pub struct SkillCatalogIndexEntry {
    ...
    /// (not optional) so a card can never hide the permission delta (#301).
...
impl SkillCatalogIndexEntry {
    ...
    /// values an event-stream fold (#287 → [`crate::catalog_counters`])
    ...
    /// package + capability approval + dry-run + #271 install plan.
    #[must_use]
