//! `mnemos-e-skill::community_import` — the external /
//! community skill import flow.
//!
//! ## Overview
//!
//! - [`CommunitySkillDecision`] — `{ Pending=1, Accepted=2,
//!   Quarantined=3, Rejected=4 }`.
//! - [`CommunitySkillImport`] — `{ source_hash_32, package, decision,
//!   review_hash_32, no_commerce_scan_hash_32 }`.
//!
//! ## Dry-run-first flow
//!
//! An external import is **dry-run first**: the package is parsed,
//! its signature/provenance checked, the malicious-fixture suite
//! run, the capability diff checked for hidden permissions
//! ([`CapabilityDiff::is_consistent`]), the eval command presence checked,
//! and the no-commerce scan run
...
//! execution** — enabling stays the local install state machine gated by
//! a signed install receipt.
//!
//! ## Determinism + offline boundary
//!
//! [`decide_import`] is a pure function of the six gate booleans (the import
//! report is deterministic). No network, wallet, secret,
...
// ===========================================================================
// 1. CommunitySkillDecision
// ===========================================================================

/// The decision of a community import dry-run.
...
/// output of an existing verifier gate; this module never
/// re-runs those gates, it folds their verdicts.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub struct CommunityImportEvidence {
    /// The package carried an author signature.
    pub signature_present: bool,
    /// The provenance chain validated.
    pub provenance_ok: bool,
    /// The capability diff is consistent — no hidden permission.
    pub capability_consistent: bool,
    /// The malicious-fixture suite passed.
    pub malicious_fixture_clean: bool,
    /// An eval command/score is present.
    pub eval_present: bool,
    /// The no-commerce scan passed.
    pub no_commerce_clean: bool,
}

impl CommunityImportEvidence {
    /// Build evidence, deriving `capability_consistent` from the capability
    /// diff and `no_commerce_clean` from the no-commerce scan of the imported
    /// manifest TOML, so the canonical gates are actually consumed (not
    /// re-implemented).
    #[must_use]
    pub fn from_gates(
...
// ===========================================================================
// 4. CommunitySkillImport record
// ===========================================================================

/// The record of an external import dry-run.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub struct CommunitySkillImport {
    ...
    /// Hash binding the no-commerce scan outcome to the policy version.
...
impl CommunitySkillImport {
    ...
    /// no-commerce scan outcome to the policy version. Deterministic for a
...
/// Bind the no-commerce scan outcome (clean/dirty) to the policy-version
...
mod tests {
    ...
    fn no_commerce_violation_rejected_via_scan() {
        // A manifest with a checkout key fails the no-commerce scan inside `from_gates`.
