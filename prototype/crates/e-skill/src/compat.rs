//! `mnemos-e-skill::compat` — the compatibility
//! constraint model.
//!
//! ## Overview
//!
//! - [`CompatibilityDecision`] — `{Compatible=1, Warn=2,
...
//!   a Mnemos version range, a chain-env hash (a `StageCChainEnv` via hash,
//!   reused), an OS/GPU hash, a toolchain hash, and a model/provider hash.
...
// ===========================================================================
// 1. CompatibilityDecision — 4-variant decision
// ===========================================================================

/// Compatibility decision for a skill against a host. The 4-variant
/// set is fixed (no `#[non_exhaustive]`).
...
/// The compatibility constraint a skill places on its host.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
...
impl SkillCompatibility {
    /// Deterministic compatibility decision against `host`.
    #[must_use]
    ...
    /// signature covers compatibility.
    #[must_use]
