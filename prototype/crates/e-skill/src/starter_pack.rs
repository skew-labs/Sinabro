//! `mnemos-e-skill::starter_pack` — atom #311 · D.4.0 — the starter skill
//! pack manifest.
//!
//! ## Canonical OUT (§4.4 — ATOM_PLAN line 340)
//!
//! - [`SkillStarterPack`] — §4.4 `{ bundle_hash_32, packages, compatibility,
...
//!   packs, not a paid marketplace** (§311 광기). The pack is content-addressed:
...
//!   always yields the same pack digest (§311 criterion — *pack manifest digest
//!   stable*).
//!
//! ## Reuse (#296-#310)
//!
//! A pack never re-mints a package or a compatibility model: members are
//! [`crate::package::SkillPackageDigest32`] (#242) and the pack-level
//! [`crate::compat::CompatibilityDecision`] (#251 / §4.4) is the **worst-case
//! fold** of the members' decisions — one unsafe member blocks the whole pack
//! (§311 광기 "one incompatible member blocks the pack"). Building a pack that
...
//! not verified (#296-#310 catalog surface).
//!
//! ## No-commerce + offline boundary
//!
//! A starter pack carries only content digests and a compatibility decision —
//! no price, checkout, revenue, or payment field (G-D-NO-COMMERCE). Building a
...
/// content digest, its host-compatibility decision (#251), and the 32-byte
/// hash of its eval score (#245). These are the only inputs a pack needs; a
/// member is always a package that already passed the catalog surface.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub struct StarterPackMember {
    /// Content address of the member package (#242).
    pub package: SkillPackageDigest32,
    /// The member's compatibility decision against the target host (#251).
    pub compatibility: CompatibilityDecision,
    /// 32-byte hash of the member's eval score (#245). Folded into the pack
...
// ===========================================================================
// 4. SkillStarterPack — §4.4 curated bundle
// ===========================================================================

/// A curated starter pack (§4.4). Built by [`Self::build`] from a verified
...
pub struct SkillStarterPack {
    ...
    /// Worst-case compatibility fold over the members (§4.4).
    pub compatibility: CompatibilityDecision,
