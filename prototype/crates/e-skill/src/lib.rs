...
//!
//! Phase 0 critical-path crate. Each finished module keeps
//! `cargo build --workspace` green.
//!
//! Filled so far:
//! - [`manifest`][]: TOML manifest parse + validate.
//!   Canonical home for [`manifest::SkillManifest`] (private fields +
//!   `pub const fn` accessors for invariant protection), [`manifest::SkillId`]
//!   (`#[repr(transparent)]` newtype over `u16`),
//!   [`manifest::ManifestError`] (`#[non_exhaustive]`
...
//!   filesystem). Reuses [`mnemos_m_agent::tool_schema::ToolId`]
//!   verbatim for the declared-tool slice; the `UnknownTool`
//!   rejection cross-pins against [`manifest::KNOWN_TOOL_IDS`] —
//!   the const allow-set matching the builtin discriminants
//!   `{ReadFile=1, WriteFile=2, RunCommand=3}` (the `Builtin` enum
//!   cross-pins via the const so the canonical set stays one
//!   source-of-truth).
//! - [`builtins`][]: Phase 0 builtin whitelist
...
//!   bytes, so the "lengths only" guarantee holds
//!   by-construction), and the free function [`builtins::dispatch_builtin`]
//!   (validator-only — every rejection folds through
//!   [`mnemos_a_core::MnemosError::tool_denied`]; real
//!   IO / spawn is deferred to the later isolated routing layer). The canonical
...
//! ## Signed skill package surface
//!
//! Stage D wraps the [`manifest::SkillManifest`] in a signed
//! [`package::SkillPackageV1`] and never re-mints core canonical types. The
//! new modules form one verifier pipeline:
//! - [`package`][]: the `SkillPackageV1` aggregate + content-digest spine.
//! - [`package_policy`][]: the no-commerce forbidden-field scan.
//! - [`capability_diff`][]: the permission diff shown before use/install.
//! - [`eval`][]: the eval score + reproducible-command / tests digests.
//! - [`provenance`][]: single-parent content-addressed lineage.
//! - [`signature`][]: the offline author signature over the content digest.
//! - [`package_toml`][]: the canonical TOML normal form.
//! - [`bundle`][]: extraction-safe bundle layout + artifact digest tree.
//! - [`compat`][]: the compatibility constraint model.
//! - [`verify`][]: `verify_skill_package` — the typed verifier API.
//!
//! The malicious-fixture gate, property corpus, and verify
//! bench live in `tests/` + `benches/`.
//! ## WASM Tier-2 sandbox policy + try-before-use
//!
...
//! install-plan, and rollback surfaces. It is a *policy + declarative-fixture*
//! layer: it carries the
...
//! for sandbox trace evidence and never re-mints a core canonical type.
//! ## Registry / provenance / install Move ABI + Rust bindings
//!
...
//! - [`install_state`][]: the `InstallState` lifecycle enum, byte-pinned to
//!   the Move `STATE_*` constants, plus the runtime-usable decision.
//! - [`chain_bindings`][]: manual fixed-layout BCS encoders for
...
//! - [`ptb`][]: PTB dry-run builders gated by the `GasStationPolicy`
//!   (package binding + gas cap + wildcard reject); dry-run / policy-only, no
//!   signing, one acyclic new edge `e-skill -> g-wallet`.
//!
//! The Move-Prover lineage acyclicity proof is authored under
//! `prototype/move/mnemos_skill_registry/prover/` and deferred: the invariant
//! is runtime-enforced + `sui move test`-green now.
#![deny(missing_docs)]
...
// Catalog index / cards / search / ranking / recommend
#[doc(no_inline)]
...
// Catalog cache / anti-gaming / fuzz / bench
#[doc(no_inline)]
...
// Starter pack / community import / author guide /
