//! `mnemos-e-skill::catalog_counters` — atom #297 · D.3.1 — downloads and
...
//! revoked installs. Ranking (#303) must show both so popularity can never
...
/// the registry event stream (§4.4). `Downloaded` is the weak floor;
...
/// A single off-chain verified-install observation (§4.4). Carries no user,
...
impl VerifiedInstallReceipt {
    ...
    /// Map an on-chain [`InstallState`] (#284) onto a verified-install
