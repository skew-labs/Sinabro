//! `mnemos-e-skill::package_toml` — atom #248 · D.0.7 — the canonical TOML
...
//!   `[extensions]` table (the "extension-prefixed" escape hatch of §248).
//! - [`parse_package`] — CRLF-normalizing parser. `\r\n` is collapsed to
//!   `\n` before parsing, so CRLF and LF inputs produce identical results
//!   (§248 CRLF/LF-stable test). Duplicate keys reject (the `toml` parser
//!   errors natively).
...
//!   canonical encoding (§248 광기 + round-trip criterion).
//!
...
/// so a canary in the input cannot escape through this channel (atom #39
...
/// base-manifest parsing (§242 invariant) and any drift fails a test, not a
/// user.
