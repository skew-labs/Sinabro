//! `mnemos-e-skill::install_receipt` — atom #315 · D.4.4 — the local
//! use/install receipt.
//!
//! ## Canonical OUT (§4.5 — ATOM_PLAN line 353-363)
//!
...
//!   produce the same id (§315 criterion — *receipt replay deterministic*).
//! - [`LocalInstallReceipt`] — §4.5 `{ id, skill, package, user, state,
...
//!   no price/payment field (§315 광기, G-D-NO-COMMERCE).
//!
//! ## Authority (reuse #271 / #300 / #301)
//!
//! A receipt is minted only when [`crate::install_plan::InstallPlan::evaluate`]
//! (#271) returns [`InstallDecision::Proceed`] — i.e. the signed package was
//! verified, the host is compatible (#300
//! [`crate::compat::CompatibilityDecision`] via
//! [`compatibility_admits_install`]), the capability diff (#301 / #244) was
...
//! (§4.5), never a freshly-minted lifecycle enum.
//!
//! ## Offline boundary
//!
...
pub enum ReceiptError {
    /// The install plan was blocked (#271). Carries the exact block reason so
...
// ===========================================================================
// 3. compatibility gate (reuse #300 CompatibilityDecision)
// ===========================================================================

/// Map a #300 compatibility decision (§4.4) to the install precondition: only
...
// ===========================================================================
// 5. LocalInstallReceipt — §4.5
// ===========================================================================

/// The local record that a use/install flow completed (§4.5). Minted only on
...
pub struct LocalInstallReceipt {
    ...
    pub user: SuiAddress,
    /// The local lifecycle state recorded (§4.5, reused from `rollback`).
    pub state: LocalSkillState,
    /// The approved capability-diff display digest (#301 / #244) — binds the
...
/// 2. The #271 [`InstallPlan`] is evaluated against `pre` and `observed_digest_32`;
...
/// `human_digest_32` (#301 / #244); `user`/`trace` are offline typed values.
///
