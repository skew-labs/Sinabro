//! `mnemos-e-skill::catalog_cache` — the signed local
//! catalog cache with a replayable event source.
//!
//! A [`CatalogCache`] is a deterministic, content-addressed snapshot of the
//! catalog: a set of [`SkillCatalogIndexEntry`] whose three popularity
//! counters are **re-derived by replaying the install-event stream**
//! ([`VerifiedInstallReceipt`], the off-chain projection of the on-chain
//! typed events) through the same order-independent
...
//!
//! The core invariant: the cache **accelerates discovery but never carries
...
pub struct CatalogCacheEntry {
    /// The denormalized, counter-folded catalog entry.
...
impl SignedCatalogCache {
    ...
    /// package + dry-run + capability approval + the install plan.
