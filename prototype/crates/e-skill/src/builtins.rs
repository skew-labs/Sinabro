//! `mnemos-e-skill::builtins` — the Phase 0 builtin
...
//!   the [`crate::manifest::KNOWN_TOOL_IDS`] allow-set via
...
//!   `&'static` choice makes runtime injection of
//!   new programs impossible — the only way to extend the whitelist is
//!   a source edit that re-builds the agent. A private field plus
//!   `pub const fn` constructor / accessor and `pub fn contains` keep
//!   the whitelist invariant-protected.
//! - [`PHASE0_COMMAND_ALLOWLIST`] — the canonical Phase 0
...
//!   [`_BUILTIN_OUTCOME_SIZE_IS_12`]. Output is returned as lengths only
//!   (no body retained, redacted) — this struct has NO
...
//!   constructor.
//! - [`dispatch_builtin`] — pure validator. The signature admits NO IO
//!   path argument, and `run_command` is designed to pair with the
//!   isolated tier-0/tier-1/tier-2 routing layer that is NOT
...
//!
//! The spec forward-commits `run_command` to a sandboxed routing layer
//! (isolated tier-0/tier-1/tier-2 routing) that does
...
//! dependency that the validator signature does not need, and (c)
//! produce a `BuiltinOutcome` whose `stdout_len_u32` is "what the
//! unsandboxed process printed" — not "what the agent is allowed to
//! observe" (these diverge once the isolated routing lands). Validator-only
...
//! the later layer that wires the isolated routing. This module is the
//! policy boundary, not the IO boundary.
//!
//! ## Reuse map
//!
//! | symbol                  | how |
//! |-------------------------|-----|
//! | [`mnemos_a_core::MnemosError::tool_denied`] | every rejection path folds through this constructor — no module-local `MnemosError` variant added |
//! | [`mnemos_a_core::ToolProgram`]              | denial telemetry; `Cargo/Git/Sui/Walrus` map by name, anything else → `Other` |
//! | [`mnemos_a_core::ToolDenyReason`]           | `Program` for non-allowlisted, `ArgumentShape` for empty / missing args |
//! | [`crate::manifest::KNOWN_TOOL_IDS`]         | cross-pinned to `Builtin` discriminants by compile-time const |
//!
//! ## Design notes
//!
//! 1. **`request_id_u64` is hard-coded to `0`.** The
//!    `dispatch_builtin(b, args, allow)` signature admits NO `request_id`
//!    parameter; the [`mnemos_a_core::MnemosError::tool_denied`]
//!    constructor requires one. The `0` sentinel is documented here
//!    so a later wiring layer (the one that calls `dispatch_builtin`
//!    from the agent loop) can wrap the call with the actual
//!    `RuntimeTaskId`. This module itself never reads the request id, so
//!    folding it through `0` neither leaks information nor masks a
//!    real id — it is a placeholder by signature.
//! 2. **`Builtin` is closed (no `#[non_exhaustive]`).** A `#[repr(u8)]`
//!    enum whose discriminants are part of the wire contract is closed —
...
//!    until the allow-set grows simultaneously, per
//!    [`crate::manifest::KNOWN_TOOL_IDS`] doc comment.
//! 3. **`PHASE0_COMMAND_ALLOWLIST` is a `pub const`, not a function.**
//!    The contents (`cargo/git/sui/walrus`) are declared
//!    directly; exporting the canonical
...
//!    const-validated at compile time if a future caller needs it; the
...
/// single byte with an explicit value pinned to the
/// canonical declaration `{ReadFile=1, WriteFile=2, RunCommand=3}`.
/// Cross-pinned to [`KNOWN_TOOL_IDS`] by
...
/// Order matches [`mnemos_a_core::ToolProgram`]
...
/// field that can carry the actual bytes — the "lengths only, no body
/// retained" guarantee holds by-construction.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl BuiltinOutcome {
    ...
    /// the job of the later layer that wires the isolated routing;
...
///   → [`ToolProgram::Other`]).
///
/// `request_id_u64` is hard-coded to `0` per design note 1 — the
/// validator signature does not admit a request id and the surrounding
/// wiring layer is the right place to thread one through.
...
/// Saturating cast `usize → u16` — `arg_count_u16` in the
/// `tool_denied` signature is 16-bit; `args.len()` is `usize`.
...
/// from the shared enum — the order here matches the order in
...
mod tests {
    ...
    /// The ReadFile / WriteFile builtins dispatch through
    ...
    /// RunCommand admits every program in
    ...
    /// [`ErrorCode::ToolDenied`] — the shared reuse contract.
    #[test]
    ...
    /// Cross-pin between [`Builtin`] discriminants and
    /// [`KNOWN_TOOL_IDS`]. Kept as an internal guard so a reviewer sees
    /// the invariant explicitly rather than relying solely on the
    ...
    /// The canonical [`PHASE0_COMMAND_ALLOWLIST`] matches the
    /// declared set verbatim and in the
