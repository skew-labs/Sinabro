//! WASM Tier-2 sandbox policy surface.
//!
...
//! policy** for a skill sandbox. By design it does **not** embed a real WASM
//! execution engine: `wasmtime` / `wasmi` / `wasmer` are absent from the
//! offline build cache, and the canonical types carry no engine handle. Instead
//! every "run" is evaluated against a *declarative fixture* that states the
//! module's content id and the resources / hostcalls / paths it declares it
//! will touch; the policy layer maps that declaration to a
//! [`WasmSandboxDecision`] with no ambient authority, no host I/O, no
//! allocation bomb, and no panic. A later stage may add a real engine in
//! `n-sandbox` under explicit approval — the limits, grants, hostcall table,
//! and decision surface authored here are the contract it will be required to
//! enforce.
//!
...
//! ## Modules
//!
//! - [`limits`][]: the deny-small per-run resource budget.
//! - [`module_id`][]: the content address of a sandbox module.
//! - [`grant`][]: owner/skill/permission/epoch-scoped capability grants.
//! - [`fs_policy`][]: filesystem deny-by-default.
//! - [`net_policy`][]: network deny-by-default.
//! - [`secret_policy`][]: wallet/chain/secret deny matrix.
//! - [`hostcalls`][]: the closed, versioned hostcall table.
//! - [`determinism`][]: replayable logical time + run-id seed.
//! - [`meter`][]: fuel/memory/stack/hostcall metering.
//! - [`output`][]: redacted output envelope + size cap.
//! - [`trace`][]: sandbox eval trace (JSONL, `StageDTraceLink`).
...
// ===========================================================================
// WasmSandboxDecision — single outcome enum for every Tier-2 evaluation
// ===========================================================================

/// Sandbox decision — the single outcome enum for every Tier-2
