//! `mnemos-e-skill::compat_solver` — atom #300 · D.3.4 — the compatibility
//! solver over single skills and skill **bundles**.
//!
//! Reuses the #251 [`SkillCompatibility`] constraint model (its `evaluate` /
...
//! installable). The toolchain dimension (A [`ToolId`]) is folded in by
//! [`solve_with_tools`]: a skill that needs a tool the host does not provide is
//! [`CompatibilityDecision::Incompatible`].
//!
//! The §4.4 `SkillStarterPack` *type* (a named, hash-bound bundle) is owned by
//! the later starter-pack atom (#311 · `starter_pack.rs`); this solver operates
//! over a plain `&[SkillCompatibility]` set so it can be reused by both the
//! catalog (#296–#305) and the starter-pack cluster without minting that type
//! here.
...
/// Solve a single skill's compatibility against `host` (reuses #251). Pure and
...
/// `override_evidence` (the mismatch is never overrideable, per #251).
#[must_use]
