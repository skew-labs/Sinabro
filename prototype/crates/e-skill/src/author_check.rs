//! `mnemos-e-skill::author_check` — atom #313 · D.4.2 — the skill author
//! checklist contract.
//!
//! ## Canonical OUT (§313)
//!
//! A simple, ordered authoring path (§313 광기): `manifest -> fixtures -> eval
...
//! not an afterthought — the guide in `docs/skill_author_guide.md` uses the
...
//! `contrib dry-run` (§313 criterion).
//!
//! ## Reuse (#248 / #243 / #244, contract-shape of #305)
//!
//! The manifest step reuses [`parse_package`] (#248) and
//! [`scan_no_commerce`] (#243); the capability step reuses the #244
//! [`CapabilityDiff`] consistency check and rejects a declaration that drifts
//! from the actual diff. The checklist output shape mirrors the #305 CLI card
...
pub enum AuthorCheckError {
    /// The manifest TOML failed to parse against the canonical schema (#248).
    ManifestParse,
    /// The manifest carries a commerce-shaped field (#243).
    NoCommerce,
    /// The declared capabilities do not match the actual capability diff, or
    /// the diff is itself inconsistent (#244).
    CapabilityDrift,
}
...
/// Check the manifest step: it must parse against the canonical schema (#248)
/// AND be free of commerce-shaped fields (#243).
...
/// self-consistent (#244 — no hidden permission). A mismatch is a docs/code
/// drift and rejects.
...
pub struct AuthorChecklist {
    /// Manifest parses + no-commerce (#248 / #243).
    pub manifest_ok: bool,
    /// Test fixtures present.
    pub fixtures_ok: bool,
    /// Reproducible eval command/score present (#245).
    pub eval_command_ok: bool,
    /// Declared capabilities match the actual diff (#244).
    pub capability_declared_ok: bool,
    /// Provenance author signature present (#246 / #247).
    pub provenance_signed_ok: bool,
    /// Local try-before-use dry-run passed (#266).
    pub dry_run_ok: bool,
}
...
/// steps are validated here (reusing #248 / #243 / #244); the remaining steps
/// (fixtures, eval, provenance signature, dry-run) are supplied as upstream
/// gate verdicts.
#[must_use]
...
mod tests {
    ...
    fn guide_example_parses_and_passes_dry_run() {
        // The doc/guide fixture is the canonical sample — it must parse (#248)
        // and pass the manifest step (the `contrib dry-run` criterion).
