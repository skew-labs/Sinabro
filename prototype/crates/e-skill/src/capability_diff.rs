//! `mnemos-e-skill::capability_diff` — atom #244 · D.0.3 — the permission
//! diff shown before a skill is used or installed.
//!
//! ## Canonical OUT (§4.1 — ATOM_PLAN line 180-190)
//!
//! - [`SkillRuntimePermission`] — §4.1 10-variant `#[repr(u8)]` runtime
...
//!   the A [`CapabilityKind`] cannot express (file / network / wallet /
//!   chain / secret / tool). The three memory/anchor permissions
//!   (`MemoryRead`/`MemoryWrite`/`AnchorChunk`) map onto the A
//!   `CapabilityKind` (`ReadMemory`/`WriteMemory`/`AnchorChunk`) — that is
//!   the only place Stage D touches the A capability surface (§1 reuse).
//! - [`CapabilityDiff`] — §4.1 added/removed permission masks + the mapped
//!   A capabilities + declared tool ids + a stable `human_digest_32`. The
...
//!   permission** and is rejected (G-D-CAPABILITY).
//!
//! No install plan may proceed without this diff digest (§244 criterion).
//! No field here grants a live wallet / secret action — these are *labels*
//! only; the WASM-sandbox capability token (#256+) is what gates execution.
...
// ===========================================================================
// 1. SkillRuntimePermission — §4.1 10-variant runtime permission label
// ===========================================================================

/// Stage D runtime permission (§4.1). `#[repr(u8)]` 1-byte discriminant.
/// The 10-variant set is pinned by §4.1 (no `#[non_exhaustive]`).
...
pub enum SkillRuntimePermission {
    ...
    Network = 3,
    /// Wallet access (label only here — no live wallet in Stage D Session 1).
    Wallet = 4,
...
// ===========================================================================
// 2. CapabilityDiff — §4.1 added/removed masks + mapped A caps + digest
// ===========================================================================

/// Permission diff shown before use/install (§4.1). The masks are over
