//! `mnemos-e-skill::review_queue` — atom #318 · D.4.7 — the local/community
//! registry review queue.
//!
//! ## Canonical OUT (§318)
//!
//! A local, self-hostable review queue (§318 광기). The public catalog accepts
//! a candidate skill only after: the import dry-run report (#312
//! [`CommunitySkillImport`]), the author checklist (#313
...
//! ## Reuse (#312 / #313)
//!
//! [`ReviewQueue::submit`] requires a complete [`AuthorChecklist`] (#313) and
//! stores the [`CommunitySkillImport`] (#312) verbatim; the queue never
...
//! (§318 criterion — *queue replay deterministic*). Pure, offline; no network,
...
pub enum ReviewQueueError {
    /// The submission's author checklist (#313) is incomplete.
...
impl ReviewQueue {
    ...
    /// (#313). A re-submission for the same package replaces the import and
