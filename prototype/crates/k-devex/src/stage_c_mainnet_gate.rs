//! Stage C mainnet gate approval-wait state (C-WP-08 · atom #238 · C.2.19).
//!
//! Canonical OUT (§4.4): [`MainnetGateReceipt`].
//!
//! # Madness invariants (atom #238)
//!
...
//!   because it delegates to [`MainnetChecklist::ready_state`] (atom #204) which
...
//! * **Pause dominates.** If the incident pause (atom #237) is engaged, the
...
//!
//! * **reuse: #204** — [`MainnetChecklist`](crate::stage_c_checklist::MainnetChecklist)
//!   supplies the green-mask + evidence + `ready_state` ceiling (k-devex).
//! * **reuse: #225** — [`MainnetPackageLock`](mnemos_d_move::stage_c_package_lock::MainnetPackageLock)
//!   is the package/bytecode/prover/gas-baseline commitment bound into the
//!   receipt (d-move).
//! * **reuse: #237** — [`IncidentPause`](crate::stage_c_pause::IncidentPause)
//!   supplies the pause override (k-devex).
//! * The execution posture reuses the §4.1
//!   [`MainnetExecutionState`](mnemos_a_core::stage_c_env::MainnetExecutionState)
//!   (atom #173, a-core); no parallel mainnet-state enum is minted.
//!
...
/// The §4.4 mainnet gate receipt: the bound checklist, the package lock, and the
/// gated execution posture this gate *permits* (never `Executed`).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub struct MainnetGateReceipt {
    /// The mainnet readiness checklist (atom #204).
    pub checklist: MainnetChecklist,
    /// The package lock binding bytecode/prover/gas-baseline (atom #225).
...
mod tests {
    ...
    /// `c2_19_all_green_results_in_approval_pending` — an all-green checklist
    /// with bound evidence and a clear pause yields `ApprovalPending`, never
    /// `Executed`, and is not executable.
    #[test]
    ...
    /// `c2_19_missing_approval_cannot_execute` — a partial checklist (one step
    /// missing) stays `Locked` and is not executable; a zero-evidence all-green
    /// checklist is likewise `Locked`.
    #[test]
    ...
    /// `c2_19_pause_forces_paused` — an engaged incident pause forces `Paused`
    /// even with an all-green, evidence-bound checklist; still not executable.
    #[test]
    ...
    /// `c2_19_receipt_binds_checklist_and_lock` — the receipt carries the exact
    /// checklist and package lock it was evaluated from.
    #[test]
