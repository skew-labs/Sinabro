//! Stage C incident pause / rollback gate (C-WP-08 · atom #237 · C.2.18).
//!
//! Canonical OUT: a pause state for the mainnet gate, the Gas Station sponsor
//! path, and the mainnet ceremony path.
//!
//! # Madness invariants (atom #237)
//!
...
//!   mirroring the [`BurnInWindow`](crate::stage_c_burn_in::BurnInWindow) (atom
//!   #235) withhold-on-anomaly discipline.
...
//!
//! * **reuse: #218** — the sponsor decision this gate withholds is the
//!   `g-wallet` `GasStationDecision::accepted` boolean (atom #218). This module
...
//!   verdict — is proven in the `o-stage-c-e2e` integration crate (Sinabro
//!   Physical Law 1, same pattern as the atom #234 ceremony binding).
//! * **reuse: #235** — [`BurnInWindow`](crate::stage_c_burn_in::BurnInWindow)
//!   supplies the anomaly / paused signal that auto-engages the pause.
//! * **reuse: #234** — [`CeremonyTranscript`](crate::stage_c_ceremony::CeremonyTranscript)
...
pub enum PauseReason {
    ...
    OperatorManual = 1,
    /// A burn-in window observed an anomaly or was paused (atom #235).
    BurnInAnomaly = 2,
    /// A Gas Station / sponsor-path alert engaged the pause (atom #218 surface).
    GasStationAnomaly = 3,
    /// A read-only canary monitor flagged an anomaly (atom #236 surface).
    CanaryAnomaly = 4,
}
...
/// The incident-pause / rollback gate state (§4.4 canonical OUT for atom #237).
///
...
impl IncidentPause {
    ...
    ///
    /// This reuses the atom #235 [`BurnInWindow`](crate::stage_c_burn_in::BurnInWindow)
    ...
    /// Gate a Gas Station sponsor decision (atom #218): a sponsor request may
    ...
    /// Build a mainnet ceremony transcript (atom #234) only if the pause is
...
mod tests {
    ...
    /// `c2_18_pause_blocks_sponsor_decision` — while paused, even an accepted
    /// sponsor decision is withheld; while running, an accepted decision passes
    /// and a rejected one stays rejected.
    #[test]
    ...
    /// `c2_18_pause_blocks_ceremony` — while paused the ceremony path is blocked
    /// before any signer boundary; while running it composes with the real
    /// atom #234 transcript builder.
    #[test]
    ...
    /// `c2_18_resume_requires_evidence_hash` — a pause cannot be lifted with an
    /// all-zero evidence hash; a non-zero hash lifts it and is recorded.
    #[test]
    ...
    /// `c2_18_anomaly_from_burn_in_pauses` — a clean burn-in window does not
    /// engage the pause; a window with an anomaly (or a paused window) does,
    /// with [`PauseReason::BurnInAnomaly`].
    #[test]
