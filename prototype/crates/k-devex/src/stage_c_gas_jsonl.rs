//! Stage C gas/effect trace JSONL line builder (C-WP-02A · atom #186 · C.0.15).
//!
//! Canonical OUT: the append-only JSONL record shape for gas + effect samples,
//! mirrored by the JSON Schema `ops/schemas/stage_c_gas_trace.schema.json`.
//!
//! # Madness invariants (atom #186)
//!
//! * **No raw payload body, no secret labels.** The line is built only from a
//!   typed [`GasTraceSample`] (atom #178) and [`EffectDelta`] (atom #185) — both
//!   are scalar / 32-byte-id records with no body field. Any free-text note must
//!   arrive as a [`RedactedLogValue`] (atom #46 `redact_for_log`), which dropped
...
//! * **Append-only, Stage E-ready.** The record carries an explicit
//!   [`STAGE_C_GAS_TRACE_SCHEMA`] version so later stages can evolve the schema
