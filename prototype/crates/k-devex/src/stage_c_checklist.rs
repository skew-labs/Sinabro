//! Six-step mainnet readiness checklist.
//!
//! Public surface: [`MainnetChecklistStep`], [`MainnetChecklist`].
//!
//! # Invariants
//!
...
//!   [`Executed`](MainnetExecutionState::Executed) requires the later
//!   operator-approval ceremony (multisig and timelock), not this checklist.
//! * **No re-mint.** The execution posture reuses
//!   [`MainnetExecutionState`] (from `a-core`); no parallel mainnet-state
...
/// The six ordered gate steps of the mainnet readiness checklist.
///
...
pub enum MainnetChecklistStep {
    /// Step 1 — deterministic testnet replay evidence.
    StageBEvidence = 1,
    /// Step 2 — Move Prover + gas trace bound to one package digest.
    ProofAndGas = 2,
    /// Step 3 — no open High/Critical audit findings.
    AuditResolved = 3,
    /// Step 4 — multisig roster + timelock policy present.
    MultisigTimelock = 4,
    /// Step 5 — deny-by-default Gas Station policy green.
    GasStationPolicy = 5,
    /// Step 6 — explicit operator approval.
    OperatorApproval = 6,
}
...
/// The mainnet readiness checklist: a bitmask of green steps, the current
