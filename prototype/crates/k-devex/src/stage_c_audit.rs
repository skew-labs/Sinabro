//! Audit finding state machine.
//!
//! Public surface: [`AuditFindingSeverity`], [`AuditFindingState`],
//! [`AuditFinding`].
//!
//! # Invariants
//!
...
//!   past the mainnet checklist (whose audit step consumes [`audit_resolved`]).
...
//!   [`StageCTraceLink`](mnemos_a_core::trace::StageCTraceLink) trace stamp
//!   (atom/attempt/gate), so the four-field signature is implemented without
//!   re-minting an owner type.
//! * **No re-mint.** Evidence reuses [`StageCEvidenceRef`]; no parallel
//!   evidence/trace type is introduced.
...
/// Severity of an audit finding. Higher discriminant = more serious.
...
/// Lifecycle state of an audit finding.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
...
/// A single audit finding: a stable id, a severity, a lifecycle state,
...
pub struct AuditFinding {
    ...
    /// Content-addressed evidence ref (the fix/acceptance record). Reuses
    /// [`StageCEvidenceRef`]; its trace stamp identifies the owner.
