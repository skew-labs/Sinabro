//! Stage C burn-in window (C-WP-07 · atom #235 · C.2.16).
//!
//! Canonical OUT (§4.4): [`BurnInWindow`].
//!
//! # Madness invariants (atom #235)
//!
...
//!   (atom #49) — a runaway producer can never wrap the count to a small
...
//!
//! * **reuse: A metrics** — the anomaly counter follows the
//!   [`crate::metrics`] monotonic-saturating-counter discipline; the concrete
//!   [`MetricsExporter`](crate::metrics::MetricsExporter) reuse lands in the
//!   atom #236 canary monitor that drives this window.
//! * **reuse: #234** — a burn-in window gates the same mainnet ceremony whose
//!   transcript atom #234 builds.
//!
//! No live action: this is monitoring state. `MainnetExecutionState` stays
//! `Locked`.

/// Burn-in window monitoring state (§4.4).
