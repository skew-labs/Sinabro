//! Skill-registry / install gas trace + Gas Station allowlist extension.
//!
//! This module extends the Gas Station to the eight bounded
//! skill-registry / install Move entries (the [`SkillPtbAction`] set). It mints
//! NO new canonical type and NO new sponsorable function:
//! the action identity is the `e-skill` [`SkillPtbAction`], the policy is
//! [`GasStationPolicy`], the verdict is [`GasStationDecision`], and the
//! per-action regression ladder reuses [`GasRegressionDecision`]. The only
//! new surfaces are the per-action baseline ([`SkillGasBaseline`]) and the
//! append-only evidence line — both `k-devex` evidence tooling, parallel to
//! [`build_gas_trace_line`](crate::stage_c_gas_jsonl::build_gas_trace_line).
//!
//! # No commerce
//!
...
//! # Dependency note
//!
//! The sibling gas-trace module deliberately keeps `k-devex` WITHOUT a
//! `k-devex -> g-wallet` edge by consuming the single sponsor decision as a
//! `bool` and binding it in `o-stage-c-e2e` (see the comments in `lib.rs`). This
//! module is different: a TYPED eight-action allowlist evaluator over
//! [`GasStationPolicy`] is not bool-reducible, so it takes a direct, ACYCLIC
//! `k-devex -> g-wallet` and `k-devex -> e-skill` edge (neither crate, nor its
//! dependency closure, depends on `k-devex`). The reuse is types only — no
//! signing, no wallet, no secret, no network, no chain action. Offline /
...
/// The append-only schema id stamped on every skill-gas-trace record.
...
/// copy of the `stage_c_gas_jsonl` helper kept private to this module so the
/// sibling module keeps its current (private) API.
...
impl SkillGasBaseline {
    ...
    /// absolute hard cap, reusing the [`GasRegressionDecision`] ladder:
...
/// the [`GasStationPolicy`] checks and returning a [`GasStationDecision`].
