//! Prometheus text exposition for the MNEMOS Phase 0 metrics surface.
//!
...
//! cover the 7 Phase 0 axes from `MNEMOS_ATOM_PLAN.md` §4.K:
...
/// MNEMOS Phase 0 counter axes — `#[repr(u8)]` discriminants per §4.K.
///
/// Variants are explicitly numbered 1..=7 so the discriminant is a stable
/// wire-style value (atom #2 / #3 precedent for explicit `#[repr(u8)]` enums)
/// and so `index()` can map `discriminant - 1` directly into the counter
/// array without a match table.
...
/// Prometheus text-exposition counter exporter for the MNEMOS Phase 0 axes.
///
...
impl MetricsExporter {
    ...
    pub fn render(&self) -> String {
        ...
        // along the hot exposition path (not heap-zero by contract — render
        // is explicitly "not hot" per atom #49 criterion — but the bound is
        // deterministic so this stays cheap).
...
mod tests {
    ...
    fn k0_4_no_secret_labels() {
        ...
        // `RuntimeEnv::from_pairs` filter set from atom #5 — KEY / TOKEN /
    ...
    fn metric_discriminants_match_canonical_spec() {
        // §4.K canonical OUT pins discriminants 1..=7 in this exact order.
