//! Stage C mainnet ceremony transcript builder (C-WP-07 · atom #234 · C.2.15).
//!
//! Canonical OUT: a reproducible, hash-addressed ceremony transcript binding the
...
//!   supplies the bound evidence hash (k-devex, same crate).
...
//! * **reuse: #214** — the `exact_tx_digest_32` is the
//!   `MainnetSignerEnvelope::tx_digest_32` (g-wallet, atom #214). This module
...
//!   integration crate, which already depends on both `g-wallet` and `k-devex`
//!   (per Sinabro Physical Law 1: the test home owns both symbols).
//!
...
pub struct CeremonyTranscript {
    ...
    /// the atom #214 `MainnetSignerEnvelope::tx_digest_32`.
