...
//!
//! This crate provides the Prometheus text exposition
//! surface — `metrics::Metric` (`#[repr(u8)]` 7-axis enum) and
//! `metrics::MetricsExporter` (fixed `[AtomicU64; 7]`, heap 0, no label
//! dimension by construction so secret exposure is 0). Other developer-experience
//! concerns (CI, systemd, bootstrap, runbooks) are
//! configuration/scripts/docs and so do not contribute Rust types — they live
//! outside this crate root.
#![deny(missing_docs)]

pub mod metrics;
pub mod stage_c_audit;
// Mainnet ceremony transcript
...
// Reuses `MainnetChecklist`, `MainnetPackageLock` (d-move), and the
// `MetricsExporter`. The signer-envelope digest is consumed
...
// Reuses `BurnInWindow`, `CeremonyTranscript`, `MainnetChecklist`
// (same crate), and `MainnetPackageLock` + `MainnetExecutionState`
// (d-move / a-core, existing edges). No `k-devex -> g-wallet` edge: the sponsor
// decision is consumed as a `bool`, bound in `o-stage-c-e2e`.
pub mod stage_c_mainnet_gate;
pub mod stage_c_pause;
// The Stage D skill-registry / install gas trace
// + Gas Station allowlist extension. UNLIKE the Stage C modules above (which
// avoided a `k-devex -> g-wallet` edge by consuming the sponsor decision as a
// `bool` and binding it in `o-stage-c-e2e`), this module needs a TYPED eight-action
