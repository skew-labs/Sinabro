//! Stage C sponsor signer request/response boundary.
//!
//! The sponsor signer request/response boundary built on the
//! [`MainnetSignerEnvelope`](crate::stage_c_signer_envelope::MainnetSignerEnvelope).
//!
//! # Invariants
//!
...
//!   be signed; the key stays behind the signer boundary
...
//! # Reuse
//!
//! * [`MainnetSignerEnvelope`] is the exact-bytes commitment.
//! * The presented policy hash is the Gas Station policy hash this signature is
//!   bound to.
//! * The `lease_id` originates from a hot-wallet-capped
...
pub struct SponsorSignerRequest {
    ...
    /// The gas-coin lease id this request consumes.
