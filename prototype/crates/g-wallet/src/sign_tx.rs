//! Sign Sui Move transaction intent messages with the borrowed
...
//! signing primitive. This module anchors that prefix as
...
//! the c-walrus [`SignatureBytes`] (`pub struct
...
//! agree byte-for-byte.
//!
//! **Gas-only bot wallet.** This signing surface is the bot
...
//!    self-trip on the explanatory doc-comment itself; same
//!    self-tripping-canary precedent used elsewhere in this crate.)
//!  - The caller-side contract (enforced upstream by `d-move`'s
//!    `SuiCallBuilder::to_dry_run_bytes`) is that
//!    `intent_tx_bytes` was produced by a dry-run-verified
//!    `mnemos::memory_root::add_chunk` call. This module itself does not
//!    encode that ownership in the type system (the bytes are
//!    intentionally opaque — wrapping them in a typed `DryRunBytes` is
//!    deferred to a future revision that wires the dry-run + sign path
...
//! here (the same declared-but-reserved precedent continues).
...
/// Sui signature-scheme flag for ed25519 keys (canonical shape:
/// `SignatureFlag(u8)` with `ed25519 = 0`). The flag itself is NOT
/// prepended to the 64-byte signature returned by [`sign_move_tx`] —
/// it is exposed as a typed constant so future additions to this crate
/// that need the prefixed wire form (e.g. authority-signature serialisation,
...
/// `IntentScope::PersonalMessage` (the `sign_msg` surface) is structurally
...
/// equal 64. If a future revision ever re-widens the signature byte
/// array, this fails the build before any test runs — the G domain
/// refuses to silently re-cast a wider blob.
...
/// Sui signature-scheme flag byte (canonical shape). `repr(transparent)`
...
/// `intent_tx_bytes` length / shape checks live in the *caller*
/// (`d-move`'s dry-run + a future intent-builder addition).
#[must_use]
pub fn sign_move_tx(key: &ScopedSecretKey, intent_tx_bytes: &[u8]) -> SignatureBytes {
...
mod tests {
    ...
    /// `g0_2_signs_known_intent_vector`.
    ///
    ...
    fn g0_2_signs_known_intent_vector() {
        ...
        // Representative dry-run-style payload (the actual `d-move`
    ...
    ///
    /// Bot wallet = gas-only low balance. The Phase 0 signing
    ...
    fn g0_2_only_gas_wallet_scope() {
        ...
        // upstream by `d-move`'s dry-run; the bot wallet sees no
