//! Stage B sign Sui tx intent, testnet-only.
//!
//! A testnet-only Sui
//! transaction signing helper. It signs the byte-stable dry-run
//! representation of a Stage B [`StageBCallBuilder`] under the Sui
...
//!   (Stage A), disjoint from the chunk-sign domain and
//!   the personal-message scope `[3,0,0]` — a tx signature cannot be
//!   replayed as a chunk or personal-message signature.
//! * **No secret escapes.** The seed is borrowed from the caller's
//!   [`ScopedSecretKey`] for the call only (Stage A zeroize posture).
//!
//! # Reuse map
//!
//! * [`StageBCallBuilder`] and its testnet-gated
//!   `to_dry_run_bytes` carrier.
//! * The [`ScopedSecretKey`] that signs.
...
pub fn sign_testnet_call(key: &ScopedSecretKey, builder: &StageBCallBuilder) -> SignatureBytes {
    // `to_dry_run_bytes` is the byte-stable, unsigned carrier. It is
