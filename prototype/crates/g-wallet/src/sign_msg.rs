//! G.0.3 — sign Sui *personal-message* intents with the borrowed
//! ed25519 secret seed from atom #33's [`ScopedSecretKey`].
//!
//! Canonical OUT (`MNEMOS_ATOM_PLAN.md` §4.G atom #35, line 679):
//!
//! ```rust,ignore
//! pub fn sign_message(key: &ScopedSecretKey, message: &[u8]) -> SignatureBytes;
//! ```
//!
//! ## 광기 (ATOM_PLAN line 1171 + §9.5)
//!
...
//! signed payload. Atom #34 ([`crate::sign_tx`]) pinned the
...
//! other intent scope. This atom pins the **distinct**
...
//!
//! **Placeholder → real signing path (ATOM_PLAN line 1171).** The
//! returned [`SignatureBytes`] is byte-identical to the
//! [`SignaturePlaceholderV1::signature`] field declared by atom #7 in
...
//! [`SignatureBytes`] (atom #7); the secret-seed accessor is the atom
//! #33 [`ScopedSecretKey::as_bytes`] borrow; the signing primitive is
//! `ed25519_dalek::SigningKey::sign` (already a transitive workspace
//! dep via atom #34, no new dep added).
//!
//! **Gas-only bot wallet (§9.5) — message surface is opaque.** This
//! signing surface, like atom #34, takes ONLY `message: &[u8]` (opaque
...
//! `-> SignatureBytes` is total per §4.G line 679 — every Sui
...
//! emitted* — atom #33's declared-but-reserved precedent reused; atom
//! #34's no-emission precedent extended. Recorded in
//! `no_op_decisions.jsonl` for Session 2 acceptance.
//!
...
//! prefix. Atom #35 does NOT impose that BCS wrapping inside
//! [`sign_message`]: the `message: &[u8]` argument is treated as
//! opaque, mirroring atom #34's `intent_tx_bytes: &[u8]` opaque
//! contract (the BCS shaping is upstream of the signing surface in
//! both atoms). A future atom that wires the off-chain Sui personal-
...
/// atom #34's `IntentScope::TransactionData=0` — is documented at one
/// canonical source.
...
/// `IntentScope::TransactionData` (atom #34 surface,
/// [`SUI_INTENT_PREFIX_TRANSACTION_DATA`](crate::sign_tx::SUI_INTENT_PREFIX_TRANSACTION_DATA)
...
/// bytes (the same width atom #34 pinned for the transaction-data
...
/// (ed25519-dalek 2.x default; atom #34 precedent).
///
...
/// live in the *caller*; this atom keeps the signing surface opaque
/// over the message bytes, matching atom #34's opaque-`intent_tx_bytes`
/// contract.
#[must_use]
pub fn sign_message(key: &ScopedSecretKey, message: &[u8]) -> SignatureBytes {
...
mod tests {
    ...
    // this module (atom #33 / #34 precedent).
    ...
    /// `g0_3_signs_message_vector` (ATOM_PLAN line 1172).
    ///
    ...
    ///     `SignaturePlaceholderV1` (the placeholder → real path of
    ///     ATOM_PLAN line 1171), with the surrounding placeholder
    ///     verifying as a well-formed structure (scheme = Ed25519,
    ///     public_key = derived ed25519 pubkey).
    #[test]
    fn g0_3_signs_message_vector() {
        // Fixed seed — distinct from atom #34's vector seed to keep
        ...
        // Placeholder → real path (ATOM_PLAN line 1171): a c-walrus
    ...
    /// `g0_3_message_domain_separated_from_tx` (ATOM_PLAN line 1172).
    ///
    /// Domain-separation invariant — the structural reason for atom
    /// #35 to exist as a distinct entry point from atom #34. Pinned
    /// across three independent witnesses:
    ///
    ...
    ///  3. The atom #35 source carries no user-funds-surface
    ///     identifier (gas-only bot wallet contract, §9.5). Asserted
    ///     by reading `include_str!("sign_msg.rs")` and grepping a
    ///     `concat!`-assembled needle list (atom #34 precedent so the
    ///     canary does not self-trip on its own assertion source).
    #[test]
    fn g0_3_message_domain_separated_from_tx() {
        ...
        // does NOT contain the verbatim literals (atom #34 self-
        // tripping-canary precedent; needles in CamelCase / snake_case
        // form match the same pattern used by `g0_2_only_gas_wallet_scope`).
