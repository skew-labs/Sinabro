...
//!
//! The first canonical surface here is an at-rest-encrypted Sui ed25519
//! keypair gated behind a passphrase, with the in-memory secret
//! confined to a `Drop`-zeroizing `ScopedSecretKey` that structurally
//! cannot be logged or cloned (`Debug` / `Display` / `Clone` / serde
//! absent ⇒ those code paths fail to compile).
//!
//! The second canonical surface is the [`sign_tx::sign_move_tx`] entry
//! point that prepends the canonical 3-byte Sui intent prefix
//! `[TransactionData=0, V0=0, Sui=0]` to a caller-supplied
//! `intent_tx_bytes` slice and returns the 64-byte ed25519 signature as
//! the c-walrus [`SignatureBytes`](mnemos_c_walrus::SignatureBytes)
//! type (reused, not redeclared — c-walrus owns the canonical 64-byte
...
//!
//! The third canonical surface is the [`sign_msg::sign_message`] entry
//! point that prepends the **distinct** 3-byte Sui personal-message
//! intent prefix `[IntentScope::PersonalMessage=3, V0=0, Sui=0]`
...
//! be replayed under the transaction-data scope (the `[0,0,0]` prefix),
//! and vice versa. The placeholder → real signing path is witnessed by
//! the `g0_3_signs_message_vector` test: the returned `SignatureBytes`
...
//!
//! The fourth canonical surface is the [`rotate::rotate_key`] entry
//! point: given a caller-supplied `&SealedKeypair`, the old passphrase,
//! and a new passphrase, returns the freshly-generated
...
//! [`SealedKeypair::create_encrypted`] path; the transient
...
//! is zeroized by `Drop`). The returned [`rotate::RotationReport`]
//! carries **only** the (old, new) Sui address pair — no secret
//! material crosses the rotation surface. The first emission site for
//! [`WalletError::KeyRotation`] (declared alongside the keystore,
//! reserved through the signing surfaces) lands here: rotation refuses
//! when `new_pass == old_pass` (silent same-pass rotation defeats the
//! purpose) and is also surfaced as a structural canary on the
//! cryptographically-impossible address-collision branch. The "no
//! signing gap during rotation" requirement is satisfied by
//! construction — `rotate_key` takes `&SealedKeypair` (NOT `&mut`), so
//! the OLD sealed file remains usable for [`sign_move_tx`] /
//! [`sign_message`] until the caller decides to overwrite it on disk;
//! the [`rotate::tests::g0_4_e2e_sign_after_rotation`] test witnesses
...
// Stage B testnet wallet core. Each module is a thin Stage B
// testnet-policy surface over the Stage A crypto above — no parallel
// crypto is minted. Network selection reuses the one-variant
// `StageBNetwork` so a production network is unrepresentable across the
// whole package.
...
// Stage C mainnet multisig / timelock / signing envelope + deny-by-default
// Gas Station policy. These are typed approval-machinery surfaces —
// multisig roster, proposal envelope, timelock policy, exact signer
// envelope, no-single-key guard, Gas Station policy schema and
// dry-run/effect-shape checker. No live egress, no wallet signing, no gas
// spend: every surface produces `Locked` / `ApprovalPending` state, never
// `Executed`. Reuses the `SuiAddress`/`ObjectId`/`GasBudgetMist`/
// `SuiCallBuilder`/`EffectDelta` (d-move) and the `StageCTraceLink` /
// `MainnetExecutionState` (a-core) — no parallel type is minted.
...
// Stage C signer isolation boundary + cold-treasury / hot-sponsor topology.
// Prepare-only datatypes — the API process holds a zero-sized key-less
// handle, the cold treasury is bound to a `threshold >= 2` multisig roster,
// and auto-refill is disabled by default. No live egress, no wallet signing,
// no gas spend; `MainnetExecutionState` stays `Locked`. Reuses
// `SealedKeypair`/`ScopedSecretKey`, `SuiAddress`/`GasBudgetMist`,
// `MainnetSignerEnvelope`, and `MultisigRoster` — no parallel type is minted.
pub mod stage_c_wallet_topology;
// Stage C sponsor hot-wallet cap policy, sponsor signer request/response
// boundary, and gas-coin lease pool. Pure typed policy / boundary /
// bookkeeping — no key material, no signer, no tx submitter. Reuses
// `SuiAddress`/`ObjectId`/`GasBudgetMist` (d-move) and
// `MainnetSignerEnvelope`; no parallel type is minted.
// `MainnetExecutionState` stays `Locked`.
...
// ----- Stage B testnet wallet canonical re-exports -------------------------
...
// ----- Stage C mainnet approval-machinery canonical re-exports -------------
...
// ----- Stage C signer isolation canonical re-exports -----------------------
...
// ----- Stage C sponsor / gas-coin canonical re-exports ---------------------
