//! Stage B wallet trace redaction (atom #152 · B.4.6, WorkPackage B-WP-03).
//!
//! Canonical OUT (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.4): a redacted wallet trace
//! event. The madness spec: "traces include address suffix / gas / tx digest
//! only; key material never enters trace / log / metrics evidence."
//!
//! # Madness invariants (§4.4 / atom #152)
//!
...
//!   public tx digest, and the atom #81 [`StageBTraceLink`] — and **no**
...
//!
//! * **reuse: #81** — [`StageBTraceLink`] (`b-memory/src/stage_b_handoff.rs`),
//!   the canonical per-action trace stamp.
//! * **Stage A redaction** — the same posture as the #82 network module and
//!   the #88 owner module: no field carries raw secret/attacker bytes onward.
...
/// A redacted wallet trace event (§4.4 canonical OUT).
///
...
pub struct StageBWalletTrace {
    ...
    /// The atom #81 per-action trace stamp.
...
impl StageBWalletTrace {
    ...
    /// The linked atom #81 trace stamp.
    #[inline]
    #[must_use]
...
mod tests {
    ...
    /// `b4_6_trace_linked` — the atom #81 trace stamp round-trips verbatim and
