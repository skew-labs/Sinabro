...
//! passphrase + address-change report. Closes Stage G.
//!
//! Public API surface:
//!
//! ```rust,ignore
...
//! CSPRNG via the [`SealedKeypair::create_encrypted`] path.
...
//! [`zeroize::Zeroize::zeroize`] on the 32-byte buffer; no copy of
//! the old seed escapes [`rotate_key`]. The
...
//!
//! **No signing gap during rotation (line 1181).** The
...
//!
//! [`WalletError::KeyRotation`] was declared in the wallet's error
//! type but reserved unused until now. This function is its FIRST
//! emission site, on two structural branches:
//!
...
//! through plain unseal (both collapse into
...
//! `ScopedSecretKey`, `SuiAddress`) flows in unchanged from the
//! g-wallet `keystore` module and the d-move `SuiAddress` type. This
//! is a pure surface addition with zero new transitive crate
//! dependency. The
...
/// crate (`ScopedSecretKey` derives nothing) is to keep
...
///    `Drop` impl zeroizes the 32-byte buffer. No copy of the old
///    seed escapes this function.
...
///   refuses fail-closed; this is the first emission site for
///   `KeyRotation`).
pub fn rotate_key(
    old: &SealedKeypair,
    old_pass: &str,
    ...
    // unseal step (PBKDF2 + AEAD authentication is the
    ...
    // 32-byte pubkey prefix of each ciphertext).
...
mod tests {
    ...
    /// `g0_4_rotation_changes_address` (line 1182).
    ///
    ...
    /// `g0_4_old_key_zeroized` (line 1182).
    ///
    /// Old-key zeroization is realised by two layered guarantees:
    ///
    ///  1. The [`ScopedSecretKey`] type's `Drop` impl calls
    ///     `zeroize::Zeroize::zeroize` on its 32-byte buffer. This is
    ///     a compile-time-pinned invariant (the `Drop` impl exists on
    ///     the type) and is independently witnessed by the
    ///     `g0_1_secret_key_has_no_debug` test plus the `Drop` impl
    ///     present in `keystore.rs`. This function inherits that
    ...
    fn g0_4_old_key_zeroized() {
        ...
        // does not contain the verbatim literal (avoiding a
        // self-tripping canary).
        ...
        // dropped at scope end and zeroized on drop.
        ...
        drop(new_scoped); // Drop → zeroize on scope exit.

        let old_scoped_again = old
            .unseal(old_pass)
            .expect("old still usable post-rotation");
        drop(old_scoped_again); // Drop → zeroize on scope exit.
    }

    /// `g0_4_e2e_sign_after_rotation` (line 1182).
    ///
    /// End-to-end witness across the wallet rotation and signing
    /// surface (`create_encrypted` / `unseal`, `sign_move_tx`,
    /// `sign_message`, `rotate_key`):
    ///
    ...
    ///     is preserved across rotation: a
    ...
    fn g0_4_e2e_sign_after_rotation() {
        ...
        // line 1181). Re-unseal OLD and sign again; the
        ...
        // separation preserved post-rotation.
    ...
    /// [`SealedKeypair::create_encrypted`] —
    ...
    /// Witness reuse-only surface: this module adds ONE public type
    /// and ONE public function. `ScopedSecretKey` re-exported from
    /// the keystore module is used internally; here we pin that the
    /// test module can still construct one via the
    /// `#[cfg(test)] pub(crate)` constructor, proving the
    /// cross-module test helper surface remains stable after this
    /// addition.
    #[test]
