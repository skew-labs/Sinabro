//! Stage B sign personal chunk message (atom #150 · B.4.4, WorkPackage
//! B-WP-03).
//!
//! Canonical OUT (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.4): sign a Stage B chunk
//! digest under the atom #89 chunk-sign domain, producing a 64-byte ed25519
//! [`SignatureBytes`] that the reused
//! [`verify_stage_b_chunk`](mnemos_b_memory::chunk_signature::verify_stage_b_chunk)
//! accepts. This is the *sign* half of the chunk-signature seam the #89
//! verifier doc reserved "the production sign path" for.
//!
//! # Madness invariants (§4.4 / atom #150)
//!
...
/// Sign a Stage B [`ChunkDigest32`] under the atom #89 chunk-sign domain with
...
/// already a validated 32-byte commitment (atom #86).
#[must_use]
...
mod tests {
    ...
    /// the atom #86 / #89 test helper — a `content` body, genesis parent, no
    ...
    /// then pair it with its derived testnet address (atom #149).
    ...
    /// transaction-data domain (atom #34 `sign_move_tx`) over the SAME digest
