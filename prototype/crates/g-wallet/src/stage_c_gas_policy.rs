//! Stage C Gas Station policy schema.
//!
//! Primary types: [`GasStationPolicy`], [`GasSponsorMode`],
...
//! # Reuse
//!
//! * [`ObjectId`](mnemos_d_move::types::ObjectId),
//!   [`GasBudgetMist`](mnemos_d_move::types::GasBudgetMist) — the package id
//!   and gas-budget unit are the d-move types, not re-minted.
//! * The threat model this policy enforces.

use blake2::{Blake2b, Digest, digest::consts::U32};
use mnemos_d_move::types::{GasBudgetMist, ObjectId};

/// Domain separator for the policy hash (bound by the signer envelope's
/// `policy_hash_32`).
const POLICY_HASH_DOMAIN: &[u8] = b"mnemos.stage_c.gas_station_policy.v1";

/// Whether/how a sponsor pays for a transaction.
...
/// A function the Gas Station may sponsor. Deny-by-default: only these
...
/// Why a Gas Station sponsorship request was rejected.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
...
/// The official-trust verdict for a sponsor.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
...
/// policy and evidence schema, with an expiry epoch.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
...
/// The deny-by-default Gas Station policy.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
...
impl GasStationPolicy {
    ...
    /// `Blake2b-256` over the canonical byte form — the value a signer
    /// envelope binds as `policy_hash_32`.
...
mod tests {
    ...
    /// changes when any field changes (binds the signer envelope).
    #[test]
