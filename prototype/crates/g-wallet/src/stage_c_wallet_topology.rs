//! Stage C cold treasury / hot sponsor topology (C-WP-06B · atom #228 · C.2.9).
//!
//! Canonical OUT: the cold-treasury / hot-sponsor wallet topology.
//!
//! # Madness invariants (atom #228)
//!
...
//!   `&`[`MultisigRoster`] (atom #211, `threshold >= 2` by construction) and
...
//! * **reuse: A `GasBudgetMist`** — [`mnemos_d_move::types::GasBudgetMist`]
//!   (`d-move/src/types.rs:109`); the cap fields are typed gas units, not raw
//!   `u64`.
//! * **reuse: #211 `MultisigRoster`** —
//!   [`crate::stage_c_multisig::MultisigRoster`]; the cold treasury binds to its
//!   `signer_hash`. No parallel roster is minted.

use mnemos_d_move::types::{GasBudgetMist, SuiAddress};

use crate::stage_c_multisig::MultisigRoster;

/// The cold-treasury / hot-sponsor wallet topology (canonical OUT).
...
pub struct ColdHotTopology {
    ...
    /// (atom #211 `MultisigRoster::signer_hash`).
...
impl ColdHotTopology {
    ...
    /// roster (atom #211, `threshold >= 2`), so a single-key cold treasury is
