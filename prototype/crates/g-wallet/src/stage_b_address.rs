//! Stage B testnet address derivation.
//!
//! Derives a Stage A
//! [`SuiAddress`] from a 32-byte ed25519 signing public key, and maps that key
//! to / from the Stage A [`SignaturePlaceholderV1::public_key`] field. This
//! is the public-key → address binding seam the owner module
//! explicitly deferred to "the d-move binding seam"
...
//! * **No owner⇔key equality is asserted at construction.** Like the
//!   owner module, [`owner_binding`] carries the derived owner address and the signing key
//!   side-by-side; the binding is a typed carrier, and the *derivation* (this
//!   module) is what makes the owner match the key.
//!
//! # Reuse map
//!
//! * [`SigningPublicKey`] / [`OwnerPublicKeyBinding`]
//!   (`b-memory/src/owner.rs:78` / `:134`).
//! * Stage B d-move binding types (the [`SuiAddress`] this
//!   seam produces feeds `MemoryRootAnchorArgs` owner fields downstream).
//! * The scoped secret whose public key flows through this
//!   address seam after signing.
...
/// one scheme Stage B's bot wallet uses.
...
/// for every other length — no canonical error variant is minted (following
/// the same reject-as-predicate pattern as the owner module).
#[must_use]
...
/// as a Stage A [`OwnerPublicKeyBinding`]. Unlike
