//! Sealed Sui keypair with at-rest authenticated encryption.
//!
//! Canonical shape:
//!
//! ```rust,ignore
...
//!
//! ## Disparity from prose ("age encryption")
//!
//! The prose names the `age` crate; the canonical struct
...
/// primitive choice is recorded in one place.
type Pbkdf2HmacSha256 = Hmac<Sha256>;

/// Byte width of the at-rest PBKDF2 salt.
pub const KDF_SALT_BYTES: usize = 16;

/// Byte width of the stored nonce surface. The full
...
/// Sui signature-scheme flag prefix for ed25519 addresses.
...
/// no extra columns. The plaintext secret never
/// reaches disk.
///
...
/// serialisation fails at compile time. Accessors are
/// intentionally minimal: callers that need the bytes for downstream
/// signing borrow `&[u8; 32]` and let `Drop` run when the
/// scope closes.
...
impl ScopedSecretKey {
    ...
    /// `ScopedSecretKey` so deterministic signing test vectors
...
/// Errors produced by the sealed-keystore surface.
...
///
/// `Sign` and `KeyRotation` are declared here but their emission sites
/// land in the signing and rotation modules; they are
/// declared-but-not-yet-emitted at this point.
#[derive(Debug)]
pub enum WalletError {
    ...
    /// or truncated blob.
    Decrypt,
    /// Reserved for the signing paths (`sign_move_tx` / `sign_message`).
    /// Declared here; not emitted by this module.
    Sign,
    /// Reserved for key rotation (`rotate_key`). Declared here; not
...
mod tests {
    ...
    /// Witness that the canonical struct widths match the declared shape.
    #[test]
