//! Stage C Gas Station dry-run / effect-shape checker (C-WP-05 · atom #218 ·
//! C.1.17).
//!
//! Canonical OUT (§4.3): [`GasStationDecision`].
//!
//! # Madness invariants (atom #218)
//!
...
//!   (atom #185 dry-run) must equal the expected shape for the claimed
//!   function; otherwise [`GasStationRejectReason::EffectShape`].
//!
//! # Reuse map (atom contract)
//!
//! * **reuse: #185** — [`EffectDelta`](mnemos_d_move::stage_c_effect_delta::EffectDelta).
//! * **reuse: #217** — [`GasStationPolicy`](crate::stage_c_gas_policy::GasStationPolicy)
//!   and the §4.3 reject/trust enums.
//! * **reuse: A [`SuiCallBuilder`](mnemos_d_move::sdk::SuiCallBuilder)** — the
//!   typed Move-call routing record; its `to_dry_run_bytes` is the decode step.
//! * **reuse: [`StageCTraceLink`](mnemos_a_core::trace::StageCTraceLink)** — the
//!   §4.0 trace stamp carried on the decision.
