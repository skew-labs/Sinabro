//! Stage B testnet tx submitter.
//!
//! A testnet-only submit
//! wrapper for a signed PTB. Design intent: the submitter is disabled by
//! default and only used once explicitly enabled; the dry-run path remains
//! primary.
//!
//! # Invariants
//!
...
//!   which is the seam a future live-enabled integration calls — the default
...
//!   tests, a live transport in a future integration). The byte-stable
//!   dry-run carrier (`to_dry_run_bytes`) stays the primary,
...
//! # Reuse map
//!
//! * The testnet-only signed call ([`SignatureBytes`]) this
//!   submitter would wrap.
//! * The testnet endpoint / network guard posture.
...
/// explicit, testnet-gated seam a live-enabled integration uses.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
...
impl StageBSubmitter {
    ...
    /// live-enabled integration calls; the default path never reaches it.
    ///
    /// # Errors
    ///
