//! Stage B testnet key rotation (atom #153 · B.4.7, WorkPackage B-WP-03).
//!
//! Canonical OUT (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.4): a testnet key rotation
//! report. The madness spec: "rotation changes owner key, zeroizes old scoped
//! secret, and records audit trace." Built on the Stage A §4.G
//! [`rotate_key`](crate::rotate::rotate_key) (#36), wrapped under Stage B
//! testnet policy.
//!
//! # Madness invariants (§4.4 / atom #153)
//!
...
//!   leak secret material. (This is the explicit resolution of the WorkPackage
//!   physics `WALLET_SECRET_REDACTION_AMBIGUOUS` advisory for #153.)
//!
...
//! * Stage A §4.G: [`rotate_key`] / [`RotationReport`] (#36), and the #148
...
/// A redacted testnet key-rotation report (§4.4 canonical OUT).
///
...
///
/// The `trace` stamp links this rotation into the atom #81 trace chain; the
