//! Stage C signer isolation boundary (C-WP-06B · atom #227 · C.2.8).
//!
//! Canonical OUT: a signer boundary spec for a KMS / HSM / TEE / signing
//! daemon backend.
//!
//! # Madness invariants (atom #227)
//!
...
//!   [`SignerIsolationBoundary::admit`] only for a typed §4.2
//!   [`MainnetSignerEnvelope`] (atom #214) whose binding hash the caller
...
/// rest and (in the existing atom #34 path) unseals it to a
...
impl SignerIsolationBoundary {
    ...
    /// reuses the #214 envelope's own opaque-bytes refusal.
    #[inline]
    #[must_use]
    pub fn reject_opaque_request(payload: &[u8]) -> SignerBoundaryError {
        // Bind to the #214 refusal so the two boundaries agree.
