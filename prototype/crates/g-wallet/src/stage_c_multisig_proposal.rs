//! Stage C multisig proposal envelope.
//!
//! A mainnet proposal envelope bound to a package digest and a checklist hash.
//!
//! # Invariants
//!
...
//!   signer-set hash (reused, not re-minted); approving requires
...
//! # Reuse
//!
//! * [`signer_set_hash`](crate::stage_c_multisig::signer_set_hash) and
//!   [`MultisigRoster`](crate::stage_c_multisig::MultisigRoster). The binding
//!   hash is computed by the canonical hasher.
//! * The checklist hash is the `MainnetChecklist` evidence hash (a 32-byte
//!   value), referenced by hash only; no `k-devex` type is imported here.
...
pub struct MultisigProposalEnvelope {
    ...
    /// The mainnet-checklist evidence hash the proposal is gated on.
    pub checklist_hash_32: [u8; 32],
    /// The signer-set hash of the approving roster.
...
impl MultisigProposalEnvelope {
    ...
    ///
    /// The full `roster_signers` set is recomputed through the canonical
    /// hasher and compared against the envelope's `roster_hash_32`
