//! Stage B scoped secret under testnet policy (atom #148 · B.4.2,
//! WorkPackage B-WP-03).
//!
//! Canonical OUT (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.4): the Stage A §4.G
//! [`ScopedSecretKey`](crate::keystore::ScopedSecretKey) **reused directly**
//! under the Stage B testnet boundary — no parallel secret type is minted.
//! The madness spec ("no Debug, Display, Clone, Serialize; Drop zeroizes")
...
//! so downstream Stage B wallet atoms (#150 sign-chunk, #151 sign-tx, #153
//! rotate) import one spelling, and (b) **lock the structural invariant with
...
//!
//! * **reuse: #147** — the Stage B keystore
...
/// The Stage B testnet scoped secret key. A re-export of the Stage A §4.G
...
mod tests {
    ...
    // ----- Compile-time trait-absence lock (§4.4 madness spec) -------------
    //
    // These assertions are the atom's "compile-fail trait absence" test
