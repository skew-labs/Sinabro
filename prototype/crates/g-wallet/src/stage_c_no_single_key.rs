//! Stage C no-single-key mainnet guard (C-WP-05 · atom #215 · C.1.14).
//!
//! Canonical OUT: a compile/runtime guard blocking a single-key mainnet
//! publish/sign path.
//!
//! # Madness invariants (atom #215)
//!
...
//!   `roster.threshold` (`>= 2`, atom #211) **distinct** approvers, each a
...
//!
//! * **reuse: A [`ScopedSecretKey`](crate::keystore::ScopedSecretKey)** — the
//!   per-signer secret remains the atom #33 zeroizing, non-`Debug`/`Clone`
//!   type. One `ScopedSecretKey` yields at most one approval, which is below
//!   the `>= 2` threshold by construction.
//! * **reuse: #211–#214** — the roster
//!   ([`MultisigRoster`](crate::stage_c_multisig::MultisigRoster)) and its
//!   signer-set hash; the authority is bound to the roster hash so a
...
pub struct MainnetSigningAuthority {
    /// The atom #211 signer-set hash of the roster that authorised the action.
