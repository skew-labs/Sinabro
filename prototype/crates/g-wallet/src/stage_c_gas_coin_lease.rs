//! Stage C gas coin lease pool.
//!
//! Primary type: [`GasCoinLease`].
//!
//! # Invariants
//!
...
//! # Reuse
//!
//! * [`ObjectId`](mnemos_d_move::types::ObjectId) — the gas coin id is the
//!   d-move canonical object id, not re-minted.
//!
...
/// A lease binding one sponsor gas coin to one inflight transaction.
...
/// An `O(1)`-lookup pool enforcing one-inflight-tx-per-coin.
