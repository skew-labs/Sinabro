...
//!   is unrepresentable.** The field's type
...
//! * **The gas cap is a typed unit.** `max_gas_mist` reuses the
...
//! # Reuse map
//!
//! * [`StageBNetwork`] (`b-memory/src/network.rs:73`). No
//!   second network type is minted here.
//! * [`GasBudgetMist`] (`d-move/src/types.rs:118`). No
//!   second gas-budget unit is minted here.
//!
//! `StageBWalletError` is the package-wide error type. Its emission
//! sites are spread across the package's modules; the variants are declared here so
...
/// Stage B testnet wallet configuration (canonical output).
///
...
/// Package-wide Stage B wallet error (canonical output).
///
...
/// Map the Stage A [`WalletError`](crate::keystore::WalletError) onto the
