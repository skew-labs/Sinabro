//! `mnemos-m-agent::tool_schema` — atom #24 · M.0.4 — lazy tool schema +
//! compact tool registry.
//!
//! Canonical OUT (§4.M — see ATOM_PLAN line 596-600 + atom #24 line
//! 1047-1055):
//!
//! - [`ToolId`] — `#[repr(transparent)]` newtype over `u16`. MOVED
//!   from the atom #21 `m-agent::llm` forward-decl placeholder (atom
//!   #22 [`crate::sse::SseDelta`] MOVE family pattern; atom #23
//!   [`crate::turn::TurnUsage`] MOVE family pattern). 16-bit width
//!   per §4.M line 597 commitment — up to 65 536 tools per agent
//!   instance (Hermes 96-flat-prompt baseline, §9.5). Inner `pub u16`
//!   preserved verbatim for atom #21 surface compatibility (atom #23
//!   `TurnUsage` rationale: no invariant beyond "name newtype over
//!   `u16`").
//! - [`ToolRegistry`] — compact fixed-slot registry mapping
//!   [`ToolId`] → declared schema byte width. New canonical home at
//!   atom #24. [`TOOL_REGISTRY_CAPACITY`] slots (16 in Phase 0:
//!   §10 builtin whitelist is 5 — read / write / exec / chat /
//!   memory — so 16 leaves headroom for the atom #39 e-skill
...
//!   protected at the type boundary (atom #3
...
//!   fields (no invariant beyond "two independent values"; atom #23
//!   [`crate::turn::TurnUsage`] precedent for public fields without
//!   invariant). Pre-measured byte width — the actual JSON
//!   encoder lives in atom #39 e-skill (manifest TOML → canonical
//!   JSON schema).
//! - [`LazyToolSchema<'a>`] — MOVED from atom #21 forward-decl
//!   placeholder. Borrows two slices: the declared [`ToolId`] slice
//!   AND the registry reference. Private fields + `pub const fn`
//!   constructor + accessors (atom #3
//!   [`crate::turn::TurnState`] invariant-protection precedent —
//!   "declared tools must reference the registry passed at
//!   construction" is enforced by signature, not runtime check).
//!   The §4.M 광기-사양 — "선언된 tool만 prompt 진입" — is
//!   encoded structurally: only the `declared` slice contributes
...
//!   `Result`). §4.M signature pin: free function (not method),
//!   `u32` return (not `Result`).
...
//!   `tool_registry.*` (atom #15 / #20 / #21 precedent).
...
//!   fixture used by the atom #21 `m-agent::llm` tests after the
//!   atom #24 surface change closed the public `declared` field;
...
//!   assertion below.
//!
//! ## Disabled tool = 0 bytes (광기-사양 structural encoding)
//!
...
//! will use it on this turn. The §9.5 token-saving spine inverts
//! that: the `declared` slice on each request is the per-turn
//! subset; tools absent from `declared` contribute 0 bytes to the
//! serialized schema. [`serialized_tool_bytes`] proves this
//! measurably — the `m0_4_disabled_tool_absent_from_bytes` test
//! demonstrates that removing a tool from `declared` reduces the
//! byte count exactly by that tool's slot width.
//!
//! ## MOVE family pattern (atom #22 / #23 precedent)
//!
//! Atom #21 shipped `LazyToolSchema<'a>` and `ToolId` as
//! forward-decl placeholders in `m-agent::llm`. Atom #24 promotes
//! their canonical homes to this module:
//! [`crate::llm`] now re-imports them via
//! `use crate::tool_schema::{LazyToolSchema, ToolId};` so the
//! public re-export path (`mnemos_m_agent::{LazyToolSchema, ToolId}`
//! via [`crate`] `lib.rs`) stays bit-for-bit stable.
//!
//! ## Carve-outs (Session 2 ACCEPT/RAISE)
//!
//! 1. **`LazyToolSchema` fields become private at atom #24.** §4.M
//!    line 598 shows the canonical signature with private fields;
//!    atom #21 forward-decl ran with `pub declared` for the
//!    placeholder surface (atom #21 carve-out documented this). Atom
//!    #24 promotes the canonical private surface so the "declared
//!    tools reference the registry at construction" invariant is
//!    enforced by [`LazyToolSchema::new`]. The atom #21 inline tests
//!    in [`crate::llm`] are updated to use the constructor;
//!    breaking change is scoped to the in-crate test surface — no
//!    external Phase 0 consumer of `mnemos-m-agent` exists.
//! 2. **`ToolId` keeps `pub u16` inner field.** Atom #23
//!    [`crate::turn::TurnUsage`] rationale ("no invariant beyond
//!    inner value") applies: `ToolId(u16)` is a name newtype with
//!    no constraint, so public inner is correct. The 16-bit width
//!    pin lives in the [`_TOOL_ID_SIZE_IS_2`] const block.
//! 3. **`ToolRegistry` capacity = 16 slots in Phase 0.** §4.M
//!    says "고정 슬롯, 컴팩트"; the constant is exported as
//!    [`TOOL_REGISTRY_CAPACITY`] and pinned by
//!    [`_REGISTRY_SIZE_IS_132`]. Future atoms can grow it with a
//!    new compile-time width pin.
//! 4. **`serialized_tool_bytes` is total-with-saturating, not
//!    fail-on-unknown.** §4.M line 600 pins the signature
//!    `-> u32` (no `Result`). Unknown declared ids contribute 0
//!    bytes — equivalent to "rejected from prompt". The explicit
//!    rejection surface is [`validate_declared`] (returns
//!    `Result<(), ToolSchemaError>`). The
//!    `m0_4_unknown_tool_id_rejected` test exercises
//!    [`validate_declared`]; production code paths can call
//!    validate-then-measure to fail fast or measure-only to
//!    compress on unknowns.
//! 5. **Per-slot byte width is provider-pre-measured.** Atom #24
//!    does NOT include a live JSON serializer — the registry
//!    stores pre-measured byte widths per [`ToolId`]. The actual
//!    encoder is atom #39 e-skill territory (manifest TOML →
//!    canonical JSON schema bytes). Atom #24's "schema bytes
//!    measured" test pins the structural carrier, not a JSON
//!    encode.
//! 6. **`Default` impl on [`ToolRegistry`] forwards to `new()`.**
//!    Provided for ergonomic fixtures; the empty-registry shape
//!    is intentional (matches [`EMPTY_TOOL_REGISTRY`]). The same
//!    accidental-zero risk applies whether a caller writes
//!    `ToolRegistry::new()` or `ToolRegistry::default()`, so
//!    `Default` does not add a new failure mode.
//! 7. **G-BENCH numerical regression deferred.** §4 master "schema
//!    bytes are measured (4× 압축 목표)" measurement axis is
//!    structurally encoded at atom #24 via the
//!    `m0_4_schema_bytes_measured` test (cargo test assertion on
//!    a known-fixture sum). A criterion-based bench would
//!    additionally regress the cumulative bytes across the full
//!    builtin whitelist; that bench (`benches/tool_schema.rs`)
//!    is deferred to atom #39 e-skill where the actual JSON
//!    encoder lands and the 4× compression target becomes
//!    measurable against a Hermes-shaped baseline.

#![deny(missing_docs)]

// ===========================================================================
// 1. Compile-time width pins (atom #21 / #23 precedent)
// ===========================================================================

/// `ToolId` width pin. `#[repr(transparent)]` over `u16` ⇒ exactly
/// 2 bytes. Any future widening (e.g. `u32` for "more than 65 536
/// tools per agent") would diverge from §4.M line 597 — the build
/// fails here first.
...
/// Compact fixed-slot capacity for [`ToolRegistry`]. Phase 0
/// builtin whitelist (§10 master plan) is 5 tools — read / write
/// / exec / chat / memory; 16 slots leave headroom for the atom
/// #39 e-skill manifest without dynamic allocation.
...
/// MOVED from atom #21 [`crate::llm`] forward-decl placeholder
/// (atom #22 / #23 MOVE family pattern). 16-bit width per §4.M
/// line 597 commitment — up to 65 536 tools per agent instance.
///
/// Inner `pub u16` preserved verbatim from atom #21 (atom #23
/// `TurnUsage` rationale: "no invariant beyond inner value, so
/// public inner is correct").
...
///
/// No invariant beyond "two independent values" (atom #23
/// [`crate::turn::TurnUsage`] precedent), so fields are `pub`. The
...
///
/// Private fields + `pub const fn` accessors (atom #3 / #23
/// invariant-protection precedent). The owning invariants are:
///
...
/// (`&EMPTY_TOOL_REGISTRY`) is the fixture used by the atom #21
/// [`crate::llm`] tests after the atom #24 surface change closed
...
/// namespaced under `tool_registry.*` (atom #15 / #20 / #21
/// precedent).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[non_exhaustive]
...
/// Lazy tool schema view. MOVED from the atom #21 forward-decl
...
/// accessors (atom #3 [`crate::turn::TurnState`]
...
///
/// §4.M signature pin: free function (not method), `u32` return
/// (not `Result`). The G-AGENT measurement axis (atom #24
/// criterion line 1052) reads this value directly.
...
mod tests {
    ...
    // ---- ATOM_PLAN line 1051 verbatim tests --------------------------------
    ...
    /// tool; the §9.5 token-saving spine pays only for the
    ...
    /// §4 master plan "4× 압축 목표" measurement axis — the
    /// criterion bench at atom #39 will compare this value
    ...
    fn m0_4_schema_bytes_measured() {
        ...
        // The value is a u32 (typed unit; pinned by signature
        // §4.M line 600 and the `_TOOL_ID_SIZE_IS_2` width
        // assertion above).
        let _: u32 = total;
    ...
    /// validator surface per §4.M signature pin.
    #[test]
    ...
    // ---- Scaffolding tests (atom #21 / #22 / #23 precedent) ----------------
