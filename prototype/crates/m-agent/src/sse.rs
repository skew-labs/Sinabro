//! `mnemos-m-agent::sse` — zero-alloc SSE delta parser.
//!
//! Public API:
//!
...
//!
//! An earlier revision shipped a forward-decl placeholder for [`SseDelta`] in
//! `m-agent::llm` (a `#[non_exhaustive]` `Pending(&'a [u8])` 1-variant stub)
//! so [`crate::llm::DeltaSink::on_delta`] had a typed argument. This module
//! now owns the canonical home (`m-agent::sse`); it **moves**
//! the type here and grows the variant list to its full shape. `llm.rs`
...
//! ## Wire-format scope
//!
//! The parser handles OpenAI-family SSE chat-streaming frames
//! (OpenRouter, DeepSeek, Anthropic chat-compat, OpenAI):
//!
//! ```text
...
//!    returned verbatim (the downstream `DeltaAccumulator`
...
//!    accumulator re-assembles across frames anyway).
...
// ===========================================================================
// 2. SseDelta — canonical home (moved from m-agent::llm)
// ===========================================================================

/// Parsed SSE delta from an OpenAI-family chat-streaming frame.
/// `Copy` and `#[non_exhaustive]` — variants may grow later
/// (e.g. an `Error` carrier for inline `{"error":...}` frames in
/// the tool loop) without breaking exhaustive matches.
///
...
///   (`data: {"usage":{...}}\n\n`). The downstream `DeltaAccumulator`
...
impl SseParseError {
    ...
    /// `sse.*` (mirroring `move_bind.*`, `sui_call_build.*`, `llm.*`).
    #[inline]
...
impl<'a> SseDeltaParser<'a> {
    ...
    ///
    /// The method name `next` is pinned by the canonical
    /// signature. It does **not** implement
...
/// bounded to `u8::MAX` by the canonical signature).
#[inline]
...
mod tests {
    ...
    /// `cached_tokens` split required by cache-hit measurement.
    #[test]
    ...
    // ---- proptest (identical delta sequence across arbitrary chunk splits) --
