//! Atom #28 · M.0.8 — per-call input-token envelope fixture and named
//! gate tests.
//!
//! §9.5 비협상 목표: per-call input tokens MUST stay at or below
...
//! `HERMES_TOKENS_BASELINE` (32,142 tokens). MNEMOS_ATOM_PLAN.md atom
//! #28 (line 1087) names this atom the "토큰 게이트" — the canonical
...
//!   (ATOM_PLAN line 1091, verbatim) run via `cargo test` as the
//!   G-AGENT named-test gate. They assert the envelope on the same
//!   `measured_input_envelope` the bench measures, so a regression that
//!   blows the envelope fails `cargo test` loudly without needing
//!   `cargo bench` to run first.
//!
//! * `prototype/crates/m-agent/benches/tokens.rs` (G-BENCH harness)
//!   reuses this fixture for its criterion run (input-envelope
//!   computation throughput) and for the `MNEMOS_BENCH_EMIT_BASELINE=1`
//!   JSON-commit emitter that writes the CI regression baseline.
//!
//! Reuse triangle (ATOM_PLAN atom #28 line 1094):
//!
//! * atom #24 / M.0.4 — [`crate::tool_schema::LazyToolSchema`] +
...
//! * atom #25 / M.0.5 — [`crate::cache::plan_cache_breakpoints`] +
...
//! (the §9.5 envelope is a hard CI gate, not a model-specific
//! prediction; future atoms can swap in a real tokenizer behind a
...
// ===========================================================================
// 1. Constants — §9.5 envelope, reduction ratio, baseline, tokenizer pin
// ===========================================================================

/// Hermes (96-tool flat-prompt) per-call input-token baseline.
///
/// Source pin: MNEMOS_ATOM_PLAN.md atom #28 line 1089 ("Hermes 32,142
/// 토큰 baseline vs Mnemos 콜당 입력 토큰 측정"). Frozen as the CI
/// regression denominator — changing this value rewrites the §9.5
/// reduction-ratio gate semantics and MUST be ratified by the
/// arch-source ATOM_PLAN edit, not a code-only change.
pub const HERMES_TOKENS_BASELINE: u32 = 32_142;

/// §9.5 hard cap: per-call input tokens MUST be ≤ this value.
///
/// Source pin: MNEMOS_ATOM_PLAN.md atom #28 line 1092 ("이 atom이
/// 토큰 게이트 — 콜당 입력 ≤5,000 절대상한 + 회귀 차단") + Phase 0
/// completion gate line 1349 (master plan §9.5 "콜당 입력 토큰
/// ≤5,000"). Enforced by [`m0_8_input_tokens_under_5000`].
pub const MNEMOS_INPUT_TOKENS_CAP: u32 = 5_000;

/// §9.5 비협상 reduction ratio: Hermes baseline / Mnemos measured.
///
/// Source pin: MNEMOS_ATOM_PLAN.md atom #28 title line 1087
/// ("≥10× 감소 실측"). With [`HERMES_TOKENS_BASELINE`] = 32,142 this
/// implies measured ≤ 3,214 — the strictest of the two gates.
/// Enforced by [`m0_8_vs_hermes_baseline_10x`].
...
///
/// * Phase 0 has zero LLM network deps (G-PRIVACY by construction).
/// * The §9.5 gate is a budget envelope, not a model-specific token
///   prediction — a 10–20% over-estimate is the SAFE direction for a
///   hard cap.
/// * Future atoms can swap in a real tokenizer behind a feature flag
...
/// 4 ≈ 1,125 tokens. With the cache-breakpoint plan (atom #25) the
...
/// placeholder pattern from atom #24 `five_tool_registry`). Pre-
...
pub const DECLARED_TOOL_SCHEMA_BYTES: &[(ToolId, u32)] = &[
    ...
    (ToolId(8), 200), // kill (§10.1 runaway-stop)
];
...
/// Cache-breakpoint planner cap (atom #25
...
/// the surface is bench-and-test fixture only (atom #23
...
pub struct InputEnvelopeMeasurement {
    ...
    /// the §9.5 gate compares against [`MNEMOS_INPUT_TOKENS_CAP`].
    pub estimated_input_tokens_u32: u32,
    /// Cache-breakpoint planner output count (atom #25 cap).
    pub cache_breakpoints_u8: u8,
}

/// Measure the canonical per-call input envelope using the reuse
/// triangle of atoms #24 / #25 / #27. Deterministic, allocation-free
/// at the measurement layer (the only allocation is the local
/// `ToolRegistry` carrier, which is a fixed-size array on the stack
/// per atom #24).
#[must_use]
...
// ===========================================================================
// 4. Named gate tests — ATOM_PLAN line 1091 verbatim
// ===========================================================================
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // m0_8_input_tokens_under_5000 — §9.5 absolute cap
    // -----------------------------------------------------------------------

    /// ATOM_PLAN atom #28 line 1091, test #1 verbatim. Asserts that
    /// the measured per-call input-token envelope stays at or below
    /// [`MNEMOS_INPUT_TOKENS_CAP`] (5,000) — the §9.5 hard cap.
    ...
    // -----------------------------------------------------------------------
    // m0_8_vs_hermes_baseline_10x — §9.5 비협상 reduction ratio
    // -----------------------------------------------------------------------

    /// ATOM_PLAN atom #28 line 1091, test #2 verbatim. Asserts that
    /// the Hermes baseline divided by the Mnemos measured envelope
    /// is at least [`MIN_REDUCTION_RATIO`] (10×). With the baseline
    /// frozen at 32,142 tokens the measured envelope must be ≤ 3,214
    /// — the strictest of the two §9.5 gates. The bench harness
    ...
    // -----------------------------------------------------------------------
    // Reuse-triangle sanity — atom #27 CostLedger projection composes
    // -----------------------------------------------------------------------

    /// The atom #27 [`CostLedger::project_usd_micros`] projection
    /// over the measured envelope at a representative price table is
    /// finite (saturating arithmetic, no panic) and is strictly
    /// reduced by the cached-prefix discount (carve-out 5). This is
    /// the BUILD_STATE atom #27 carry-forward L49 hook — the bench's
    /// JSON commit reports both the token envelope and `$/turn`.
    #[test]
    ...
    /// under-counts (atom #24 `serialized_tool_bytes` docs: unknown
    ...
    /// The Hermes baseline and the §9.5 cap are frozen constants;
    /// pin them so a code-only edit to either value (without an
    /// ATOM_PLAN ratification) fails this test.
    #[test]
