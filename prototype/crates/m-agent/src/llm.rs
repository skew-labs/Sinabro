//! `mnemos-m-agent::llm` — atom #21 · M.0.1 — LLM client trait + 메시지 타입.
//!
//! Canonical OUT (§4.M — see ATOM_PLAN line 580-585 + atom #21 line 1007-1015):
//! - [`Role`] (`#[repr(u8)]`) — 4-variant System=1 / User=2 / Assistant=3
//!   / Tool=4 wire-tag enum with const [`Role::tag`] / [`Role::from_tag`]
//!   round-trip (atom #2 / #7 / #15 class-discriminant precedent).
//! - [`TokenCount`] — `#[repr(transparent)]` newtype over `u32`. Typed
//!   unit; the unit-confusion barrier between "token count" and any
//!   other `u32` width (cache bytes, schema bytes, etc.). Pairs at
//!   atom #26 / #27 with `UsdMicros` for cost ledgering.
...
//!   slice plus the (forward-pending) tool schema and cache plan.
//!   §4.M canonical signature; the `tools` / `cache_plan` fields
//!   reference forward-decl placeholder types whose canonical homes
//!   are atom #22 / #23 / #24 / #25 (see §4 below).
...
//!   construction (OpenRouter → DeepSeek default per §9 master
//!   plan).
...
//!   in [`crate::sse`] as of atom #22).
//!
//! ## Forward-decl placeholders (atom #20 disparity precedent)
//!
//! The §4.M pseudocode references several types whose canonical
//! definitions belong to later atoms in the M domain (the
//! per-atom OUT lists for #22-#25 own them):
//!
//! - `SseDelta` — canonical home [`crate::sse`] (atom #22 ·
//!   M.0.2). **Resolved at atom #22**: the type moved out of this
//!   module into `m-agent::sse` and grew the variant list to the
//!   §4.M shape (`ContentText` / `ToolCallArgs` / `Done` /
...
//!   `lib.rs`) stays stable.
//! - [`TurnUsage`] — canonical home [`crate::turn`] (atom #23 ·
//!   M.0.3). **Resolved at atom #23**: the type moved out of this
//!   module into `m-agent::turn` (atom #22 `SseDelta<'a>` MOVE
//!   family pattern). This module imports the canonical symbol
//!   via `use crate::turn::TurnUsage;` so
//!   [`LlmClient::stream_chat`]'s return type and the public
//!   re-export path (`mnemos_m_agent::TurnUsage` via `lib.rs`)
//!   stay stable. Atom #23 layers `TurnState` /
//!   `DeltaAccumulator` on top of the same carrier.
//! - [`LazyToolSchema`] — canonical home [`crate::tool_schema`]
//!   (atom #24 · M.0.4). **Resolved at atom #24**: the type
//!   moved out of this module into `m-agent::tool_schema` and
//!   grew the canonical 2-field shape (`declared` + `registry`
//!   per §4.M line 598). This module imports the canonical
//!   symbol via `use crate::tool_schema::LazyToolSchema;` so
//!   [`LlmRequestView::tools`] keeps its typed field and the
//!   public re-export path (`mnemos_m_agent::LazyToolSchema`
//!   via `lib.rs`) stays stable. Atom #21 atom carve-out 1
//!   noted the future field-visibility close; atom #24 made
//!   `declared` private, with read access through
//!   `LazyToolSchema::declared()` (in-crate test surface
//!   updated; no external Phase 0 consumer).
//! - `ToolId` — canonical home [`crate::tool_schema`] (atom
//!   #24 · M.0.4). **Resolved at atom #24**: MOVED with
//!   [`LazyToolSchema`]; same MOVE family pattern as atom #22
//!   `SseDelta<'a>` / atom #23 `TurnUsage`. Public re-export
//!   path (`mnemos_m_agent::ToolId` via `lib.rs`) preserved.
//! - `CacheBreakpointPlan` — canonical home [`crate::cache`]
//!   (atom #25 · M.0.5). **Resolved at atom #25**: MOVED to
//!   `crate::cache` alongside the canonical free function
//!   [`crate::cache::plan_cache_breakpoints`]; same MOVE family
//!   pattern as atom #22 `SseDelta<'a>` / atom #23 `TurnUsage`
//!   / atom #24 `LazyToolSchema<'a>` + `ToolId`. Public
...
//!
//! This is the same family as atom #2 (17 vs 13 enum variants),
//! atom #3 (12 vs 11), atom #7 (manual BCS encoder), atom #20
//! (4 canonical + 2 implementation-private SuiCallBuilder fields)
//! — recorded for Session 2 ACCEPT/RAISE via the BUILD_STATE
//! disparity carry-forward protocol.
//!
...
//!    `Copy` enum. Atom #26 (M.0.6 tool loop) will introduce
//!    the `From<LlmError> for MnemosError` (or the inverse)
//!    when the budget axis bridges the two. At atom #21 the
//!    m-agent crate has zero workspace deps — Cargo.lock
//!    unchanged.
//! 4. **`ControlFlow<()>` not `ControlFlow<LlmError>`.** §4.M
//!    pseudocode uses `ControlFlow<()>` so a sink can request
...
//!    later atom from drifting the signature.
//! 5. **Send + Sync NOT required.** The trait does not bound
//!    impls on `Send + Sync` — adding them would constrain a
//!    blocking single-threaded test mock (and the ATOM_PLAN
//!    §4.M signature is silent on auto-traits). Future atoms
...
// ===========================================================================
// 1. Compile-time width pins (atom #2 / #20 precedent)
// ===========================================================================
...
/// over `u32` ⇒ exactly 4 bytes. Pairing types (`UsdMicros` at
/// atom #27) follow the same width; any divergence here
...
/// request serialiser at atom #24 / #25.
///
/// The four variants exhaust the OpenAI-family chat schema as
/// of the §9 master plan provider survey (OpenRouter, DeepSeek,
...
impl Role {
    ...
    /// can be folded into compile-time constants (atom #22's
    /// parser uses it to dispatch SSE frame role tokens).
    #[inline]
...
/// m-agent crate. Pairs at atom #27 with `UsdMicros` (also a
...
/// the §9.5 daily budget cap (5_000 tokens per call) and
...
impl TokenCount {
    ...
    /// constants (atom #26's `DailyTokenBudget` cap default
    /// lives in a const).
    #[inline]
...
// ===========================================================================
// 5. Forward-decl placeholders (canonical homes — atom #22..#25)
// ===========================================================================

// `SseDelta` canonical home moved to [`crate::sse`] at atom #22
// (M.0.2). This module imports the canonical symbol via the
// top-level `use crate::sse::SseDelta;` so `DeltaSink::on_delta`
// keeps its typed argument and the public re-export path
// (`mnemos_m_agent::SseDelta` via `lib.rs`) stays stable. See the
// "Forward-decl placeholders" section above for the migration note.

// `TurnUsage` canonical home moved to [`crate::turn`] at atom #23
// (M.0.3). This module imports the canonical symbol via the
...
// is preserved bit-for-bit. See the "Forward-decl placeholders"
// section above for the migration note (atom #22 `SseDelta` MOVE
// family pattern).

// `LazyToolSchema` canonical home moved to [`crate::tool_schema`]
// at atom #24 (M.0.4). This module imports the canonical symbol
...
// stays stable. See the "Forward-decl placeholders" section above
// for the migration note (atom #22 / #23 MOVE family pattern).
//
// `ToolId` canonical home moved to [`crate::tool_schema`] at atom
// #24 (M.0.4) alongside `LazyToolSchema`. Same MOVE family
// pattern; public re-export path
// (`mnemos_m_agent::ToolId` via `lib.rs`) preserved.

// `CacheBreakpointPlan` canonical home moved to
// [`crate::cache`] at atom #25 (M.0.5) alongside
// `plan_cache_breakpoints`. Same MOVE family pattern as atom
// #22 `SseDelta<'a>` (`crate::sse`), atom #23 `TurnUsage`
// (`crate::turn`), and atom #24 `LazyToolSchema<'a>` / `ToolId`
// (`crate::tool_schema`). Public re-export path
...
///
/// §4.M canonical signature (line 583). The `tools` and
/// `cache_plan` fields reference forward-decl placeholders
/// (`LazyToolSchema` / `CacheBreakpointPlan`) — see the
/// module-level "Forward-decl placeholders" carve-out.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
...
/// labels namespaced under `llm.*` (atom #15 / #20 precedent).
///
...
///   tokens. Distinct from `Transport` so the tool loop at
///   atom #26 can apply a backoff policy without parsing
///   the cause.
/// - `BudgetExceeded` — the local `DailyTokenBudget` (atom
///   #26 / M.0.6) refused the charge; the LLM call was
///   aborted before any network egress. Pairs with the
///   `MnemosError::budget_exceeded(BudgetAxis::LlmTokens, …)`
///   bridge a future atom adds at the boundary.
/// - `Protocol` — the SSE stream parsed but violated the
///   chat-protocol contract (unexpected role, malformed
///   tool-call schema). Atom #22's parser surfaces structural
...
impl LlmError {
    ...
    /// single prefix (mirrors atom #15 `move_bind.*` and
    /// atom #20 `sui_call_build.*`).
    #[inline]
...
///
/// `&mut self` so an accumulator sink (atom #23's
...
mod tests {
    use super::*;
    // atom #24 surface change: LazyToolSchema fields are private; tests
    // construct via the new() constructor and point at EMPTY_TOOL_REGISTRY
    // for the empty-registry fixture (atom #24 carve-out 1). `ToolId` is
    ...
    impl LlmClient for MockLlmClient {
        fn stream_chat(
            ...
            // Atom #22: the mock drives the sink with `ContentText`
    ...
    // ---- ATOM_PLAN line 1011 verbatim tests --------------------------------
    ...
    fn m0_1_role_tag_roundtrip() {
        // Tag values are 1..=4, matching §4.M line 580.
    ...
    /// `m0_1_mock_client_honors_trait_contract` — the third
    /// ATOM_PLAN-named test ("mock client로 trait 계약
    /// 테스트"). Verifies the [`LlmClient`] + [`DeltaSink`]
    ...
    // ---- Scaffolding tests (atom #20 precedent) ----------------------------
    ...
    fn sse_delta_borrowed_text_flows_through_sink() {
        // Atom #22: `SseDelta` canonical home is now `crate::sse`.
