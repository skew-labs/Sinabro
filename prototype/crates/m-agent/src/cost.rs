//! `m-agent::cost` — atom #27 · `M.0.7` — cost telemetry with type-safe
//! USD micros + cached-token visibility + daily hard-cap gate.
//!
//! Closes the §9.5 token-saving spine: atom #21 supplied [`TokenCount`]
//! (typed-unit width for "LLM tokens"); atom #23 supplied
//! [`TurnUsage`] (per-turn 3-counter breakdown); atom #26 supplied
//! [`DailyTokenBudget`] (saturating ledger + refusal channel via
//! [`MnemosError::budget_exceeded`]). Atom #27 projects a [`TurnUsage`]
//! through a [`PriceTable`] into an accumulated [`UsdMicros`] cost on a
//! [`CostLedger`], and exposes [`CostLedger::try_charge_and_record`] as
//! the atomic "gate then record" sibling that ties the §9.5 prepaid
//! daily cap to the cost projection in one observable step.
//!
//! ## §4.M canonical signature (line 612-616)
//!
//! ```text
//! // M.0.7 cost.rs
...
//! ```
//!
//! ## 광기 사양 (atom #27 9-field — line 1080)
//!
//! - **Typed-unit barrier (단위혼동 0).** [`UsdMicros`] is a
//!   `#[repr(transparent)]` newtype over `u32` that mirrors the atom
//!   #21 [`TokenCount`] shape. The compiler refuses any silent coercion
...
//!
//! - **Cached-token visibility (절감 가시화).** [`CostLedger`]
...
//!   with `cached_tokens_u32 == 0`. The §9.5 cache-hit ratio
//!   (`cached / prompt`) is measurable straight off the ledger.
//!
//! - **Daily hard cap (§9.5 선불).** [`CostLedger::try_charge_and_record`]
//!   gates the record against a caller-supplied [`DailyTokenBudget`]
//!   (atom #26): the budget is charged `prompt + completion`
...
//!   atom #26 monotonicity invariant ("a refused charge does not
//!   mutate `spent_u32`") composes with atom #27's by-construction
...
//!   ever decrements a field. The width pins below let the verifier
//!   catch any future layout drift via compile-time failure, and the
//!   `m0_7_ledger_monotone` test proves the runtime invariant.
//!
//! ## Reuse surface (atom #27 9-field — line 1084)
//!
//! - [`TurnUsage`] (atom #23 / `M.0.3`) — the per-turn 3-counter
//!   carrier this ledger projects through a [`PriceTable`]. Public
//!   fields per atom #23's "no invariant beyond inner value" rationale.
//! - [`DailyTokenBudget`] (atom #26 / `M.0.6`) — the daily token cap
//!   gate this atom composes with for the §9.5 prepaid spec. Private
//!   fields + `pub const fn` accessors; the
//!   [`DailyTokenBudget::try_charge`] entry point owns the
//!   `spent_u32 <= cap_u32` invariant.
//! - [`TokenCount`] (atom #21 / `M.0.1`) — the `#[repr(transparent)]`
//!   newtype over `u32` that pairs with [`UsdMicros`] at the
//!   single-byte-vs-u32 unit-confusion boundary.
//! - [`MnemosError::budget_exceeded`] +
//!   [`mnemos_a_core::error::BudgetAxis::LlmTokens`] (atom #2 /
//!   `A.0.2`) — the canonical refusal channel for any LLM-spend
//!   budget axis. Re-used verbatim; no new variant introduced.
//!
//! ## Carve-outs (Session 2 ACCEPT / RAISE)
//!
//! 1. **[`CostLedger`] fields are private at atom #27.** §4.M line
//!    614 shows the signature without explicit `pub`; the atom #26
//!    [`DailyTokenBudget`] carve-out 2 precedent applies — fields
...
//!    `pub const fn` accessors. The atom #3 `TurnState` / atom #24
//!    `LazyToolSchema` precedent applies.
//!
//! 2. **[`PriceTable`] fields are public.** §4.M line 616 signature;
//!    `PriceTable` carries no invariant beyond "two independent
//!    `u32` rates" — the atom #23 [`TurnUsage`] rationale applies.
...
//!    is the daily-cap entry point.** §4.M line 615 pins `record` as
//!    `fn record(&mut self, …)` — no `Result` return. The 광기-사양
//!    "일/월 하드캡(§9.5 선불)" lives on the sibling
//!    [`CostLedger::try_charge_and_record`] method (carve-out beyond
//!    the §4.M signature), which integrates the
//!    [`DailyTokenBudget::try_charge`] refusal channel. This split
//!    matches atom #26's pattern: [`ToolLoop::check_iter_cap`]
//!    (infallible structural check) vs.
//!    [`DailyTokenBudget::try_charge`] (the `Result`-returning gate).
//!
//! 5. **Cost formula treats cached prompt tokens as free.** §4.M
...
//!    at zero. The §4.M 광기 says "(절감 가시화)" — the discount must
//!    be visible in the ledger sum, otherwise the cached counter is
//!    a useless decoration. Atom #27 implements (b) — cached portion
...
//!    twice; this atom does not bake the discount ratio into the
//!    canonical surface.
//!
...
//!    tokens. The master plan §9.5 daily cap is the daily LLM token
...
//!
//! 7. **No criterion bench.** Atom #27 9-field line 1082 records
//!    `criterion: N/A`. Cost projection is a few `u64` multiplies +
//!    a `u32` saturating sum — not a perf hot spot. The token
//!    bench is atom #28 (M.0.8 G-BENCH).
...
// ===========================================================================
// 1. Compile-time width pins (atom #21 / #24 / #25 / #26 precedent)
// ===========================================================================

/// `UsdMicros` width pin. One `u32` field, `#[repr(transparent)]` ⇒
/// exactly 4 bytes (alignment 4). Pairs with the atom #21
...
// ===========================================================================
// 2. UsdMicros — typed-unit newtype over u32 (mirror of atom #21 TokenCount)
// ===========================================================================
...
/// over `u32` matches the atom #21 [`TokenCount`] shape and pairs with
...
/// above the master plan §9.5 per-call cost envelope (5_000 tokens ×
...
/// each rate axis. Public fields per the atom #23 [`TurnUsage`]
...
///
/// §4.M signature (line 616). `#[repr(C)]` so the layout is stable
/// across rebuilds.
...
/// accessors + named constructor (atom #26 [`DailyTokenBudget`]
/// carve-out 2 precedent; atom #3 `TurnState` invariant-protection
/// precedent). The invariants:
///
...
///    invariant.
///
/// §4.M signature (line 614). `#[repr(C)]` so the layout is stable
...
pub struct CostLedger {
    ...
    /// tracked separately for §9.5 cache-hit-ratio visibility).
    cached_tokens_u32: u32,
...
impl CostLedger {
    ...
    /// per atom #23 [`TurnUsage`] documentation).
    #[inline]
    ...
    /// Cumulative USD micros charged. §4.M canonical signature
    /// (line 615).
    #[inline]
    ...
    /// §4.M canonical: record a turn's token usage at the supplied
    ...
    /// Carve-out 4 (§9.5 일/월 하드캡): atomic charge-then-record.
    ...
    /// the ledger mutates (atom #26's monotonic refusal invariant
    /// composes with atom #27's by-construction record-after-accept
    /// guarantee).
...
// ===========================================================================
// 5. Inline unit tests (atom #21 / #24 / #25 / #26 precedent)
// ===========================================================================
...
mod tests {
    ...
    // ---- ATOM_PLAN line 1081 verbatim tests -------------------------------
    ...
    /// zero (carve-out 5 — 절감 가시화). A turn with cached prefix
    ...
    /// `m0_7_daily_cap_enforced` — verifies the §9.5 prepaid daily
    /// hard cap (carve-out 4 — `try_charge_and_record`). A charge
