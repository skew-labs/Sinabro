//! `m-agent::loop_budget` — atom #26 · `M.0.6` — tool-loop iteration cap +
//! daily token budget.
//!
...
//! Atom #26 is structural: it ships the iteration-cap carrier ([`ToolLoop`]),
...
//!   (atom #2 / A.0.2). The driver translates that error into
//!   [`LoopStop::BudgetExceeded`] for the loop's terminal signal. The
//!   Telegram alert wiring (master plan §9.5) consumes the `MnemosError`
//!   `SafeErrorReport` projection at a future atom; this atom does not
...
//!   is the bytes-on-the-wire token count. Atom #23's `TurnUsage` is the
//!   production source.
//!
...
//!
//! ## Reuse surface (atom #26 9-field — line 1074)
//!
//! - `a-core::error::MnemosError::budget_exceeded` + `a-core::error::BudgetAxis::LlmTokens`
//!   (atom #2 / A.0.2) — canonical refusal channel.
//! - `m-agent::llm::TokenCount` (atom #21 / M.0.1) — typed-unit width pin
//!   (`#[repr(transparent)]` over `u32`); the unit-confusion barrier between
//!   "tokens" and any other `u32`-shaped width on the m-agent crate.
//! - `m-agent::tool_schema::ToolId` (atom #24 / M.0.4) — referenced via the
...
//! - e-skill atom #40 builtin whitelist — referenced as the dispatch
...
//!
//! 1. **No live driver / dispatch loop.** §4.M canonical OUT pins four
//!    types only (one struct, one enum, one struct, one method). Atom
//!    #26 does **not** define a `run_tool_loop` driver. The Stage M
//!    binding atom (post-M-trait-stack) wires the iteration counter +
//!    budget + tool dispatch into a closed `Result<TurnUsage,
//!    LoopStop>` loop. Atom #26 supplies the structural primitives
//!    only — same pattern as atom #21 (M.0.1 trait surface, no live
//!    transport) and atom #25 (M.0.5 plan struct, no live request
//!    client).
//!
//! 2. **`DailyTokenBudget` fields are private at atom #26.** §4.M
//!    line 607 shows the signature with `spent_u32` / `cap_u32`;
//!    atom #24's `LazyToolSchema` MOVE-family carve-out 1 promoted
...
//!    it (direct field mutation). The atom #3 `TurnState` / atom #24
//!    `LazyToolSchema` precedent applies.
//!
//! 3. **`ToolLoop` fields are public.** §4.M signature is verbatim;
//!    `max_iter_u8` carries no invariant beyond "fits in `u8`",
//!    which the type itself enforces. The atom #23 `TurnUsage`
//!    rationale ("no invariant beyond inner value, so public is
//!    correct") applies. A named constructor is supplied for the
//!    Phase 0 default (5); operators with non-default caps use
//!    [`ToolLoop::with_max_iter`].
//!
//! 4. **No `From<LlmError>` bridge yet.** §4.M line 1070 says
//!    "budget 초과 시 `MnemosError::budget_exceeded`(reuse a-core)
//!    → Telegram alert". The Telegram alert is the
//!    `SafeErrorReport`-projection consumer at a later atom.
//!    `LlmError::BudgetExceeded` (atom #21) and
...
//!    return type.
//!
//! 5. **No criterion bench.** Atom #26 9-field line 1072 records
//!    `criterion: N/A(루프 제어; 토큰은 측정)` — loop control is
//!    a per-iter `u8` compare and a saturating-add; neither is a
//!    perf hot spot worth a criterion sweep. The token-counting
//!    bench is atom #28 (M.0.8 G-BENCH, ≤5_000 tokens/call
//!    target).
//!
//! 6. **`LoopStop` class labels.** Atom #15 / #20 / #21 / #24
...
// ===========================================================================
// 1. Compile-time width pins (atom #21 / #24 / #25 precedent)
// ===========================================================================
...
/// signature drift through the §4.M canonical pin.
...
/// Default per-turn iteration cap. Master plan §9.5 token-saving spine
/// — five tool-using rounds per turn matches the Phase 0 budget envelope
...
/// stable wire tag. The driver (Stage M binding atom) returns one of these
...
///   (atom #22 / M.0.2 `SseDelta::Done`).
/// - [`LoopStop::ToolDenied`] — the dispatch boundary refused a tool
///   call (`MnemosError::tool_denied` from a-core / atom #2, mapped
...
impl LoopStop {
    ...
    /// (mirrors atom #15 / #20 / #21 / #24 precedent).
    #[inline]
...
// ===========================================================================
// 4. ToolLoop — type-level iteration cap (§10.1)
// ===========================================================================

/// Per-turn tool-using iteration cap. One `u8` field — the type itself is
/// the cap (255 absolute maximum; default 5 per master plan §9.5). No
/// representable [`ToolLoop`] value can express an unbounded loop.
///
/// Public field per the atom #23 `TurnUsage` rationale: the only
/// invariant is "fits in `u8`", which the type enforces; no `try_*`
/// validator is needed. Named constructors are provided for the Phase
/// 0 default and for operator-supplied caps.
///
/// §4.M signature (line 609). `#[repr(C)]` so the layout is stable
/// across rebuilds; `Default` returns the Phase 0 default
/// ([`DEFAULT_MAX_ITER_U8`]).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(C)]
pub struct ToolLoop {
    /// Per-turn iteration cap. Default 5 ([`DEFAULT_MAX_ITER_U8`]);
    /// the `u8` width is the §10.1 type-level upper bound.
    pub max_iter_u8: u8,
}

impl Default for ToolLoop {
    /// Default loop carries the Phase 0 cap ([`DEFAULT_MAX_ITER_U8`]).
    #[inline]
    fn default() -> Self {
        Self::new()
    }
...
impl ToolLoop {
    /// Construct a [`ToolLoop`] with the Phase 0 default iteration cap
...
/// named constructor (atom #24 `LazyToolSchema` carve-out 1 precedent;
/// atom #3 `TurnState` invariant-protection precedent). The invariant
...
///
/// §4.M signature (line 607). `#[repr(C)]` so the layout is stable
...
impl DailyTokenBudget {
    ...
    ///
    /// `n` is the externally-measured token count (atom #23
    /// `TurnUsage` output, post-`Done` frame); the budget never
...
// ===========================================================================
// 6. Inline unit tests (atom #21 / #24 / #25 precedent)
// ===========================================================================
...
mod tests {
    ...
    // ---- ATOM_PLAN line 1071 verbatim tests -------------------------------

    /// `m0_6_loop_stops_at_max_iter` — verifies the type-level
    /// iteration cap (§10.1 — "무한루프 표현 불가"). The default
    ...
    fn m0_6_budget_exceeded_stops_and_errors() {
        // Phase 0 cap = 5_000 tokens / call (master plan §9.5).
    ...
    // ---- Scaffolding tests (atom #21 / #24 / #25 precedent) ----------------
