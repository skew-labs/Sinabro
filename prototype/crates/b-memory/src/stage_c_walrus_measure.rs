//! Walrus PUT/GET byte + latency collector.
//!
//! Canonical OUT: a Walrus PUT/GET byte-and-latency sample derived from the
//! Stage B [`WalrusRoundTripReceipt`].
//!
//! # Crate-boundary note
//!
//! This collector reuses the Stage B [`WalrusRoundTripReceipt`], which lives in
//! `b-memory` (orchestration), and `b-memory` already depends on `c-walrus`
//! (transport). Homing the collector in `c-walrus` would force a
//! `c-walrus → b-memory` edge and a cargo cycle. Per the crate-boundary split
//! (`c-walrus = transport`, `b-memory = Walrus orchestration / receipt`), this
//! collector lives in `b-memory`.
//!
//! # Invariants
//!
...
//!   `stage_b_verify_blob_id` — so **no server-reported blob id can
...
/// One Walrus PUT/GET byte-and-latency measurement.
///
...
mod tests {
    ...
    /// call it; this mirrors the test-only duplicate used to
    ...
    /// A Walrus-primary `StorageObjectRef` carrying the verified blob (const
    /// constructor).
    ...
    fn body_is_redacted_to_a_length_count_only() {
        ...
        // blob/storage, yield byte-identical receipts (the receipt contract);
