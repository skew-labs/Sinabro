//! Stage B domain-separated chunk digest (atom #86 · B.1.5).
//!
//! This module mints the `§4.1` digest surface: the two
...
//!
//! # Madness invariants (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.1 / atom #86)
//!
...
//!   (the exact string the atom #86 spec requires), and the content hash uses a
...
//! the real Walrus/Sui domain at the net-testnet feature seam (Stage B late
//! atoms), exactly as `derive_blob_id` documents its own swap point.
//!
//! # Reuse map (atom contract)
//!
//! * **reuse: #85 [`StageBChunkView`]** — the borrowed lens the digest reads
//!   through (header by value + envelope by shared borrow).
//! * **reuse: #84 [`StageBChunkHeaderV1::to_bytes`]** — the content-free 85-byte
//!   header encode is the header half of the digest input (no re-encode minted).
//! * **reuse: #85 [`MAX_STAGE_B_CONTENT_BYTES`]** — the same 1 MiB content cap is
//!   re-checked fail-closed before hashing.
//! * **reuse: A [`BLOB_ID_BYTES`](mnemos_c_walrus::BLOB_ID_BYTES)** — the content
...
/// Domain string absorbed into every [`ChunkDigest32`] (the atom #86 spec
...
/// separate domains (the "content hash와 header digest를 분리한다" madness spec):
/// the same bytes hashed as "content" versus as "chunk" can never collide.
...
/// re-declared as a second 32 constant (§4.1 `CONTENT_HASH_BYTES = BLOB_ID_BYTES`).
...
/// value a Stage B chunk signature (atom #89+) signs over — never the raw header
/// or the raw content.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(transparent)]
...
/// §4.1's chunk-error surface. Minted here because atom #86 is the first atom
/// whose canonical signature ([`stage_b_chunk_digest`]) returns
/// `Result<_, StageBChunkError>`. The variant set is declared verbatim from §4.1;
/// atom #86 constructs only [`ContentTooLarge`](Self::ContentTooLarge) (the
/// fail-closed cap re-check), and the remaining variants are constructed by the
/// later atoms that own their surfaces (`SignatureInvalid` at the verify atom
/// #89, `PublishClassDenied` at the publish atom, `ReservedFlags` mapping the
/// atom #83 flag reject, `NonCanonicalAChunk` wrapping a Stage A codec error).
/// Declaring the full set once keeps the shared error type byte-stable across
/// those atoms instead of growing it variant by variant.
///
...
pub enum StageBChunkError {
    /// A `flags_u8` bitset carried a reserved bit (the atom #83
    ...
    /// atom #86 constructs (the [`stage_b_chunk_digest`] fail-closed cap
    /// re-check before hashing).
    ContentTooLarge,
    /// A chunk signature did not verify against the digest + owner (atom #89).
    SignatureInvalid,
    /// The chunk's publish payload class is not admissible onto the public
    /// testnet (the publish atom).
    PublishClassDenied,
...
mod tests {
    ...
    /// (86, 86, 0). Matches the `/tmp/mnemos_digest_ref.py` fixture exactly.
    ...
    /// reference (`/tmp/mnemos_digest_ref.py`) byte-for-byte. Pins the content
    ...
    /// binding (#85 deferred the header↔content binding to this digest).
    #[test]
