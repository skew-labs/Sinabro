//! Stage B Walrus testnet GET **request planner** and
//! **response parser**.
//!
//! This module mints [`WalrusGetPlan`] — the borrowed,
//! trace-stamped description of a single Walrus testnet GET (blob fetch) — plus
//! the GET response parser [`parse_walrus_get_response`], which classifies
...
//! or a content-free [`WalrusClientError`]. It builds on
...
//! and the PUT planner + response parser
...
//! borrows a [`VerifiedBlobId`] (zero copy) and binds a
...
//! type: the transport execution (`stage_b_get_with_transport`) is a
//! separate concern. The GET response parser ([`parse_walrus_get_response`])
...
//!
//! ## Verified-id-only by construction
//!
//! The key invariant for a GET is *no-trust on the server's self-report*:
...
//! byte-for-byte local-derive match). A
//! [`ReportedBlobId`](crate::stage_b_put::ReportedBlobId) (the undecoded,
...
//! not a runtime branch — mirroring the PUT planner's "trace required by construction".
//!
...
//! the only reachable target — the GET-side structural equivalent of the
...
//!
//! ## Trace required by construction
//!
//! As with the PUT planner, this planner takes the trace as a
...
//! the `RESET` sentinel) is rejected fail-closed by
//! [`StageBTraceEvidence::from_trace`] / `::embed` and so is **not
//! representable** as a planner input. The [`WalrusGetPlan::trace`] field
...
//! plan time (the transport execution layer owns the runtime error surface); the
//! enum is named here only in the doc trust-boundary narrative.
//!
//! ## Crate-home rationale
//!
//! This planner reuses the `b-memory`
...
//! edge is `b-memory -> c-walrus`). Walrus client
//! wrappers live in
...
/// A planned Walrus testnet GET. Owns nothing of variable
...
/// by construction.
///
/// The fields are `pub` per the canonical registry; [`plan`](Self::plan)
...
pub struct WalrusGetPlan<'a> {
    ...
    /// The trace stamp this GET is bound to. Always a *stamped* trace
...
impl<'a> WalrusGetPlan<'a> {
    ...
    /// Steps, in order:
    ///
    /// 1. **Verified id only**: `verified` is a
    ///    [`VerifiedBlobId`], whose only construction path is the local
    ...
    pub const fn plan(
        ...
        Self {
            request,
            // Unwrap the stamped trace (atom_id_u16 != 0 guaranteed by construction).
            trace: trace.trace(),
        }
    }
...
// ===========================================================================
// GET response parser: fetched blob body, capped.
// ===========================================================================
...
/// A fetched Walrus testnet blob body.
///
...
/// mirroring the content-free posture of the rest of this module
...
/// locally-derived id is the verify stage (`verify_reported_blob_id`).
...
impl WalrusGetBody {
    ...
    /// verify stage, which derives the local id and matches it).
    #[inline]
    #[must_use]
...
/// frozen [`WalrusClientError`] surface, so a caller sees a single,
...
///   `WalrusClientError` is frozen at seven variants with no `NotFound`,
...
/// body, not a live socket — those tags belong to the transport / retry layer.
...
mod tests {
    ...
    /// `pub(crate)` encoder (chunk_vectors.rs test-helper precedent). The
    ...
    /// A GET plan can only be built from a
    /// [`VerifiedBlobId`]; the plan's request targets exactly that verified id.
    /// The no-trust invariant is type-level: there is no
    ...
    /// The GET URL composed from the plan is the
    ...
    /// blob-id segment under the sanctioned testnet base + path (the form
    /// the real aggregator parses, not hex).
    #[test]
    ...
    /// A missing/unstamped trace (`atom_id_u16 == 0`) is
    ...
    /// real trace. A stamped trace builds a plan and is preserved
    ...
    /// The only constructible endpoint is the
    ...
    /// The plan is a `Copy` value carrier (it holds a
    ...
    // -----------------------------------------------------------------------
    // GET response parser tests.
    // -----------------------------------------------------------------------

    /// An HTTP 200 with a within-cap body parses to a
    ...
    /// A 200 with an empty body is a valid (zero-length)
    ...
    /// An HTTP 404 (the aggregator does not hold the blob)
    /// maps to [`WalrusClientError::Protocol`]. The error set has no `NotFound` variant
    ...
    /// Every non-200, non-404 status with no
    ...
    /// The fetched bytes are memory content, so a
    ...
    /// `into_body` yields exactly the fetched bytes
