//! Seal stub integration with the signed chunk.
//!
//! Canonical OUT — the signed-chunk constructor path
//! checks the Seal stub policy. The invariant: chunk signing cannot
//! accidentally bless a private publish. Fixture-only, no live egress;
//! same-message approval is required for any live path, none in Stage B. This
//! module is the seam where the f-seal Seal-stub envelope
//! mapping meets the signed chunk and the chunk flags: a
...
//!
//! Canonical IN:
//! - f-seal [`stage_b_seal_envelope`] — the content-class → envelope
//!   decision, reused verbatim (the edge `b-memory -> f-seal` is one-way).
//! - [`StageBChunkFlags`](crate::chunk_schema::StageBChunkFlags) — the
...
///
/// Returns the [`StageBSealStubEnvelope`] when the class is admissible
...
mod tests {
    ...
    // bubbling; suppress prod-only clippy denies inside this module.
