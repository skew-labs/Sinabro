//! Walrus-primary persistence plan for a [`MemoryChunk`] (atom #31 · B.0.3).
//!
...
//! feature-gated transport (atom #12 · C.0.6) and the real anchor runs
//! through `d-move`'s SDK builder (atom #15 · D.0.3). This atom only
//! emits the typed manifest, so the whole test suite runs `--offline`.
//!
//! # Madness invariants (`MNEMOS_ATOM_PLAN.md` §B.0.3)
//!
//! * **Walrus is the only Phase 0 primary.** The emitted plan pins
//!   `primary == StorageBackendKind::Walrus` and there is no caller
//!   knob to swap it. `LocalEncrypted` (atom #29) is `Enabled` as a
//!   *kind* but is not addressable as the Phase 0 persistence primary
//!   in this planner — its writer lives later under `f-seal` (atom #36).
//!   `IpfsMirror` / `FilecoinArchive` are `FutureOnly` (atom #29's
...
//! * **Synthetic class only.** Phase 0's publisher (atom #8) refuses
...
//!   The Phase 1 promotion path (real-user-memory) is *not* this atom.
...
//!   envelope (atom #7), and a locally-derived `blob_id`
//!   ([`derive_blob_id`] from atom #10) computed over the chunk
//!   content bytes. This is the §10.2 self-report ban (the publisher-
...
//!   would not encode under atom #7's `MAX_CONTENT_BYTES`),
...
//!
//! * [`PublishPayload`] · [`PublishPayloadClass`] — atom #8 · C.0.2
//!   (`mnemos_c_walrus::publisher`).
//! * [`MoveAnchorArgsV1`] · [`ChunkCodecError`] · [`encoded_len_for_content_len`]
//!   — atom #7 · C.0.1 (`mnemos_c_walrus::codec`).
//! * [`derive_blob_id`] — atom #10 · C.0.4 (`mnemos_c_walrus::blob_id`).
//! * [`MemoryChunk`] · [`StorageBackendKind`] · [`StorageBackendPhase`]
//!   · [`StorageBackendRole`] — atom #29 · B.0.1 (`crate::chunk`).
//! * Anchor-args downstream consumer — atom #15 · D.0.3
...
/// ([`ChunkCodecError`] in atom #7, `StoreError` in atom #30) so the
/// error channel never owns a heap allocation and never carries user
/// content.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
#[non_exhaustive]
pub enum PersistError {
    /// The chunk's content would not encode under atom #7's codec
    ...
    /// surfaced by atom #15's `memory_root_args_from_anchor` (today
...
impl<'a> StorageWritePlan<'a> {
    ...
    /// `blob_id` is locally derived via [`derive_blob_id`] (atom #10)
...
impl MemoryPersist {
    ...
    ///   exceeds atom #7's `MAX_CONTENT_BYTES`.
    /// * [`PersistError::Publish`] — `chunk.envelope().content.len()`
    ///   exceeds atom #8's `PUBLIC_PUBLISHER_BODY_CAP_BYTES` cap.
    ...
    pub fn plan_persist(chunk: &MemoryChunk) -> Result<StorageWritePlan<'_>, PersistError> {
        ...
        // 2. Length budget — codec cap (atom #7's MAX_CONTENT_BYTES)
        //    first, then publisher cap (atom #8's body cap) via the
...
// Pin the cross-atom byte invariant: this atom assumes the publisher
// class tag for `SyntheticPublicFixture` is the single-byte value 1
// (atom #8 line 96) and that the Phase 0 primary kind is
// `StorageBackendKind::Walrus` with tag 2 (atom #29). A future drift
// on either side is caught at compile time by a zero-length array
// index (mirror of atom #29's pattern).
...
// ===========================================================================
// 5. Inline unit tests — atom #31 spine
// ===========================================================================
...
mod tests {
    ...
    /// — the only class atom #8's `PublisherPutRequest::new` admits.
    /// A Phase 1 promotion to `RealUserMemory` is a deliberate
    /// future-atom flip, not an accidental one.
    #[test]
    ...
    /// `blob_id` computed over the chunk content via atom #10's
    ...
    fn b0_3_anchor_args_match_chunk() {
        ...
        // The anchor's `seed()` projection (atom #7 surface) drops
    ...
    fn b0_3_no_ipfs_filecoin_live_writer() {
        ...
        //    — if a future atom adds such a field, the four-accessor
        //    invocation will fail to remain exhaustive and atom
