//! `stage_b_blob_id.rs` (atom #107 · B.2.6 local blob-id derive + atom #108 ·
//! B.2.7 reported blob-id verify) — the Stage B **local** Walrus blob-id
//! derivation and verification seam.
//!
//! This module mints two §4.2-row entry points:
//!
//! * [`derive_walrus_blob_id`] (atom #107): the single Stage B name for "compute
//!   the blob id of a chunk **from the local encoded bytes**, never from what a
//!   server says". It is a **thin wrapper** over Stage A's
//!   [`derive_blob_id`](mnemos_c_walrus::derive_blob_id) (atom #10 · C.0.4) —
//!   Stage B mints **no** second derivation algorithm, mirroring how atom #91
//!   [`encode_stage_b_chunk`](crate::encode_stage_b_chunk) is a thin wrapper over
//!   Stage A's `encode_chunk_v1`.
//! * [`stage_b_verify_blob_id`] (atom #108): the single Stage B name for "promote
...
//!   #11 · the §10.2 self-report-refusal seam), whose internal derivation is the
...
//!
//! # Madness invariant (`MNEMOS_STAGE_B_ATOM_PLAN.md` atom #107)
//!
...
//! holds locally — the canonical Stage B chunk wire produced by atom #91
//! [`encode_stage_b_chunk`]. A Walrus publisher's *self-reported* blob-id text
//! (`PublisherReportedBlobId`, atom #103/#104) is **never** consulted here: it is
//! only ever compared against this local derivation at the verify seam (atom #108
...
//! # Scope (atom #107 only)
//!
//! Derivation only. This atom does **not**:
//!
//! * verify a reported id or construct a
//!   [`VerifiedBlobId`](mnemos_c_walrus::VerifiedBlobId) — that is atom #108
...
//! * open a socket or touch the network (`G-B-WALRUS-OFFLINE` holds by
...
//! # Reuse (재발명 0)
//!
//! * A [`derive_blob_id`](mnemos_c_walrus::derive_blob_id) — the canonical Phase 0
...
//!   atom #12 seam `feature = "net-testnet"`, with the `&[u8] -> BlobId`
//!   signature byte-stable across the swap). The wrapper delegates verbatim.
//! * A [`BlobId`](mnemos_c_walrus::BlobId) — the 32-byte id type, returned
//!   unchanged (no Stage-B-specific id newtype).
//! * #91 [`encode_stage_b_chunk`](crate::encode_stage_b_chunk) — the production
...
///
/// `encoded_chunk` is the canonical Stage B chunk wire produced by atom #91
...
/// invariant: the verify seam (atom #108
...
pub fn derive_walrus_blob_id(encoded_chunk: &[u8]) -> BlobId {
    ...
    // (criterion sweep + baseline emitter; G-B-BENCH). The hot work is entirely
...
/// §10.2 self-report-refusal seam): it forwards the borrow straight through, so
...
/// (the Stage B PUT-response parser, atom #104, yields the reported token; wiring
/// that token into a `PublisherReportedBlobId` is the caller/round-trip seam,
/// atom #109 — out of scope here).
///
/// # Errors
...
///   the local derivation in at least one of the 32 positions (the §10.2
...
// ===========================================================================
// bridge atom #116.5 · B.2.15.5 — official Walrus RS2 oracle (net-testnet)
// ===========================================================================
//
// The two functions above are the Phase-0 **placeholder** derive/verify and are
// byte-identical under both builds. Bridge atom #116.5 adds the **official**
...
// `mnemos_c_walrus::blob_id_rs2`. Stage B atom #117 (live GET + verify) consumes
// these to promote the #116 live-PUT reported id to a real `VerifiedBlobId`; the
...
/// Merkle-root -> blob-id rule, **not** the Phase-0 placeholder
...
/// Walrus oracle instead of the placeholder. It is the path Stage B atom #117
/// uses to promote the #116 live-PUT reported id to a trust root, and like the
...
mod tests {
    ...
    /// The atom #107 known-vector input. Pinned cross-language: the expected
    /// digest below is computed by the Python oracle replicating
    /// `c-walrus::blob_id::derive_blob_id` (see the atom #107 sidecar
    /// `command_manifest.json` / `input_context.jsonl`), and re-checked here by
    ...
    // ----- atom #108 · B.2.7 reported blob-id verify -----
    ...
    /// cross-crate test cannot call it; this is a test-only duplicate (the
    /// #95/#105 test-only base64 helper precedent), used only to synthesize a
