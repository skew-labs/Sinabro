//! `owner.rs` (atom #88 · B.1.7) — the Stage B **owner ↔ signing-public-key
//! boundary**.
//!
...
//!   as Stage A's [`SuiAddress`] (`mnemos-d-move` §4.D) **verbatim**, never as a
//!   raw `String`. Atom #84's [`crate::chunk_schema::StageBChunkHeaderV1::owner`]
...
//!   [`SignaturePlaceholderV1::public_key`] field (`mnemos-c-walrus` §4.C); this
...
//!
//! # Boundary invariant (madness spec)
//!
...
//! check that an address indeed corresponds to a key) is a later atom living on
//! the Move-binding seam, where the chain's canonical scheme-flag + hash rule is
//! applied. Atom #88 mints the typed carriers, not that conversion.
//!
//! # No raw leak
//!
...
//! * #84 `StageBChunkHeaderV1.owner: SuiAddress` — the precedent that the owner
...
impl SigningPublicKey {
    ...
    /// the #81–#87 precedent — no canonical error variant is minted here (the
...
///
/// The madness-spec invariant: the public-key → `SuiAddress` derivation is the
/// exclusive job of the d-move binding seam (a later atom). This type performs
