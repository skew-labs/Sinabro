//! Stage B Walrus boundary-aware retry policy (atom #110 · B.2.9).
//!
...
//! audit append downstream). This module encodes that single madness invariant
//! (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.2 / atom #110):
//!
//! > **if bytes may have crossed, automatic second PUT is forbidden — manual
//! > reconcile only.**
//!
//! It does so as a pure, offline classifier over two canonical OUT enums (§4.2):
//!
//! ```text
...
//! boundary states fail **closed** to a manual reconcile, so the madness
//! invariant holds for every input by construction (proved over all states by the
//! `b2_9_proptest_madness_invariant` property).
//!
//! # Madness invariants (atom #110)
//!
...
//!   bridge and the GET/PUT decisions are inherent methods, not new §4.2
...
//!   mirroring the atom #103 forward-reserved error-variant precedent.
//!
//! # `WalrusRetry::Never` — forward-reserved
//!
//! `Never` is the strictest decision: *no* retry path at all, distinct from
//! `ManualReconcile` (which permits a human-driven reconcile). No function in this
//! atom produces it; it is reserved for a structurally one-shot / already-committed
//! operation that must never be retried — e.g. the atom #111 PUT idempotency
...
//! and predicates, exactly as atoms #103/#104 carried forward-reserved
//! `WalrusClientError` variants (`Transport`, `Protocol`, …) before a later atom
//! produced them.
//!
//! # Reuse map (atom contract — reuse #103-#106)
//!
...
//! * **#104** the PUT request/response path (`WalrusPutPlan` /
//!   `parse_walrus_put_response`) is the *mutating* operation whose boundary this
//!   module classifies; not imported (this atom is the pure policy, the transport
//!   execution that observes the boundary is the §4.2 `stage_b_put_with_transport`
//!   atom). Flagged, not forced — cf. the atom #82 "defer unconsumed canonical"
//!   precedent.
//! * **#105/#106** the GET request/response path (`WalrusGetPlan` /
...
/// The state of the external-mutation boundary for a Walrus operation (§4.2
/// canonical OUT).
///
/// `#[repr(u8)]` with explicit discriminants so the byte tag is stable for any
/// future tabular/diagnostic form (mirrors `StageBNetwork` and the atom #81
...
/// The retry decision implied by a [`WalrusBoundaryState`] (§4.2 canonical OUT).
///
...
impl WalrusBoundaryState {
    ...
    /// The retry decision for this boundary state — the atom #110 enforcement
    /// point.
    ///
    ...
    ///   absorbs the retry).
    ///
    /// The madness invariant — `self.requires_manual_reconcile()` implies
...
mod tests {
    ...
    /// `b2_9_bytes_crossed_forbids_auto_put` — the madness invariant's core case:
    ...
    proptest::proptest! {
        ...
        /// (sampled across all three inhabitants) the atom #110 madness invariant
