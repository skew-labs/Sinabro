//! Bounded in-memory store for [`MemoryChunk`] (atom #30 · B.0.2).
//!
...
//! `RuntimeSupervisor<CAP>` — atom #4 · A.0.4) so the whole agent has a
//! uniform "fixed-capacity madness" surface.
//!
//! # Madness invariants (`MNEMOS_ATOM_PLAN.md` §B.0.2)
//!
...
//!   a `u64` counter and wrapped in [`MemoryId`] (atom #29 · B.0.1). The
//!   atom #29 hand-off pre-arranged the *saturating sentinel*
//!   ([`MemoryId::next`] saturates at `u64::MAX`) as the surface for an
...
//!   (atom #31 · B.0.3) and is outside this atom's scope.
//!
//! # Reuse map (atom contract)
//!
//! * [`MemoryChunk::new`] — atom #29 · B.0.1 (`crate::chunk`).
//! * [`MemoryId::next`] saturating sentinel — atom #29 · B.0.1.
...
/// `ChunkCodecError` (atom #7) by virtue of being a separate concept.
...
pub enum StoreError {
    ...
    /// `get` API; reified here so atom #31's persistence layer can wrap
...
impl<const CAP: usize> InMemStore<CAP> {
    ...
    /// the id space is exhausted (the atom #29 saturating-sentinel
    ...
    /// persistence planner's job (atom #31 · B.0.3).
    ...
    pub fn append(&mut self, envelope: ChunkEnvelopeV1) -> Result<MemoryId, StoreError> {
        ...
        // Id-space exhaustion via atom #29's saturating sentinel: when
        ...
        // `CapacityExceeded` (the atom #29 hand-off contract).
...
// Pin the cross-atom byte invariant for `StoreError`: `#[repr(u8)]`
// discriminants are stable. A future drift would break this and the
// downstream persistence layer (atom #31) that surfaces `StoreError` tags.
...
// ===========================================================================
// 4. Inline unit tests — atom #30 spine
// ===========================================================================
...
mod tests {
    ...
    /// `Default` blanket so atom #31's plan layer can rely on either.
    #[test]
    ...
    /// `NotFound = 2`). Mirrors atom #29's `_TAGS_ARE_STABLE` compile
