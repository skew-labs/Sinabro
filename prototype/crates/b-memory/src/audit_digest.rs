//! Stage B audit-log entry hash.
//!
//! This module mints the one canonical OUT:
//! [`stage_b_audit_entry_hash`] — the 32-byte entry hash that a memory owner's
//! append-only audit log (`mnemos::audit_log::append`) stores per chunk.
//! The hash binds, in one domain-separated digest, the four quantities the
//! design spec requires:
//!
//! 1. **the chunk digest** — [`StageBSignedChunkV1::digest`] (a
//!    [`ChunkDigest32`](crate::ChunkDigest32)), absorbed directly;
//! 2. **the verified blob id** — the locally-verified Walrus id
//!    ([`VerifiedBlobId`]), absorbed directly;
//! 3. **the trace id** — the per-action [`StageBTraceLink`](crate::StageBTraceLink)
//!    stamp carried by the signed chunk, absorbed directly as
//!    its 11-byte little-endian serialization;
//! 4. **the owner** — bound **transitively** through the chunk digest: the
//!    digest absorbs the fixed 85-byte header
...
//!   enters it only as a content **hash**), the verified blob id is a
...
//!   from the chunk-digest / content-hash domains and the
...
//! * **Verified id only.** The blob-id input is a [`VerifiedBlobId`],
...
//!
//! The canonical OUT signature is fixed as
//! `stage_b_audit_entry_hash(signed_chunk, blob_id) -> [u8; 32]` — **two**
//! arguments. The design spec lists **four** bound quantities. The signed chunk
//! carries `{ envelope, digest, signature, trace }` and does **not**
//! carry the `owner` as a separate field (the owner lives in the chunk *header*,
//! which the signed chunk does not keep — see the signed-chunk docs). Owner is
//! therefore bound **transitively** through `signed_chunk.digest`, which the
//! signed-chunk constructor recomputed from the header whose bytes `9..41` are
//! the owner. This is a genuine binding (any owner change moves the digest and
//! thus the audit hash), faithful to the two-argument signature; it is documented
//! here rather than widened to a three-argument signature, and no new owner
...
//! as [`stage_b_chunk_digest`](crate::stage_b_chunk_digest) /
...
//! / `permute` are not exported), so — exactly as `chunk_digest` re-stated Stage
//! A's private permutation rather than importing it — it is re-stated here. This
//! keeps the module strictly inside its own file (`audit_digest.rs`; zero
//! `chunk_digest.rs` edit) and mints **no** new wire format and
...
//! byte-identical to the `chunk_digest` core by a cross-language Python reference
//! (golden vector `b1_14_audit_hash_known_vector`).
//!
//! # Reuse (zero re-invention)
//!
//! * [`StageBSignedChunkV1`] — the signed-chunk input; its `digest()`
//!   and `trace()` accessors are the two fields absorbed.
//! * trace — the [`StageBTraceLink`](crate::StageBTraceLink) stamp's
//!   `{ trace_id_u64, atom_id_u16, attempt_u8 }` serialized exactly as the
//!   header `to_bytes` lays the trace out (bytes `74..85`).
//! * A [`VerifiedBlobId`] — the local-verify trust root, absorbed via
...
/// Domain string absorbed into every audit entry hash. Disjoint from the
/// [`CHUNK_DIGEST_DOMAIN`](crate::CHUNK_DIGEST_DOMAIN) /
/// [`CONTENT_HASH_DOMAIN`](crate::CONTENT_HASH_DOMAIN) and the
...
/// (2, LE) + `attempt_u8` (1). This matches the header `to_bytes`
...
/// order:
///
/// 1. `signed_chunk.digest()` — the [`ChunkDigest32`](crate::ChunkDigest32)
...
/// present, signature-verified digest and a stamped trace, and a
/// [`VerifiedBlobId`] can only exist after a local re-derivation match,
/// so every input is already well-formed — there is nothing to reject.
///
...
/// `mnemos::audit_log::append` records — never a raw chunk body.
#[inline]
#[must_use]
pub fn stage_b_audit_entry_hash(
    ...
    // Serialize the trace stamp exactly as the header lays it out
...
// ===========================================================================
// 3. ARX hash core (Phase 0 placeholder; re-stated from chunk_digest)
// ===========================================================================
//
...
// `c-walrus::blob_id`. Re-stated here rather than imported — the same
// precedent — so this module touches only this file. No cryptographic claim.
...
mod tests {
    ...
    // suppress prod-only clippy denies inside this module (test-helper
    // precedent).
    ...
    /// Build a minimal valid envelope (mirrors the signed_chunk test helper).
    ...
    /// `derive_blob_id` + `verify_reported_blob_id` round-trip (chunk.rs
    ...
    /// golden audit-entry hash independently derived by a cross-language Python
    /// reference. Content = the known body, owner =
    /// `0x55`*32, trace = (95, 95, 0), blob witness = the known witness. This is
    /// the vector-stable test: the audit hash is a stable cross-language
    /// anchor that later golden vectors build on.
    #[test]
    fn b1_14_audit_hash_known_vector() {
        ...
        // Golden vector — independently derived by a Python mirror
        // of the ARX core; not a self-captured value.
        assert_eq!(
            hex(&h),
    ...
    /// signature-vs-spec reconciliation: changing only the owner (which is not
