//! Stage C first synthetic payload builder.
//!
//! Canonical OUT: [`WalrusMainnetPrepare`] — the description of the first
//! mainnet Walrus blob. That first blob is a **synthetic public fixture only**.
//! The builder consumes a Stage B signed-chunk ([`StageBSignedChunkV1`]) — so it
//! sits on the canonical signed-chunk path — encodes the Stage A
//! [`ChunkEnvelopeV1`](mnemos_c_walrus::codec::ChunkEnvelopeV1),
...
//!   construction (the [`StorageObjectRef`] split); this builder never
//!   emits one.
//! * **Self-report ban preserved.** The blob id is derived locally
//!   and the `storage` carries a
...
//! * **Encoded bytes recorded.** `payload_hash_32` is the
...
//!   [`StorageObjectRef`], and the [`StageCTraceLink`].
...
/// Description of the first (synthetic) mainnet Walrus blob.
...
pub fn build_synthetic(
    ...
    // local-derive path so the storage carries a real `VerifiedBlobId`.
