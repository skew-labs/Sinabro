//! Stage B parent chain encoding.
//!
//! Stage A carries a chunk's parent linkage in three independent places
...
//!    `Option<BlobId>`.
//!
//! This module mints the canonical OUT: the **parent validation
//! helpers** that hold those three views in agreement.
//! [`StageBChunkHeaderV1::new`] already pins the *header-internal* invariant
//! (flag ⇔ header `parent`); the digest binds the parent only "by
//! inclusion" (the header, parent field and all, is hashed into the digest) —
//! leaving the **cross-binding** of the header's parent against the *borrowed
//! envelope's* parent. [`parent_linkage_consistent`] closes that gap.
//!
//! # Design invariants
//!
...
//!   bound into the [`ChunkDigest32`] through the header bytes, so a
...
//!   domain of `replay_stage_b`, a later stage). The helpers are pure
...
//!   `NonCanonicalAChunk`) is `#[non_exhaustive]` and was minted **verbatim**
//!   earlier; it carries no parent-mismatch variant. Minting one here would
//!   extend that frozen set, so the parent reject is expressed as a `bool`
//!   predicate, mirroring the reject-as-predicate precedent. A later
//!   stage that owns a chunk-acceptance error surface maps a `false` here onto its
//!   own reject at that boundary.
//!
//! # Reuse map
//!
//! * **reuse [`StageBChunkHeaderV1`]** — the header carrying the `flags_u8`
//!   `HasParent` bit and the `parent: Option<BlobId>` field.
//! * **reuse [`StageBChunkFlags::is_set`]** — the flag membership test (no new
//!   flag decoder minted).
//! * **reuse [`StageBChunkView`]** — the borrowed lens pairing the header with
//!   the Stage A envelope, so the cross-binding can read both without a copy.
//! * **reuse [`stage_b_chunk_digest`](crate::stage_b_chunk_digest)** — the
...
/// The canonical OUT: validate a chunk view's parent linkage across all
/// three carriers, fail-closed.
///
/// Returns `true` iff **both** invariants hold:
///
/// 1. **Flag ⇔ header parent** (re-affirms the header-internal invariant;
...
/// 2. **Header parent == envelope parent** (the cross-binding, previously bound
///    only by inclusion in the digest): the header's parent blob id
...
/// and performs **no** self-parent / cycle check (integrity only — the design
/// spec forbids order here). It returns a `bool` rather than a
/// [`StageBChunkError`](crate::StageBChunkError): the error set is frozen
...
pub fn parent_linkage_consistent(view: &StageBChunkView<'_>) -> bool {
    ...
    // (2) header parent == envelope parent (the cross-binding). `BlobId`
...
mod tests {
    ...
    /// `b1_6_parent_digest_vector` — the parent is bound into the chunk
    /// digest (through the header bytes). Pins the golden parent-bearing digest and
    /// the golden genesis digest (both independently derived by a Python
    /// reference, whose ARX core self-checks against the known digest
