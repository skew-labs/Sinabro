//! Stage C ← A+B handoff digest (C-WP-01 · atom #171 · C.0.0).
//!
//! Canonical OUT (§4.0): [`StageCHandoffDigest`]. The companion §4.0 trace
...
//!
//! # Madness invariants (§4.0 / atom #171)
//!
...
/// The frozen Stage A+B → Stage C evidence digest (§4.0).
///
...
///
/// Fields are `pub` per §4.0. The byte order used by [`to_bytes`](Self::to_bytes)
...
pub struct StageCHandoffDigest {
    /// Hash of the Stage A atom plan (`MNEMOS_ATOM_PLAN.md`).
    pub atom_plan_a_hash_32: [u8; 32],
    /// Hash of the Stage B atom plan (`MNEMOS_STAGE_B_ATOM_PLAN.md`).
