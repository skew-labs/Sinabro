//! Stage B testnet network typed boundary (atom #82 · B.1.1).
//!
...
//! to violate*: the only canonical OUT (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.2) is
//!
//! ```text
...
//! degree of freedom, and the `G-B-NO-MAINNET` gate's "typed network guard" is
//! satisfied by construction rather than by a runtime check that could be
//! bypassed.
//!
//! # Madness invariants (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.2 / atom #82)
//!
...
//!   network.
//! * **No canonical error type is minted for the reject.** §4.2 declares a
//!   network-parse error nowhere (its error enums are `WalrusClientError` /
//!   `BlobIdError`, both downstream of this boundary). Mirroring the atom #81
//!   precedent (§4.0 declared no handoff error enum → reject expressed as a
...
//!
//! * **reuse: #81 trace** — [`StageBTraceLink`](crate::stage_b_handoff::StageBTraceLink)
//!   is the canonical per-action trace stamp. atom #82 is a *parse-only*
//!   boundary with **zero** external action, so it does not consume a trace
//!   here; the trace enters the live network plans from atom #101+ (§4.2
...
//!   would be premature (cf. atom #81's identical "defer unconsumed canonical"
//!   note).
//! * No Stage A wire/address/gas/secret type is reused here either; the chunk
//!   schema (atom #83+) and the Walrus/Sui surfaces (#101+) introduce those.
...
/// any future tabular/wire form (mirrors the atom #81 `Evidence*Class` enums).
...
impl StageBNetwork {
    ...
    /// invariant). There is intentionally no canonical error type — §4.2
