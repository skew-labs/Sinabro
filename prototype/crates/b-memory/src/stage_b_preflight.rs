//! Stage B Walrus testnet **preflight**.
//!
//! This module provides [`WalrusTestnetPreflightReport`] — a content-free
//! readiness assessment that answers "may a caller attempt a live Walrus testnet
//! action?" across five dimensions, **without writing a single byte to any
//! network**. It composes the existing content-policy and trace surfaces and
//! introduces no new wire format, no new error type, no new id/address newtype,
//! and no dependency. The whole module is pure and offline: it opens no socket,
//! resolves no DNS, and pulls in no HTTP/TLS surface.
//!
//! # Design invariants
//!
...
//!   of the testnet host is handled by the network transport layer; this offline
//!   preflight takes the resolution outcome as an *injected* `dns_resolved`
//!   signal so the entire assessment is deterministic and testable with no
//!   socket. The endpoint itself is the [`WalrusTestnetEndpoint`] — testnet by
//!   construction — so only the single sanctioned endpoint is ever preflightable
//!   (an arbitrary host is not even representable as input).
//!
//! * **Feature is the `net-testnet` compile gate.** [`feature_compiled`] reports
//!   whether the `net-testnet` feature (the only seam that links the real
//!   `reqwest` transport) is compiled in. [`assess`](WalrusTestnetPreflightReport::assess)
...
//!   transport wiring passes `feature_compiled()`. Under the default build the
//!   feature is off, so `feature_compiled()` is `false` and a report built with
//!   the real gate is not ready.
//!
//! * **Payload class reuses the publish decision.** The `payload_class`
//!   dimension is [`Ready`](PreflightReadiness::Ready) **iff**
//!   [`stage_b_publish_decision`] returns
...
//!   **fail-closed** here exactly as it does at the planner seam. No second
//!   payload-policy enum is introduced, and this module provides **no**
//!   owner-signature verifier or wallet/seal/capability surface (that override is
...
//!
//! * **Trace reuses the trace evidence.** The
...
//!   [`NotReady`](PreflightReadiness::NotReady) fail-closed; no second
//!   trace-evidence type is introduced.
//!
//! # Related surfaces
//!
//! The five dimensions compose these already-existing surfaces, which are reused
...
//! * the `net-testnet` feature — surfaced by [`feature_compiled`].
...
//! * [`StageBTraceEvidence`](crate::trace_link::StageBTraceEvidence) — the
//!   `trace` dimension; [`StageBTraceLink`](crate::stage_b_handoff::StageBTraceLink)
...
/// Whether the `net-testnet` feature — the only seam that links the real Walrus
/// `reqwest` transport — is compiled into this build.
///
/// `true` only when the crate is built with `--features net-testnet`. The
/// default (offline) build returns `false`. The real transport wiring passes this
...
/// A content-free Walrus testnet preflight readiness report across the five
/// dimensions.
///
/// Built by [`assess`](Self::assess), which performs **no** network I/O — it
/// composes the injected DNS / feature / timeout signals with the payload-class
/// decision and the trace evidence to produce a per-
...
pub struct WalrusTestnetPreflightReport {
    ...
    /// real resolve happens in the network transport layer).
    endpoint_dns: PreflightReadiness,
    /// Readiness of the `net-testnet` feature gate (injected; the real transport
    /// wiring passes [`feature_compiled`]).
    feature: PreflightReadiness,
    /// Readiness of the payload content class (publish decision: `Admit` only).
    payload_class: PreflightReadiness,
    /// Readiness of the configured timeout (within the accepted bounds).
    timeout: PreflightReadiness,
    /// Readiness of the per-action trace stamp (trace evidence; fail-closed on
    /// the missing/unstamped sentinel).
    trace: PreflightReadiness,
}

impl WalrusTestnetPreflightReport {
    /// Assess Walrus testnet readiness across the five dimensions
    /// **without writing to any network**.
    ///
    /// * `endpoint` — the [`WalrusTestnetEndpoint`]. Taken by value so a
    ...
    ///   (the real resolve happens in the network transport layer; offline here).
    /// * `feature_ready` — whether the `net-testnet` transport feature is usable;
    ///   the real transport wiring passes [`feature_compiled`].
    /// * `class` — the payload content class; mapped through
    ...
    /// * `trace` — the per-action stamp; ready iff
    ...
    pub const fn assess(
        ...
        // Trace via the trace evidence: a missing /
    ...
    /// Readiness of the payload content class.
    #[inline]
    ...
    /// Readiness of the per-action trace stamp.
    #[inline]
...
mod tests {
    ...
    /// The `payload_class` dimension is
    /// `Ready` **iff** `stage_b_publish_decision` returns `Admit`,
    /// across the full payload class set. Binds the preflight to that decision so the two
    ...
    /// The missing/unstamped trace sentinel
    /// (`atom_id_u16 == 0`) makes the `trace` dimension not ready (reusing the
    /// `from_trace` fail-closed reject) and the whole report not ready.
    #[test]
