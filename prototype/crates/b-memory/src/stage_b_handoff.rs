//! Stage B handoff lock (atom #81 · B.1.0).
//!
//! Stage B does not begin until Stage A (`MNEMOS_ATOM_PLAN.md` #0-#50) has
//! left behind verifiable evidence. This module carries the canonical
//! *value carriers* for that hand-off — it does **not** itself recompute the
//! Stage A hashes (that population is a runtime/integration concern). The
//! three canonical OUT types are taken verbatim from
//! `MNEMOS_STAGE_B_ATOM_PLAN.md` §4.0:
//!
//! * [`StageAHandoffDigest`] — the 11 evidence hashes that pin the Stage A
//!   surface (build-state, atom-plan, §4 canonical registry, and the
...
//!   and *no* remote storage locator: Stage B is not an Evidence Lake live
//!   archive stage, and consent to train is never the default.
//!
//! # Madness invariants (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.0 / atom #81)
//!
...
//!   Stage B while that mask is non-zero. No error enum is minted here —
//!   §4.0 declares none, so the "reject" is expressed as a bitmask + a
//!   boolean predicate (a *checker*, not a new canonical type).
...
//!
//! atom #81 composes only primitive byte/integer types. It deliberately
//! reuses **no** Stage A wire/address/gas/secret type — those enter Stage B
//! from atom #82 onward (network typed boundary) and #83+ (chunk schema).
//! Pulling an A canonical type here would be premature and is out of scope.
...
///
/// The atom #81 `(trace_id_u64, atom_id_u16, attempt_u8)` triple is unchanged.
/// In C-WP-01 (atom #171) the *definition* moved down to `a-core` so the low
/// crates `d-move` (gas trace, #178) and `k-devex` (evidence ref, #177) can
/// compose it through [`StageCTraceLink`](mnemos_a_core::trace::StageCTraceLink)
...
///
/// Fields are `pub` per the §4.0 canonical registry. The byte order used by
...
pub struct StageAHandoffDigest {
    /// Hash of `MNEMOS_BUILD_STATE.md` (Stage A next-atom truth). Slot 0.
    pub build_state_hash_32: [u8; 32],
    /// Hash of `MNEMOS_ATOM_PLAN.md` (Stage A arch source). Slot 1.
    pub atom_plan_hash_32: [u8; 32],
    /// Hash of the §4 canonical registry section. Slot 2.
...
/// per-atom sidecar, the replay command and the env-lock, plus a redaction
...
///
/// Fields are `pub` per the §4.0 canonical registry; the Stage B constructor
...
pub struct EvidenceBundleManifestV1 {
    ...
    /// Hash over the 21-file sidecar directory contents.
...
// Pin the evidence-class discriminants exactly as declared in §4.0. Any drift
// fails the build via a zero-length array index (atom #29 pattern).
...
// ===========================================================================
// 6. Inline unit tests — atom #81 spine
// ===========================================================================
...
mod tests {
    ...
    /// `b1_0_handoff_hash_parse` — `to_bytes` → `from_bytes` is an identity
    /// round-trip over all 11 slots, and the buffer width is exactly 352.
    #[test]
    ...
    /// `b1_0_missing_evidence_reject` — an all-missing digest reports every
    /// slot missing and is not complete; a complete digest reports `0` and is
    /// complete; a single zeroed slot is pinpointed by its bit.
    #[test]
    ...
    /// `b1_0_trace_id_width` — every field carries its full type width with
    /// no truncation, and the constructor preserves all three components.
    #[test]
    fn b1_0_trace_id_width() {
        ...
        // Atom #81 fits in the u16 field with vast headroom.
        let t81 = StageBTraceLink::new(7, 81, 0);
        assert_eq!(t81.atom_id_u16, 81);
        assert_eq!(t81.attempt_u8, 0);
    }

    /// `b1_0_manifest_default_false` — the Stage B constructor always emits
    /// `training_eligibility = false`, regardless of class inputs.
    #[test]
    ...
    /// `b1_0_remote_locator_absent` — the Stage B constructor leaves the
    /// remote storage locator all-zero (= absent); flipping any byte makes
    /// the predicate report "present".
    #[test]
    ...
    /// `b1_0_evidence_class_tags_stable` — discriminants match §4.0 exactly
    /// (mirrors the compile-time pins as a runtime witness).
    #[test]
