//! Stage B trace-link embedding (atom #94 · B.1.13).
//!
//! Every Stage B chunk header (atom #84 [`StageBChunkHeaderV1`]) already carries
//! a non-optional atom #81 [`StageBTraceLink`] field, so a chunk can never be
//! detached from its replay/measurement stamp *at the type level*. This module
//! mints the canonical OUT for atom #94: [`StageBTraceEvidence`] — the
//! **content-free** evidence carrier that the log / metrics seam reads so that
//! "기억과 측정이 분리되지 않는다" (memory and measurement are never separated)
//! becomes an executable invariant, not only a struct field.
//!
//! # Madness invariants (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.0 / atom #94)
//!
...
//!   [`StageBChunkError`](crate::StageBChunkError) variant) mirrors the atom
//!   #81–#87/#93 precedent; the frozen `#[non_exhaustive]` §4.1 error set is not
//!   widened here.
//!
...
//!
//! # Reuse map (atom contract — reuse: #81, #84)
//!
//! * **reuse: #81 [`StageBTraceLink`]** — the `(trace_id, atom_id, attempt)`
//!   stamp, used verbatim from [`stage_b_handoff`](crate::stage_b_handoff). No
//!   second trace/stamp type is minted.
//! * **reuse: #84 [`StageBChunkHeaderV1`]** — the content-free chunk header,
...
//!
//! # Scope note — the a-core log / metrics *emission* seam is a later atom
//!
//! The atom #94 madness line says every chunk points to its trace **and** to
//! "a-core log/metrics evidence". The atom's `reuse` field is `#81, #84` only —
//! it does **not** list the Stage A `a-core` logging/metrics canonical, and
//! `mnemos-b-memory` carries no `mnemos-a-core` dependency. Adding one here
//! would step outside the reuse contract and into a later integration atom's
//! scope (the same deferral discipline by which the sign path stays in
//! `g-wallet` #150 and the publish owner-flag stays at §4.4). So atom #94 mints
//! the content-free, redaction-safe **projection** ([`evidence_ids`] /
//! [`StageBTraceEvidence`]) that the a-core log/metrics seam will consume, and
//! defers the actual `a-core` emission wiring to that seam.
//!
...
/// trace stamp.
///
/// This is the atom #94 canonical OUT: an *evidence* type combining the atom
/// #84 header with its atom #81 [`StageBTraceLink`] (exactly the
/// "A canonical을 조합하는 … evidence 타입" category that §4.0 permits Stage B to
/// mint). It carries **only** the trace — never the chunk body, owner address,
/// or parent blob — so it is redaction-safe by construction and can be emitted
/// to a log / metrics line without any content leaving the machine.
///
...
impl StageBTraceEvidence {
    ...
    /// missing/unstamped sentinel (`atom_id_u16 == 0`) fail-closed.
    ///
    /// `atom_id_u16 == 0` is treated as "missing" because Stage A atom `#0` is
    /// `RESET` — never a real memory-producing action — so a zero atom id means
    /// the action is not bound to any atom and its evidence trail would be
    /// detached from measurement.
    #[inline]
...
mod tests {
    ...
    /// `owner` and `trace`, reusing the atom #84 validated constructor.
