//! `stage_b_measure.rs` (atom #118 · B.2.17 trace telemetry for Walrus) — the
...
//!
//! This module mints the atom #118 canonical OUT: [`StageBWalrusMeasure`] — a
//! per-action measure record that pairs the Walrus action *kind*
//! ([`WalrusActionKind`]) with the atom #112 content-free [`WalrusDiagnostics`].
//! It is exactly the "A canonical을 조합하는 … evidence 타입" category that §4.0
//! permits Stage B to mint: it composes earlier-atom canonicals and introduces
...
//!
//! # Madness invariant (`MNEMOS_STAGE_B_ATOM_PLAN.md` atom #118)
//!
...
//!   the action [`kind`](Self::action) and the atom #112 [`WalrusDiagnostics`]
...
//!   redaction pass (the `b2_17_no_payload_body` proof, mirroring the atom #112
//!   `b2_11_canary_payload_not_logged` seam).
//!
//! * **Trace id linked (fail-closed).** [`record`](Self::record) composes the
//!   atom #112 [`WalrusDiagnostics::record`], whose only constructor path runs
//!   through the atom #94 [`StageBTraceEvidence::from_trace`] fail-closed gate:
...
//!   G-B-TRACE gate enforced at the type level — the `b2_17_trace_id_linked`
//!   proof). The emitted evidence carries the same atom number the memory side
//!   stamped, so "기억과 측정이 분리되지 않는다" stays an executable invariant.
//!
...
//!   plan's fourth telemetry dimension — *failure* — is **not** a separate
//!   action kind: it is carried by the reused #112 `status`
...
//!   `action = put|get|verify` *and* `status = walrus.<error>` (OD-1, user-locked
//!   via AskUserQuestion 2026-05-31; the `b2_17_failure_via_status` proof).
//!
...
//!   the atom #118 "A §4.A logging/redaction + A §4.K metrics" reuse applied as a
//!   **discipline** — `mnemos-b-memory` carries no `mnemos-a-core` dependency, so
//!   the redaction-by-construction + allowlist pattern (atom #94 / #112 deferral
...
//!   later integration atom's scope.
//!
//! # Reuse (재발명 0)
//!
//! * #112 [`WalrusDiagnostics`](crate::stage_b_diag::WalrusDiagnostics) — the
...
//! * #94 [`StageBTraceEvidence`](crate::trace_link::StageBTraceEvidence) —
...
//!   type is minted.
//! * #103 [`WalrusClientError`](crate::stage_b_put::WalrusClientError) /
//!   #110 [`WalrusRetry`](crate::stage_b_retry::WalrusRetry) — their content-free
//!   `class_label`s are read through the #112 diagnostics; the frozen
//!   `#[non_exhaustive]` §4.2 error set is not widened.
//!
...
/// body, or provider text. Three variants only — *failure* is the reused atom
/// #112 `status` dimension, not a fourth action kind (OD-1, user-locked
/// 2026-05-31), so success-vs-failure has a single source of truth.
...
///
/// The atom #118 canonical OUT. It records **only** the action
/// [`kind`](Self::action) and the atom #112 [`WalrusDiagnostics`] — and
...
pub struct StageBWalrusMeasure {
    ...
    /// The atom #112 content-free diagnostics: status / latency / retry / trace.
    /// Holds no body; carries the fail-closed atom #94 trace evidence.
...
impl StageBWalrusMeasure {
    ...
    /// success or `Some(e)` for the content-free atom #103 error class (this is
    /// how a *failure* is recorded — there is no `Failure` action kind);
    /// `latency_ms_u32` is the measured cost; `retry` is the atom #110 decision;
    /// `trace` is the atom #81 per-action stamp.
    ///
    /// Returns `None` if `trace` is the missing/unstamped sentinel
    /// (`atom_id_u16 == 0`) — a Walrus action not bound to a real atom mints no
    /// measure record (the G-B-TRACE invariant, reusing the atom #112
    ...
    /// The composed atom #112 content-free diagnostics (status/latency/retry/trace).
    #[inline]
...
mod tests {
    ...
    /// from `json`. Mirrors the atom #112 allowlist helper; deliberately minimal
    ...
    /// `trace id linked` — the G-B-TRACE invariant: a measure record cannot be
    ...
    /// is `action = put|get|verify` *and* `status = walrus.<error>` (OD-1). The
    /// emitted line carries both, so the plan's put/get/verify/**failure**
    ...
    fn b2_17_failure_via_status() {
        ...
        // Action kind is preserved; failure rides on the #112 status dimension.
