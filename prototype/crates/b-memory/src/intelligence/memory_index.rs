...
//! retrieval (sinabro agent-core lane, step 1 of
//! `ops/evidence/stage_g/agent_loop/MEMORY_INDEX_DESIGN.md` §8).
//!
...
//! memories' content (read CU `O(K)`, design §4) — the record is the cheap
//! thing, the content stays cold.
//!
//! # The two physical laws (design §0 — drift unrepresentable, not policed)
//!
...
//!   `verify_reported_blob_id` seam promotes a reported id against, §10.2
//!   self-report ban). Change one byte and you have a *different* address,
//!   not a stale pointer.
...
//!   the memory's hash.
//!
//! # Byte-level lock (design §1 — cross-language)
//!
...
//! the named `OFFSET_*` constants, and the Python cross-language
//! re-computation (`scripts/check_memory_index_record_layout.py`; plan-time
//! verified 2026-06-10: offsets `0/8/40/72/328/330/332/333/334`, size 336,
...
//!
//! # Drift invariants owned here (design §7)
//!
//! | # | invariant | enforcement |
//! |---|---|---|
//! | D1 | location ↔ content never drift | `blob_id_32` = [`derive_walrus_blob_id`] of the blob bytes; re-derivable via [`MemoryIndexRecord::verify_blob_location`] |
//! | D2 | summary ↔ content never drift | `summary = derive_summary(content)` pure, hash-bound; re-derive + compare |
//! | D3 | record layout never drifts | `#[repr(C)]` + const asserts + Python↔Rust width lock |
//! | D6 | a read returns the claimed bytes | [`MemoryIndexRecord::verify_against_content`] re-hashes + re-derives |
//!
//! Fail-closed privacy default (threat model GATE-1): an unclassified memory
...
//!
//! * [`MemoryId`] (atom #29) — monotone local id, stored as its raw `u64`.
//! * [`ContentHash32::of`] (atom #86) — domain-separated content hash.
//! * [`derive_walrus_blob_id`] (Stage B) — local Walrus location derivation.
//! * [`MemoryTier`] (atom #326) — tier byte via [`MemoryTier::tag`].
//! * [`MAX_IMPORTANCE_SCORE`] (atom #322) — importance stays in `0..=10000`.
//! * [`MAX_STAGE_B_CONTENT_BYTES`] (atom #85) — the same 1 MiB content cap,
//!   re-checked fail-closed before hashing (mirrors `stage_b_chunk_digest`).
//!
//! # Build-order status (design §8)
//!
//! * step 1 — the record + deterministic summary (sections 1-5).
//! * step 2 — the trust-tier retrieval selectors (section 6), consumed by
//!   the `memory index` / `memory read <id>` read-only dispatch verbs.
//! * step 3 — the chunk fold (section 7, the index is a CACHE; the chunks
//!   are the truth) + the cache image codec (section 8, re-derivable).
//! * step 4 — the agentic loop driver consumes this same surface.
//!
...
/// Fixed summary capacity in bytes (design §1 `SUMMARY_CAP`). The summary is
/// UTF-8, zero-padded to this width; the index stays `O(N) × fixed`.
pub const SUMMARY_CAP: usize = 256;

/// Total record width in bytes — Python plan-time verified 2026-06-10 and
/// compile-time asserted below. A layout drift is a build failure.
...
/// Fail-closed privacy class default (threat model GATE-1): an unclassified
/// memory is treated PRIVATE, never accidentally shareable. The fold
/// projection (design §8 step 3) consumes this constant when a chunk carries
/// no explicit classification.
pub const UNCLASSIFIED_IS_PRIVATE: bool = true;

/// Owner-facing privacy classification of one memory (threat model GATE-1 /
/// IV2). The byte VALUES mirror the index record's `private_u8` encoding
...
/// lock; Python plan-time verified 2026-06-11).
///
...
/// frontier-bound turn ([`catalog_select`] / IV2) — after the redaction gate,
...
/// Byte offset of `memory_id_u64` (design §1 `@0`).
pub const OFFSET_MEMORY_ID: usize = 0;
/// Byte offset of `content_hash_32` (design §1 `@8`).
pub const OFFSET_CONTENT_HASH: usize = 8;
/// Byte offset of `blob_id_32` (design §1 `@40`).
pub const OFFSET_BLOB_ID: usize = 40;
/// Byte offset of `summary` (design §1 `@72`).
pub const OFFSET_SUMMARY: usize = 72;
/// Byte offset of `summary_len_u16` (design §1 `@328`).
pub const OFFSET_SUMMARY_LEN: usize = 328;
/// Byte offset of `importance_u16` (design §1 `@330`).
pub const OFFSET_IMPORTANCE: usize = 330;
/// Byte offset of `tier_u8` (design §1 `@332`).
pub const OFFSET_TIER: usize = 332;
/// Byte offset of `private_u8` (design §1 `@333`).
pub const OFFSET_PRIVATE: usize = 333;
/// Byte offset of the explicit two-byte tail pad (design §1 `@334`).
...
/// One fixed-layout catalog entry per memory (design §1).
///
...
pub struct MemoryIndexRecord {
    ...
    summary_len_u16: u16,
    /// Importance score in `0..=MAX_IMPORTANCE_SCORE` (Stage D scale).
    importance_u16: u16,
    /// [`MemoryTier`] stable tag (`1..=4`; `4` = tombstone, design IV3).
    tier_u8: u8,
...
// Compile-time byte lock (D3): a drift in size, alignment or ANY field offset
// is a build failure, before any test runs. Mirrors the Python side
// (`scripts/check_memory_index_record_layout.py`) for the cross-language lock.
...
pub enum MemoryIndexError {
    ...
    TailPadNonZero,
    /// Re-hashing the presented content did not match `content_hash_32` (D6).
    ContentHashMismatch,
    /// Re-deriving the presented blob bytes did not match `blob_id_32` (D1).
    BlobIdMismatch,
...
// ===========================================================================
// 4. Deterministic summary — the pure `f(content)` (design §2, Phase 0)
// ===========================================================================

/// Derive the deterministic Phase-0 summary of `content`: the rule-based
/// extractive head (design §2, "no LLM ⇒ no non-determinism ⇒ no drift, and
...
impl MemoryIndexRecord {
    ...
    /// ([`derive_walrus_blob_id`]). The fold projection (step 3) uses
    ...
    /// Importance score (`0..=MAX_IMPORTANCE_SCORE`, Stage D scale).
    #[inline]
    #[must_use]
    ...
    /// retrieval (design IV3), so corruption degrades to denial, never to
    /// exposure.
    #[inline]
    #[must_use]
    ...
    /// an explicit `0` is shareable (threat model GATE-1).
    #[inline]
    #[must_use]
    pub const fn is_private(&self) -> bool {
        self.private_u8 != 0
    ...
    /// same constants the compile-time asserts and the Python checker pin).
    #[must_use]
    ...
    /// D6 / law-2 read-side verify: re-hash the presented content and
    ...
    /// D1 / IV4 read-side verify: re-derive the Walrus location of the
...
// ===========================================================================
// 6. Retrieval selectors — design §5 (step 2), pure + trust-tier aware
// ===========================================================================

/// Why a `memory read` candidate was denied (design §5; data-free, `Copy`).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[non_exhaustive]
pub enum MemoryReadDeny {
    /// The id is not in the index (an unindexed memory is unreadable).
    NotInIndex,
    /// The record is tombstoned — terminal, never resurrected (IV3 layer 1).
    Tombstoned,
    /// The record is private and the read is frontier-bound (IV2/D7): a
...
/// List-time catalog selection (design §5 `memory index`): tombstoned records
/// are excluded ALWAYS (IV3, list layer — a deleted memory is never a
/// retrieval candidate); private records are excluded ONLY when the catalog
/// is frontier-bound (IV2 pre-filter / D7 — the local owner surface and a
    ...
    records
        ...
        .filter(|record| {
            if record.is_tombstone() {
                // IV3 — a deleted memory is never a retrieval candidate.
                return false;
            }
            if frontier_bound && record.is_private() {
                // IV2/D7 — a private record never lists for a frontier turn.
                return false;
            }
            true
        })
...
/// Read-time gate (design §5 `memory read <id>`): the id must be in the
/// records (else [`MemoryReadDeny::NotInIndex`]), not tombstoned (IV3, else
/// [`MemoryReadDeny::Tombstoned`]) and — when frontier-bound — not private
/// (IV2/D7, else [`MemoryReadDeny::PrivateToFrontier`]). Tombstone denial
/// outranks privacy (the stronger no-resurrection signal).
///
/// Content integrity (IV4/D6) is the NEXT gate, on the bytes the caller then
...
// ===========================================================================
// 7. Index fold — the index is a CACHE; the chunks are the truth (design §3)
// ===========================================================================

/// Outcome of one index fold (D4): the projected records plus honest skip
...
/// ([`UNCLASSIFIED_IS_PRIVATE`], GATE-1 fail-closed); importance IS scored
/// (P1-2). Production dispatch folds the persisted store through
...
/// Fold owner-CLASSIFIED chunks into the index: a PURE projection (design
/// §3, D4). The index never becomes a second source of truth — re-running
...
/// loss.
///
/// Projection (P1-2; each refines later WITHOUT a format change):
/// * `importance` = the deterministic Stage-D [`ImportanceModel`] score over
///   `{recency rank, access count = 0, content length}` — a pure function of
///   the input sequence (L2: same store, same scores). The recency rank is
...
///   time; IV2 — ONLY an explicit [`MemoryPrivacy::Shareable`] record may
///   later list frontier-bound, and the redaction gate still applies).
/// * `tier` = `Recent`, except the delete truth ([`TombstonePolicy`])
///   projects `DeletedTombstone` (IV3 — `catalog_select` then excludes it).
    ...
    for (position, (chunk, privacy)) in items.into_iter().enumerate() {
        ...
        let importance_u16 = if deleted {
            ...
            let features = ImportanceFeatures {
                ...
                // inside the saturation points (Python-verified 2026-06-11).
...
// ===========================================================================
// 8. Index image codec — the local cache FORMAT (design §3)
// ===========================================================================
//
// The serialized image is the at-rest cache representation. HONEST SURFACE
// NOTE: the workspace has no real at-rest cipher yet (`f-seal` is the Seal
// STUB), so the encrypted-at-rest WRITE binds when the real seal/crypto seam
// lands — this codec deliberately carries NO key material and claims no
// confidentiality. Until then the image is a caller-managed artifact whose
// loss or corruption is a cache miss by construction (decode fails closed ⇒
// re-fold from chunks).
...
mod tests {
    ...
    /// D3 — the byte lock, runtime-visible: size 336, align 8, and the full
    /// Python-verified offset table (`0/8/40/72/328/330/332/333/334`).
    #[test]
    ...
    /// D6 + law 2 — verify detects drift: wrong content fails the hash, and
    ...
    /// D1 — the location IS the content: `from_content` binds both hashes to
    ...
    fn record_binds_location_and_hash_to_content() {
        ...
        // the encoded wire (the step-3 fold shape).
    ...
    /// byte image; tombstone reads as tombstone (IV3); ONLY an explicit `0`
    /// is shareable; the unclassified default is private (GATE-1).
    #[test]
    ...
    /// D5 + D7 — list layer: tombstoned records never list (either tier
    ...
    /// D5 + D7 — read layer, exhaustive over tier × private × frontier: a
    ...
    /// D4 — the fold is a pure deterministic projection: same chunks + same
    /// delete truth ⇒ bit-identical records; fields project per the P1-2
    /// choices (scored importance, fail-closed private via the unclassified
    /// wrapper, tombstone tier from the delete truth) and `catalog_select`
    /// then excludes the tombstone (the step-2 chain).
    #[test]
    fn fold_is_deterministic_and_projects_fields() {
        ...
        // Step-2 chain: the folded tombstone never lists.
    ...
    /// D1/D4 — a chunk with a VERIFIED Walrus ref binds that location (the
    ...
    /// P1-2 — the fold SCORES importance deterministically (L2): exact
    /// Python-pinned fixtures (2026-06-11); appended-later never scores
    ...
    /// P1-2 — a tombstoned chunk scores the importance FLOOR 0 (the model
    ...
    /// P1-2 — the classified fold projects the OWNER class per chunk; the
    /// unclassified wrapper stays all-private (GATE-1); and the IV2 chain
    ...
    fn fold_classified_projects_owner_classes() {
        ...
        // IV2 — the frontier catalog lists ONLY the explicit shareable record.
    ...
    /// P1-2 — MemoryPrivacy byte lock: the persisted tag values mirror the
    ...
    /// D4 — the cache image round-trips exactly; EVERY corruption class is a
    ...
    fn index_image_round_trip_rejects_corruption_and_recovers() {
        ...
        // RECOVERY (the D4 point): every decode failure above degrades to a
