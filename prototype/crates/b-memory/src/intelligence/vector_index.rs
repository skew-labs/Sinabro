//! Custom int8-quantized vector index.
//!
//! [`HnswInt8Config`] carries the HNSW tuning parameters (`m`,
...
//! default bound is **≤ 80 bps**). Search is an int8-quantized scan today.
//!
//! ## Reserved graph layer
//!
//! `m_u8` / `ef_construction_u16` / `ef_search_u16` are validated and persisted
//! but the current search is an **int8 exact (flat) scan**, not yet a navigable
//! small-world graph traversal. The requirement here is recall-loss-bounded,
//! with no performance or scale requirement yet, and it
//! deliberately avoids over-claiming a graph that is not yet built; the graph
//! traversal that consumes the `m`/`ef` parameters is a later addition. This
//! boundary is documented explicitly so the reserved-versus-active split is clear.
//!
...
/// HNSW + int8 vector index configuration.
