//! Memory Intelligence module boundary (Stage D Cluster 6, atom #321 · D.5.0).
//!
...
//!
//! ## Cross-WorkPackage boundary note (DeleteSemantics)
//!
//! [`DeleteSemantics`] is declared here as a shared §4.6 module-boundary enum so
//! that atom #327 ([`user_model`]) `UserModelDelta` can compile inside this
//! WorkPackage (D-WP-06, #321-#327). The atom plan attributes `DeleteSemantics`
//! *plus its tombstone policy* to atom #328 (D-WP-08, #328-#329): the
...
//! [`portability`] module (#329) consumes that policy for export / import /
...
// ===========================================================================
// 1. DeleteSemantics — shared §4.6 enum (cross-WP boundary, see module doc)
// ===========================================================================

/// How a deleted memory is removed (§4.6). Declared at the module boundary so
/// atom #327 `UserModelDelta` can attach deletion semantics inside D-WP-06;
/// atom #328 (D-WP-07) imports this enum and owns the tombstone / resurrection
...
pub enum DeleteSemantics {
    ...
    /// silently re-materialized (the #328 policy enforces zero resurrection).
    Tombstone = 1,
...
// ===========================================================================
// 2. StageDEvidenceRef — redacted evidence reference (§4.0)
// ===========================================================================

/// A redacted reference to a Stage D evidence artifact (§4.0): a 32-byte path
...
/// recommendation authority (G-D-EVIDENCE-SUMMARY). This boundary constant
...
// ===========================================================================
// 3. StageDPolicyObservation — measurement-only self-evolution signal (§4.6)
// ===========================================================================

/// Kind of measurement-only policy observation (§4.6). Each names a surface the
...
/// A measurement-only self-evolution observation (§4.6).
///
...
/// itself into a production policy mutation. This hardens the §4.6 field (which
/// the plan sketches as `pub`) into the G-D-SELF-EVOLUTION-MEASURE invariant —
/// an observation is evidence, never authority. Promotion is a Stage E decision
...
// ===========================================================================
// 4. ReadOnlyBaseline — read-only A/B observation (atom #321)
// ===========================================================================
