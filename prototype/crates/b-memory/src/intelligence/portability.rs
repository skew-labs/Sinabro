//! Portable memory bundle + offline replay.
//!
//! A [`PortableMemoryBundle`] is the user-owned, portable root of a memory:
...
//! ([`TombstonePolicy::tombstone_hash_32`]). [`export_bundle`] produces one;
...
//!   [`TombstonePolicy`]; a tombstoned id is never re-applied, so
...
//! [`StageDEvidenceRef`] provenance summary — never a live
...
/// Portable memory bundle — the user-owned root for export / import / replay.
///
/// Exactly the four fields; there is deliberately **no** policy field, so an
...
pub struct PortableMemoryBundle {
    /// Verified root blob id (never a bare server-reported id).
    pub root_blob: VerifiedBlobId,
    /// Stage B replay transcript hash (carried verbatim, never re-minted).
...
/// Compute the bundle `user_model_hash_32` from a [`UserModelDelta`]: a
...
/// Replay-portability report — the outcome of replaying a bundle's candidate
...
impl ReplayPortabilityReport {
    /// Whether the replay upheld both criteria: the transcript is stable
...
impl ProviderMigration {
    ...
    /// Every candidate is routed through the [`TombstonePolicy`]: a tombstoned
