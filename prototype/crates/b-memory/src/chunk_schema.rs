...
//! downstream Stage B chunk surface (`StageBChunkHeaderV1`,
//! `StageBChunkView`, …) names them through one Stage B path, and
...
//! * **Reject is a predicate, not an invented canonical error.** The schema
//!   declares a `StageBChunkError` whose variants (`ContentTooLarge` / `SignatureInvalid`
//!   / `PublishClassDenied` / `NonCanonicalAChunk`) belong to the later content-
//!   cap / signature / publish layers. Minting that enum now — with four unused
//!   variants — would be premature, so the reserved-flag reject is expressed as
//!   an `Option` (`None` = rejected), following the reject-as-predicate
//!   precedent used elsewhere in the schema. The full `StageBChunkError` is minted
//!   by the module that first owns its other variants.
//!
//! # Reuse map
//!
//! * **reuse: Stage A [`ChunkKind`] / [`MemoryRole`]** — chunk kind/role wire tags,
//!   re-exported, not redefined.
//! * **reuse: Stage A [`PublishPayloadClass`]** — content classifier, re-exported.
...
//! * **reuse: network boundary** — `StageBNetwork` (the testnet typed guard)
//!   is the live-network sibling; this module is a pure tag/flag surface with zero
//!   external action, so it consumes no network value (the network enters at the
//!   Walrus/Sui layers).
...
/// the chunk header wire form (mirrors the byte-stable `Evidence*Class` and
/// `StageBNetwork` enums).
...
pub enum StageBChunkFlags {
    ...
    /// the name must never imply confidentiality).
    SealStubbed = 4,
}
...
impl StageBChunkFlags {
    ...
    /// predicate, following the same precedent used elsewhere. The
    /// `StageBChunkError::ReservedFlags` variant is minted by the later module that owns
...
// ===========================================================================
// StageBChunkHeaderV1 — fixed chunk header
// ===========================================================================

/// Stage B chunk schema version carried in [`StageBChunkHeaderV1::schema_version_u8`].
///
/// The chunk schema lists this const inside the signed-chunk-schema block; the
/// header is its first and defining consumer, so it is minted here (the
/// `from_tag`-style reverse map and the full chunk codec — `encode_stage_b_chunk`
/// / `decode_stage_b_chunk` — remain Stage A's wire and arrive in later modules).
...
/// `StageBChunkError`: the full error enum belongs to the content-cap /
/// signature / publish layers, and reject-as-predicate follows the same
/// precedent used elsewhere. The later module owning `StageBChunkError::ReservedFlags`
/// maps this `None` onto it at that boundary.
///
/// The public fields mirror the canonical declaration; `new` is the
...
impl StageBChunkHeaderV1 {
    ...
    pub fn new(
        ...
        // Reserved-flag reject (reuse the fail-closed validator).
    ...
    /// allocation** per encode. The encoded length is
...
/// (the design rule). The value `1_048_576 = 2^20` fits in a `u32` and is
...
/// allocation-free. Stage B does not
/// mint a new chunk wire — the body still rides Stage A's envelope; this view is
/// the zero-copy lens a digest / signer pass reads through.
///
/// # Invariants (constructed via [`new`](Self::new), fail-closed)
///
...
/// *digest* (`stage_b_chunk_digest`), which hashes header and envelope
/// together. This module owns only the borrowed lens + the content cap.
///
/// `new` returns `Option<Self>` (`None` = rejected) — reject-as-predicate,
/// following the shared precedent. The `StageBChunkError::ContentTooLarge` variant is minted
/// by the later module that owns the full chunk-error surface and maps this `None`
...
impl<'a> StageBChunkView<'a> {
    ...
    pub fn new(header: StageBChunkHeaderV1, envelope: &'a ChunkEnvelopeV1) -> Option<Self> {
        ...
        // Content cap: reject oversized bodies
...
mod tests {
    ...
    fn b1_2_flag_tag_roundtrip() {
        ...
        // Discriminants are exactly the canonical byte values.
        ...
        // canonical value (no `from_tag` exists in Stage A, and none is invented).
    ...
    /// `b1_3_header_encode_layout` — the design spec: ownership (owner) and
    ...
    // -----------------------------------------------------------------------
    // borrowed chunk view + content cap
    // -----------------------------------------------------------------------
