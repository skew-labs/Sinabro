//! Stage B round-trip receipt — the **content-free** receipt minted after a
//! Walrus PUT→GET round trip completes.
//!
//! This module mints one entry point: [`WalrusRoundTripReceipt`] — the
//! evidence record proving that a chunk was published and read back, carrying
//! the **verified** blob id (the local-derive trust root), the storage
//! reference (the lock-in breaker), the measured PUT/GET latencies, the
//! transferred *length*, and the per-action [`StageBTraceLink`]. It composes
//! earlier Stage A / Stage B canonical types — a receipt/evidence record — and
//! introduces no new wire, no new error type, and no new id/address newtype.
//!
//! # Invariant
//!
...
//!   [`VerifiedBlobId`](mnemos_c_walrus::VerifiedBlobId), whose sole
...
//!   [`StageBTraceLink`] verbatim; [`trace_evidence`](Self::trace_evidence)
//!   projects it into the content-free [`StageBTraceEvidence`], which
...
//! # Reuse
//!
//! * [`VerifiedBlobId`](mnemos_c_walrus::VerifiedBlobId) — the local-verify
//!   trust root, carried unchanged (no Stage-B receipt-local id type).
//! * [`StorageObjectRef`](crate::StorageObjectRef) — the backend/role/
...
///
/// Fields are `pub` per the canonical registry; [`new`](Self::new) and
...
pub struct WalrusRoundTripReceipt {
    /// The locally-verified blob id the round trip published and
    ...
    /// The storage reference: backend kind / role / phase, the
    ...
    pub bytes_u32: u32,
    /// The per-action trace stamp `(trace_id, atom_id, attempt)`.
    pub trace: StageBTraceLink,
}

impl WalrusRoundTripReceipt {
    /// Construct a receipt from its six components.
    ///
    ...
    /// `bytes_u32` field is a `u32`, and a silent `as u32` truncation would
    ...
    /// Project this receipt's trace into the content-free
...
mod tests {
    ...
    /// call it; this is the test-only duplicate used to synthesize a *correctly*
    ...
    /// (`stage_b_verify_blob_id`): derive the local id, encode it as
    ...
    /// a const constructor (no receipt-local storage type).
    ...
    fn b2_8_trace_linked() {
        ...
        // Missing/unstamped sentinel — fail-closed to None.
