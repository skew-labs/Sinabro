//! Memory chunk model (atom #29 · B.0.1).
//!
...
//! canonical [`ChunkEnvelopeV1`] (atom #7 · C.0.1) together with a
...
//! chunk has been planned for persistence (atom #31 · B.0.3) — i.e. a chunk
//! that has only been appended to the in-memory store (atom #30 · B.0.2)
//! carries no backend pointer.
//!
//! # Madness invariants (`MNEMOS_ATOM_PLAN.md` §B.0.1)
//!
//! * **Codec reuse, zero duplicate wire.** [`MemoryChunk`] holds a real
//!   [`ChunkEnvelopeV1`] from `c-walrus`; no separate wire encoding exists
//!   in `b-memory`. Encode/decode goes through atom #7's
//!   `encode_chunk_v1` / `decode_chunk_v1`. This is the cross-language
//!   schema-lock contract carried forward from atom #7.
//! * **Monotone local id.** [`MemoryId`] is a single `u64`; ids are minted
//!   by the in-memory store (atom #30) and only ever increase within a
//!   single supervisor lifetime. Crash recovery (atom #32) re-derives ids
...
//!   the responsibility of the persistence plan (atom #31). Decoupling
//!   here keeps the in-memory store (atom #30) network-blind.
//!
//! # Reuse map (atom contract)
//!
//! * [`ChunkEnvelopeV1`] — atom #7 · C.0.1 (`mnemos-c-walrus::codec`).
//! * [`VerifiedBlobId`] — atom #10 · C.0.4 (`mnemos-c-walrus::blob_id`).
...
/// (atom #30 · B.0.2). The wrapped `u64` is private; ids are obtained
/// either from the store's `append` return value or by reconstructing the
/// `MemoryId` from on-chain anchors during replay (atom #32 · B.0.4).
///
/// A `MemoryId` is **not** a content-addressable digest; the canonical
/// content hash lives in [`StorageObjectRef::content_hash_32`] (B.0.1) and
/// the anchor seed (atom #7's `MoveAnchorSeedV1`).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash, Ord, PartialOrd)]
#[repr(transparent)]
pub struct MemoryId(u64);

impl MemoryId {
    /// Construct a `MemoryId` from a raw `u64`. The store (atom #30) is
    /// the only intended caller — external callers obtain a `MemoryId`
    /// via the store's `append` return value or via crash replay.
    /// Exposed here so the cross-crate boundary between B.0.1 (chunk
    /// type) and B.0.2 (store) does not require re-opening this module
    /// in a future atom.
    #[inline]
    ...
    /// silently — the saturating sentinel is what atom #30's
    /// `b0_1_memory_id_monotone` and B.0.2's `CapacityExceeded` will
...
/// (Phase 1+). No tag overlap with codec enums (atom #7) by virtue of
...
impl StorageBackendKind {
    ...
    /// (atom #31) consults before emitting a write plan. This is a
    ...
    /// as a `const fn` for downstream serialization layers (atom #31)
...
pub enum StorageBackendPhase {
    ...
    /// persistence planner at atom #31).
    DryRunOnly = 2,
...
/// chunk envelope (atom #7's `BlobId`-domain bytes), and an optional
/// [`VerifiedBlobId`] from atom #10. The blob id is `Some(_)` exactly
/// when the backend is `Walrus` and the publisher's reported id has been
/// locally re-derived and byte-matched (the §10.2 self-report ban).
///
...
impl StorageObjectRef {
    ...
    /// (atom #10 self-report-refused). Used by the persistence planner
    /// (atom #31) when emitting a Walrus PUT plan.
    #[inline]
    ...
    /// 32-byte canonical content hash (atom #10 `BlobId` domain).
    #[inline]
...
/// canonical bytes in this Phase 0 in-mem world; persistence (atom #31)
...
impl MemoryChunk {
    ...
    /// (atom #31 · B.0.3) and is performed via [`with_storage`].
    /// Intended caller: the in-memory store (atom #30 · B.0.2).
    #[inline]
    ...
    /// side — atom #31 produces the post-plan chunk, the store may
    /// then replace the slot. Re-attachment overwrites a previous
    /// `Some(_)`; this matches the persistence-replan contract for
    /// recovered chunks (atom #32).
    #[inline]
    #[must_use]
    ...
    /// Optional persistence reference. `None` before atom #31 emits a
    /// plan; `Some(_)` after persistence has been planned.
    #[inline]
    ...
    /// Convenience predicate for B.0.2 tests; equivalent to
...
// Pin the cross-atom byte invariant: atom #29 assumes `MemoryId` is the
...
// zero-length array index (mirror of atom #10's pattern).
...
// ===========================================================================
// 6. Inline unit tests — atom #29 spine
// ===========================================================================
...
mod tests {
    ...
    /// `tests/` (atom #29 follow-on) carries the full Walrus path.
    ...
    /// `MNEMOS_ATOM_PLAN.md` §4.B. A future drift in any enum value
