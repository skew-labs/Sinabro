//! #166 (B.5.3) — transcript hash determinism + replay report + the top-level
//! [`replay_stage_b`].
//!
//! Canonical OUT (§4.5): [`StageBTranscriptHash32`], [`StageBReplayReport`], and
//! [`replay_stage_b`].
//!
//! Madness clause (plan #166): *the same inputs always produce the same
//! transcript hash across process restarts and event fetch order.* Offline; no
//! live egress.
//!
//! ## How determinism is achieved
//!
//! `replay_stage_b` walks the (already coordinate-normalized, #163) anchor stream
...
//! (atom #108 [`derive_walrus_blob_id`], the same primitive that derives blob
...
/// §4.5 transcript hash — a 32-byte deterministic digest of a full replay.
///
...
/// §4.5 replay report — the transcript hash plus the applied / duplicate /
...
pub fn stage_b_transcript_hash(records: &[u8]) -> StageBTranscriptHash32 {
    ...
    // `benches/stage_b_replay_transcript.rs` (criterion; G-B-BENCH). The hot work
...
/// §4.5 — replay the Stage B custody chain from a normalized input, producing a
/// deterministic [`StageBReplayReport`].
///
/// Walks the anchor stream then the audit stream, applying each event through the
/// #165 state machine, recording a transcript, and hashing it once. Returns
...
/// established earlier, by the #163 normalizer that builds the input).
