//! Walrus blob fetch + signed chunk decode + digest match.
//!
//! Canonical OUT: fetch verified blobs and decode
//! [`StageBSignedChunkV1`]. Invariant: *replay uses verified blob id only;
...
//! chunk ([`encode_stage_b_chunk`]) and derives the blob id **locally**
//! ([`derive_walrus_blob_id`]). A claimed `blob_id` is honored only if
...
//! Walrus GET supplies the bytes through the separate ceremony rail and
...
/// envelope and running the local derive.
///
...
impl<'a> ReplayBlobIndex<'a> {
    ...
    /// sizes; no `std::collections`, mirroring the crate's dedup style).
