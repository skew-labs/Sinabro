//! Stage B replay (Cluster 4 · B.5.0–B.5.5) — deterministic crash-recovery
...
//!
//! Submodules:
//! - [`events`] (#163): normalize the on-chain `ChunkAnchored` / `AuditAppended`
//!   stream into a totally-ordered, len32-validated form.
//! - [`blob_fetch`] (#164): fetch by [`VerifiedBlobId`] only and decode the
//!   signed chunk, checking the decoded digest against the anchored digest.
//! - [`cursor`] (#165): the replay state machine that reuses the Stage A
//!   [`ReplayCursor`](crate::replay::ReplayCursor) and yields one decision per
//!   anchor.
//! - [`transcript`] (#166): the deterministic transcript hash + replay report,
//!   and the top-level [`replay_stage_b`].
//!
//! Offline by construction: nothing in this module performs network or wallet
//! I/O. The one approved live roundtrip (#168) drives this code from a separate
...
/// §4.5 replay error — the **whole-stream abort** conditions for
