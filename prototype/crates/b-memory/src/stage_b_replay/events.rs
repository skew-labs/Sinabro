//! Event snapshot normalizer.
//!
//! Canonical OUT: normalized Sui `ChunkAnchored` and
...
//! names here are *event-type references* (the on-chain shapes minted by the
//! Move contracts), not a live action: this file only reorders and validates
//! an already-observed event stream.
//!
//! ## Why normalize first
//!
//! A Sui RPC may return events for a checkpoint in any order and may repeat a
//! page on retry. Replay determinism requires that the *order* the
//! cursor applies anchors in is a pure function of the on-chain history,
...
/// the call-args form carrying `root`, the verified [`MoveAnchorArgsV1`], and
...
pub struct StageBChunkAnchoredEvent {
    ...
    /// Typed anchor call args, digest already len32-validated.
...
impl StageBChunkAnchoredEvent {
    ...
    /// adapter [`MemoryRootAnchorArgs::try_from_move_vectors`] — the
...
pub struct StageBAuditAppendedEvent {
    /// On-chain ordering coordinate.
    pub coord: StageBEventCoord,
    /// Typed audit-append call args, entry hash already len32-validated.
...
impl StageBAuditAppendedEvent {
    ...
    /// [`AuditAppendArgs::try_from_move_entry_hash`].
    #[inline]
    pub fn from_move_vectors(
        coord: StageBEventCoord,
        log: ObjectId,
...
pub fn normalize_event_stream(
    ...
    // the linear/sort style of `replay_from_anchors`.
