//! #165 (B.5.2) — replay cursor and apply order.
//!
//! Canonical OUT (§4.5): [`StageBReplayState`], [`StageBReplayDecision`], and the
//! per-event apply logic that the top-level
//! [`replay_stage_b`](super::transcript::replay_stage_b) (#166) walks.
//!
//! Madness clause (plan #165): *replay reuses the Stage A
...
//!   events at the same on-chain coordinate), which the normalizer (#163) already
//!   refused.
//! - **MissingBlob** / **DigestMismatch** — the verified-blob outcome from
//!   [`fetch_for_anchor`] (#164).
...
/// §4.5 replay input — the on-chain anchor and audit streams (already normalized
/// into on-chain order by the #163 normalizer) plus the set of published signed
...
pub struct StageBReplayInput {
    /// Ordered `ChunkAnchored` call-args (§4.3).
    pub anchors: Vec<MemoryRootAnchorArgs>,
    /// Ordered `AuditAppended` call-args (§4.3).
...
/// §4.5 replay state — the Stage A [`ReplayCursor`] wrapped with Stage B
/// progress counters.
///
/// `cursor` is the canonical Stage A crash-recovery cursor (atom #32): its
...
/// §4.5 per-anchor / per-event decision. `#[repr(u8)]` with the exact §4.5
...
impl StageBReplayDecision {
    /// The §4.5 discriminant byte. Used by the transcript hash (#166).
    #[inline]
...
pub(crate) fn apply_anchor(
    ...
    // (3) verified-blob + digest match (#164).
