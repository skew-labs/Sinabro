//! `chunk_signature.rs` (atom #89 · B.1.8) — the Stage B **chunk-sign domain**
//! and the digest-level signature **verify** surface.
//!
//! A Stage B memory chunk is signed over its [`ChunkDigest32`] (atom #86), never
//! over the raw header or the raw body. This module mints the two things atom #89
//! owns:
//!
...
//!   ([`OwnerPublicKeyBinding`], atom #88), it reconstructs the domain-separated
//!   preimage and checks the ed25519 signature against the binding's public key.
//!
//! # Madness invariant (`MNEMOS_STAGE_B_ATOM_PLAN.md` atom #89)
//!
//! > chunk signature domain과 Sui tx signature domain을 분리한다. 같은 key라도
//! > message domain 혼동 불가.
//!
...
//! `mnemos-g-wallet` atoms #34 / #35 pin between the two Sui scopes, extended here
//! to a third, Stage-B-private scope. The domain is also distinct from the digest
//! domains (`CHUNK_DIGEST_DOMAIN` / `CONTENT_HASH_DOMAIN`, atom #86): hashing and
...
//!
//! # Scope of atom #89 — verify only; production sign is a later atom
//!
//! §4.1 declares two function signatures over the digest:
//!
//! ```rust,ignore
...
//! ```
//!
//! Atom #89's reuse field is `#86, #88` — it deliberately does **not** pull in
//! `mnemos-g-wallet`. The production sign path (which borrows a
//! `ScopedSecretKey`) is `atom #150` (`g-wallet/src/stage_b_sign_message.rs`,
//! canonical OUT "sign chunk digest for #89", reuse `#89, #148`). Atom #89 mints
//! the **domain** + the **preimage** (so #150 signs byte-identically to what this
...
//! [`OwnerPublicKeyBinding`] rather than §4.1's `owner: SuiAddress`: an ed25519
//! check needs the 32-byte public key, and the public-key → `SuiAddress`
//! derivation was explicitly deferred by atom #88 to the d-move binding seam
//! (`atom #149`, "owner pubkey matches chunk owner"). The binding carries the
...
//! the d-move binding's job, unchanged from atom #88.
//!
...
//! `mnemos-g-wallet` signing surface (atoms #34 / #35) — one 64-byte
...
//! ARX placeholder (atom #86, "no cryptographic claim"); the signature over it is
//! genuine and swaps to the real digest at the net-testnet seam with no change to
//! this verify surface.
//!
//! # Reuse (재발명 0)
//!
//! * #86 [`ChunkDigest32`] — the value signed over (its 32-byte width is the only
//!   variable part of the preimage).
//! * #88 [`OwnerPublicKeyBinding`] / [`SigningPublicKey`] — the verify key carrier.
//! * A `mnemos-c-walrus::SignatureBytes` — the 64-byte signature type, reused
//!   verbatim (no parallel signature wrapper minted).
//! * `ed25519-dalek` — the verify primitive, already a workspace dependency
//!   (`mnemos-g-wallet` since atom #34); newly added to `b-memory` for this verify.
...
/// atom #89 madness spec: "chunk signature domain과 Sui tx signature domain을
/// 분리한다"). This **is** the Stage B chunk-sign domain.
///
/// Distinct from atom #86's digest domains (`CHUNK_DIGEST_DOMAIN` /
...
/// signer (atom #150) and this verifier agree on the exact preimage length.
...
/// is the exact byte sequence the signer (atom #150) signs and
...
///   `SignatureInvalid` variant atom #86 reserved "at the verify atom #89".
///
...
/// seam's job (atom #149), unchanged from atom #88. This verify answers only "is
...
mod tests {
    ...
    // suppress prod-only clippy denies inside this module (b-memory #86/#88
    // precedent).
    ...
    /// Build a minimal valid envelope (mirrors the chunk_digest.rs #86 test
    ...
    /// for the atom #150 wallet signer; the production sign path is deferred).
    ...
    /// (here the atom #86 digest domain, and separately the Sui personal-message
    ...
    fn b1_8_wrong_domain_fails() {
        ...
        // (a) sign over `CHUNK_DIGEST_DOMAIN || digest` (atom #86's hashing
    ...
    /// both atom #86 digest domains and is disjoint from both Sui intent scopes
