//! Stage B trust-boundary attestation surface.
//!
//! # What this module is
//!
//! Declares three attestation types
//! ([`StageBTrustMode`], [`SafetyKernelBuildRef`], [`StageBTrustBoundaryReceipt`])
//! recorded for a live Walrus testnet action. A live testnet action must record
//! *which client build* spoke to the network and *who owns* the resulting
//! memory, so this module provides the minimal data-free surface needed to
//! capture that: it is the attestation companion to the live PUT path.
//!
//! # Invariants
//!
...
//!    is a user-controlled [`SuiAddress`], reused verbatim.
...
//!    The hash fields are `pub` (per the registry shape) so a caller can
...
///
/// Tag values are pinned so the one-byte wire form is stable across the project.
/// There is no `#[non_exhaustive]` — exactly these four modes are fixed.
...
/// action belongs to.
///
/// Fields are `pub`; [`new`](Self::new) is
