//! Stage B Walrus testnet PUT **request planner** (atom #103 · B.2.2).
//!
//! This module mints the §4.2 canonical OUT [`WalrusPutPlan`] — the borrowed,
//! policy-gated, trace-stamped description of a single Walrus testnet PUT — plus
//! the Cluster 2 client error enum [`WalrusClientError`]. It is the third
//! Cluster 2 atom, after #101's
...
//! content-class policy **before** any transport type is touched, and binds an
//! atom #81 [`StageBTraceLink`] to the planned request so every external action
//! is traceable. It performs **no** PUT, opens no socket, and constructs no HTTP
//! type: the transport execution (`stage_b_put_with_transport`, §4.2) is a later
//! atom. The PUT response parser (#104 · `parse_walrus_put_response`) lives in
...
//!
//! The planner reads the atom #113
//! [`stage_b_publish_decision`](crate::stage_b_policy::stage_b_publish_decision)
//! — the three-way decision layered over atom #93's [`stage_b_publish_allowed`]
...
//!
//! The planner takes the trace as an atom #94 [`StageBTraceEvidence`], not a bare
//! [`StageBTraceLink`]. `StageBTraceEvidence` can only exist for a *stamped*
//! trace (`atom_id_u16 != 0`); the unstamped / missing sentinel
//! (`atom_id_u16 == 0`, Stage A atom `#0` `RESET`) is rejected fail-closed by
...
//! because a plan without a real trace cannot be built. The §4.2
...
//!
//! [`WalrusClientError`] is minted here (the first signature in Cluster 2 to
//! return it — the atom #86 [`StageBChunkError`](crate::StageBChunkError)
//! "first-returning signature mints the §-set" precedent, user-ratified). Its
...
//! deliberately distinct types at distinct layers (user-locked 2026-05-30). The
//! full §4.2 variant set is minted frozen `#[non_exhaustive]`; this atom consumes
...
//! variants are forward-reserved for the named later atoms (see each variant
...
//!
//! ## Crate-home decision (#101/#102, user-ratified — applies to #103)
//!
//! The plan's `file:` field names `c-walrus`, but #103 reuses #93 / #94 / #101
//! (all `b-memory` types) and binds them above the raw transport. Hosting the
//! planner in `c-walrus` would require `c-walrus` to import `b-memory` types and
//! form a cargo-rejected `c-walrus -> b-memory -> c-walrus` cycle. The user
//! re-confirmed the Cluster 2 home decision for the whole #101-#120 cluster:
//! Walrus client wrappers live in `b-memory`, `c-walrus` stays raw transport.
//! Only the plan's `file:` crate is corrected; the §4.2 canonical OUT shape is
//! honoured verbatim.
//!
...
// Re-exported in the §4.2 reuse map; named here so the constructor signature and
// the doc links resolve without a full path at every use site.
use mnemos_c_walrus::EpochCount;

/// Stage B Walrus client error (§4.2). The orchestration / policy / trace /
/// blob-verify / boundary error layered *above* the raw `c-walrus` transport,
/// minted in `b-memory` at atom #103 (the first Cluster 2 signature to return
...
pub enum WalrusClientError {
    ...
    /// candidate URL/endpoint maps a #101 allowlist reject here. At atom #103 the
    /// endpoint is the already-validated #101 [`WalrusTestnetEndpoint`] (testnet
    ...
    /// **Consumed at #103**: [`WalrusPutPlan::plan`] returns this when atom #93's
    /// [`stage_b_publish_allowed`](crate::content_policy::stage_b_publish_allowed)
    ...
    /// reserved for the transport execution atom (`stage_b_put_with_transport`,
    /// §4.2); never produced by the offline planner.
    Transport,
...
/// A planned Walrus testnet PUT (§4.2 canonical OUT). Owns nothing of variable
...
///
/// The fields are `pub` per the §4.2 canonical registry; [`plan`](Self::plan) is
...
pub struct WalrusPutPlan<'a> {
    ...
    /// The atom #81 trace stamp this PUT is bound to. Always a *stamped* trace
...
impl<'a> WalrusPutPlan<'a> {
    ...
    /// Steps, in order:
    ///
    /// 1. **Content-class policy before transport** (atom #113 decision layer
    ///    over atom #93): consult
    ///    [`stage_b_publish_decision`](crate::stage_b_policy::stage_b_publish_decision).
    ...
    ///
    /// `endpoint` is the already-validated #101 [`WalrusTestnetEndpoint`]
    ...
    pub const fn plan(
        ...
        // 1. Stage B content-class policy gate (atom #113), BEFORE any c-walrus
...
// ===========================================================================
// atom #104 (B.2.3) — PUT response parser: reported (untrusted) blob id.
// ===========================================================================
...
/// the locally derived id (`verify_reported_blob_id`, atom #108). Keeping the
...
/// match are the verify stage (`verify_reported_blob_id`, atom #108). Both
...
///   verify-stage (#108) tag, listed only so callers see the boundary.
///
...
mod tests {
    ...
    /// A stamped trace evidence for atom #103, for the happy-path tests.
    ...
    /// `b2_12_planner_consumes_decision` — the atom #113 integration: the planner
    /// gate is driven by [`stage_b_publish_decision`], not the bare #93 predicate.
    ...
    // --- atom #104 (B.2.3) — PUT response parser ---------------------------
