//! Stage B Walrus testnet endpoint allowlist.
//!
//! This module mints [`WalrusTestnetEndpoint`] — the canonical binding of
...
//! typed network boundary. It is the Walrus testnet client entry
...
//! cycle that cargo rejects. The field list is honoured verbatim.
//!
//! ## Allowlist invariant
//!
...
//! through the return value (the redaction-by-construction precedent).
//!
//! This module is pure and offline: it performs no I/O, opens no socket, and
//! pulls in no HTTP/TLS surface. The only network it can name is testnet.
//!
...
/// canonical shape verbatim. The type is a plain `Copy` value carrier with
...
impl WalrusTestnetEndpoint {
    ...
    /// the [`StageBNetwork::parse_label`] allowlist) and rejecting every
