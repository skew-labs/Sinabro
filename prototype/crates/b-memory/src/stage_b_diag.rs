//! `stage_b_diag.rs` — the Stage B
//! **content-free** diagnostics record emitted for a Walrus testnet action.
//!
//! This module mints [`WalrusDiagnostics`] — the
//! redacted Walrus diagnostics schema. It is exactly the kind of type that
//! composes a canonical receipt/evidence value: it
//! composes existing canonical types and introduces no new wire, no new error
...
//!   [`SuiAddress`](mnemos_d_move::SuiAddress), no URL/host, and — unlike a
//!   round-trip receipt — not even a transferred *length*. Only those four
//!   diagnostic dimensions are recorded, so a length count is out
...
//!   reuses [`WalrusClientError`] — a data-free `Copy` tag whose
...
//!   status enum is minted and the frozen `#[non_exhaustive]` error set is
//!   not widened.
//!
//! * **Retry decision carried verbatim.** [`retry`](Self::retry) is the
//!   [`WalrusRetry`] decision; its namespaced
...
//!   [`StageBTraceEvidence`], whose only constructor
...
//!   bound to a real trace mints **no** diagnostics record (the
//!   `b2_11_trace_required_fail_closed` proof), enforced at
//!   the type level.
//!
...
//!   proof). This is the "Stage A redaction" pattern applied as a
//!   *discipline* — `mnemos-b-memory` carries no `mnemos-a-core` dependency, so
//!   the redaction-by-construction pattern is
...
//!   status. No new error type, no widening of the frozen error set.
//! * [`WalrusRetry`](crate::stage_b_retry::WalrusRetry) — the retry
...
///
/// It records **only** the four diagnostic
/// dimensions — status, latency, retry, trace — and deliberately
...
pub struct WalrusDiagnostics {
    ...
    /// `Some(e)` is the data-free [`WalrusClientError`] class. No
    /// content can leak through this tag.
    status: Option<WalrusClientError>,
    /// Measured wall-clock cost of the action in milliseconds.
    latency_ms_u32: u32,
    /// The retry decision for the action.
    retry: WalrusRetry,
    /// The content-free trace evidence binding this action to a real
    /// trace event (`atom_id_u16 != 0` guaranteed by the constructor).
...
impl WalrusDiagnostics {
    ...
    ///
    /// `status` is `None` for success or `Some(e)` for the content-free
    /// error class. `latency_ms_u32` is the measured cost; `retry` is the
    /// retry decision; `trace` is the per-action stamp.
    ///
    /// Returns `None` if `trace` is the missing/unstamped sentinel
    /// (`atom_id_u16 == 0`) — a Walrus action not bound to a real trace mints no
    /// diagnostics record, reusing the
    ...
    /// The content-free status of the action: success or the error class.
    #[inline]
    ...
    /// [`WalrusClientError::class_label`] (`walrus.*`). Safe to emit in
    ...
    /// The bound content-free trace evidence.
    #[inline]
...
mod tests {
    ...
    /// `trace required (fail-closed)` — a record cannot
    /// be built for the missing/unstamped sentinel (`atom_id_u16 == 0`); a real
    /// stamped trace records.
    #[test]
    fn b2_11_trace_required_fail_closed() {
        // Missing/unstamped — fail-closed to None.
    ...
    /// `Some(err)` projects to the namespaced class label.
    #[test]
    ...
    fn b2_11_retry_and_latency_preserved() {
        ...
        // The retry label is the content-free class label.
