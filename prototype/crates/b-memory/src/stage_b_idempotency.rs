//! Stage B Walrus PUT idempotency ledger.
//!
//! A Walrus testnet PUT crosses an **external-mutation boundary** (see
...
//! (the canonical [`WalrusPutLedger`] type):
//!
//! * The **trace id** is the [`StageBTraceLink::trace_id_u64`] carried
//!   by a fail-closed [`StageBTraceEvidence`] — an unstamped sentinel
//!   (`atom_id_u16 == 0`) is not representable as evidence, so a ledger key can
//!   never be detached from a measured action.
//! * The **content digest** is the [`ContentHash32`] of the payload
...
//!
//! The middle row is the key invariant: a prior attempt that ended in
//! [`WalrusBoundaryState::BytesMayHaveCrossed`] or
//! [`WalrusBoundaryState::UnknownAfterBoundary`] reuses the
...
//! secret, no payload body. The [`WalrusClientError`](crate::WalrusClientError)
...
///
/// The [`WalrusBoundaryState`] type exposes [`WalrusBoundaryState::tag`] but no
/// inverse (and that module is frozen), so the inverse lives here as a local
/// helper. The three arms mirror the discriminants `1/2/3`; the
...
struct LedgerEntry {
    /// The [`ContentHash32`] bytes of the payload body that was PUT.
    content_hash: [u8; CONTENT_HASH_BYTES],
    /// The trace id (the per-attempt-spanning logical key).
    trace_id_u64: u64,
...
///
/// Data-free `Copy` tag wrapping the reused [`WalrusRetry`]; it carries
...
pub enum WalrusPutDecision {
    ...
    /// inner [`WalrusRetry`] is the boundary decision of that prior
...
/// A PUT idempotency ledger keyed by **content digest + trace id**.
///
/// One entry per trace id (the logical PUT spanning retry attempts).
...
impl WalrusPutLedger {
    ...
    ///
    /// See the module-level decision grid. The key invariant lives in the
    /// `Retry` arm: it reuses the [`WalrusBoundaryState::retry`]
...
mod tests {
    ...
    fn b2_10_duplicate_same_digest_after_unknown_boundary_is_manual_reconcile() {
        // The invariant: same bytes after an unknown boundary returns
    ...
    fn b2_10_boundary_tag_roundtrip() {
        // Pins boundary_from_tag against the #[repr(u8)] discriminants so a
