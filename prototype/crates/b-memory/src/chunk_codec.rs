//! `chunk_codec.rs` (atom #91 · B.1.10 encode + atom #92 · B.1.11 decode) — the
//! Stage B **canonical codec**.
//!
//! This module mints the §4.1 [`encode_stage_b_chunk`] entry point: the single
...
//!
//! # Madness invariant (`MNEMOS_STAGE_B_ATOM_PLAN.md` atom #91)
//!
//! > A `encode_chunk_v1`의 thin wrapper다. 새 BCS wire를 만들지 않고, Stage B
//! > digest/signature preimage만 추가한다.
//!
...
//!
//! * the domain-separated **digest** preimage — atom #86
...
//! (`§4.3`) and the verified-blob decode (atom #92) both read.
//!
//! # Why a named wrapper rather than a bare re-export
//!
//! A `pub use` would also keep the bytes identical, but the canonical OUT in
//! §4.1 is a `fn` with a Stage B name. Giving Stage B one explicit
//! `encode_stage_b_chunk` symbol means the Walrus PUT plan, the blob-id
//! derivation and the atom #92 decode all reference a single Stage B entry point
...
//! # Scope (atoms #91 / #92)
//!
//! Encode (#91) and decode (#92). [`decode_stage_b_chunk`] is the matching thin
...
//! `sign_stage_b_chunk(digest, &ScopedSecretKey)` is atom #150. This wrapper
...
//! [`stage_b_chunk_digest`](crate::stage_b_chunk_digest) (atoms #85 / #86) and is
...
//! # Reuse (재발명 0)
//!
//! * A [`encode_chunk_v1`](mnemos_c_walrus::codec::encode_chunk_v1) — the canonical
//!   V1 encoder; the wrapper delegates to it verbatim.
//! * A [`ChunkEnvelopeV1`](mnemos_c_walrus::codec::ChunkEnvelopeV1) — the input
//!   envelope, reused verbatim (no Stage-B-specific envelope).
//! * A [`ChunkCodecError`](mnemos_c_walrus::codec::ChunkCodecError) — the error
//!   surface, returned unchanged (no new Stage B variant).
//! * #85 [`StageBChunkView`](crate::StageBChunkView) — the borrowed lens; a caller
//!   holding a view encodes `view.envelope` (exercised in tests).
//! * #90 [`StageBSignedChunkV1`](crate::StageBSignedChunkV1) — the signed unit; a
...
/// The Stage B digest (atom #86) and signature (atom #89) preimages are separate
...
/// at the view/digest layer (atoms #85 / #86) and is expressed as
...
///
/// This is the §4.1 canonical OUT for atom #92 (B.1.11). It is a **thin,
...
/// client GETs (#101) and the replay path (#163) re-reads stays consistent
/// with the Move anchor (§4.3 `ChunkAnchored.blob_id`).
///
/// The madness clause "decode 후 re-encode가 입력과 다르면 NonCanonical로
/// reject" is satisfied by reuse, not reinvention: Stage A's decoder already
...
mod tests {
    ...
    /// Build a minimal valid envelope (mirrors the signed_chunk.rs #90 test
    ...
    fn b1_10_golden_bytes() {
        ...
        // Golden literal (Python-verified in Session 1: decimal
    ...
    /// not exercised here — it would need a 13 MB+ allocation; see
    ...
    /// `b1_10_encode_via_stage_b_view` — reuse #85: a caller holding a
    ...
    /// `b1_10_encode_via_signed_chunk` — reuse #90: a caller holding a minted
    ...
    /// digest (#86) → sign-preimage (#89) → signed chunk (#90) → encode (#91).
    #[test]
    ...
    // ---------------------------------------------------------------------
    // atom #92 (B.1.11) — decode_stage_b_chunk + noncanonical reject.
    // ---------------------------------------------------------------------

    /// std-only deterministic PRNG for the property loop. The proptest crate is a
    /// b-memory dev-dep but `#92` keeps its codec property coverage self-contained
    /// in a fixed-seed splitmix64 sweep (the dedicated codec property/fuzz infra
    /// is atom #97); no randomness source, fully reproducible by Session 2.
    ...
    /// `b1_11_prop_roundtrip_and_no_panic` — G-B-PROPTEST (std-only): over 512
