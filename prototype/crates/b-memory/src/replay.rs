...
//! CIDs, or Filecoin deal ids — all banned) but from the
//! on-chain anchor stream itself. The anchor is the ground-truth
//! ledger: each [`MoveAnchorArgsV1`] (surfaced into
//! `b-memory` by the persistence planner) names a
//! `blob_id` (content-derived), a [`ChunkKind`],
...
//!
//! This module pairs with the runtime supervisor: the supervisor
...
//! `MemoryId` sequence that the in-memory store
//! would re-emit on a fresh boot from the same anchor history. The
//! supervisor + systemd pair handles the
...
//!   The ban on backend self-report is enforced by the type
...
//!   parent)` (the anchor derives `PartialEq` / `Eq` / `Hash`), so a
...
//!   change that tags anchors with their backend would necessarily
//!   change `MoveAnchorArgsV1`'s wire shape (the `c-walrus` codec)
//!   and would surface as a compile-time test failure on
//!   the codec's `public_type_sizes_v1` and on the planner's
...
//! # Reuse map
//!
//! * [`MoveAnchorArgsV1`] · [`ChunkKind`] —
//!   `mnemos_c_walrus::codec`.
//! * [`MemoryId`] · [`MemoryId::new`] —
//!   `crate::chunk`.
//! * [`PersistError::Anchor`] —
//!   `crate::persist`. The `Anchor` variant was declared but not
//!   emitted by the persistence planner (reserved for future SDK-side
//!   anchor projection failures); this module is its first emission
//!   site, used for the `u32::MAX` overflow guard.
//! * Crash-recovery framing — the runtime supervisor's
...
/// in-memory store's post-boot rehydration: the
...
mod tests {
    ...
    /// blob id locally — the canonical anchor
    /// constructor used across the persist and replay tests.
    ...
    fn b0_4_backend_location_does_not_change_replay() {
        ...
        //    leak impossible. If a future change adds such a field,
        //    the codec's `public_type_sizes_v1` test would change and
