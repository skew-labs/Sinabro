//! Criterion bench harness — atom #86 · B.1.5 (digest throughput).
//!
//! Measures the Stage B chunk-digest AI-HOT paths and pins their
//! allocation behaviour:
//!
//! 1. `stage_b::content_hash` — [`ContentHash32::of`], the body-only hash. Its
//!    cost scales with the body size (the ARX absorb walks the content), so it
//!    is swept across the content-size ladder. Allocation-free: the body is
//!    absorbed in place.
//! 2. `stage_b::chunk_digest` — [`stage_b_chunk_digest`], the full domain-
//!    separated digest (cap re-check + content hash + header/content bind). Also
//!    swept across the ladder; allocation-free (stack header encode + stack
//!    hashes). The borrowed envelope is built **once, outside** the measured
//!    loop so the body `Vec` allocation is not attributed to the digest.
//!
//! The content-size ladder mirrors `benches/chunk_schema.rs` (64 B / 1 KiB /
//! 16 KiB).
//!
//! Two run modes share the same binary (mirrors `benches/chunk_schema.rs`):
//!
//! * Default `cargo bench` → criterion statistical sweep.
//! * `MNEMOS_BENCH_EMIT_BASELINE=1 cargo bench --bench chunk_digest` → bypass
//!   criterion and write a single canonical `chunk_digest_baseline.json` record
//!   (deterministic 100 warmup + 1000 measured iterations per cell). Path
//!   defaults to `chunk_digest_baseline.json`, overridable via
...
//!
//! Smoke gate: `cargo bench --bench chunk_digest --no-run --offline --locked`
...
use mnemos_b_memory::{
    ContentHash32, StageBChunkFlags, StageBChunkHeaderV1, StageBChunkView, StageBTraceLink,
    stage_b_chunk_digest,
};
...
fn bench_content_hash(c: &mut Criterion) {
    let mut g: BenchmarkGroup<'_, _> = c.benchmark_group("stage_b::content_hash");
    for &size in SIZE_LADDER {
        let body = vec![0u8; size];
        g.throughput(Throughput::Bytes(size as u64));
        g.bench_function(format!("size_{size}B"), |b| {
            b.iter(|| {
                let h = ContentHash32::of(black_box(&body));
                black_box(h);
            });
        });
    }
...
fn bench_chunk_digest(c: &mut Criterion) {
    let mut g: BenchmarkGroup<'_, _> = c.benchmark_group("stage_b::chunk_digest");
    for &size in SIZE_LADDER {
        // Envelope + header + view built ONCE outside the measured loop so the
        // body `Vec` allocation is not attributed to the digest.
        ...
        g.bench_function(format!("size_{size}B"), |b| {
            b.iter(|| {
                let d = stage_b_chunk_digest(black_box(&view));
                black_box(d.is_ok());
            });
        });
    }
...
fn measure_content_hash(size: usize) -> (u128, AllocDelta) {
    let body = vec![0u8; size];
    for _ in 0..WARMUP_ITERS {
        black_box(ContentHash32::of(&body));
    }
    let s = alloc_snapshot();
    let t0 = Instant::now();
    for _ in 0..MEASURED_ITERS {
        black_box(ContentHash32::of(black_box(&body)));
    }
...
fn measure_chunk_digest(size: usize) -> (u128, AllocDelta) {
    let envelope = build_envelope(size);
    let header = build_header(size as u32);
    let view = StageBChunkView::new(header, &envelope).expect("within cap");
    for _ in 0..WARMUP_ITERS {
        black_box(stage_b_chunk_digest(&view).is_ok());
    }
    let s = alloc_snapshot();
    let t0 = Instant::now();
    for _ in 0..MEASURED_ITERS {
        black_box(stage_b_chunk_digest(black_box(&view)).is_ok());
    }
    let ns = t0.elapsed().as_nanos();
    let e = alloc_snapshot();
    (ns, alloc_delta(s, e))
...
fn emit_baseline_json(path: &str) {
    ...
    for &size in SIZE_LADDER {
        let (ns_total, delta) = measure_content_hash(size);
        rows.push(row("content_hash", size, ns_total, delta));
    }
    for &size in SIZE_LADDER {
        let (ns_total, delta) = measure_chunk_digest(size);
        rows.push(row("chunk_digest", size, ns_total, delta));
    }
    ...
    std::fs::write(path, s.as_bytes()).expect("write chunk_digest_baseline.json");
...
fn main() {
    if std::env::var("MNEMOS_BENCH_EMIT_BASELINE").is_ok() {
        let path = std::env::var("MNEMOS_BENCH_BASELINE_PATH")
            .unwrap_or_else(|_| "chunk_digest_baseline.json".to_string());
        emit_baseline_json(&path);
        return;
    }

    let mut c = Criterion::default().configure_from_args();
    bench_content_hash(&mut c);
    bench_chunk_digest(&mut c);
    c.final_summary();
}
