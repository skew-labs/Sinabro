//! Stage C status/budget/kill GA smoke (C-WP-01 · atom #175 · C.0.4).
//!
...
//!
//! * **`/status` is not a slash verb in this codebase.** The §4.K slash grammar
//!   (atom #46) is `{budget, clear, skill, kill}` — there is no `status`
...
//!   `m-agent` (atom #27 · M.0.7); `j-ux` depends only on `a-core` + `e-skill`,
