//! `slash.rs` — Control command grammar (atom #43 · J.0.3).
//!
//! # Why this madness
//!
//! Phase 0 ships zero live control-rail wiring. This atom defines only
...
//! ("파싱은 고정 enum(임의 명령 실행 불가)", ATOM_PLAN line 1263).
//!
//! The grammar deliberately stops short of the side-effect wiring:
//!
//! - `/kill` is the §10.1 폭주 즉시정지 trigger, which a Stage F/H atom
//!   routes to [`mnemos_a_core::runtime::RuntimeSupervisor::request_shutdown`]
//!   (atom #3, reuse concept only — this module imports zero a-core
//!   types and performs zero supervisor side effects).
//! - `/budget` is a cost-ledger (atom #27, M.0.7) query, which a later
//!   atom routes to [`mnemos_m_agent`] — again, concept-reuse only.
//! - `/skill <id>` carries the integer payload as a
//!   [`mnemos_e_skill::manifest::SkillId`] (atom #39), the only
//!   cross-crate import on this surface.
//!
//! Stage F/H promotes `/kill` and `/budget cap` from the normal /
//! background queue onto an express control rail
//! ("Stage F/H가 `/kill`·`/budget cap`을 normal/background queue가
//! 아닌 express control rail로 배선한다", ATOM_PLAN line 1263); this
//! atom carries no routing field. The decision shape is byte- and
//! state-pure (no I/O, no allocations on the success paths beyond the
//! payload integer parse, no `unsafe`).
//!
//! Reuse:
//! - atom #39 (`SkillId`): `#[repr(transparent)] pub struct SkillId(pub u16)`
//!   (`prototype/crates/e-skill/src/manifest.rs:182`) — re-used verbatim
//!   for the `/skill <id>` payload. Adding `mnemos-e-skill` as a
//!   path-dep is the only Cargo.toml delta this atom introduces.
//! - atom #27 (cost telemetry) and atom #3 (supervisor shutdown):
//!   concept-reuse only. No code path here calls into either crate;
//!   Stage F/H performs that wiring per the express control rail
//!   schedule above.
//!
//! Canonical OUT (verbatim from ATOM_PLAN §4.J line 731-733):
//!
//! ```text
...
/// type ("파싱은 고정 enum", ATOM_PLAN line 1263). Variant order is
/// the order in which the commands appear in the canonical OUT
/// signature (`/budget`, `/clear`, `/skill <id>`, `/kill`).
///
/// The enum is `Copy` because every payload it carries is itself
/// `Copy` — [`SkillId`] is `#[repr(transparent)]` over `u16` per
/// atom #39 (`prototype/crates/e-skill/src/manifest.rs:182`).
/// Snapshotting a parse result before dispatch therefore never moves
/// the value out of any surrounding borrow.
///
/// `#[non_exhaustive]` reserves the right for a later Stage F/H atom
/// to add additional control-class commands without breaking
/// downstream `match` exhaustiveness — but this atom locks the four
/// canonical OUT variants in place; no other variant exists today.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[non_exhaustive]
pub enum SlashCommand {
    /// `/budget` — query the daily/period cost ledger (atom #27 ·
    /// M.0.7). This atom only emits the variant; the cost-ledger read
    /// is wired by a later atom.
    Budget,
    /// `/clear` — reset the current turn / progressive-edit state. The
    /// concrete reset action is wired by a later atom (the m-agent
    /// turn engine and the j-ux progressive editor are the eventual
    /// consumers).
    Clear,
    /// `/skill <id>` — switch the active skill manifest. The payload
    /// is a [`SkillId`] (atom #39); the actual manifest re-bind is
    /// wired by a later atom.
    Skill(SkillId),
    /// `/kill` — §10.1 폭주 즉시정지 trigger. Stage F/H routes this
    /// to the supervisor's `request_shutdown` on the express control
    /// rail (ATOM_PLAN line 1263). This atom emits only the variant.
    Kill,
}
...
/// `/kill` is the "임의 명령 실행 불가" spine (ATOM_PLAN line 1263):
/// a smuggled argument cannot ride on top of a recognised control
...
// ===========================================================================
// 3. Tests — three test names verbatim per ATOM_PLAN line 1264
// ===========================================================================
...
mod tests {
    ...
    fn j0_3_parses_budget_clear_skill_kill() {
        ...
        // Unary commands with trailing payload are rejected — the
        // "임의 명령 실행 불가" spine (ATOM_PLAN line 1263).
    ...
    fn j0_3_unknown_slash_is_none() {
        ...
        // collapse to `None` ("값이 u16 범위를 벗어남").
        assert_eq!(
    ...
    fn j0_3_skill_id_parsed() {
        ...
        // inner u16 — atom #39 exposes the inner field as `pub u16`.
