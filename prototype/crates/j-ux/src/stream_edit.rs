//! `stream_edit.rs` — Progressive edit throttle decision.
//!
//! # Why this design
//!
//! Phase 0 never performs a Telegram `editMessageText` call. This module
...
//! API call) is wired by a future addition — this surface is byte-
//! and clock-pure (zero I/O, zero allocations, no `unsafe`, no new
//! dependencies on the j-ux Cargo.toml).
//!
//! Reuse: upstream concept reuse only from `SseDelta` and
//! `TurnState`. This module never touches the SSE token
//! strings themselves; it consumes only the integer `pending_bytes_u32`
//! count that the upstream sse buffer exposes — the delta flows from
//! the sse buffer via a zero-copy connection, consuming the
//! pending_bytes_u32 count without touching token strings.
//!
...
//! (first-call <1.2s, median 1.8s) lives in the J-crate
//! integration and is *not* asserted by this module — the criterion here
//! is the throttle decision time-base only (the edit throttle operates
//! on a time basis; the latency target is enforced at integration).
//!
//! Canonical shape:
//!
//! ```text
...
///
/// The three field widths are pinned by the canonical signature:
/// `u16` for the interval (≤ ~65 s — Phase 0 expects
/// sub-second values, e.g. 1_500 ms = the "median 1.8 s"
...
pub struct ProgressiveEditor {
    ...
    /// edit cadence (expects 1_200–1_800 ms).
...
///    rate-limit budget (`j0_2_no_flush_when_idle` spine).
///
...
// ===========================================================================
// 3. Tests — three test names verbatim per line 1254
// ===========================================================================
...
mod tests {
    ...
    /// accessor (same size-pin pattern as `j0_1_allowlist_is_static`).
    #[test]
