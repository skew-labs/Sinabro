//! `telegram.rs` — Telegram gateway authorization spine (atom #41 · J.0.1).
//!
//! # Why this madness
//!
//! Phase 0 ships zero live Telegram transport. This atom defines only the
...
//! operator id (§9.5 광기 "allowlist 우회/평문 비밀 송출 = 차단",
//! ATOM_PLAN line 240 G-UX gate).
//!
//! The gateway carries no token field. The Bot API token is owned by a
//! separate secret-gated boundary (a later J.0.x wiring atom); this struct
...
//! *who* was rejected ("인가 실패는 조용히 거부 / 정보 노출 0").
//!
//! Reuse: atom #6 (`mnemos-a-core` boot wiring) — this module imports no
...
//!
//! Canonical OUT (verbatim from ATOM_PLAN §4.J line 722-727):
//!
//! ```text
...
///
/// Inner `pub i64` per atom #24 `ToolId(pub u16)` / atom #39
/// `SkillId(pub u16)` precedent: "no invariant beyond the inner value,
/// so the public inner field is correct" (atom #23 `TurnUsage` lineage).
...
/// extra operator id at runtime ("런타임 추가 불가 = 우회 차단",
/// ATOM_PLAN line 1243).
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct Allowlist {
    /// Allowlisted Telegram user ids. The `'static` lifetime is the
    /// 광기 spine: runtime additions are syntactically impossible
    /// (§9.5 allowlist 우회 차단).
...
/// no transport reason is carried in the value. This is the "조용히
/// 거부 / 정보 노출 0" spine (ATOM_PLAN line 1243): neither the derived
/// `Debug` nor any future `Display` impl on this enum can describe *who*
/// was rejected or *why* the transport failed. The variant labels
/// themselves are intentionally coarse class tags (atom #2
/// `MnemosError` `RedactionClass` lineage).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[non_exhaustive]
pub enum GatewayError {
    ...
    /// channel. Realised by a later J.0.x wiring atom.
    SendFailed,
...
/// owned by a separate secret-gated boundary (a later J.0.x wiring atom)
...
// ===========================================================================
// 5. Tests — three test names verbatim per ATOM_PLAN line 1244
// ===========================================================================
...
mod tests {
    ...
    fn j0_1_unknown_user_rejected() {
        ...
        // `-1`, or `0` substring). This is the "정보 노출 0" spine.
    ...
    ///    in §2), so no external caller can construct an `Allowlist`
    ...
    fn j0_1_allowlist_is_static() {
        ...
        // it — atom #6 [[mainnet-safety-over-speed]] precedent).
