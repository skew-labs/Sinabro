//! Stage C Walrus PUT retry/idempotency safety.
//!
...
//!   computed by reusing the canonical [`classify_transport_failure`] decision
...
//!   `a-core`; the `StageCTraceLink` stamp is applied at the `d-move` /
//!   integration composition layer (mirrors the
