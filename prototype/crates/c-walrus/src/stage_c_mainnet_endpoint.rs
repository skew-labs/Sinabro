//! Stage C Walrus **mainnet** endpoint typed prep (C-WP-06A · atom #220 · C.2.1).
//!
//! Canonical OUT: [`MainnetWalrusEndpoint`] — a typed Walrus **mainnet**
//! endpoint wrapper that is constructed *only* from a sealed §4.4 mainnet
//! config ([`SealedMainnetConfig`], atom #219) and still drives the Stage A
...
//! **no** network I/O: this atom is *prepare-only*. No PUT, no GET, no socket.
//!
//! # Madness invariants (atom #220)
//!
...
//!   is `false` (always, by atom #219 construction). A config that somehow
...
//!   (atom #219 already enforces this at parse time; re-checked here so a
...
//!   (atoms #8 / #9).
//!
//! # New crate edge (C-WP-06A architecture note)
//!
...
//! workspace member, so the edge is acyclic. It mirrors the user-blessed
//! `b-memory`/`d-move`/`k-devex -> a-core` (C-WP-01) and `g-wallet -> a-core`
//! (C-WP-05) edges. Path-only: pure types, no network / HTTP / TLS.
...
/// A typed Walrus **mainnet** endpoint (canonical OUT, atom #220).
///
...
impl MainnetWalrusEndpoint {
    ...
    pub fn from_sealed_config(
        ...
        // Defense-in-depth config gates (atom #219 already enforces these at
        // parse time; re-checked so a directly built config literal cannot
        // bypass them).
...
mod tests {
    ...
    /// `c2_1_checklist_missing_reject` — a config with a zero checklist receipt
    /// hash is rejected (defense in depth over atom #219's parse-time check).
    #[test]
    fn c2_1_checklist_missing_reject() {
        ...
        // An executable config is also rejected (cannot happen via atom #219,
        // re-checked here).
    ...
    // never invoked here — this atom issues no PUT/GET.
    ...
    /// `c2_1_transport_trait_reuse` — the official mainnet endpoint resolves to
    /// the sanctioned hosts and composes with the canonical
    /// `PublisherTransport` / `AggregatorTransport` traits (compile-time proof;
    /// no PUT/GET is issued).
    #[test]
    ...
    /// `c2_1_custom_url_reject_unless_self_host` — a custom URL is rejected in
    /// official mode and accepted (when a valid `https` Walrus URL) in
    /// self-host mode.
    #[test]
    ...
    /// `c2_1_ipfs_filecoin_endpoint_rejected` — `ipfs://` / `filecoin://` /
    /// non-`https` / query-injected endpoints are rejected even in self-host
    /// mode.
    #[test]
