...
//! to the default unless the feature is explicitly enabled.
//!
//! Contract:
//!
...
//!   this transport only re-asserts it via the strongly typed
//!   borrow it receives.
//! - The synchronous `reqwest::blocking::Client` is used so that the c-walrus
//!   crate stays runtime-agnostic. The caller threads bring
...
//! is forbidden). The round-trip integration test ships
...
pub enum ReqwestTransportInitError {
    ...
    /// timeout could hang forever (no unbounded waits).
    TimeoutZero = 1,
    ...
    fn get_blob(
        ...
        let response = match send_result {
            ...
            Err(err) => {
                ...
                // `NoExternalMutation` (matches `classify_aggregator_transport_failure`).
