//! Stage C mainnet blob local verify plan (C-WP-06A · atom #224 · C.2.5).
//!
//! Canonical OUT: §4.4 [`MainnetSyntheticBlobReceipt`] — the receipt minted
...
//! §10.2 self-report ban, atom #10).
//!
//! # Madness invariants (atom #224)
//!
...
/// derivation, and mint the §4.4 receipt on success.
///
/// `payload_hash_32` (atom #221) and `ceremony_hash_32` (atom #223) are carried
...
mod tests {
    ...
    /// `c2_5_synthetic_receipt_vector` — a correctly reported synthetic blob
    /// verifies and the receipt carries the locally derived id + the supplied
    /// hashes.
    #[test]
    ...
    /// `c2_5_mismatch_reject` — a reported id derived from *different* content
    /// is rejected as a root mismatch; no receipt is produced.
    #[test]
    ...
    /// `c2_5_unverified_reported_id_cannot_enter_receipt` — a malformed reported
    /// text cannot mint a receipt; the only path to a `VerifiedBlobId` is the
    /// local-derive verify, so an unverified id is structurally excluded.
    #[test]
