//! Walrus aggregator GET transport.
//!
//! # Invariants
//!
...
//!    response body is never cloned into a `Vec` past the cap). The
...
//!   byte-stable cross-language form, so this module inherits that convention
//!   rather than inventing a second encoding. The net feature is
...
//!   reuse target). No new variants are added: the
...
//!   [`crate::publisher::publish_blob_with_transport`] but this module does not
//!   emit it. The net feature may introduce tracing emission keyed
...
/// (a hex segment is rejected live with HTTP
...
impl AggregatorGetUrl {
    ...
    /// bytes — the exact form the real Walrus testnet aggregator parses;
    /// a hex segment is rejected by the live aggregator.
...
pub fn validate_aggregator_get_url(url: &str) -> Result<AggregatorGetUrl, PublisherClientError> {
    ...
    // decode to a 32-byte blob id. A hex segment, a wrong
...
pub enum FetchStopReason {
    ...
    /// network transport (the offline classifier surfaces oversized
...
/// with a fake transport while the net feature provides a real
/// reqwest-backed implementation under a feature flag.
///
/// The trait reuses [`PublisherTransportResponse`] /
/// [`PublisherTransportFailure`] verbatim so a single fake
...
///
/// Retry policy: the loop retries iff
...
// Static reuse marker for `BLOB_ID_BYTES`. The aggregator never decodes a
...
// Static reuse marker for `BoundaryState`. The aggregator collapses every
...
mod tests {
    ...
    /// URL for a known blob id is byte-identical to the URL-safe
    /// base64 form the real Walrus testnet aggregator served HTTP 200 for. The
    /// 32 id bytes are the local RS2 derivation
    ...
    /// live with HTTP 400, which is why the base64url form is used.
    #[test]
