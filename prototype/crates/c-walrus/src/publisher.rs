//! Walrus publisher PUT transport.
//!
//! # Invariants
//!
...
//! # Notes
//!
//! * [`PublisherClientError`] is introduced here (the canonical signature
//!   references it but does not define it). The shape follows the existing
//!   `ChunkCodecError` pattern: `Copy + non_exhaustive`, `Display + Error` with
...
//!   module; the caller (or a reqwest-backed publisher implementation) owns
//!   the wall-clock wait. This keeps `publish_blob_with_transport` synchronous
//!   and offline testable.

use crate::codec::BLOB_ID_BYTES;

// ===========================================================================
// 1. Module-level wire / policy constants
// ===========================================================================
...
/// Walrus public testnet (see invariant 4 above).
...
pub enum PublishPayloadClass {
    ...
    /// Rejected.
    PrivateProvenance = 6,
    /// The AEAD CIPHERTEXT of a user memory record. DISTINCT from
    ...
    /// leaks no plaintext (memory is stored ciphertext-only, never plaintext). ADMITTED
    /// to the publisher; the plaintext classes above stay rejected (secret-zero holds).
    EncryptedUserMemory = 7,
    /// A PUBLIC agent-registry artifact (a GitHub-style repo where visibility —
    /// public or private — is user-configurable). PLAINTEXT, deliberately admitted
...
impl<'a> PublisherPutRequest<'a> {
    ...
    /// [`PublishPayloadClass::SyntheticPublicFixture`] (see invariant 4).
    pub const fn new(
        endpoint: PublisherEndpoint,
        epochs: EpochCount,
        payload: PublishPayload<'a>,
        ...
        match payload.class {
            // ciphertext of user memory is ADMITTED (the AEAD key never leaves
            ...
            // PublicRegistryArtifact is deliberately admitted (a public agent
...
/// — a later verification step promotes this to `VerifiedBlobId` via local
...
/// with a fake transport while a separate module provides a real reqwest-backed
...
///
/// Retry policy (see invariant 2 above): the loop retries iff
...
impl PublisherClientError {
    ...
    /// `class_label` pattern from `MnemosError` and `ChunkCodecError`.
    #[inline]
    pub const fn class_label(&self) -> &'static str {
        match self {
...
// Static reuse marker against the codec's blob-id width. The publisher does
// not decode the reported blob-id text, but we want a const-evaluable
// assertion so the reuse claim against `crate::codec::BLOB_ID_BYTES` cannot
// silently drift.
