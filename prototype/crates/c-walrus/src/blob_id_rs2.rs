//! Official Walrus **RS2 blob-id oracle** — `feature = "net-testnet"` only.
//!
//! # Why this module exists
//!
//! The default/offline [`derive_blob_id`](crate::blob_id::derive_blob_id) is a
//! Phase-0 **placeholder** ARX digest: deterministic and
//! domain-separated, but *not* the real Walrus blob id. So a publisher-reported
//! id from a live testnet PUT can never byte-match the
//! placeholder, and
...
//! the `walrus blob-id --n-shards 1000` CLI uses. It proves 3-way
//! parity on the fixture: `walrus blob-id` CLI == `walrus-core` ==
...
//!   [`verify_reported_testnet_blob_id`]); the Stage B publish path consumes those.
...
/// live PUT — used. The `walrus blob-id --n-shards 1000` invocation
...
/// of the Phase-0 placeholder. This is the path the Stage B publish uses to
/// promote a live-PUT reported id to a trust root. The existing
