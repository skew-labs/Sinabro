//! Locally derived Walrus blob identifiers (atom #10 · C.0.4).
//!
...
//! [`derive_blob_id`] — the Walrus-side §10.2 self-report ban.
//!
//! # Madness invariants
//!
...
//!   carried by atom #13 / C.0.7 alongside the codec bench; this atom
...
//!
//! # Phase 0 derivation algorithm — placeholder, atom #12 swap
//!
...
//! disk inside this offline workspace. Per [`BUILD_STATE`] atom #10
//! disparity marker (mirroring the atom #2 "17 enum" and atom #3 "12 enum"
//! discrepancies), the derivation **body** is implemented here as a
...
//!
//! atom #12 (`C.0.6`, `feature = "net-testnet"`) is the canonical seam where
...
//!
//! [`BUILD_STATE`]: ../../../../../../MNEMOS_BUILD_STATE.md
...
/// algorithm; atom #12 introduces a different tag (`v1` or the real Walrus
/// domain) so the swap is visible at the byte level.
...
pub enum BlobIdError {
    ...
    /// This is the §10.2 self-report-refusal trigger.
    RootMismatch,
    /// (`net-testnet` only) The official Walrus RS2 blob-id oracle
    /// ([`crate::blob_id_rs2`], bridge atom #116.5) could not compute a local id
    ...
    /// placeholder verify (atoms #10/#11) never yields it.
...
impl VerifiedBlobId {
    ...
    /// bridge atom #116.5) to wrap a **locally derived** id after it has matched
    ...
    /// synthesise a `VerifiedBlobId` — the §10.2 self-report ban is preserved.
    #[cfg(feature = "net-testnet")]
    #[inline]
...
/// docstring for the swap point at atom #12 (`C.0.6`,
/// `feature = "net-testnet"`). The function is zero-copy on `content`:
/// the only allocation is the 32-byte [`BlobId`] on the caller stack.
#[inline]
pub fn derive_blob_id(content: &[u8]) -> BlobId {
    // AI-HOT: criterion target `benches/blob_id.rs` (atom #13 carries
    // the criterion infrastructure; the marker pre-pins the seam).
...
// ===========================================================================
// 5. verify_reported_blob_id — §10.2 self-report refusal
// ===========================================================================
...
/// deferred to atom #12 where the encoding is pinned by a real round-trip.
///
/// `pub(crate)` (visibility-only widening, bridge atom #116.5) so the
...
/// public 32-byte content address. `None` on a malformed text. E14-W2: lets a client
...
///
/// Canonical blob-id text encoder (bridge atom #116.75 · B.2.15.75): the
...
// Pin the cross-atom byte invariant: atom #10 assumes `BLOB_ID_BYTES == 32`
// (atom #7 / C.0.1) and `WALRUS_BLOB_ID_TEXT_LEN_BASE64URL == 43`
...
mod tests {
    ...
    fn verified_blob_id_is_repr_transparent_over_blob_id() {
        ...
        // to `BlobId([u8; 32])` so atom #12 (the network swap) can store
