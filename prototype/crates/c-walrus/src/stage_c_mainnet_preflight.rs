//! Stage C Walrus mainnet read/preflight.
//!
//! Canonical OUT: [`WalrusMainnetPreflight`] — a **read-only** preflight check
//! for a [`MainnetWalrusEndpoint`]. It validates the policy that a
...
//! # Invariants
//!
//! * **No PUT.** This module is read-only/preflight. It produces an evidence
...
//!   disabled by default and only the explicit ceremony rail (operator
//!   approval) can enable it.
//! * **Synthetic-only policy reuse (no re-mint).** The preflight reuses the
//!   [`PublishPayloadClass`] policy (only
//!   [`SyntheticPublicFixture`](PublishPayloadClass::SyntheticPublicFixture)
//!   is admissible) and the body cap
...
pub enum PreflightReject {
    ...
    NonSyntheticClass = 2,
    /// The observed body exceeds the public body cap.
    BodyOverCap = 3,
}
...
/// disabled-by-default ceremony gate (operator approval); when
...
mod tests {
    ...
    /// Preflight refuses when the mainnet-PUT feature is off (disabled by
    /// default).
    #[test]
    ...
    /// A synthetic, within-cap payload with the feature on yields a preflight
    /// record (no PUT issued).
    #[test]
    ...
    /// A non-synthetic payload class is rejected.
    #[test]
