//! Stage C hosted/self/none sponsor-mode config (C-WP-07 · atom #232 · C.2.13).
//!
//! Canonical OUT (§4.3): a [`GasSponsorMode`]-mirroring sponsor-mode config.
//!
//! # Madness invariants (atom #232)
//!
...
//!
//! * **reuse: #217** — this config mirrors the §4.3
...
/// `GasSponsorMode` (§4.3) discriminants, usable from `a-core` without a
/// dependency cycle.
...
/// The secret-free sponsor-mode config (§4.3). No field can hold key material.
...
mod tests {
    ...
    fn discriminants_mirror_gas_sponsor_mode() {
        // Byte-exact value-mirror lock against the §4.3 GasSponsorMode values.
    ...
    fn env_secret_reject() {
        ...
        assert_eq!(
            SponsorModeConfig::from_parts(
                ...
                // fixture cannot itself trip the atom #233 scan).
