//! Phase 0 structured, allowlist-only JSON logging surface.
//!
...
//! [`RedactedLogValue`].
//!
//! Reuse: `emit_config_failure_log` consumes the
//! [`SafeErrorReport`] projection so the line can never carry a leaked
//! cause. `emit_shutdown_log` takes the scalar fields of a
...
/// scalar arguments mirror the relevant fields of the
...
mod tests {
    ...
    // Allowlist key sets (per the wire spec).
