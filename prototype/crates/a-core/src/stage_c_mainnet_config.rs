//! Stage C sealed mainnet config.
//!
//! Provides a sealed mainnet config whose chain env is
...
//!   prepared without binding the checklist receipt that gates it.
//! * **No re-mint, no inverted dependency.** The execution posture reuses the
//!   [`MainnetExecutionState`](crate::stage_c_env::MainnetExecutionState)
//!   and [`StageCChainEnv`](crate::stage_c_env::StageCChainEnv) (the
//!   same crate). The checklist is referenced **by its 32-byte
...
//! # Reuse map
//!
//! * **reuse** — [`MainnetExecutionState`](crate::stage_c_env::MainnetExecutionState),
//!   [`StageCChainEnv`](crate::stage_c_env::StageCChainEnv).
//! * **reuse** — the `MainnetChecklist` evidence hash, by value (a
//!   `[u8; 32]`), not by type.
...
pub struct SealedMainnetConfig {
    ...
    /// The checklist evidence hash this config is gated on. Non-zero by
    /// construction.
