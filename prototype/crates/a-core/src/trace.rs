//! Cross-stage action trace primitives (Stage C C-WP-01 · atoms #81 / #171).
//!
...
//! (the dependency root) so that the low crates `d-move` (gas trace, atom
//! #178) and `k-devex` (evidence ref, atom #177) can stamp a trace without
//! taking a cyclic dependency on `b-memory`.
//!
//! * [`StageBTraceLink`] — the atom #81 `(trace_id_u64, atom_id_u16,
//!   attempt_u8)` triple. It was originally defined in
//!   `b-memory/src/stage_b_handoff.rs`; in Stage C (C-WP-01) the *definition*
...
//! * [`StageCTraceLink`] — the §4.0 Stage C stamp. It *composes* (never
...
//! * [`StageDTraceLink`] — the §4.2 Stage D stamp. It *composes* (never
...
//!   It is minted here by D-WP-02 (the first WorkPackage to consume it) per
//!   the no-wrong-file-home rule, so no consumer crate inverts a dependency
...
//!   `u16` by type; the Stage C atom space (#171-#240) and the gate registry
...
///
/// Fields are `pub` per the Stage B §4.0 canonical registry; [`new`](Self::new)
...
pub struct StageBTraceLink {
    ...
    /// Stage A+B+C atom space (#0-#240 and beyond) fits with headroom.
    pub atom_id_u16: u16,
...
// ===========================================================================
// 2. StageCTraceLink — Stage B trace + Stage C atom + gate id  (§4.0)
// ===========================================================================

/// Stage C per-artifact trace stamp (§4.0 canonical registry).
///
...
/// artifact. A gas sample (atom #178), an evidence ref (atom #177), a Gas
/// Station decision (§4.3) and a mainnet ceremony record (§4.4) all carry one
/// of these.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub struct StageCTraceLink {
    ...
    /// covers the #171-#240 Stage C span with headroom.
    pub stage_c_atom_u16: u16,
    /// Gate-registry id that produced the artifact (e.g. the numeric id of
    /// `G-C-GAS-TRACE`). A `u16` keeps the gate space open-ended.
    pub gate_id_u16: u16,
}
...
// ===========================================================================
// 3. StageDTraceLink — Stage C trace + Stage D atom + sandbox event id  (§4.2)
// ===========================================================================

/// Stage D per-artifact trace stamp (§4.2 canonical registry — minted here by
/// D-WP-02, the first WorkPackage to consume it; the trace-link family stays in
/// `a-core` so no consumer crate inverts a dependency to stamp a trace).
///
...
/// outcome (allow / deny / meter-exceeded), every try-before-use run (§4.2
...
pub struct StageDTraceLink {
    ...
    /// covers the #241-#330 Stage D span with headroom.
