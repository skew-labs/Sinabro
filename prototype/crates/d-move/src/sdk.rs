//! `mnemos-d-move::sdk` — Sui Rust SDK call builder.
//!
//! Canonical OUT:
...
//!   `(&self)` self-contained; the canonical struct text only enumerates the
//!   four routing fields. The two extra fields are an implementation detail.
//! - [`SuiCallBuilder::add_chunk`] — the sole constructor.
...
//! - The `MoveAnchorArgsV1` anchor chain terminates at the
//!   `MemoryRootArgs` projection; this module
//!   does not touch Walrus surfaces directly.
//!
//! ## Carve-outs
//!
//! 1. **Package id placeholder.** The `mnemos::memory_root` package id
//!    is not known until the testnet deploy completes
//!    (the deploy leaves `mnemos = "0x0"` until the operator
//!    fills `published-at` post-publish). This module pins the on-builder
//!    `package` field to [`MNEMOS_MEMORY_ROOT_PACKAGE_PLACEHOLDER`] =
//!    32 zero bytes so the dry-run byte size is stable for measurement.
//!    The setter that swaps in a real published-at id is a later
//!    decision carried forward from the deploy step.
...
//!    around it. This module itself does not call any RPC, does not
//!    consult `sui` CLI, and does not load a wallet — keeping the
//!    build satisfiable in `--offline` mode.
//! 3. **Factory-absence carry-forward.** The `mnemos::memory_root`
//!    Move module exposes no `init_memory_root` factory. This SDK builder
//!    still produces the byte string for the
//!    `add_chunk(root, blob_id, kind, parent)`
//!    Move entry, but a live `add_chunk` against a real on-chain root
//!    cannot execute until the factory-resolution path is chosen.
//!    Dry-run measurement is decoupled from it.
//! 4. **No intent prefix.** A real Sui intent message starts with
//!    `[scope, version, app_id]` (3 bytes) before the BCS-encoded
//!    `TransactionData`. This module's `to_dry_run_bytes` emits ONLY the
//!    routing + args portion — adding the intent prefix + `TransactionData`
//!    wrapper is the domain of the future signing layer where wallet
//!    keypairs first appear. The current output is therefore a
//!    measurement-only carrier, not a signable transaction blob.
//! 5. **uleb128 helper kept local.** The `uleb128_encoded_len_u32` helper
//!    stays in `c-walrus::stream` rather than being
//!    promoted to the private `c-walrus::wire` module; this module
//!    follows the same pattern and ships a local `append_uleb128_u32`.
...
/// 11-byte ASCII string `b"memory_root"` is the source of truth; the
/// entry-function definition is anchored in the file
...
/// Move signature (`public entry fun add_chunk(...)`).
pub const MNEMOS_MOVE_FUNCTION_ADD_CHUNK: &str = "add_chunk";

/// Placeholder 32-byte package id used in [`SuiCallBuilder::add_chunk`]
/// until the operator-side testnet-deploy step fills in the real
/// `published-at` value. Mirrors the `mnemos = "0x0"` placeholder line
/// left in `prototype/move/Move.toml` for the same operator step. Pinned
/// to 32 zero bytes so the dry-run byte size is stable across rebuilds.
...
// ===========================================================================
// 2. Compile-time reuse / drift markers
// ===========================================================================
...
/// Pins the Move module name at 11 bytes. The Move source anchors
/// `module mnemos::memory_root`; if the module is ever renamed, the
/// build fails here before the dry-run oracle even runs.
const _MNEMOS_MOVE_MODULE_NAME_LEN_IS_11: [(); 0 - !(MNEMOS_MOVE_MODULE_NAME.len() == 11)
    as usize] = [];

/// Pins the Move function name at 9 bytes. The Move
...
///   submit.
/// - `ArgEncode` — reserved for future fallible arg encodes. At present
///   the only encode step is [`encode_memory_root_args_bcs`]
...
///   fit in `u32`. The variant is preserved verbatim from the
///   canonical signature so future work can layer fallible arg
...
pub enum CallBuildError {
    ...
    /// argument shapes; never emitted by this module itself.
    ArgEncode,
}
...
impl CallBuildError {
    ...
    /// prefix (mirrors the `move_bind.*` namespace).
    #[inline]
...
/// match the canonical signature byte-for-byte. Two
/// implementation-private fields (`root` and
/// `encoded_args`) are required to make [`SuiCallBuilder::to_dry_run_bytes`]
...
pub struct SuiCallBuilder {
    ...
    // ------------- implementation-private (beyond the canonical fields) -----
    root: ObjectId,
...
impl SuiCallBuilder {
    ...
    /// projection of the call args; `gas` is
    /// the typed gas budget. The package id is pinned to
    /// [`MNEMOS_MEMORY_ROOT_PACKAGE_PLACEHOLDER`] (32 zero bytes) until
    /// the testnet-deploy operator step hands off the real
    /// `published-at` value through the deploy-resolution path.
    #[inline]
    pub fn add_chunk(
        root: ObjectId,
        args: &MemoryRootArgs,
    ...
    /// The Move module name. This is always
    ...
    /// The Move function name. This is always
    ...
    // ---- implementation-private accessors (diagnostic) --------------------
...
/// `c-walrus::wire::append_uleb128_u32` semantics; kept local
