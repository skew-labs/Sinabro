//! Stage C gas trace sample type.
//!
//! Public types: [`GasTraceFunction`], [`GasTraceSample`].
//!
//! # Invariants
//!
...
//!   package reuses the Stage A [`ObjectId`]; the trace reuses the
...
/// Which Move function a [`GasTraceSample`] measured. `#[repr(u8)]` with
/// fixed discriminants so the tag is byte-stable.
...
/// One structured gas measurement.
///
/// Fields are `pub`. The MIST split is `computation + storage`
