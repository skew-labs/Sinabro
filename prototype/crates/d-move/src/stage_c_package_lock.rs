//! Stage C mainnet package lock.
//!
//! Canonical output: [`MainnetPackageLock`].
//!
//! # Invariants
//!
...
//!   the standalone `sui-prover` proved (`prover_hash_32`) and the
//!   package the gas baseline was measured against (`gas_baseline_hash_32`).
//!   [`MainnetPackageLock::verify`] rejects a presented bytecode,
...
//!   the later operator-approval ceremony, never this lock.
...
//! # Reuse map
//!
//! * **reuse: `ObjectId`** — [`crate::types::ObjectId`]
//!   (`d-move/src/types.rs:156`). No second 32-byte object-id type is minted.
//! * **reuse** — the prover/gas coupling lives in
...
//!   gate/evidence level
//!   so no `d-move -> k-devex` cargo edge is created (cycle-avoidance
//!   precedent).
...
/// A mainnet package lock (canonical output).
///
...
pub struct MainnetPackageLock {
    ...
    /// The package digest the standalone `sui-prover` proved.
    pub prover_hash_32: [u8; 32],
    /// The digest of the gas baseline the package was measured
    /// against.
...
mod tests {
    ...
    // The real production Move package content digest (the
