//! Gas regression comparator with prover coupling.
//!
//! Canonical shape: a gas regression report for `add_chunk` / `audit_log::append`
//! that couples the [`classify_gas_regression`] verdict with a
...
//!   cap (the `add_chunk` 800k ceiling) is red even when the package
...
pub struct GasCompareInput {
    /// The measured gas sample (from the gas-trace collectors).
    pub sample: GasTraceSample,
    /// The gas baseline to classify against.
