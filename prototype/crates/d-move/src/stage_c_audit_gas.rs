//! `audit_log::append` dry-run gas collector (C-WP-02A · atom #181 · C.0.10).
//!
//! Canonical OUT: a dry-run gas collector for the Stage B `audit_log::append`
//! Move call.
//!
//! # Madness invariants (atom #181)
//!
...
//!   the `memory_root::add_chunk` memory-anchor cost (atom #180).
//! * **Route through the canonical builder — no alternate PTB.** The collector
//!   builds the call exclusively via [`StageBCallBuilder::audit_append`] (atom
//!   #134), from the Stage B [`AuditAppendArgs`] (atom #132). It never assembles
//!   a Move call by hand.
...
//!   dev-inspect / dry-run response parsed elsewhere); this atom performs no
//!   network egress.
