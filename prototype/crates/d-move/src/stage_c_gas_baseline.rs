//! Stage C gas baseline + regression classifier (C-WP-01 · atom #179 · C.0.8).
//!
//! Canonical OUT (§4.1): [`GasTraceBaseline`], [`GasRegressionDecision`],
//! [`classify_gas_regression`].
//!
//! # Madness invariants (atom #179)
//!
...
/// Per-package gas baseline (§4.1): the maximum accepted spend for the two
...
/// The regression verdict for one sample against a baseline (§4.1).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
...
const fn relevant_baseline_mist(function: GasTraceFunction, baseline: &GasTraceBaseline) -> u64 {
    match function {
        ...
        // add_chunk maximum in this atom (only add_chunk / audit have their own
        // baseline slot in §4.1).
...
/// Classify a gas sample against a baseline (§4.1).
///
