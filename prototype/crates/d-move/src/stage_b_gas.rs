//! `mnemos-d-move::stage_b_gas` — atom #135 · B.3.14 — Stage B gas budget
//! cap policy over the reused atom #15 [`crate::types::GasBudgetMist`].
//!
//! ## Canonical OUT (ATOM_PLAN lines 982-991)
//!
...
//!   ceiling default.
//!
//! 광기 사양 (ATOM_PLAN line 986): MIST cannot be confused with a raw token
//! count or a blob byte length because the budget crosses every surface here
//! as the atom #15 [`GasBudgetMist`] `#[repr(transparent)]` newtype — never a
//! bare `u64`. The cap is enforced before dry-run.
//!
//! ## OD-A (no §4 canonical signature for a gas cap policy — Option-A protocol)
//!
//! The Stage B §4 canonical registry (`MNEMOS_STAGE_B_ATOM_PLAN.md` §4.3,
//! lines 313-316) declares the Move-binding shapes but NOT a gas-cap policy
//! type. §4 line 226 explicitly authorises Stage B to mint NEW types that
//! *combine* A canonical (here: a `policy` over [`GasBudgetMist`]). So this
//! atom mints [`StageBGasBudgetPolicy`] / [`StageBGasError`] as new policy
//! surfaces while REUSING the gas typed unit verbatim (no second gas type).
//!
//! The §4.3 line 316 [`crate::StageBMoveBindError`] channel is frozen at five
//! variants (`stage_b_types` OD-2: "a sixth variant would be drift" — the
//! cross-language schema lock). Its `GasBudgetZero` variant is the *call
//! builder* layer's zero guard (atom #134, `require_nonzero_gas`). The cap
//! and the addition-overflow rejects have NO home in that frozen channel, so
//! this atom routes ALL three policy-layer rejects through a dedicated
//! [`StageBGasError`] enum and leaves `StageBMoveBindError` byte-stable. This
//! is the same reuse+verify+disparity-flag resolution used by atoms
//! #2 / #3 / #122 / #124 / #125 / #132.
//!
//! ## OD-B (`STAGE_B_DEFAULT_MAX_GAS_MIST` is a policy ceiling, not a protocol constant)
//!
...
//! (atom #134 dry-run bytes are 65 / 130 / 162 / 123 bytes), so a 1-SUI cap is
...
//! (`G-B-NO-MAINNET`), so this ceiling never gates real funds.
#![allow(clippy::module_name_repetitions)]

use crate::types::GasBudgetMist;

// ===========================================================================
// 1. Compile-time reuse pin (atom #10 / #15 / #132 precedent)
// ===========================================================================

/// Pins the reuse: a [`GasBudgetMist`] is exactly 8 bytes (`u64`). Any drift
/// in the atom #15 newtype width breaks the build via a zero-length-array
...
/// (1 SUI = 10^9 MIST). See OD-B: this is a Stage-B-internal conservative
...
/// atom #132 [`crate::StageBMoveBindError::class_label`] discipline.
///
/// This is a SEPARATE channel from the frozen five-variant
/// [`crate::StageBMoveBindError`] (§4.3 line 316). The cap and overflow
/// rejects have no home there, and adding a sixth variant would break the
/// cross-language schema lock — see OD-A.
...
pub enum StageBGasError {
    ...
    /// at atom #134, routed through this policy channel).
    GasBudgetZero,
...
impl StageBGasBudgetPolicy {
    ...
    /// ([`STAGE_B_DEFAULT_MAX_GAS_MIST`] = 1 SUI in MIST). See OD-B.
    #[inline]
...
mod tests {
    ...
    /// ATOM_PLAN line 987 test #1 — `zero reject`. A zero request is rejected
    /// on both the single-budget and the accumulated path, and an empty slice
    /// (total zero) is rejected too.
    #[test]
    ...
    /// ATOM_PLAN line 987 test #2 — `cap reject`. Over the ceiling is
    /// rejected; the inclusive boundary `requested == max` and one below are
    /// accepted (and the validated value is returned verbatim).
    #[test]
    ...
    /// ATOM_PLAN line 987 test #3 — `checked addition`. A normal sum stays
    /// under the ceiling and returns the total; a `u64` overflow is caught as
    /// `GasBudgetAdditionOverflow` (not a silent wrap, not a panic).
    #[test]
    ...
    /// Reuse pin — the gas unit stays the atom #15 8-byte newtype (no second
    /// gas type minted by this atom).
    #[test]
    ...
    /// OD-B — the default ceiling is 1 SUI = 10^9 MIST and threads through
    /// `with_default_cap`.
    #[test]
