//! `mnemos-d-move::types` — atom #15 · D.0.1 — MemoryRoot Rust bindings.
//!
//! Canonical OUT (§4.D) wired in this module:
//! - `GasBudgetMist` newtype over `u64` so a gas budget is never confused
//!   with a token count, a chunk byte length, or a blob id (§raw-byte Q4
//!   typed-unit; `[[newtype-typed-units-zero-cost-abstraction-pattern]]`).
...
//!   `vector<u8>` with a `len == 32` invariant proved by Move Prover
//!   in atom #18 (`D.0.4`). The boundary check between the two is
//!   carried by [`MoveBindError::RootHashLen`].
//! - `MoveBindError` (`Copy`, `#[non_exhaustive]`, class-label namespaced
//!   `move_bind.*`) — failure modes raised by the binding layer; atoms
//!   #16 / #17 / #20 emit the `EpochNotMonotone` / `OwnerMismatch`
//!   variants from their own enforcement sites. `RootHashLen` is the
//!   only variant emitted in atom #15 (currently only reachable via the
//!   future `try_from_root_hash_slice` constructor that atom #20 will
//!   add — at atom #15 the public surface for converting an anchor only
//!   ever sees a fixed-size `[u8; 32]`, so the channel exists for
//!   downstream symmetry).
//! - `memory_root_args_from_anchor(anchor, owner, epoch_u64)` — the
//!   single conversion entrypoint specified by ATOM_PLAN line 551.
//!   Takes a verified `MoveAnchorArgsV1` (atom #7 canonical reuse,
//!   already `BlobId([u8; 32])`-typed; `c-walrus::blob_id::VerifiedBlobId`
//!   gate carried by atom #10) plus an owner address and an epoch
//!   counter, returns a `MemoryRootArgs`. The conversion is total at
//!   atom #15 (always `Ok`); the `Result` wrapper is kept so future
//!   atoms that bolt on epoch-monotonicity or owner-mismatch checks do
//!   not break the canonical signature.
//!
//! ## Cross-language schema lock (§V.3 — BCS test vector)
//!
...
//! epoch_u64 = 42) byte-for-byte; the Python oracle at
//! `ops/evidence/phase_0/atom_015/oracle_bcs_memory_root_v0.py`
//! independently reproduces the same 72-byte string.
//!
//! ## Carve-outs (documented for Session 2 verifier ACCEPT / RAISE)
//!
//! 1. `MoveBindError::EpochNotMonotone` + `MoveBindError::OwnerMismatch`
//!    are **defined** but **not emitted** by any function in atom #15.
//!    They are the cross-atom failure channel that atoms #16 / #17 /
//!    #20 consume (the `[[helper-define-then-consume-enforce]]`
//!    discipline allows definition ahead of consumption when the
//!    consumer atom is fully specified in the same plan; see ATOM_PLAN
//!    lines 953-1000 for §D.add_chunk / §D.transfer_root / §D.sdk).
//! 2. `memory_root_args_from_anchor` currently always returns `Ok`. The
//!    `Result` wrapper is preserved verbatim from ATOM_PLAN line 551 so
//!    that atom #16 / #20 can add fallible checks without an API break.
...
//!    the on-chain module (atom #16 increments it inside `add_chunk`).
//!    The Rust SDK never sets either field directly.

use mnemos_c_walrus::{BLOB_ID_BYTES, MoveAnchorArgsV1};

// ===========================================================================
// 1. Compile-time reuse markers (atom #10 precedent)
// ===========================================================================

/// Pins the Rust-side root-hash length to the atom #7 / C.0.1
...
// ===========================================================================
// 3. Typed-unit newtypes (§raw-byte Q4 typed-unit discipline)
// ===========================================================================
...
/// is owned by `SuiCallBuilder::add_chunk` in atom #20.
...
// ===========================================================================
// 4. Memory-root call args (§4.D canonical)
// ===========================================================================
...
/// infrastructure and by `add_chunk` (atom #16) respectively.
///
...
pub struct MemoryRootArgs {
    ...
    /// `vector<u8>` with a `len == 32` invariant proved by atom #18
    /// (`D.0.4` Move Prover).
    pub root_hash: [u8; BLOB_ID_BYTES],
    /// Monotonically-increasing epoch counter. The monotonicity
    /// invariant is enforced by atom #18 (`D.0.4` Move Prover) and
    /// raised through [`MoveBindError::EpochNotMonotone`] by atom #16
    /// (`D.0.2` `add_chunk`).
    pub epoch_u64: u64,
}
...
/// mirrors the atom #7 / #8 / #9 / #10 / #11 `class_label()`
/// discipline.
///
/// At atom #15 only [`MoveBindError::RootHashLen`] is conceptually
/// reachable (via a future fallible constructor from a runtime
/// `&[u8]`); atoms #16 / #17 / #20 consume the rest:
/// - `EpochNotMonotone` — emitted by `add_chunk` (#16) on `next <= prev`.
/// - `OwnerMismatch` — emitted by `add_chunk` (#16) and `transfer_root`
///   (#17) when `ctx.sender() != root.owner`.
...
///
/// The function is total at atom #15 (always `Ok`). The `Result`
/// wrapper is preserved verbatim from ATOM_PLAN line 551 so atoms
/// #16 / #20 can layer fallible checks (epoch monotonicity, owner
