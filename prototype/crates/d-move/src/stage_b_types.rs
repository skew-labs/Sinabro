//! `mnemos-d-move::stage_b_types` — atom #132 · B.3.11 — Stage B Rust↔Move
//! anchor/audit call-arg bindings.
//!
//! Canonical OUT (Stage B §4.3, ATOM_PLAN lines 313-316) wired here:
...
//!   `root` object id (atom #15 [`crate::types::ObjectId`]), the verified
//!   Walrus `anchor` (atom #7 `MoveAnchorArgsV1`, already `BlobId([u8; 32])`
...
//!   the complete five-variant set declared verbatim in §4.3 line 316.
//!
//! ## OD-1 (user-locked 2026-05-31): `digest` is `[u8; 32]`, NOT `ChunkDigest32`
//!
//! §4.3 line 314 names the `digest` field type as `ChunkDigest32`. That
//! type lives in `b-memory` (`crates/b-memory/src/chunk_digest.rs:118`,
//! atom #86). `b-memory` ALREADY depends on `d-move`
//! (`crates/b-memory/Cargo.toml`, reused by the shipped #101-#120 Walrus
...
//!
//! Resolution (Option-A reuse+verify+disparity-flag protocol, same as
//! atoms #2 / #3 / #122 / #124 / #125): `digest` is a raw `[u8; 32]` here.
...
//! single-file atom's scope: it would mutate the immutable atom #86.
//!
//! ## 광기 사양 — Move `vector<u8>` length checked at the boundary
//!
...
//!
//! ## OD-2 (user-locked 2026-05-31): full five-variant error mint
//!
//! [`StageBMoveBindError`] mints all five variants verbatim from §4.3
//! line 316 in one unit, preserving the cross-language schema lock. Only
//! [`StageBMoveBindError::Len32`] is emitted by atom #132 (the boundary
//! adapters). `OwnerMismatch` / `EpochNotMonotone` / `NetworkNotTestnet` /
//! `GasBudgetZero` are the cross-atom channel consumed by atom #134
//! (`B.3.13` PTB/call-builder dry-run: mainnet reject, gas-zero reject)
//! — the `[[helper-define-then-consume-enforce]]` discipline allows
//! definition ahead of consumption when the consumer atom is fully
//! specified in the same plan.
//!
...
// ===========================================================================
// 1. Compile-time reuse marker (atom #10 / #15 precedent)
// ===========================================================================

/// Pins the Stage B Move-boundary `vector<u8>` length to the atom #7 /
/// C.0.1 `BLOB_ID_BYTES = 32`. Any drift breaks the build via a
...
// ===========================================================================
// 3. Move-binding failure channel (§4.3 line 316, full five-variant set)
// ===========================================================================
...
/// from §4.3 line 316 (cross-language schema lock — a sixth variant would
/// be drift). Class labels are namespaced `stage_b_move_bind.*`, mirroring
/// the atom #15 [`crate::types::MoveBindError`] `class_label()` discipline.
///
/// At atom #132 only [`StageBMoveBindError::Len32`] is emitted (the
/// `try_from_move_*` boundary adapters). The other four are the cross-atom
/// channel consumed by atom #134 (`B.3.13`).
...
pub enum StageBMoveBindError {
    ...
    /// the targeted `MemoryRoot` / `AuditLog`. (Consumed by atom #134.)
    OwnerMismatch,
    /// The epoch counter handed to the binding layer was not strictly
    /// greater than the previous on-chain value. (Consumed by atom #134.)
    EpochNotMonotone,
    /// The targeted network was not a Stage B testnet — Stage B forbids
    /// mainnet by construction. (Consumed by atom #134.)
    NetworkNotTestnet,
...
// ===========================================================================
// 4. memory_root anchor call args (§4.3 line 314)
// ===========================================================================
...
/// order (consumed in declaration order by the #133 BCS parity vectors).
/// `Copy` is safe because every field is `Copy`.
///
/// `digest` is a raw `[u8; 32]` rather than `b-memory::ChunkDigest32`; see
/// the module-level OD-1 note for the cargo-cycle reason.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(C)]
pub struct MemoryRootAnchorArgs {
    /// 32-byte on-chain object id of the target `MemoryRoot`.
    root: ObjectId,
    /// Verified Walrus anchor (atom #7 `MoveAnchorArgsV1`; carries the
    /// locally-derived `blob_id`, the chunk `kind`, and an optional
    /// `parent` blob id).
    anchor: MoveAnchorArgsV1,
    /// 32-byte content digest. §4.3 nominal type is `ChunkDigest32`; held
    /// here as raw bytes per OD-1 (the typed value is unwrapped by the
...
// ===========================================================================
// 5. audit_log append call args (§4.3 line 315)
// ===========================================================================
...
mod tests {
    ...
    /// ATOM_PLAN line 954 test #1 — `len32 accepted`.
    #[test]
    ...
    /// ATOM_PLAN line 954 test #2 — `wrong len rejected`. Both a short
    /// (31) and a long (33) vector are rejected with `Len32`, on both
    /// boundary adapters.
    #[test]
    ...
    /// ATOM_PLAN line 954 test #3 — `no raw address string in core`.
    ...
    fn b3_11_full_five_variants_have_namespaced_unique_class_labels() {
        ...
        // All five present (OD-2 full mint) and namespaced.
