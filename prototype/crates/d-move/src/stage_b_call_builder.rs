//! `mnemos-d-move::stage_b_call_builder` — Stage B
//! testnet-only PTB/call-builder dry-run.
//!
//! Canonical OUT: testnet-only call
...
//! - `mnemos::memory_root::add_chunk` (anchors a chunk via the
//!   [`MemoryRootAnchorArgs`]),
//! - `mnemos::audit_log::append` (appends an audit entry via the
//!   [`AuditAppendArgs`]).
//!
...
//! code path here — exactly like the [`crate::sdk::SuiCallBuilder`]
//! measurement carrier, extended to three call shapes and gated on network.
//!
//! ## Cargo-cycle network mirror
//!
//! The canonical Stage B network type is `StageBNetwork` with its
//! `parse_label` testnet-only allowlist, which lives in `b-memory`
//! (`crates/b-memory/src/network.rs`). `b-memory` ALREADY depends
...
//! that forces `stage_b_types`'s `digest: [u8; 32]` (not `b-memory::ChunkDigest32`).
//!
//! Resolution (reuse + verify locally): this module MIRRORS the canonical
//! testnet-only allowlist locally as
...
//! rejected with [`StageBMoveBindError::NetworkNotTestnet`]. The one
//! cross-crate caveat: the `"testnet"` label string is duplicated here
//! rather than imported from `StageBNetwork::CANONICAL_LABEL`; a future
//! change to the canonical label would not auto-propagate. Relocating
//! `StageBNetwork` down to a cycle-free crate is out of this single-file
//! module's scope.
//!
//! ## Network input is a `&str` label, not the canonical enum
//!
//! The test list requires a "mainnet reject" path. The canonical
...
//! `network_label: &str`.
//!
//! ## Call-arg wire is the REAL Move entry ABI
//!
...
//!   taken verbatim from the [`MoveAnchorArgsV1`] (whose own doc
...
//! serialized root, no digest); the [`StageBCallBuilder`] must NOT reuse
//! the struct-parity wire verbatim. That BCS struct-parity wire is
//! retained ONLY as parity/replay evidence in [`encode_anchor_args_bcs`] /
//! [`encode_audit_append_args_bcs`] (still cross-pinned to the fixture
...
//! are deferred to the future signing layer. Module / function names are
//! all `< 128` bytes and `num_object_inputs <= 1`, so each uleb128 prefix
//! is a single byte; the totals are pinned by the
...
/// the module-level cargo-cycle note). Parsing is ASCII-case-insensitive
/// after trimming surrounding whitespace, exactly as the canonical
/// `parse_label`.
pub const STAGE_B_CALL_TESTNET_LABEL: &str = "testnet";

// ===========================================================================
// 2. Compile-time width pins
// ===========================================================================
...
// ===========================================================================
// 3a. BCS struct-parity wire lengths
//     (parity / replay EVIDENCE ONLY — NOT the PTB call-arg wire)
// ===========================================================================

/// Canonical BCS struct-parity length of a `MemoryRootAnchorArgs` with
/// `parent == None`. = 100. Pinned against the
/// `b3_12_anchor_args_bcs_parity_vector_parent_none` fixture. This is the
/// struct-parity wire (`root ‖ blob_id ‖ kind ‖ parent ‖ digest`),
...
/// Canonical BCS struct-parity length of a `MemoryRootAnchorArgs` with
/// `parent == Some`. = 132. Pinned against the
...
/// against the `b3_12_audit_append_args_bcs_parity_vector` fixture.
...
/// Compile-time pin: anchor (parent-none) parity wire is 100 bytes.
const _ANCHOR_NONE_BCS_LEN_IS_100: [(); 0 - !(STAGE_B_ANCHOR_ARGS_BCS_LEN_PARENT_NONE == 100)
    as usize] = [];
/// Compile-time pin: anchor (parent-some) parity wire is 132 bytes.
const _ANCHOR_SOME_BCS_LEN_IS_132: [(); 0 - !(STAGE_B_ANCHOR_ARGS_BCS_LEN_PARENT_SOME == 132)
    as usize] = [];
/// Compile-time pin: audit-append parity wire is 65 bytes.
...
/// (mirrors the `add_chunk` gas guard, routed through the Stage B
/// error channel).
#[inline]
...
/// encoder stays correct for any length. Local copy of the uleb128 idiom
/// (kept local — promote only if a fourth production consumer appears).
#[inline]
...
/// Encode the REAL `add_chunk` pure call-arg wire from the
...
/// Encode a [`MemoryRootAnchorArgs`] to its BCS struct-parity wire,
/// reproducing the fixture layout byte-for-byte:
/// `root[32 raw] ‖ blob_id[vec] ‖ kind[u8] ‖ parent[vec] ‖ digest[vec]`.
/// `parent == None` → empty vector (`0x00`); 100 bytes parent-none, 132
/// bytes parent-some.
///
/// **This is replay/parity EVIDENCE, NOT the `add_chunk` PTB call args.**
/// The [`StageBCallBuilder`] must NOT use
/// this wire as call args (it would inject a serialized `root` and a phantom
/// `digest` the real entry never takes); the builder uses
/// [`encode_add_chunk_pure_args`] instead. This encoder exists so the
/// tests can keep cross-pinning the fixture lengths/layout.
...
/// Encode an [`AuditAppendArgs`] to its BCS struct-parity wire,
/// reproducing the fixture layout byte-for-byte:
...
/// records the planned transaction byte size.
///
...
impl StageBCallBuilder {
    ...
    /// object id (from the [`MemoryRootAnchorArgs`]) becomes the
    ...
    /// call wire. Network + gas gated as in
    ...
    /// `log` object id (from the [`AuditAppendArgs`]) becomes the
    ...
    /// version/digest — those are owned by the future signing layer).
    ...
    /// The planned transaction byte size ("tx byte size
...
mod tests {
    ...
    fn b3_13_build_calls_all_three_kinds_on_testnet() {
        ...
        // Package id is the placeholder (32 zero bytes) on all three.
        for b in [&cr, &ac, &ap] {
    ...
    fn b3_13_add_chunk_pure_args_are_blob_kind_parent_only_no_root_no_digest() {
        ...
        // appear anywhere in the pure call-arg wire.
        assert!(
    ...
    // ---- struct-parity EVIDENCE is preserved (NOT the call wire) ----------

    #[test]
    fn b3_13_anchor_bcs_matches_atom_133_layout_and_lengths() {
        // parent = None → 100 bytes (the
        // b3_12_anchor_args_bcs_parity_vector_parent_none fixture). Parity evidence.
        ...
        // Field boundaries (mirror of the b3_12_anchor_wire_field_boundaries fixture).
    ...
    fn b3_13_audit_bcs_matches_atom_133_layout_and_length() {
        // 65 bytes (the b3_12_audit_append_args_bcs_parity_vector fixture).
