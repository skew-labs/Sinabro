//! Stage C PTB byte-size minimizer (C-WP-03A · atom #189 · C.0.18).
//!
...
//! * **No re-mint.** The call tag reuses the §4.1 [`GasTraceFunction`]; the
//!   trace reuses the §4.0 [`StageCTraceLink`]. No new call-kind / trace newtype
//!   is minted here.
...
/// the §4.0 [`StageCTraceLink`] so a report is greppable by Stage C atom + gate.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
