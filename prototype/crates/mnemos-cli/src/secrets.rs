//! §4.2 secret reference + inline-secret scanning (atom #406 F.0.5).
//!
...
/// §4.2 — where a secret reference resolves (the value is never loaded here).
#[repr(u8)]
...
/// §4.2 — a status-only view of a secret reference.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// SI-1 — a secret value that physically cannot be rendered, logged, persisted,
/// or serialized. The PD-4 structural invariant: the leak state is
...
/// layout of `T` (PD-5: no runtime overhead).
///
