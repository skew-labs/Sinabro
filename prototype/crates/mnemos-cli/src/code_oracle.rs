//! `code_oracle` — the CODE-class verification oracle.
//!
...
//!
//! Token-minimal and drift-free: the oracle is 100% LOCAL (0 external LLM tokens),
...
//! Verified by real probe: `sui move build` runs offline in this sandbox
//! and genuinely discriminates (valid ⇒ exit 0, type-error ⇒ exit 1). A nuance found and
//! corrected: `sui` resolves the framework from `$HOME/.move` when
...
/// Wall-clock cap for one Move build oracle (ms). Measured < 1s; this is a generous
...
fn run_sui_move_oracle(answer: &str, sub: &str) -> VerificationEvidence {
    ...
    // (kernel-DENIED here) and fails. Observed: HOME=cold-dir ⇒ exit 1
...
/// The CODE oracle STRENGTHENED with behavioral unit tests (the "compile + tests"
/// stage, EvalPlus-motivated): runs `sui move test` — which
...
/// which merely COMPILES (a failing assert exits 1, a passing
...
/// real `sui move test` oracle (COMPILE + behavioral unit tests — strictly stronger
...
/// cross-memory) ride the autonomous R-E-W loop, so here they are `Absent`. The
/// MODEL's answer text is fed only to the deterministic COMPILER/TEST-RUNNER, never to
/// `verify` (no self-certification — the hidden-oracle boundary).
#[must_use]
...
mod tests {
    ...
    /// LIVE TEST oracle (macOS + `sui` present): a module with a PASSING `#[test]`
    ...
    fn live_test_oracle_is_strictly_stronger_than_build() {
        ...
        // a module that COMPILES but whose test FAILS — the behavioral reject.
