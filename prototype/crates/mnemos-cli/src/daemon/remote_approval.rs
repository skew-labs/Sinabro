...
//! (ENDGAME). Ties the three E4 halves into the loop the owner asked for.
//!
//! When the autonomy runtime hits a gated action it cannot fire (`FrontierDenied` —
//! no grant), the owner is PINGED ([`build_approval_ping`] = an -redacted send;
//! [`ping_through_center`] = the single dedupe gate, one `ApprovalRequest` per
...
//! , action-bound, replay-refused via the now load-bearing
//! [`ApprovalSyncLedger`]; on approval a NARROW single-shot grant is minted
//! (the unforgeable path). That grant ARMS the runner
...
//! [`crate::telegram::inbound_auth::authenticate_and_mint`] (the owner-path
...
pub enum RemoteApprovalReject {
    /// the sender is not the owner pin — dropped.
    SenderNotOwner = 1,
    /// The reply was not an `approve`/`deny` verb — ignored.
    UnrecognizedReply = 2,
    /// the reply named no known pending action.
    NoSuchPendingAction = 3,
    /// missing-hash or replay — refused.
    ReplayOrUnbound = 4,
    /// The ceremony/arm refused (should not occur for a valid approve).
    MintFailed = 5,
}
...
pub enum RemoteApprovalOutcome {
    ...
    /// `Approved` (egress) — an egress approval can never authorize a mutate.
    ApprovedMutate {
...
impl RemoteApprovalCoordinator {
    ...
    /// the chat cycle classifies inbound updates against ONE source of truth
    ...
    /// Ingest ONE untrusted inbound update: auth + mint (.T4 via
...
/// The dedupe key for a pending action's ping (one `ApprovalRequest` per
...
/// Build the -redacted outbound ping for a pending action. EVERY byte passes the
...
/// new live-send path is added — the wall is unchanged).
#[must_use]
...
/// Decide + record the ping through the SINGLE dedupe gate
...
/// advances the monotone offset, and ingests each owner reply into the
...
// ============================================================================
// (⑯) — the background "away → ping → reply → proceed" SERVE LOOP. A bounded
// poll-and-arm cycle composed over the proven E3/E4 primitives. It MINTS nothing
// (the mint is `authenticate_and_mint`, reached only via `ingest_update`); it
// only INSTALLS an already-minted grant and PROCEEDS the ONE denied action.
// custody/funds are untouched (DP8).
// ============================================================================
...
/// replay-refuse / monotone-offset are reused, never reimplemented.
...
/// (`Copy`) and delegates to [`poll_and_ingest`] — the ONE getUpdates edge (DP4).
...
pub struct ServeArm<'a> {
    ...
    /// across the loop (the caller never re-creates it per window/cycle).
    pub coordinator: &'a mut RemoteApprovalCoordinator,
    /// The dedupe gate for the approval ping (one ApprovalRequest per action).
    pub center: &'a mut NotificationCenter,
}

/// The bounded parameters of one serve cycle. `poll_windows_max` is the per-cycle
/// long-poll window cap ( no unbounded spin within a cycle).
pub struct ServeParams<'a> {
    ...
    /// derived from it: `sha256("frontier egress: " + task)`).
    pub task: &'a str,
    /// The per-cycle poll-window cap (bounded).
...
pub enum ServeCycleOutcome {
    ...
    /// was installed → the ONE denied action PROCEEDED (single-shot).
    ...
    /// failure). The action STAYS DENIED — fail-closed, zero egress.
    DeniedNoReply {
        /// The action that stays denied (its hash).
        action_hash_32: [u8; 32],
    },
    /// The ping could not be built/delivered (secret-shaped or a dedupe error) —
...
/// Run ONE bounded "away → ping → reply → proceed" serve cycle (⑯, part 1).
///
...
/// `FrontierDenied`, register the denied action + PING the owner through the
/// `build_approval_ping` (dry-run; secret-shaped ⇒ fail-closed) and the
/// `ping_through_center` dedupe gate; (3) POLL the injected source for the owner's
/// reply across the BOUNDED `poll_windows_max` windows; (4) on an `Approved`
...
/// proceeds; (5) no approval / a deny / a transport failure ⇒ the action
/// STAYS DENIED.
///
/// This function MINTS nothing (the mint lives in `authenticate_and_mint`,
...
/// (DP8). The `coordinator` is the caller's LONG-LIVED state (its ledger is
/// persisted across the loop).
pub fn serve_poll_arm_cycle(
    ...
    // 2. FrontierDenied — register the denied action + PING (dry-run + dedupe).
    ...
    // 3. POLL the source for the owner's reply across the BOUNDED windows.
    ...
    // 5. No approval within the bounded windows ⇒ the action STAYS DENIED.
    ServeCycleOutcome::DeniedNoReply { action_hash_32 }
}

// ============================================================================
// (⑱) — the TELEGRAM REMOTE-CONTROL chat cycle: a FREE-FORM owner message →
...
// : a chat prompt is not an approve/deny reply, so it is a different
// control flow (local-turn-then-reply, never a frontier escalation). It MINTS
// nothing (the mint is `authenticate_and_mint`, reached only on the
// `ApprovalReply` route via `coordinator.ingest_update`); the chat turn is LOCAL +
// READ-class + zero-egress; the only outbound is the reply, gated by the
// owner-armed session. Custody/funds untouched.
// ============================================================================
...
/// variant maps to a threat (#). No variant except `Replied` performs an
/// outbound send; no variant reaches a custody/chain symbol.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ChatTurnOutcome {
    /// a non-owner update — dropped before ANY work (no turn, no send,
    /// nothing recorded).
    NotOwnerDropped,
    /// a secret-shaped inbound — WITHHELD before the agent ever saw it
    ...
    /// (Option A): a chat prompt with NO armed session — surfaced as a card
    ...
    NoAnswer,
    /// the turn's answer was whole-secret (every fragment denied ⇒ outgoing
    ...
    /// session (turn → redact → armed send). `sent` = the
    ...
    /// classified, nothing sent, fail-closed (bounded).
    PollFailed,
}

/// A redacted chat reply ready to deliver: the send WALL (proves the body
...
/// SAME `redact` choke the ping/consult egress use — a whole-secret answer
/// yields `None` (the reply is WITHHELD). So a chat reply whose text never
/// passed redaction is UNREPRESENTABLE (mirrors [`RedactedTelegramSend`]'s).
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct RedactedChatReply {
    /// The send wall — a `RedactedTelegramSend` whose payload hash IS the
    /// redacted body hash (built only from the receipt below).
    send: RedactedTelegramSend,
    /// The secret-free reply text (the answer that passed the choke ENTIRELY —
    /// `secret_fragments_denied == 0`). Carried to the ONE `sendMessage` edge.
    text: String,
}

impl RedactedChatReply {
    /// Build a redacted reply from a LOCAL turn's `answer`. Runs the
    ...
    /// ; a secret-free answer yields the dry-run send (bound to the redacted
    ...
    /// The caller flips the send live ONLY inside an armed session.
    #[must_use]
    fn build(answer: &str) -> Option<Self> {
        ...
        .ok()?;
        // Whole-secret ⇒ WITHHOLD (nothing passed the choke). Identical
    ...
    /// (produced ONLY on the armed branch of [`serve_chat_cycle`]).
    #[must_use]
    ...
    /// The send wall (the redacted-body proof the codec preflight demands).
    /// Named `redacted_send` (not `send`) so the call site never collides with the
    /// reqwest socket send-call the e0b verifier pins.
    #[must_use]
...
/// it AUTHORIZES NOTHING: the card is delivered ONLY through the SOLE redact wall
...
impl TradeCard {
    ...
    /// field ⇒ the WHOLE card is WITHHELD (`None`, ). The card authorizes nothing;
...
/// and adopts the monotone offset (reuse, no second poll path); a scripted
...
/// (reuse). Reached only when the owner runs `daemon serve-chat` with
...
/// lives — so the single-dispatch-truth holds: NO `.send_live_*` execute path
...
pub trait ChatReplySink {
    ...
    /// transport may still report a wire failure). The reply carries the wall
...
/// live-egress execute home — NOT here. The dispatch builds the closure that
/// calls `TelegramTransport::send_live_message`; this core references no
/// `.send_live_*` path (preserved). The scripted reply sink for tests is its
/// own `#[cfg(test)]` type.
...
pub struct ChatArm<'a> {
    ...
    /// route (replay-refused across windows, reuse).
    pub coordinator: &'a mut RemoteApprovalCoordinator,
}

/// The bounded parameters of one chat serve cycle. `poll_windows_max` is the
/// per-cycle window cap ( no unbounded spin within a cycle).
...
/// Run ONE bounded chat serve cycle: poll the injected raw-update
/// source across BOUNDED windows, classify each update fail-closed
/// ([`classify_inbound`]: owner-pin → secret-withhold → split), and
/// per disposition:
/// - `NotOwner` / `WithheldSecret` ⇒ DROP before any turn.
...
/// - `ChatPrompt` ⇒ with NO armed session, surface a card only
///   (NO turn, NO send); otherwise run a LOCAL, READ-class, zero-egress turn
///   ([`run_agent_loop_with`] with `file_policy = None` + `web_seam = None`),
///   redact the answer (whole-secret ⇒ withheld), flip the reply live + send
///   it through the armed-gated sink, and consume one session action so replies are
///   bounded by the grant's `max_actions`.
///
/// MINTS nothing (the mint lives in `authenticate_and_mint`, reached only via
/// the `ApprovalReply` route's `ingest_update`); touches no custody/chain symbol.
/// The chat turn never reaches the frontier transport. The
...
pub fn serve_chat_cycle(
    ...
    // ONE source for the classify pin — never drifts from the approval pin.
    let owner_chat_id = coordinator.owner_chat_id();
    let mut outcomes: Vec<ChatTurnOutcome> = Vec::new();
    // BOUNDED poll windows ( no unbounded spin within a cycle).
    for _ in 0..params.poll_windows_max {
        ...
        for update in &updates {
            // CLASSIFY fail-closed: owner-pin FIRST → secret-withhold
            // → approval-vs-free-form split. A non-owner / secret-shaped
            ...
            match classify_inbound(update, owner_chat_id) {
                ...
                InboundDisposition::ChatPrompt(text) => {
                    // / Option A: the arm gates the ENTIRE loop. With NO
                    ...
                    // a LOCAL, READ-class, zero-egress turn — `file_policy =
                    ...
                    // the `redact` choke gates the reply — a
                    ...
                    // flip the send live ONLY on the armed branch (the
...
mod tests {
    ...
    // A spoofed sender arms nothing and resolves nothing — the pending action
    ...
    // A replayed approval is refused on the second ingest — the grant is
    // minted exactly once.
    #[test]
    ...
    // The outbound ping is built through the redact choke (a redacted send whose
    ...
    // The single dedupe gate suppresses a duplicate ping for the SAME pending
    ...
    // The offset adoption is monotone: a lower offset never rewinds it.
    #[test]
    ...
    // ---- (⑯): the background poll-and-arm SERVE LOOP, hermetic (0 real net) ----
    ...
    /// second poll path (sender-pin/replay/offset reused). `#[cfg(test)]` ONLY.
    ...
    /// proves a transport failure keeps the action denied, fail-closed.
    ...
    // stays denied — exactly one frontier egress.
    #[test]
    ...
    // a denied-then-NO-REPLY cycle ends fail-closed — the bounded windows
    ...
    // a transport FAILURE (token missing analogue) keeps the action denied —
    ...
    // a SPOOFED (non-owner) reply NEVER proceeds — the sender pin (reused, not
    ...
    // the ledger PERSISTS across the loop — ONE long-lived coordinator; the
    ...
    fn serve_loop_ledger_persists_no_double_proceed() {
        ...
        // ledger replay-refuses ⇒ NoAction ⇒ stays denied; NO second proceed.
    ...
    // / the loop runs under the REAL `RuntimeHandle::spawn` std-thread
    ...
    // ---- (⑱): the TELEGRAM REMOTE-CONTROL chat cycle, hermetic (0 real net) ----
    ...
    /// polled (the bound proof). `#[cfg(test)]` ONLY.
    ...
    /// proves a transport failure is fail-closed (PollFailed).
    ...
    /// A secret-shaped fixture the choke scanners catch (the canonical
    ...
    // A NON-OWNER chat message runs NO turn and sends NOTHING, even with the
    ...
    // A SECRET-SHAPED owner inbound is WITHHELD before the agent sees it
    ...
    // (Option A) — an owner chat prompt with NO armed session is a CARD ONLY:
    ...
    // /RC3/RC5 — an ARMED owner chat prompt runs a LOCAL turn (zero egress),
    ...
    // A WHOLE-SECRET answer from the LOCAL turn is WITHHELD: the reply is
    ...
    // /RC8 — an armed session of `max_actions = 1` bounds the replies: TWO
    ...
    // The poll window count is BOUNDED by `poll_windows_max`: an empty
    ...
    // A poll transport FAILURE is fail-closed: PollFailed, no turn, no send.
    #[test]
    ...
    // The chat cycle runs under the REAL `RuntimeHandle::spawn`
    ...
    fn trade_card_renders_seven_fields_and_routes_through_the_redact_wall() {
        ...
        // the 7-field envelope + the oracle verdict + the approve/deny id16 (the surface).
        for needle in [
            "action:",
            "network:",
            "protocol:",
    ...
    fn trade_card_with_a_secret_shaped_field_is_withheld_whole() {
        ...
        // redact wall every chat reply passes — never partially echoed.
        let card = TradeCard {
