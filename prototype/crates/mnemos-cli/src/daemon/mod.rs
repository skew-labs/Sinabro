//! `sinabro daemon` — the operational daemon + task/session control rail.
//!
...
//!   the redact choke inside the agent loop).
//! - [`remote_approval`][]: the "away → ping → reply → proceed"
//!   orchestration. When the runner hits a gated action it cannot fire (no grant),
//!   the owner is PINGED (redacted, deduped); the owner's untrusted phone
...
//!   (unforgeable) ARMS the runner ([`runtime::AutonomyRuntime::install_egress_grant`])
