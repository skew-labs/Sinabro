//! Autonomy runtime — the bounded background runner.
//!
...
//! the runner works autonomously — local-first, egress-bounded, killable. Written
//! against a dedicated threat model, authored before this code.
//!
//! Shape:
...
//!   against the live `(now, used)` — never cached.
...
//!   calls a caller-supplied driver to advance one tick.
//!
//! The runner holds ONLY READ (free; the recall READ is minted INSIDE the
...
//! `(now, used)` every turn — a grant is never a cached blank cheque. No
//! field can hold a wallet/secret; custody is unreachable. Every
//! local turn re-obeys the ⑧ loopback discipline (re-redact), and every assembled
//! outbound byte still passes the `redact` choke inside the loop (the runner
//! adds no socket and sees no raw bytes). The model cannot mint an
...
pub enum TurnOutcome {
    ...
    /// Control halted this tick (pause / express STOP) — no side effect.
    Paused,
    /// The budget cap re-check refused the next side effect (fail-closed):
    ...
    /// grant — fail-closed, ZERO egress, no silent downgrade to local.
    FrontierDenied,
    /// The job is terminal (killed / done) — `tick` is a no-op (no zombie).
    Terminated,
}
...
/// The typed outcome of one owner-authorized mutate PROCEED step (ENDGAME).
...
pub enum MutateProceedOutcome {
    ...
    /// fail-closed: ZERO side effect, no silent fallback.
    MutateDenied,
...
/// minimal token + cost estimate, so the cap re-check is exercised on
...
/// the security spine (.R10). Holds ONLY an optional owner-armed
...
pub struct AutonomyRuntime {
    ...
    turn_u32: u32,
    /// The OPTIONAL owner-armed MUTATE-LOCAL grant (ENDGAME — a tier-correct
    ...
    /// the capability is re-derived live per action. Tier-distinct from the
    /// egress grant: an EgressGrant cannot occupy this field (type). No field
    /// can carry a wallet/secret; custody is unreachable.
    ...
    /// GRANT (not a capability, never a key/wallet) so the `ChainTxCapability` is
...
impl AutonomyRuntime {
    /// Arm the runner for ONE background job. Holds READ freely (minted
    ...
    /// (interactive never starves). `admission_max_concurrent_u32` is the
    ...
    pub fn arm(
        ...
        // the interactive lane preempts via the reserved-slot arithmetic.
    ...
    /// armed escalation). Whatever transport is injected, it carries its OWN
    /// `redact` + loopback-bind walls; a denied route runs NO transport
    /// (zero egress).
    pub fn tick(
        &mut self,
        now_epoch_ms: u64,
        ...
        // 1. Control re-read (no-zombie / preempt): a terminal job is a
        ...
        // 2. Budget-kill cap RE-CHECK before the side effect. A lowered cap
        ...
        // . Expired / rate-exceeded / revoked ⇒ None (fail-closed). The
        ...
        // 4. Route via the typed selector. The model cannot mint an
        //    EgressCapability (E0d) ⇒ cannot self-route to the frontier.
        ...
        match route {
            ...
            ConsultRoute::LocalLoopback | ConsultRoute::Frontier => {
                // 5. Run ONE bounded agent loop. Redact is intact INSIDE the
    ...
    /// a killed job never resurrects).
    pub fn resume(&mut self) {
        let _ = self.inbox.resume(self.job_id_u64);
        self.control.resume();
    }

    /// Kill the job — terminal + IRREVERSIBLE (no zombie, no resurrection).
    ...
    /// (fail-closed; the job continues LOCAL-only).
    ...
    /// re-derived per turn) and holds no secret/wallet.
    ...
    /// effects (ENDGAME). Sources: a tier-correct telegram approval (a
    ...
    /// grant; the capability is re-derived per action) and holds no
    /// secret/wallet. Tier-distinct: an `EgressGrant` cannot be installed
    /// here (the parameter type is `MutateGrant`), so an egress approval
    ...
    /// Install an owner-armed COMPOSITE BOLD SESSION grant (ENDGAME / ⑳) — arms
    ...
    /// `(now, used)` ( / ⑳), fail-closed. The session is
    ...
    /// CUSTODY is unreachable (uninhabited) and DOWNLOAD is absent.
    ...
    /// (fail-closed; the agent-proposed side effects stop).
    ...
    /// mutate window (ENDGAME). The order is the security spine: control
    ...
    /// (fail-closed on expired/rate/revoked) → (only then) the SINGLE gated
    /// chokepoint ([`execute_authorized_mutate`]). No grant / an invalid
    /// grant ⇒ [`MutateProceedOutcome::MutateDenied`] — ZERO side effect, no silent
    /// fallback. Custody is unreachable. `action` is the
    ...
    pub fn proceed_authorized_mutate(
        ...
        // RE-DERIVE the capability at the LIVE (now, used) — never cached.
        ...
        // action, so the rate count advances exactly once per proceed.
    ...
    /// key/wallet. Tier-distinct: only a `CustodyGrant` installs here.
    ...
    /// custody is unreachable (`CustodyCapability` uninhabited). On a `Ran`, the action +
    ...
    /// even while the background job is live (interactive never starves).
    ...
    /// / ⑱ — the remote-control reply gate). RE-DERIVES the
    ...
    /// Consume ONE egress action of the armed session's rate budget (ENDGAME
    /// / ⑱) — called once per DELIVERED remote-control reply so replies are bounded
    /// by the grant's `max_actions` (RC8). A delivered reply IS an egress
...
/// NO business logic: it locks the shared [`AutonomyRuntime`], honors a
...
impl RuntimeHandle {
    ...
    /// turn and breaks; [`join`](Self::join) then returns (no zombie).
    pub fn kill(&self) {
        if let Ok(mut rt) = self.shared.lock() {
            rt.kill();
        }
...
mod tests {
    ...
    /// / with NO grant, the autonomous default is LOCAL and a bounded
    ...
    /// / SECURITY PROOF: an autonomous FRONTIER escalation WITHOUT a
    ...
    /// a VALID owner-armed grant lets a frontier escalation fire, and the
    ...
    /// ENDGAME — the FULL "away → ping → reply → proceed" loop, hermetic (no
    ...
    /// an EXPIRED grant (now >= expires_at) fails the frontier closed.
    #[test]
    ...
    /// a REVOKED grant fails the frontier closed (and a local turn still runs).
    #[test]
    ...
    /// lowering the cap to zero stops the NEXT autonomous turn fail-closed
    ...
    /// a KILLED job's tick is a no-op (Terminated), and resume can NEVER
    ...
    /// a paused job performs no side effect; resume re-enables it.
    #[test]
    ...
    /// the single background job never starves the interactive lane — an
    ...
    /// (threaded teardown): the REAL worker thread terminates on `kill` and
    ...
    /// ENDGAME (⑬) — the FULL "away → ping → reply → PROCEED-EXECUTE" loop
    ...
    fn e10_mutate_away_ping_reply_proceed_executes_then_caps() {
        ...
        // AWAY: no mutate grant ⇒ a proceed is DENIED (zero side effect).
        assert!(matches!(
        ...
        // TIER-CORRECT mutate grant (NOT an EgressGrant).
        let reply =
    ...
    /// A broad owner-armed MUTATE grant authorizes up to `max_actions`,
    ...
    /// (tier separation) — an installed EGRESS grant grants ZERO mutate
    ...
    /// / ⑳ — installing a composite BOLD SESSION grant arms BOTH halves: the
    ...
    /// / ⑳ — revoking the bold session (both halves) stops the next action
