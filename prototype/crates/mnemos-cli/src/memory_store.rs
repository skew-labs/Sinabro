...
//! later sub-steps over this base.
//!
...
//!    not a second cipher.
//! 2. **DL-2 deterministic ciphertext (a deliberate design choice).** `aes-gcm-siv` is
...
//! Held as a `Secret<[u8; 32]>` in [`MemoryCipher`] — structurally
...
/// The at-rest cipher. Holds the 32-byte key as a [`Secret`] so it is
...
// ===========================================================================
// Persisted store — content-addressed, encrypted, replay-recoverable (-b)
// ===========================================================================

/// Record magic (4 bytes) — `MNMC` = MNemos Memory Chunk.
pub const RECORD_MAGIC: [u8; 4] = *b"MNMC";

/// Record format version. v2: the sealed plaintext is
...
/// time, so no readable data is dropped and no legacy
...
impl PersistedStore {
    ...
    /// The raw on-disk record bytes (AEAD CIPHERTEXT) of every `.mc` record,
    ...
    /// Every memory as `(memory_id, topic_summary, ciphertext)` for the
    ...
    /// Decode a FETCHED record blob (e.g. a sub-store blob fetched back from
    ...
    /// Seal the Walrus MAIN-INDEX manifest with the LOCAL AEAD key, bound to the
    ...
    /// Open a fetched Walrus MAIN-INDEX blob back to the manifest bytes (AEAD
    ...
    /// Seal a Skew history-time-series WINDOW with the LOCAL AEAD key, bound to the
    ...
    /// Open a sealed Skew history window back to its plaintext bytes (AEAD tag + history-AAD
    ...
    /// Seal a CERTIFIED strategy corpus entry (the canonical strategy TOML as a pattern memory)
    ...
    /// Open a sealed strategy corpus entry back to its plaintext bytes (AEAD tag + corpus-AAD
    /// verified). `Err` (fail-closed) on wrong key / tampered / wrong-AAD blob.
    ...
    /// Settings-sync: seal a (secret-screened) config TOML with the LOCAL AEAD key,
    ...
    /// Settings-sync: open a fetched Walrus settings blob back to the config TOML
    ...
    /// Semantic codebase index: seal the serialized vector store with the LOCAL key,
    ...
    /// Semantic codebase index: open a sealed index blob back to the serialized bytes
...
/// Mint a `MemoryChunk` for a user-authored memory `text` at `id`.
...
/// because the OTel exporter writes its span files with THIS one
/// implementation (⑨ — one atomic-write discipline, zero drift).
...
mod tests {
    ...
    /// The AAD path binds: a blob sealed under one associated-data
    ...
    /// A legacy v1 record (`id || wire`, version byte 1, no class, no
    /// AAD) is REJECTED-as-skip, never misparsed. The live store held ZERO v1
    /// records at upgrade time; this pins the fail-closed
    ...
    /// A version-flipped v2 record, RENAMED to its new content-
    ...
    /// Lifecycle — the owner class persists across a "restart" (a NEW
    ...
    /// Recall is LIVE + redacted from a REAL store. A
    ...
    /// path is WITHHELD + locks the loop down (redaction on recall, from
