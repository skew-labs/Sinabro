//! `zerog_chain` — 0G Chain (EVM) anchor PREPARE for an oracle-verified pattern.
//!
...
//! `curl`/`cast` — the network legs live outside the binary.
//! `CustodyCapability` stays uninhabited; this module names no custody symbol.
//!
...
//!   "oracle-verified". Reproducibility is `include_bytes!`-checked.
//!
...
/// The LOCKED pattern hash: sha256 of the frozen, `sui move build`-verified Move
...
/// The locked anchor calldata (the encoder applied to the seam-locked inputs).
#[must_use]
...
/// Build the OWNER-runbook lines for the anchor (PURE — only strings). The agent
...
mod tests {
    ...
    fn encoder_reproduces_the_cross_language_golden() {
        // Rust encoder ⟂ Python ⟂ solc: the locked calldata is byte-identical.
