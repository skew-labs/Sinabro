//! Interactive REPL read-eval loop.
//!
...
//! The `setup` / `setup memory` wizards render the onboarding flows.
//!
//! Live boundary: render / dispatch only. Every side-effect verb stays
//! same-message approval-gated and disabled — [`crate::dispatch::run`]
...
//!   replaced the hand-rolled raw-mode line editor.
...
//!   path and the Plain proof stays valid).
...
/// Acknowledge a STOP-class control on the express rail: the ack is
...
/// The 5-step setup wizard (Provider -> MemoryOwner -> Telegram -> Privacy
...
/// The mandatory first-run 4-axis memory wizard (storage / identity /
...
/// Render the live audit game tree (`audit scan/explain/repro-plan`). The
...
/// Render the live memory commands surface (`memory status/list/query/
...
/// Render the live evidence pack (`evidence pack`). The hash-linked
...
/// Render the live skill discovery/use/state surface (`skill search/
...
/// Render the live daemon supervisor + task/session inbox + reconnect
...
/// Render the live control-express-under-load surface (`control
...
fn handle_line<W: Write, E: Write>(
    ...
    // Interactive onboarding surfaces.
    ...
    // `audit scan/explain/repro-plan` drive the live audit game tree
    ...
    // `memory status/list/query/export/delete/replay` drive the live memory
    ...
    // `evidence pack` drives the live hash-linked evidence pack manifest
    ...
    // `skill search/inspect/recommend/use/enable/disable/update/remove`
    ...
    // `daemon status/inbox/reconnect/supervisor` drive the live daemon
    ...
    // `control status/express` drive the live control-express surface
...
pub(crate) fn run_trimmed_line<W: Write, E: Write>(
    ...
    // Auto-redact secret-shaped input before it persists in history.
...
/// The cooked read-eval loop: greet, then read a line with line-buffered
...
pub fn launch() -> io::Result<()> {
    ...
    // preserves the Plain proof + the headless `run_repl` tests.
...
mod tests {
    ...
    fn audit_scan_drives_live_game_tree_candidate_not_finding() {
        // `audit scan` drives the full live pipeline (not the bare dispatch
    ...
    fn skill_verbs_drive_live_security_first_sandbox_bound_quarantine_sticky() {
        // bare `skill <verb>` drives the live skill surface through the
    ...
    fn daemon_verbs_drive_live_no_secret_killable_one_state_hash() {
        // bare `daemon <verb>` drives the live daemon surface through the
    ...
    fn control_verbs_drive_live_express_lane_under_load() {
        // bare `control <verb>` drives the live control-express surface
    ...
    fn rich_tty_path_boxes_greeting_prompt_and_response() {
        ...
        // Plain path, so this purely ADDS coverage — the Plain proof is intact.
