//! REPL shell boundary (atom #409 F.1.0; presentation reworked in G-WP-12).
//!
...
//! is the canonical OUT and is fully testable via injected input lines (no
//! terminal required).
//!
//! On a real TTY the interactive surface is a `reedline` chat loop ([`chat`])
//! preceded by a one-shot `ratatui` [`splash`] (G-WP-12); the hand-rolled
...
/// refinement happens in each namespace's handler (later F-WP atoms); the
