...
//! the closed grammar and injected skill-id hints from the skill-index cached
...
//! map half lives in [`crate::commands::context`].

use crate::grammar::{self, CliNamespace};

/// A cached skill-id hint injected from the skill index. `stale` marks a hint
