...
//! them. The p95 computation here is the testable canonical surface; the criterion
//! statistical harness is deferred, and
//! `benches/repl_latency.rs` is an offline `std::time` harness that scores real
//! measurements against this budget.

/// Perceived-latency p95 budgets, in milliseconds.
...
impl LatencyBudget {
    /// The default budget: keypress ≤ 16ms,
