//! `metamorphic_oracle` — O-4: the summarization METAMORPHIC deterministic oracle (the
//! Oracle Bootstrap's second domain class; master plan §6.3 S2 + §6.4 R2 + §6.9). The S2
//! analog of the S1 [`crate::reconcile_oracle`]: where reconciliation re-derives an
...
//! never the judge** (no reward-hacking, §6.5). 0 LLM tokens, 0 IO, no clock/RNG — pure
...
//!
//! ## ★ THE METAMORPHIC LAW — a SOUND REJECTOR (§6.3, Chen et al. 2018)
...
//!   and still OMIT the key point or mislead by selection. Per the R2 ladder rung
//!   (rejector-only, owner-locked O-4 seam Q3), a `NotFalsified` verdict NEVER admits a write.
...
//! the fenced Bucket-B residue (they need a human/LLM in the loop forever, §6.3). Use a
...
/// The metamorphic verdict (mirrors the [`crate::reconcile_oracle`] verdict shape; R2 §6.4).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
