//! `skew_read` — MNEMOS × SKEW K-0b: the PURE, byte-locked decode layer for reading the Skew
//! `skew_otc` program's on-chain accounts (DEVNET). Pairs with the `web3_rpc` `getProgramAccounts`
//! READ method (K-0b-1): the dispatch glue fetches the raw JSON-RPC result over the EXISTING
...
//! `skew_chain_m1/skew-runtime/src/onchain.rs:92-138`). Nothing here is hand-guessed
//! ([[no-formula-guessing]]); the live `4nANvajB…` $1,000 account is a regression test vector.
//!
//! ## Honest scope (K-0b)
...
pub enum SkewAccountKind {
    ...
    ReferenceSnapshot,
    /// An OTC settlement receipt (441 bytes; the volume / realized-price time-series source, K-3).
    SettlementReceipt,
    /// A per-market lazy funding accumulator (74 bytes; the funding-rate time-series source, K-3).
    FundingState,
...
/// volume / realized-price time-series source, K-3). DERIVED by the documented Anchor account
...
/// funding-rate time-series source, K-3). DERIVED by `sha256("account:FundingStatePda")[..8]` (same
...
impl UraBalance {
    ...
    /// the skew-kernel codec uses (`pending_profit` EXCLUDED, UNIFIED §6). Checked: `None` on overflow
...
/// A decoded `ReferenceSnapshotPda` — the validated per-market price reference (K-3 price series).
...
pub struct ReferenceSnapshotData {
    ...
    pub exponent: u8,
    /// @75 The §17 guard-pass bitmask (`ALL_PASS` on a validated snapshot).
...
/// A decoded `SettlementReceiptPda` — an OTC settlement event (K-3 volume / realized-price series).
...
/// A decoded `FundingStatePda` — the per-market lazy funding accumulator (K-3 funding-rate series).
...
pub struct PerpPositionView {
    ...
    /// @112(body) Realized-but-unbacked PnL (u128 atoms; UNIFIED §6; NOT withdrawable).
...
/// byte-exact from the verified Skew source; never hand-rolled ([[no-formula-guessing]]).
#[must_use]
...
/// never hand-rolled ([[no-formula-guessing]]).
#[must_use]
...
mod tests {
    ...
    /// PROVE every discriminator (the 6 live-confirmed + the 2 K-3 derived) is reproduced by the
    ...
    /// not a guess ([[no-formula-guessing]]).
    #[test]
    fn discriminators_match_anchor_derivation() {
        ...
        // the 2 NEW K-3 discriminators, derived by the SAME proven formula:
        assert_eq!(
