//! [7] Owner-box / remote-shell lane (B⑪, ⑪-class) — the highest-risk surface. Threat
//! model: `ops/evidence/stage_g/agent_loop/REMOTE_SHELL_THREAT_MODEL.md` (IV-RS1..IV-RS9).
//!
//! # The one place sinabro runs a command on a REMOTE box
//!
//! A NEW credentialed-remote capability (no SSH lane existed). Owner triangle (2026-06-17):
//! transport = OpenSSH subprocess; command surface = READ-only diagnostic allowlist v1.
...
//!   literal per variant. An arbitrary shell / a write / a chain-write is UNREPRESENTABLE
//!   (IV-RS1).
...
//!   handles auth, IV-RS4/RS5). [`render_remote_run`] requires an [`EgressCapability`]
//!   witness (owner-armed; the model holds no constructor — IV-RS3).
//! * the output is `redact()`-gated (IV-RS6); custody/chain-write HARD-LOCKED (PD-6, IV-RS7).
...
/// constructed, so only a fixed READ diagnostic can run on the remote box (IV-RS1).
...
impl RemoteCommand {
    ...
    /// write / arbitrary command — yields `None`, never a guessed command, IV-RS1).
    #[must_use]
...
/// Validate the OWNER-CONFIGURED remote host (IV-RS2) — PURE, no IO. Admit only
...
/// The SHARED remote-run pipeline (IV-RS1..IV-RS9). REQUIRES an [`EgressCapability`] witness
