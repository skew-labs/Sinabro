//! P4-1 OTel trace export (sinabro 1.0 mega-lane, roadmap §3.C).
//!
//! Threat model: `ops/evidence/stage_g/agent_loop/OTEL_EXPORT_THREAT_MODEL.md`
//! (⑨ IV-O1..O6) — THE FIRST OBSERVABILITY-EXPORT SURFACE in the core. One
//! ANSWERED consult ceremony may, when the OWNER opts in via
...
//!
//! Drift-0 physics (the 8 cross-cutting laws):
//! - L1 content-addressing: the filename IS the sha256 of the bytes;
//!   `traceId`/`spanId` are domain-separated sha256 derivations of the run's
//!   captured truth — never random, never a counter.
//! - L2 deterministic projection: [`project_otlp_json`] is a PURE function of
//!   the receipt facts + ceremony-captured times. No second truth is minted;
//!   the export re-derives from [`crate::agent_loop::AgentLoopOutcome`]
//!   exactly like the terminal receipt lines do.
//! - L3 byte-lock: the encoding is pinned byte-for-byte by an INDEPENDENT
//!   Python derivation (`scripts/check_otel_export_schema.py`, golden
//!   sha256 `83d39923…`) against [`GOLDEN_OTLP_JSON`].
//! - L5 fail-closed: strict env resolve (`unset`/`0` off · `1` on · anything
...
//!   returned `Ok`).
//! - L6 bounded: one ~2KB file per explicitly-opted-in ceremony; string
//!   attributes are char-safe capped; the exporter runs ONCE, post-loop,
//!   off the egress path (zero loop/prompt byte delta).
//!
//! Shareable-tier content (⑨ IV-O2): a collector reading the directory is an
//! UNAUDITED process (the ⑧ loopback-peer argument), and the owner's pipeline
//! may forward off-box — so the bytes hold ONLY counters, const typed labels,
//! hex digests, and two variable strings: the response-echoed `model`
//! (redaction-belted + capped) and the tool trail as a VERB-ONLY projection
//! (first whitespace token per entry — model-authored bytes and filesystem
//! paths are structurally absent). The question, the answer, memory content,
//! and file content are NOT inputs to this module.
...
/// The opt-in environment variable (⑨ IV-O4; the `measure` namespace's
/// "telemetry is opt-in, off by default, no egress" contract made real).
pub(crate) const SINABRO_OTEL_EXPORT_ENV: &str = "SINABRO_OTEL_EXPORT";

/// Domain separator for the content-derived 16-byte `traceId` (L1).
const TRACE_ID_DOMAIN: &[u8] = b"SINABRO-OTEL-V1-TRACE";
/// Domain separator for the content-derived 8-byte `spanId` (L1).
const SPAN_ID_DOMAIN: &[u8] = b"SINABRO-OTEL-V1-SPAN";
/// Char-safe byte cap on the response-echoed model attribute (L6).
const MODEL_ATTR_CAP_BYTES: usize = 200;
/// Char-safe byte cap on the joined verb-only trail attribute (L6).
const TRAIL_ATTR_CAP_BYTES: usize = 400;
/// Sub-directory of `data_dir()` that holds the span files (⑨ IV-O3).
const OTEL_DIR_NAME: &str = "otel";
/// Filename suffix — `<hex64>.otlp.jsonl`, one JSON line per file.
const OTEL_FILE_SUFFIX: &str = ".otlp.jsonl";

/// The strict tri-state resolve of `SINABRO_OTEL_EXPORT` (⑨ IV-O4).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) enum OtelExportSetting {
    /// Unset or the explicit `0`: no export, no receipt line (the default
    /// surface is byte-unchanged — baseline preservation).
    Off,
    /// The exact value `1`: export one span for this ceremony.
    On,
    /// Any other value: the export is REFUSED with a visible typed line —
    /// never a silent default, never a guessed intent (L5).
    Invalid,
}

/// Strictly resolve the raw environment value. Whitespace is NOT trimmed —
/// `" 1"` is an Invalid value, not a sloppy On (the ⑧ strict-resolve
/// precedent: a config byte either matches the contract or is refused).
#[must_use]
...
/// Why an export write was refused (typed, fail-closed; ⑨ IV-O4).
...
/// ceremony-captured wall-clock pair (L4: time is CAPTURED once as an input,
/// then the projection is deterministic over it).
#[derive(Clone, Debug)]
...
/// numbers (⑨ IV-O6 rule, Python-locked).
...
/// The VERB-ONLY trail projection (⑨ IV-O2): first whitespace token of every
/// trail entry, comma-joined, char-safe capped. Deterministic; drops paths,
/// memory ids, and any model-authored remainder structurally.
#[must_use]
...
/// Content-derived span identifiers (L1): `traceId` = first 16 bytes of a
...
/// The PURE deterministic projection (⑨ IV-O1): receipt facts → ONE
/// OTLP/JSON `ExportTraceServiceRequest` line. Emission order, separators,
/// id widths, int64-as-string, enum-as-number, the 18-attribute set, and the
/// status mapping are all byte-locked by [`GOLDEN_OTLP_JSON`] and the
/// independent Python derivation. Total over every stop class: a
/// non-`loop.completed` stop maps to `status {"code":2,"message":…}` (the
/// v2 failure-path sites will reuse this fn unchanged).
#[must_use]
...
/// The production span directory: `$HOME/.mnemos/otel/` (⑨ IV-O3 — the
/// P1-1 `data_dir` truth, one fixed sub-directory, no variable component).
...
/// The ONE consult-side seam both executors consume (the
/// `consult_memory_load` single-implementation precedent — one fn, zero
/// drift between the frontier and local routes). Returns the receipt line
/// to append, or `None` when the setting is Off (the default surface stays
/// byte-unchanged).
...
/// VISIBLE typed line (⑨ IV-O4: a side artifact may fail loudly, but it
/// never destroys the answer card and never fails silently).
...
fn run_consult_export(
    ...
    // ⑨ IV-O2 belt: the response-echoed model id is the only foreign-
    // controlled string that reaches the file; a hostile local server (⑧)
    // echoing a key-shaped "model" must not get it persisted for third-party
...
// ---- E5-3: read the REAL exported OTel spans back (the GUI audit feed) --------
//
// The GUI advertised an OTel trail but had NO reader (it re-rendered chat cards).
