//! `autonomy_evolve` — the autonomous Read-Execute-WRITE evolution loop (the
//! evolution spine).
//!
...
//! WRITTEN. It is the anti-hallucination break wired to persistence: a pattern is written ONLY when
...
//! Each written pattern carries a PERFORMANCE-TRACKING score that is REINFORCED
...
//!
//! token-min + drift-0: this whole decision is DETERMINISTIC RUST, 0 IO,
...
//! layer that CONSUMES this core. custody/funds stay HARD-LOCKED: pure, no IO.
...
pub struct PatternCandidate {
    ...
    /// The CLASS of the oracle that produced the admit verdict (the
...
/// the perf score (already reinforced for this verified-good outcome).
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct EvolutionWrite {
    ...
    pub content: String,
    /// The performance-tracking score (reinforced on this verified-good write).
    pub perf: PerfScore,
    /// Whether this pattern is DOUBLY VERIFIED — confirmed by TWO INDEPENDENT
...
pub struct EvolutionOutcome {
    ...
    /// Pattern keys whose oracle did NOT verify them (no Write — the anti-hallucination gate).
...
impl EvolutionOutcome {
    ...
    /// The number of WRITTEN patterns that are DOUBLY VERIFIED (two
...
/// The DETERMINISTIC WRITE decision (the anti-hallucination break wired to persistence): for each
...
/// performance-tracking score by key (a fresh pattern has the default); a written pattern's score is
...
pub fn select_evolution_writes(
    ...
    // Anti-hallucination held-out CANARY: if the deterministic gate has COLLAPSED (a known
    ...
    for c in candidates {
        ...
        // Performance-tracking: this verified-good pattern reinforces its perf score.
...
/// The well-known expert-kind label for a finance reconciliation pattern.
...
/// Build a WRITE candidate from a finance reconciliation receipt — the SINGLE bridge that
/// wires the deterministic [`crate::reconcile_oracle`] (the invariant oracle) into the
...
/// checker's TYPED verdict does (the no-LLM-judge boundary).
#[must_use]
...
/// The well-known expert-kind label for a recognition-synthesized pattern.
pub const RECOGNITION_KIND: &str = "recognition";

/// Build a WRITE candidate from an induced-checker verdict — the SINGLE bridge that wires
/// the [`crate::recognition_synth`] checker (the induced oracle) into the EXISTING ACCUMULATE
...
/// PROVISIONAL pattern (held-out zero-false-accept gated; a quantitative α-budget is a future refinement).
#[must_use]
...
/// The well-known expert-kind label for a summarization metamorphic pattern.
pub const METAMORPHIC_KIND: &str = "summary_metamorphic";

/// Build a WRITE candidate from a summarization metamorphic verdict — the SINGLE bridge that
/// wires the [`crate::metamorphic_oracle`] checker (the metamorphic SOUND REJECTOR) into the
/// EXISTING write gate ([`select_evolution_writes`], reused — no second write path). Unlike the
/// reconcile / recognition bridges (which ADMIT a verified pattern), this is REJECTOR-ONLY: the
...
/// anti-hallucination fabrication gate). The model never reaches `verify`; only the checker's TYPED verdict does
/// (no-LLM-judge). HONEST LOCK: a `NotFalsified` is provisional, NOT a faithful-summary cert.
#[must_use]
...
/// The well-known expert-kind label for a certified SKEW strategy pattern.
pub const STRATEGY_KIND: &str = "skew_strategy";

/// Build a WRITE candidate from a Skew strategy's conformal SHADOW certification — the SINGLE
/// bridge that wires the deterministic strategy cert ([`crate::skew_strategy::certify_strategy`] →
/// [`crate::verification::strategy_receipt`]) into the EXISTING ACCUMULATE write gate
...
/// pattern is `Strategy` ⟂ `CrossMemory` ⇒ DOUBLY VERIFIED); an UNcertified strategy does NOT (it
/// lands in `unverified`, never written — the anti-hallucination fabrication gate against the "wrong memory
/// written as success → collapse" failure mode). The model never reaches `verify` — only the
/// deterministic conformal cert bit does (the no-LLM-judge boundary). HONEST LOCK: an admitted
/// strategy's PROPOSALS stay in-bounds, NOT that it is PROFITABLE; shadow money 0; the live sub-budget
/// is the owner-armed path.
#[must_use]
...
// ===========================================================================
// Pattern-memory format + perf ledger (PURE codec; the IO layer persists these)
// ===========================================================================
...
mod tests {
    ...
    /// TWO-DERIVATION at WRITE time: a Code-class admit (compile oracle) ⟂ the
    ...
    /// THE ACCUMULATE CLOSURE: a RECONCILED finance certificate,
    /// bridged in, flows through the EXISTING write gate and ACCUMULATEs — and
    /// because the invariant axis ⟂ the write-time cross-memory axis, it is DOUBLY VERIFIED.
    /// A VIOLATED certificate (a sound reject) NEVER ACCUMULATEs (the anti-hallucination gate holds). No
    ...
    /// THE RECOGNITION ACCUMULATE: a CERTIFIED induced-checker `Accept` flows through the
    /// EXISTING write gate and ACCUMULATEs as a DOUBLY-VERIFIED (Induced ⟂ CrossMemory)
    /// pattern; an uncertified accept / a `Reject` / an `Escalate` NEVER ACCUMULATEs (the
    /// certify-before-accumulate + anti-hallucination gates). No LLM judge: the induced verdict is the only
    /// admission input.
    #[test]
    ...
    /// THE METAMORPHIC REJECTOR (rejector-only — NOTHING accumulates): a
    ...
    /// THE STRATEGY ACCUMULATE: a CONFORMAL-CERTIFIED Skew strategy flows through the EXISTING
    /// write gate and ACCUMULATEs as a DOUBLY-VERIFIED (Strategy ⟂ CrossMemory) pattern; an
    /// UNcertified strategy NEVER ACCUMULATEs (the certify-before-accumulate + anti-hallucination gates — the
    ...
    /// THE ANTI-HALLUCINATION WRITE GATE: only an admits_write (oracle-Verified) candidate is
    ...
    fn only_verified_patterns_are_written() {
        ...
        // Performance-tracking: a fresh verified-good pattern is reinforced once.
    ...
    /// Performance-tracking reinforcement accumulates across runs: a pattern with a prior score is
