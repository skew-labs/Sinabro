...
//!
//! # The user-owned signing boundary (IV-K2-4 / IV-K2-5)
...
//! `agent_loop` reaches no signer symbol (the model holds no signer and types no key, IV-K2-12).
//!
...
impl IsolatedSigner {
    ...
    /// placed in the model's context (IV-K2-5). Returned inside a zeroizing string so the secret
