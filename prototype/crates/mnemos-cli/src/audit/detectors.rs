//! Audit detector pattern surface.
//!
//! A unified surface over the detectors (static, Solana/Anchor,
...
//! ([`DetectorSurface::direct_finding_count`]). This module
//! performs no live action.
//!
//! Reuse (no reinvention): [`StaticCandidate`] / [`DetectorKind`],
//! [`SolanaPattern`], [`SuiMovePattern`], [`AuditCandidate`] /
//! [`AuditProfile`] / [`AuditScanView`].
...
/// Wrap an [`AuditCandidate`] as a candidate-only game-tree node (a
...
impl DetectorSurface {
    ...
    /// Wrap a REAL source-scan [`AuditCandidate`] (from
    ...
    /// Aggregate detector candidates into a local-only scan view.
    #[must_use]
    pub fn scan(
        profile: AuditProfile,
        changed_only: bool,
