//! Local repro runner receipt (atom #526 · G.3.10).
//!
//! [`LocalReproRunnerReceipt`] is the §3 record that a candidate was reproduced
...
//! mandatory hash, is refused (`G-G-AUDIT-GAME-TREE`). A receipt may record a
//! non-reproduced outcome (a defended invariant) without becoming a finding. This
//! module performs no live action.
//!
//! Reuse (no reinvention): mirrors the Stage E exploit-repro signal — a finding
...
/// §3 — a receipt that a candidate was (or was not) reproduced locally.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
