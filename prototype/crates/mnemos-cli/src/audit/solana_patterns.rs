//! Solana / Anchor audit pattern surface (atom #523 · G.3.7).
//!
...
//! detection produces a Stage F [`AuditCandidate`] that is candidate-only
//! (`local_repro_done = false`) — it can never become a finding without a local
//! repro receipt (`G-G-AUDIT-GAME-TREE`). There is no Solana RPC / Anchor client
//! here (consistency-lock "No Solana Active Tool Surface Yet"): this module is a
//! pure pattern descriptor and performs no live action.
//!
//! Reuse (no reinvention): [`AuditCandidate`] / [`AuditProfile`] from
//! [`crate::commands::eval_core`]; the Skew/Strike economic-invariant candidate
//! diet is the source corpus, never a live target.
