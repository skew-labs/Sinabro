//! Defended invariant memory.
//!
//! A move that does not break is not a failure — it is memory that lowers the
//! cost of the next read. [`DefendedInvariantMemory`] is the record of an
...
//!
//! Reuse (no reinvention): the scope/expiry replay idiom mirrors the
//! memory replay path; the reward-neutral outcome maps to the no-reward
//! labels (a defended invariant is never a positive training signal).

use crate::sha256_32;

/// The memory of an explored-but-defended invariant.
...
impl DefendedInvariantMemory {
    ...
    /// no-reward labels) and never a finding. Always `true`.
    #[must_use]
