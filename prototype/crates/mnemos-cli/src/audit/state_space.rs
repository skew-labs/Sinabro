//! Bounded protocol state space (atom #517 · G.3.1).
//!
//! An audit game tree searches a *bounded* state space, never an unbounded fuzz.
//! [`ProtocolStateSpace`] is the §3 record of the bounded axes — account, object,
...
//! "production probe" axis is forbidden outright (`G-G-AUDIT-GAME-TREE`: no
...
//! B/C/D chain-state concepts and the Stage F audit namespace.
...
/// §3 — the bounded protocol state space an audit search explores.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl ProtocolStateSpace {
    /// Whether every §3 axis is bounded non-zero.
    #[must_use]
