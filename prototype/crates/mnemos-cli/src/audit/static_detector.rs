//! Static detector bridge (atom #525 · G.3.9).
//!
...
//! a Stage F [`AuditCandidate`] (candidate-only); a low-confidence detection is
//! quarantined as a likely false positive. No detection can become a finding
//! without a local repro receipt (`G-G-AUDIT-GAME-TREE`). This module performs no
//! live action.
//!
...
pub struct StaticCandidate {
    ...
    /// The reused Stage F candidate (candidate-only).
...
/// Aggregate static candidates into a local-only scan view (reuse the Stage F
/// [`AuditScanView`]).
#[must_use]
pub fn scan(
    profile: AuditProfile,
