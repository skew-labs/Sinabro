...
//! (AE6). The render carries counts + STATIC rule labels + impact scores
//! only; the source anchors stay hashed, so no raw source byte can leak.
//!
//! Reuse (no reinvention): [`crate::commands::source_scan::scan_tree`] /
//! [`crate::commands::source_scan::rust_rule_hashes`] · [`DetectorSurface`] ·
...
pub fn run_source_detect(root: &Path, profile: AuditProfile) -> SourceDetectReport {
    ...
    // node (never a finding): direct_finding_count is structurally 0.
    ...
    // compile-time constants, never source bytes (secret-zero).
...
/// a finding, never a severity. The shared render for BOTH the dispatch verb
/// and the loop tool.
#[must_use]
...
mod tests {
    ...
    fn a_source_candidate_cannot_become_a_finding_without_a_repro_receipt() {
        // Verify-before-expose: a REAL source candidate (as `scan_tree`
        ...
        // ⇒ it STAYS a candidate (false-positive 0 by construction).
