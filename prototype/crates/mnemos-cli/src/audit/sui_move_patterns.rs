//! Sui / Move / Walrus audit pattern surface (atom #524 · G.3.8).
//!
...
//! produces a Stage F [`AuditCandidate`] that is candidate-only
//! (`local_repro_done = false`) — never a finding without a local proof / replay
//! (`G-G-AUDIT-GAME-TREE`). This module is a pure pattern descriptor and performs
//! no live action.
//!
//! Reuse (no reinvention): [`AuditCandidate`] / [`AuditProfile`] from
//! [`crate::commands::eval_core`]; the B/C/D Move/Walrus evidence is the source
