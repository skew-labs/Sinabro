//! Audit impact prior (atom #519 · G.3.3).
//!
//! Suspected paths are ranked by plausible impact, not by how scary a diff looks.
//! [`ImpactPrior`] is the §3 record of the per-axis impact estimate (funds at
...
//! deterministic — sorted by score, ties broken by ascending index
//! (`G-G-AUDIT-GAME-TREE`). This module performs no live action.
//!
//! Reuse (no reinvention): the reward-firewall philosophy of the Stage E reward
//! pipeline — impact is evidence-weighted, never self-asserted.

/// §3 — the per-axis impact prior for a suspected audit path. Each field is in
