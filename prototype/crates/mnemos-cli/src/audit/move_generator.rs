...
//! into a child node. [`AuditMoveGenerator`] is the record of the generator
...
//! production probe or a random-fuzz mode. This module
//! performs no live action.
//!
//! Reuse (no reinvention): [`crate::sha256_32`]; a reproduced node routes to a
//! finding only through the [`crate::commands::eval_core::route_to_finding`]
...
/// A node in the audit search tree.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// The audit move generator config + running sequence counter.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
