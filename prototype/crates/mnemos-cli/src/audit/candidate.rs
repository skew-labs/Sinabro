//! Candidate-before-finding boundary (atom #521 · G.3.5).
//!
...
//! the Stage F [`AuditCandidate`] with its game-tree origin and gates promotion
//! through the canonical [`route_to_finding`]: a candidate with no reproduced,
//! local-only receipt that backs its node can never become a finding
//! (`G-G-AUDIT-GAME-TREE`, `G-G-EARLY-REWARD-WEIGHTS`). This module performs no
//! live action.
//!
...
/// A Stage G audit-game-tree candidate: a Stage F [`AuditCandidate`] plus the
/// game-tree origin and the search node it sits on.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct AuditGameTreeCandidate {
    /// The reused Stage F candidate (rule / location / invariant / evidence).
