//! Audit invariant graph.
//!
//! The `audit scan` ([`crate::commands::eval_core::AuditScanView`]) is a
//! local-only candidate *surface*. The audit game tree starts one layer
//! earlier: an audit begins from the invariants that must not break, never from a
//! grep smell. [`AuditInvariantGraph`] is the canonical record of that node
...
/// The audit invariant graph: the bounded set of invariants an audit must
