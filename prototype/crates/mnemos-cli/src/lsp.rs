//! LSP language intelligence — a sandboxed language-server subprocess as a
//! READ capability.
//!
//! ## Thesis (the verify-oracle ethos)
//!
...
//! * REDACTION: the rendered diagnostics pass the `redact` wall before
...
//! ## Scope (v1)
//!
//! `lsp diagnostics <path>` only, Move + Rust. NOT a
//! general multi-language IDE (a non-goal). `definition` / `references` are
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
enum LspDeny {
    /// fail-CLOSED: no kernel sandbox on this host — the server is NEVER
    /// run unsandboxed.
    SandboxUnavailable,
...
/// `/dev/null`; every OTHER write stays denied (SBPL "last rule wins"). The
/// path-scoped grant keeps the agent-facing capability READ (read-only
...
impl LspServer {
    ...
    fn spawn_sandboxed(
        ...
        // fail-closed — never an unsandboxed fallback.
        ...
        command
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::null());
        // the child sees ONLY the allowlist (rust-analyzer needs PATH +
...
/// redaction belt: a secret-shaped diagnostic ⇒ the whole render is withheld.
#[cfg(feature = "lsp")]
