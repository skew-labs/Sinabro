//! Operational prompt / workpackage status view.
//!
//! The operational status view shows where Sinabro is in the WorkPackage
...
//! never a false-green. This is a pure projection — no I/O, no live action.
//!
//! Reuse (no reinvention): the prompt strip is
...
pub struct WorkPackageStatusView {
    /// The prompt strip status (workspace / model / budget / approvals).
...
impl WorkPackageStatusView {
    ...
    /// reused prompt strip; the rest are the workpackage coordinates.
    #[must_use]
