//! A⑤ git-as-capability-type — git `status`/`diff`/`log`/`show`/`blame` as a
//! sandboxed READ capability (CURSOR PARITY TIER-2; design:
//! `ops/evidence/stage_g/agent_loop/CURSOR_PARITY_REFRAME_DESIGN.md` §3 A⑤ + §6 A⑤).
//!
//! ## Thesis (git mapped onto the capability tiers)
//!
//! MNEMOS itself is not git, but the owner's PROJECTS are. v1 (owner-locked
//! 2026-06-16) = the READ tier ONLY: `status`/`diff`/`log`/`show`/`blame` (free,
...
//!   the A① move-analyzer profile needed). The NETWORK is ALWAYS kernel-DENIED (the
...
//! * REDACTION (SI-2): the git output passes the `redact()` wall; secret-shaped ⇒
//!   WITHHELD.
...
/// The v1 READ-only git subcommand allowlist (owner-locked 2026-06-16 = core 5).
...
/// presence probe; `None` ⇒ absent ⇒ honest-degrade). Mirrors the A① resolver.
#[must_use]
...
fn run_git_read(subcommand: &str, args: &str) -> Result<String, GitDeny> {
    ...
    // fail-closed: never run git unsandboxed (IV-S9).
...
pub fn render_git_read(subcommand: &str, args: &str) -> GitReadRender {
    match run_git_read(subcommand, args) {
        Ok(stdout) => {
            // SI-2: a secret-shaped git output is WITHHELD (never surfaces).
...
/// SI-2 redaction gate (the SAME canonical `redact()` wall the loop / web fetch /
...
// ---------------------------------------------------------------------------
// A⑤ v2 EGRESS — git push (owner-armed, origin-only). The FIRST credentialed
// network-WRITE in the agent: `git push origin <branch>` to the repo's configured
// `origin` (owner-locked scope), run under a BESPOKE sandbox profile that ALLOWS
...
// ref and the remote is the literal `origin`). custody/funds HARD-LOCKED (PD-6).
// ---------------------------------------------------------------------------

/// The owner-arm ceremony phrase for `daemon git-push` (a dedicated phrase for
/// ceremony separation; the grant TIER reused is `GrantTier::Egress` — no new tier,
/// owner-locked 2026-06-17).
...
/// redact-belted (SI-2). The capability witness is consumed (proves the owner armed
...
pub fn render_git_push(
    ...
    // is the LITERAL "origin" (owner-locked scope); the ONLY user-derived token is
