//! §4.1 command model (atom #404 F.0.3).
//!
...
/// §4.1 — risk class of a command; drives the approval requirement.
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// §4.1 — approval gate a command must pass before any side effect.
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ApprovalRequirement {
    ...
    Multisig = 4,
    /// Forbidden in Stage F entirely (e.g. train execution).
    ForbiddenInStageF = 5,
}

/// §4.1 — identity of a command: closed namespace + verb hash + grammar version.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// §4.1 — a fully classified command ready for dispatch.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// §4.1 — the trace record every executed command emits.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// The closed, total risk→approval mapping. Training is always forbidden in
/// Stage F; this is the single place that decision is made.
#[must_use]
...
impl CommandEnvelope {
    ...
    /// Whether this command is forbidden in Stage F (train execution).
    #[must_use]
