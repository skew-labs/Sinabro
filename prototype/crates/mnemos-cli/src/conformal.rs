//! `conformal` — the EXACT, float-free Clopper-Pearson false-accept-budget certification.
//! Certifies the false-accept budget [THM: conformal — Vovk; Angelopoulos-Bates
//! 2021; RCPS/LTT]. The quantitative refinement of the qualitative zero-false-accept gate:
...
//!
//! ## The one identity that makes it float-free + EXACT
...
//! ⇒ no — the "n≈10" boundary, reproduced exactly.)
//!
//! ## Design constraints (zero drift, no new crate)
...
//! float / clock / RNG. Cross-checked against `u128` for small values + the reference table, so the
...
//! everywhere. The residual (OOD, the tacit standard) stays the fenced Bucket-B residue.

use std::cmp::Ordering;

/// The phase-transition threshold `α*_safe ≈ 0.27`: a hard domain is globally stable below
...
/// The default certification at the reference budget (`FAR ≤ α*_safe = 27/100` @ 95%).
#[must_use]
...
mod tests {
    ...
    /// THE reference TABLE, reproduced EXACTLY (α*_safe = 27/100 @ 95%): 0 false-accepts in 10 held-out
