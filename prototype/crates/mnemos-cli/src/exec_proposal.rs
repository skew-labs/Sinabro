...
//! (ENDGAME E10, "AGENT ACTS"). Threat model:
...
//! The split of authority is the whole design (IV-A1/IV-A2): the MODEL only
...
//! denied); the OWNER alone authorizes execution (E10-2: a terminal typed-phrase
...
//! timeout + byte-cap, IV-A6), so there is no target, no staleness, no
//! atomic-replace. It REUSES the proven `file_edit` discipline: AEAD-sealed
//! (P1-1 [`MemoryCipher`], content-derived nonce ⇒ idempotent re-propose),
...
//!
//! The SandboxTier is NOT carried here (IV-S7 / D-A4): it is fixed by the
...
//! `MutateCapability`-gated sandbox run) is wired in E10-2 dispatch — there is no
...
//! secret-shaped proposed command at mint time (IV-A8 — an unreviewable command
...
/// Pending exec-proposal cap (IV-A2/W8 analog): a save beyond this is a typed
...
// ===========================================================================
// 2. PROPOSE — answer-block extraction + mint walls (IV-A2/A8)
// ===========================================================================
...
pub enum ExecProposeDeny {
    ...
    CommandTooLarge,
    /// The proposed command is secret-shaped (IV-A8 — refused outright; an
    /// unreviewable command must not exist).
    SecretShaped,
    /// The proposed command's intent is a chain WRITE / signing / fund-moving
    /// operation (IV-A12 — structurally refused at PROPOSE time, so a chain-write
    ...
    /// exists, `CustodyCapability` is uninhabited, and SI-5 allowlists no chain
    /// host); chain-WRITE/sign/funds are HARD-LOCKED ALWAYS (PD-6).
    ChainWriteIntent,
    /// The proposed command's intent is a git FORCE-PUSH (history rewrite to a remote)
    /// — an IRREVERSIBLE escalation (E13-4 / ⑳ IV-BS3, the escalation family sibling of
    ...
    /// adjacent escalation (E13-4 / ⑳ IV-BS3). Structurally refused at PROPOSE time, so a
    ...
    /// exfil; `CustodyCapability` is uninhabited; key material stays HARD-LOCKED, PD-6).
    KeyExportIntent,
...
/// (IV-A12). A command is chain-write-intent iff ANY of its tokens (split on
...
/// wallet key exists (env-scrubbed), `CustodyCapability` is uninhabited, and SI-5
...
/// (IV-A12) — `true` iff any token matches [`CHAIN_WRITE_INTENT_TOKENS`]. Used by
...
/// Private key-material tokens for the KEY-EXPORT escalation (E13-4 / ⑳ IV-BS3): an
...
/// IRREVERSIBLE escalation (E13-4 / ⑳ IV-BS3). STRUCTURAL co-presence (NOT a flat
...
/// escalation (E13-4 / ⑳ IV-BS3). `true` iff a `keygen` / `keytool` token is present
...
/// the caller's canonical redaction verdict over the command (IV-A8) — a bool so
...
pub fn mint_exec_proposal(
    ...
    // IV-A12 — a chain-WRITE / signing / fund-moving command is structurally
    ...
    // the absent wallet key, the uninhabited `CustodyCapability`, and SI-5;
    // chain-WRITE/sign/funds stay HARD-LOCKED ALWAYS (PD-6).
    if command_is_chain_write_intent(command) {
        return Err(ExecProposeDeny::ChainWriteIntent);
    }
    // E13-4 / ⑳ IV-BS3 — the escalation family (force-push / key-export) is refused at
    ...
    // IV-A8 — a secret-shaped command is refused outright (an unreviewable
...
// ===========================================================================
// 3. The sealed, content-addressed, bounded exec-proposal store (IV-A2/W6/W8)
// ===========================================================================
...
/// the P1-1 memory store and the `file_edit` proposal store; a DIFFERENT magic /
...
mod tests {
    ...
    // ---- IV-A12: chain-write intent structurally refused at propose time --
    ...
    // ---- E13-4 / ⑳ IV-BS3: force-push + key-export refused at propose time --------
