//! `agent_orchestrator` — the two-model orchestration loop (the orchestrator
//! spine).
//!
...
//! transports — the loop driver is byte-untouched. Two-model by
...
//! model output stays advisory until verified (the verify oracle gates "success").
//!
//! Drift-0: the orchestration CONTROL FLOW is deterministic given the
...
pub enum OrchestratorStop {
    ...
    /// runaway plan never fans out unbounded work.
    PlanTooLarge,
    ...
    /// fail-closed; never a guessed order or a deadlock.
    DepCycle,
}
...
pub struct RoutedImpl {
    ...
    /// The Typed-Write-Admission receipt: the
...
impl OrchestratedOutcome {
    ...
    /// Sub-tasks whose verification receipt ADMITS a permanent Write (only an
    /// oracle-`Verified` receipt admits; advisory / unverified
...
/// The result of the PLAN phase alone (Plan Mode's INERT half): the frontier plan text + the
...
/// PLAN + DECOMPOSE only (phases 1-2 of [`run_orchestrated_consult`]) — the INERT half of Plan
...
/// (for Plan Mode, owner-APPROVED) sub-task list: ROUTE each sub-task -> local IMPLEMENT loop ->
...
pub fn run_orchestrated_from_subtasks(
    ...
    for subtask in subtasks {
        ...
        // The Typed-Write-Admission verify step — the receipt comes from the
        // class-typed ORACLE evidence, NEVER the model's self-judgment.
...
/// Run the two-model orchestration loop: frontier PLAN -> deterministic
...
/// `model_id` placed on the wire. `run_agent_loop_with` is called
...
pub fn run_orchestrated_consult(
    ...
    // Composed from the two reusable phases (Plan Mode shares these): PLAN+DECOMPOSE then, on a
...
// ===========================================================================
// The PARALLEL worker fleet: the IMPLEMENT phase
...
// oracle (the model is NEVER an arbiter — drift-0).
// ===========================================================================

/// The hard cap on how many sub-tasks ONE decomposed plan may fan out into — a
/// runaway / adversarial frontier plan must not spawn unbounded work. A
...
/// chunks of ≤ this many threads, never M threads at once.
...
/// SERIALLY, in id-order, after every wave joins (drift-0; /FG3).
struct WorkerImpl {
    idx: usize,
    port: u16,
    model_id: String,
    ...
    OF: FnMut(&SubTask, &AgentLoopOutcome) -> VerificationEvidence,
{
    ...
    // a runaway plan never fans out unbounded work.
    ...
    // validate the deps DAG fail-closed ⇒ deterministic topological waves.
    ...
    // per-wave concurrency cap ∩ the host's parallelism (≥1).
    ...
    // zombie thread outlives the call (std-thread, no tokio, no crate).
    ...
    // waves joined (the oracle is pure ⇒ order-independent; the model
...
mod tests {
    ...
    /// THE TWO-MODEL ORCHESTRATION PROOF: a frontier plan that
    ...
    fn full_chain_routes_each_subtask_and_synthesizes() {
        ...
        // Both sub-tasks are Code class; no oracle evidence (Absent) ⇒
    ...
    /// PLAN-ONLY (the INERT half): the frontier PLAN + deterministic decompose run WITHOUT
    ...
    /// FROM-SUBTASKS (the active half): implement+synthesize over an owner-APPROVED sub-task
    ...
    /// The verify oracle gates Write admission — a passing CODE oracle
    ...
    /// MACRO PER-PORT: a routing table that maps each kind to its OWN worker PORT
    ...
    // ---- PARALLEL fan-out (bounded, oracle-gated, drift-0, fail-closed) ----
    ...
    /// DRIFT-0 PARITY: the PARALLEL fan-out produces the IDENTICAL
    ...
    /// Fail-closed: a cyclic `deps` graph stops typed (`DepCycle`) —
    ...
    /// Bounded fan-out + joins (FG4): 10 INDEPENDENT sub-tasks (>
    ...
    /// Fail-closed: a plan that decomposes into more than
