//! `skew_oracle` — the PURE, byte-locked TRADE ORACLE. Given a proposed Skew
...
//! signing, money 0**. Sinabro acts as an additional verification lane atop
//! Skew's on-chain solvency: on the collateralized path the trade's
...
//!   "free collateral at risk + funding," weaker than "initial escrow = max loss".
...
//!   portfolio comes from the chain-read layer). The owner bounds are PLAIN NUMBERS — the oracle never
//!   touches the grant machinery; the real `CustodyGrant` authorize + signing transport live in the
...
/// `CustodyGrant` authorize lives in the execution chokepoint). Every field is an UPPER limit.
...
/// `commands::grant::CustodyDenied` (the custody authorize) so the two layers speak the same language.
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
pub enum CollateralPolicyClass {
    ...
    Wcc,
    /// A policy id outside the recognized set (carries the raw u16).
    Other(u16),
}
...
/// `certified.bound`), so the oracle re-derives the EXACT on-chain escrow (amount-binding).
...
/// SUM the program pulls into the shared vault (amount-binding); fail-closed `None` on any
...
/// The model PROPOSES one of these; the oracle DECIDES. (No signing payload — that lives in the
/// execution chokepoint.)
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum SkewTrade {
...
mod tests {
    ...
    fn sigma_escrows_bounded_by_budget_theorem() {
        // The theorem on the collateralized path: a sequence of in-bounds trades keeps
