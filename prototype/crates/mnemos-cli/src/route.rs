//! Router execution-state FSM (canonical).
//!
//! Defined here at its *first consumer* — the cockpit status bar — and reused
//! unchanged by the provider/model router. This is the single definition of
//! [`RouteExecutionState`]; later consumers `use crate::route::RouteExecutionState`,
...
/// The router finite-state-machine execution state.
#[repr(u8)]
...
/// A single routing event recorded in a [`RouterDecisionTrace`].
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum RouteEvent {
    /// Default local-base execution — no external egress.
    LocalOnly,
...
pub struct RouteStep {
    /// The route state the decision was made in (visible).
    pub state: RouteExecutionState,
    /// The route recommended after folding trajectory health.
...
/// A rich, replayable consult-trace entry. Every provider consult
...
/// typed trigger, and the same-message approval-summary hash. The provider
...
pub struct ConsultTraceEntry {
    ...
    /// SHA-256 of the same-message approval summary that authorized the send.
...
/// A deterministic, replayable record of routing decisions. The same input
...
impl RouterDecisionTrace {
    ...
    /// Record a provider consult. Every consult appends one entry — the
    ...
    /// Record the split serving metrics for a generation — TTFT / TPOT /
...
/// The full split CU evidence bundle for one served route. Every
/// speed claim attaches this: no route claims "fast" without latency percentiles,
/// the TTFT/TPOT/prefill/decode/queue split, throughput, prefix-hit + KV-hit
...
impl PerfScorecard {
    ...
    /// Requires ordered, present latency percentiles (p50 ≤ p95 ≤ p99 and
    /// p99 > 0), a consistent serving-metric split (no aggregate-one-number
...
mod decision_trace_tests {
    ...
    /// The default route is local-first: FAST/NORMAL authorize 0 external
    /// tokens, so the local base model is the default executor.
    #[test]
    ...
    /// Trajectory health folds into the recommended route (guard binding):
    ...
    /// Unhealthy route states are recorded visibly and never render green.
    #[test]
    ...
    /// A fallback is recorded as a visible event; a fallback missing
    ...
    /// A consult escalation is a visible (non-silent) event carrying its
    /// typed trigger.
    #[test]
    ...
    /// Deterministic replay: the same input sequence twin-runs to identical
    ...
    /// Falsifiability — different inputs must produce different fingerprints
    ...
    // ---- rich consult trace (provider, hashes, cost, latency, advisory) ----
