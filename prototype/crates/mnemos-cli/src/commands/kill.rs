...
//! rendering. After a hard-kill + restart, the durable
//! job journal is reconciled so any orphaned (still-live) job is cleaned up to
//! `Killed` — broken recovery and zombie green are impossible. A repeated kill
//! is idempotent.
//!
//! Reuse: the job rail and its no-zombie transition are the
...
impl KillController {
    ...
    /// restart can recover). In this implementation it is the current rail state.
    #[must_use]
