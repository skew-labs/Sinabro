//! Global budget command (atom #468 · F.8.1 `/budget`).
//!
...
//! overrun blocks dispatch *before* any side effect (`G-F-SAFETY`).
//!
...
//! side effect can start (`G-F-CONTROL-EXPRESS`). A lower is one-way — a cap can
...
//!
//! Reuse (no reinvention): the token / cost / deadline / route-consult / approval
//! decision is the canonical [`crate::commands::budget::BudgetCap`] (Stage A
//! budget), reconstructed from the current lowerable caps on each check so a
//! tightened cap is enforced without re-minting the authorize logic; the gas cap
//! is the canonical [`mnemos_d_move::GasBudgetMist`] typed unit (Stage C gas
//! caps); the red/yellow/green verdict is [`crate::tui::RenderTruth`]. This
...
pub struct GlobalDispatch {
    /// The token / cost / deadline / route request (canonical Stage A type).
...
impl GlobalBudget {
    ...
    /// (`G-F-CONTROL-EXPRESS`: every side effect re-reads control state).
