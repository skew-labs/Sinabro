//! Audit trail command (atom #472 · F.8.5).
//!
...
/// (atom #472 madness spec). Every variant carries a trace id + evidence hash.
#[repr(u8)]
...
impl AuditAction {
    ...
    /// reload path; E5-1). An out-of-range byte is refused (fail-closed) so a
...
impl AuditEntry {
    ...
    /// record body; E5-1). Identical bytes to the content the seal hash covers,
    ...
    /// reload path; E5-1). Refuses an out-of-range action byte (fail-closed); the
...
impl AuditTrail {
    ...
    /// Filter by Stage F atom id (bounded O(n)).
    #[must_use]
