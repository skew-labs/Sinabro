...
//! detector routing — core eval command group.
//!
...
//!
//! Reuse: the gas harness is the canonical
...
//! [`AuditFinding`]. There is no on-disk measure-trace type — the local
//! [`EvalRunView`] is its grounded stand-in.
...
/// Map a gas regression verdict onto the cockpit render truth (reuse of
...
/// A B/C performance feature that must pass a paired A/B comparison before
...
pub enum PromotableFeature {
    /// Cache-safe token compression.
    TokenCompress = 1,
    /// Prompt prefix cache.
    PrefixCache = 2,
    /// Speculative draft/verify decoding.
    Speculative = 3,
    /// Quantized KV-cache mode.
    KvMode = 4,
    /// Route-FSM steering.
    RouteFsm = 5,
}
...
/// Route a *locally reproduced* candidate into a canonical
