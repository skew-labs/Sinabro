//! `sinabro gas status` — gas sponsor mode / policy / quota status.
//!
//! The `GasStatusView`: a read-only projection of the gas posture. It shows
...
//! or raw balance).
//!
//! A distinction made structural here: **gas/measure telemetry is
...
/// The read-only gas status view. Holds no secret: balances and policy are
...
pub struct GasStatusView {
    ...
    /// The daily burn cap, in MIST. The hot wallet stays below this.
