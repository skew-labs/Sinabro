//! `sinabro contrib` — contributor local dry-run.
//!
...
//! Accepted (every security gate passed). It is the community-integration
...
//!
//! Reuse: the checklist is the canonical
//! [`mnemos_e_skill::author_check::AuthorChecklist`]; the import decision
//! is the canonical [`mnemos_e_skill::community_import::CommunitySkillImport`];
//! the secret scan is the canonical
...
pub enum ContribReject {
    ...
    /// The community-import dry-run did not Accept the package (a
    ...
    /// A live contribution (upload / on-chain write) was attempted. Forbidden —
    /// no live action.
...
impl ContribDryRun {
    ...
    /// Attempt a live contribution (upload / on-chain). Always refuses — no live action.
