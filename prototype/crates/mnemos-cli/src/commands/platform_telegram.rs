...
//! approval-queue rules (F-WP-06C, atom #459 · F.7.0 platform telegram
//! bind/test/pause/sync).
//!
...
//! diverge by origin (master plan §210/§307). Platform pause rides the *same*
...
//! notification queue is full (G-F-CONTROL-EXPRESS). Approval requests, job-done,
...
//! never silently lost — G-F-NOTIFY), and every delivered approval is tracked so
//! none is orphaned (G-F-TASK-INBOX).
//!
//! Reuse (no reinvention): the bind / authorize / test primitive is the canonical
...
//!
//! Disparity note (atom #459 reuse): the atom plan lists `MessageEnvelope` under
//! *reuse*, but no `MessageEnvelope` existed anywhere on disk (verified absent
//! across the whole `prototype/` tree). This atom mints its grounded form here,
//! next to the [`CommandEnvelope`] it wraps (USER-LOCKED 2026-06-07, OD-2 Option
//! A). This module performs no live action.
...
/// envelope-equality invariant, master plan §307).
#[repr(u8)]
...
/// bridge (master plan §210/§307). It pairs a transport [`PlatformOrigin`] with
...
/// the normal and background queues (G-F-CONTROL-EXPRESS). Platform pause rides
...
pub struct TestMessageReport {
    ...
    pub would_send: bool,
    /// Whether live transport ran. Always `false` in Stage F.
    pub transport_live: bool,
}

/// A bind view over the canonical [`TelegramGateway`] (authorization-only; the
/// gateway holds no bot token and performs no transport). In Stage F the
...
impl TelegramBindingView {
    ...
    /// Whether live Telegram transport is open. Always `false` in Stage F — the
    /// Bot API transport is a later (post-Stage-F) wiring atom.
    #[must_use]
    ...
    /// in Stage F: an authorized user yields `would_send = true` but
...
/// The user-configurable notification classes (atom #459 광기 spec).
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq, Hash)]
...
/// stays within the p95 ≤ 50ms budget (atom #459 criterion).
...
impl NotificationCenter {
    ...
    /// Attempt to deliver `n`. `transport_ok` models the (Stage F: simulated)
...
mod tests {
    ...
    /// E0e-1 — SI-4 single dispatch truth (the behavioral half; the structural
    /// "exactly one executor" half is `ops/evidence/stage_g/e0e_si456_grep.sh`).
    ...
    fn render_bounded_no_commerce_and_p95_within_50ms() {
        ...
        // Local status p95 <= 50ms (atom #459 criterion).
