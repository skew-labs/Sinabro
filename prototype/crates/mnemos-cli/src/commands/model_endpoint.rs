//! local / vLLM endpoint config — no run (atom #429 F.3.2).
//!
//! `sinabro model endpoint add/test`. Configures and *validates* endpoint
//! reachability config only — Stage F never starts a vLLM training or serving
...
//! as a [`SecretRefView`] reference (`G-F-SECRET-ZERO`).
//!
...
pub struct EndpointView {
    ...
    pub is_local: bool,
    /// Whether a training/serving job may be spawned — invariant `false` in Stage F.
...
impl EndpointRegistry {
    /// Stage F never spawns a training / serving job from an endpoint.
...
mod tests {
    ...
    fn no_spawn_training() {
        ...
        // straight from EndpointRegistry::SPAWN_TRAINING_ALLOWED — Stage F never
