...
//! controls (F-WP-06B, atom #454 · F.6.3).
//!
//! Evidence packing / verification / replay is local and deterministic;
//! archiving is dry-run / background only in Stage F and can never imply training
//! consent.
//!
//! Reuse (no reinvention): a packed evidence record is the Stage E
...
//! bundle is the Stage B [`mnemos_b_memory::EvidenceBundleManifestV1`]
...
pub enum EvidenceReject {
    ...
    ReplayMismatch,
    /// A live archive upload is denied in Stage F (dry-run only).
    #[error("live archive denied")]
    LiveArchiveDenied,
}

/// A `sinabro evidence pack` projection over a Stage E [`EvidenceLakeReceipt`].
...
pub struct EvidencePackView {
    ...
    /// Training eligibility carried by the receipt (display only; Stage F never
    /// executes training).
...
/// A `sinabro evidence archive --dry-run` plan: archiving never uploads in
/// Stage F and never implies training consent. The default is the safe dry-run
/// plan.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Default)]
pub struct EvidenceArchivePlan {
    /// Whether a live upload would occur — always `false` in Stage F.
...
impl EvidenceArchivePlan {
    ...
    /// Attempt a live archive — always refused in Stage F.
    pub const fn try_live_archive(&self) -> Result<(), EvidenceReject> {
        Err(EvidenceReject::LiveArchiveDenied)
    }
}

/// A `sinabro evidence bundle` projection over a Stage B
...
mod tests {
    ...
    /// Build a Stage E evidence receipt. `archive_locator` is left zero so no
    ...
    fn training_eligibility_displayed_not_executed() {
        // The flag is shown for transparency; Stage F never runs training.
