//! Capability tiers as TYPES.
//!
//! Authority the model can NEVER widen, enforced by the type system:
//! - **READ** — always granted, no approval, no witness. Reads are not
...
//! - **CUSTODY (blanket)** — unbounded funds / wallet / chain / mainnet.
...
//!
//! The invariant: a capability is mintable ONLY by (a) the type system
...
/// READ authority — always granted. No approval, no grant, no witness;
...
impl ReadCapability {
    /// READ is always granted — no approval, no grant, no witness.
    #[must_use]
...
/// Autonomous DOWNLOAD authority for one bounded GET-into-/tmp action ( ⑲).
...
/// Owner-path (ENDGAME): re-derive a MUTATE-LOCAL capability from an
...
/// Owner-path (ENDGAME / ⑲): derive a DOWNLOAD capability from an owner-armed
...
/// Owner-path (CURSOR PARITY v2 EGRESS): derive an EGRESS capability from an
...
/// CUSTODY (blanket) authority — UNBOUNDED funds / wallet / chain / mainnet. HARD-LOCK AS A TYPE:
...
mod tests {
    ...
    fn read_is_always_granted() {
        // READ takes no witness: the type system hands it out freely.
