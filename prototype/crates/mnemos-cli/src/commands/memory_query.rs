...
//!
//! These are read-only projections over canonical `b-memory` truth. The
...
//! memory root hash, the replay cursor, the StorageBackend receipt, the
...
pub enum SuiAnchorState {
    ...
    AnchoredTestnet = 2,
    /// The root carries a mainnet-gate anchor (read-only provenance).
...
impl MemoryListView {
    ...
    /// Whether raw private content is shown — always `false` by default.
    #[must_use]
...
impl MemoryRootView {
    ...
    /// [`StorageObjectRef`], the [`StageBTranscriptHash32`], the
