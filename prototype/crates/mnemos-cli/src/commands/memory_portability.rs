...
//! portability command group.
//!
//! Read-only projections + thin wrappers over the canonical replay and
//! portability surfaces (both owned by `b-memory`). Three invariants are
//! structural:
//!
//! * **Root hash + transcript preserved.** Export/import/replay carry the bundle's
//!   verified root blob and transcript verbatim ([`import_bundle`] rejects
...
//!   evidence hash only — never a live upload
...
pub struct MemoryExportView {
    ...
    /// Redacted 16-hex prefix of the replay transcript hash.
...
pub struct MemoryReplayView {
    ...
    /// Redacted 16-hex prefix of the carried transcript hash.
    pub transcript_redacted: String,
    /// Whether both criteria held (transcript stable + zero resurrections).
...
/// never a live upload. The backend is always a `FutureOnly` label.
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct MemoryDryRunPlan {
    ...
    /// Whether a live upload occurs — always `false`.
