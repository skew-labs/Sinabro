//! Incident pause / resume (atom #471 · F.8.4).
//!
...
//! (`G-F-CONTROL-EXPRESS`), and every side effect must re-check the pause state
//! ([`IncidentController::preflight`]) before it runs. Resume is gated: it
//! requires the same typed approval an admin action does.
//!
//! Reuse (no reinvention): the risk taxonomy and the approval requirement are the
//! canonical Stage-C/D safety policies [`crate::command::CommandRisk`] /
...
impl IncidentController {
    ...
    /// (`G-F-CONTROL-EXPRESS`: every side effect re-reads control state). Identical
