//! `sinabro release` — launchable package dry-run (atom #462 · F.7.3 release
//! launchable package smoke).
//!
...
//! smoke result — but Stage F never actually publishes a public release
//! ([`ReleaseDryRun::try_publish`] always refuses with
//! [`ReleaseReject::LiveReleaseForbiddenInStageF`]).
//!
//! Reuse (no reinvention): the baked-secret scan is the canonical Stage E
...
//!
//! # Secret custody proof (physics warning #462 resolution — G-F-SECRET-ZERO)
//!
...
//! `Copy` + `Debug`-scalars-only per its #381 custody proof). This module stores
...
pub enum ReleaseReject {
    ...
    InstallSmokeFailed,
    /// A live publish was attempted. Stage F never publishes a public release.
...
impl ReleaseDryRun {
    ...
    /// Always `true`: Stage F release is dry-run only.
    #[must_use]
    pub const fn is_dry_run(&self) -> bool {
        true
    }

    /// Always `false`: Stage F never publishes a public release.
    #[must_use]
    pub const fn published(&self) -> bool {
        false
    }

    /// Attempt to publish for real. Always refuses in Stage F — there is no code
