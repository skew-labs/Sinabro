...
//! (F-WP-06A, atom #453 · F.6.2 multisig/timelock commands).
//!
...
//!   so Stage F never executes live. Every decision is a denial — the surface is
//!   fail-closed.
//!
//! Reuse (no reinvention): the proposal envelope is the canonical
...
//! type — it projects the Stage C ceremony state.
...
/// a denial in Stage F — the surface is fail-closed and never returns an "allow".
#[repr(u8)]
...
pub enum TimelockExecuteDecision {
    ...
    /// forbidden in Stage F (the state never reaches `Executed`).
...
impl TimelockExecuteDecision {
    ...
    /// Always `true`: every Stage F decision is a denial (fail-closed; no live
...
pub struct MultisigTimelockView {
    ...
    /// The mainnet execution posture (never `Executed` in Stage F).
...
impl MultisigTimelockView {
    ...
    /// fixed order — signatures, then timelock maturity — and Stage F always
    /// denies live execution at the end. There is no override parameter: a local
    ...
    /// Whether the CLI may execute the queued action live. Always `false` in
    /// Stage F — the timelock/multisig ceremony is displayed, never exercised.
    #[must_use]
...
mod tests {
    ...
    fn propose_is_roster_bound() {
        let r = roster();
        let p = proposal(&r);
        // The proposal binds the roster's signer-set hash (atom #211 hasher).
    ...
    fn matured_and_signed_still_forbidden_in_stage_f() {
        // Signatures met + matured -> Stage F still forbids live execution.
        let v = view(2, MainnetExecutionState::TimelockQueued);
