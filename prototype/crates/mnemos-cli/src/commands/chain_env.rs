//! `sinabro chain env` — chain environment status (F-WP-06A, atom #451 ·
//! F.6.0 chain env/testnet/mainnet).
//!
...
//!   never exercised in Stage F (no live action).
...
//!   [`MainnetExecutionState::Executed`] ceremony — which Stage F never reaches.
//!   A testnet env carrying a mainnet-ladder posture is an *env mismatch* and
//!   also renders `Red`.
//!
//! Reuse (no reinvention): the env + execution posture are the Stage C
...
//! type — it is a projection over A/C/D/g-wallet truth.
...
pub struct ChainEnvView {
    ...
    /// is executable, and Stage F never reaches it).
...
impl ChainEnvView {
    ...
    ///   `MainnetPrepared` env whose mainnet write path is still gated (the Stage
    ///   F default, since `Executed` is never reached);
    /// * `Yellow` — a `MainnetPrepared` env reporting `Executed` (a live mainnet,
    ///   never produced in Stage F);
    /// * `Green`  — a consistent `TestnetVerified` env (the Stage F operating env).
    #[must_use]
...
mod tests {
    ...
    #[test]
    fn mainnet_gate_red() {
        // A prepared mainnet whose write path is still gated (the Stage F
        // default) renders Red — the mainnet write is blocked.
        let v = view(
            StageCChainEnv::MainnetPrepared,
            MainnetExecutionState::Locked,
        );
