...
//! EXACTLY what a consult will use (a deterministic projection — no second
//! truth, no fork).
//!
...
//! (which value wins? — exactly the no-silent-fallback ambiguity),
...
//! model can never self-select its own runtime/model (the auto-router is
...
/// The frontier model id when `OPENROUTER_MODEL` is unset/blank — the
...
/// the only TEXT chatbot served is `qwen2.5-omni` (32k ctx, OpenAI
...
impl RuntimeRoute {
    ...
    /// to activate" — the route is phrase-driven).
    #[must_use]
