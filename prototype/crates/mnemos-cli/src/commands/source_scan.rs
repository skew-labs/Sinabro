//! Real source-tree audit scan (ENDGAME E5-1).
//!
...
/// E11-2: the static Rust pattern-rule labels paired with their `rule_id_hash_32`
...
/// `pub(crate)` so the A④-rg find-in-files walk ([`crate::search`]) reuses the SAME
