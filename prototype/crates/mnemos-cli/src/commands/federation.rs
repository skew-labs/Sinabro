//! `sinabro federation` — federation opt-in / DP-budget controls.
//!
...
//! are visible. Crucially, opting in does *not* start anything: a
//! federated round stays locked ([`FederationControlView::round_locked_in_stage_f`]
//! is always `true`) — this surface only exposes
//! the control + status, never the training itself.
//!
//! There is no federation type defined in any crate yet, so this module is a
//! local, locked control-surface view only. It introduces no new crate
...
pub enum FederationMode {
    ...
    /// leaves.
    Off = 1,
    /// The user has explicitly opted in. (The round itself is still
...
impl FederationControlView {
    ...
    /// Always `true`: a federated round is locked. Opting in records
    /// intent + budget but cannot start training here.
    #[must_use]
...
mod tests {
    ...
    fn opt_in_is_explicit_and_still_locked() {
        ...
        // Opting in never bypasses the federation lock, and never
