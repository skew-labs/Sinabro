...
//!
//! A read-only control surface over the AtomDiet corpus: it previews
...
//! eligible — all without mutating any locked dataset shard. This build may
//! prepare/validate but never silently rewrites locked truth.
//!
//! Reuse: the source record is the canonical
...
pub enum DatasetIngestReject {
    ...
    RedactionResidue,
    /// A locked dataset shard cannot be mutated.
...
pub struct DatasetIngestView {
    ...
    /// Whether the source record is training eligible (display only — training
...
impl DatasetIngestView {
    ...
    /// Whether a locked dataset shard may be written — always `false`.
    #[must_use]
    pub const fn locked_shard_write_allowed() -> bool {
        false
    }

    /// Attempt to write a locked shard — always refused.
