...
//!   is the invariant `true`: the execution posture defaults to
...
//!
//! Reuse: the package-lock evidence (bytecode / prover / gas
//! baseline digests) is the [`MainnetPackageLock`]; the multisig
...
/// "execute now" variant — both operations are dry-run.
#[repr(u8)]
...
pub struct PackageGateView {
    ...
    /// The mainnet execution posture (dry-run by default).
...
impl PackageGateView {
    ...
    /// Whether the gate is in dry-run (no execution). Always `true`:
    /// only [`MainnetExecutionState::Executed`] is executable, and it is never
    /// reached.
    #[must_use]
    ...
    /// still does not execute — a real publish would require the
...
mod tests {
    ...
    fn publish_and_upgrade_default_dry_run() {
        ...
        // Even a fully-green gate is still dry-run (never executes).
