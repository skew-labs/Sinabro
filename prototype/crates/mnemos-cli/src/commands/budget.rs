//! Token / cost / deadline budget cap enforcement.
//!
...
//! [`consult_token_cap`]: `Fast`/`Normal`/`Lockdown`
...
//! mirror the `DailyTokenBudget` in `crates/m-agent/src/loop_budget.rs`,
...
/// The MNEMOS per-call input-token ceiling: no single dispatch may send
