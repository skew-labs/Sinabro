//! `sinabro setup memory` wizard + StorageBackend receipt projection.
//!
...
//!   backend is admissible as a label but has no live writer
...
/// canonical [`StorageBackendKind`]: only Walrus has a live writer;
...
pub enum MemoryStorageMode {
    ...
    LocalOnly = 1,
    /// Walrus testnet primary backend (the only live writer).
    WalrusTestnet = 2,
...
/// gas authority are NEVER the same key.
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// Privacy / learning posture chosen in the wizard. The default is fully
/// private: learning off, egress none. The wizard cannot enable training.
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum PrivacyLearningMode {
    /// Private: learning off, egress none (the default).
    PrivateLearningOff = 1,
    /// Local-learning-only opt-in: still no external egress, never training
...
impl PrivacyLearningMode {
    ...
    /// Whether this is the private, learning-off default.
    #[must_use]
...
/// present, and whether a live writer exists.
...
pub struct MemoryBackendReceiptView {
    ...
    /// Whether a live writer exists for this backend (only Walrus
...
impl MemoryBackendReceiptView {
    ...
    /// `true` when the backend is a mirror/archive with no live writer.
    #[must_use]
