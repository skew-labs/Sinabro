//! Compression + context-window + KV-cache mode status (atom #432 F.3.5).
//!
//! `sinabro model compress status`. Three status-only surfaces, none of which
//! starts a serving job in Stage F:
//!
//! * **context compression** — reports its loss and risk; it can shrink the
//!   token footprint but can never hide that it dropped information
//!   (`G-F-TOKEN-SAVING`). A specialized compressor per source kind (compiler /
...
//!   from memory that excludes deleted ids (Stage B delete/replay truth, never
//!   bypassed — `G-F-MEMORY`).
...
//!   risk, and the Stage H canary requirement are visible, but no serving job is
//!   started here (`G-F-PROVIDER`).
...
//!   or private memory (`G-F-PROMPT-CACHE-BOUNDARY`, `G-F-ADAPTIVE-ROUTER`).
...
/// deleted. Reuses Stage B delete/replay truth: a deleted memory can never
...
/// A KV-cache / quantization serving mode. Status-only in Stage F.
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl KvCacheMode {
    ...
    /// Whether this mode requires a Stage H quantized-serving canary before it
...
/// expected VRAM / latency / quality risk, the Stage H canary requirement, and
/// the prefill/decode split candidacy. `serving_started` is the invariant
/// `false` — Stage F starts no serving job.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct KvCacheModeStatus {
    ...
    /// Whether a Stage H canary is required before enabling.
    ...
    /// Invariant `false`: no serving job is started in Stage F.
