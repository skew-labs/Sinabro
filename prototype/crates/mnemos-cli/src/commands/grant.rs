//! Unforgeable owner-armed capability grants (ENDGAME E0c).
//!
//! This module names two ARMABLE capability tiers: **EGRESS** (provider consult /
...
//!
//! # The invariant (unforgeable token)
//!
...
//!   forge is a COMPILE error ("the bad state does not compile");
...
//! here — there is no custody [`GrantTier`] variant ( HARD-LOCK).

use crate::repl::approval::ApprovalPrompt;

const ZERO32: [u8; 32] = [0u8; 32];

/// Which armed tier a grant authorizes. CUSTODY is deliberately ABSENT:
...
pub enum GrantTier {
    ...
    /// explicit, distinct ceremony (per MUTATE-LOCAL).
    MutateLocal = 2,
    /// Bounded network download — an owner-armed GET that fetches UNTRUSTED bytes
    /// into a temp file (ENDGAME / ⑲). STRICTER than egress: it egresses AND
    ...
    /// (a tier is armed only by its own phrase). CUSTODY is still ABSENT.
    MutateDownload = 3,
    ...
    /// within the egress bound) — ENDGAME / ⑳. It composes the EXISTING grant
    /// types (no new capability type). It does NOT arm download and CUSTODY
    /// is still ABSENT. A `BoldSession` ceremony arms ONLY a [`BoldSessionGrant`]
    ...
    /// kill). ONCHAIN PIVOT C-0: REPLACES the blanket funds-lock with a BOUNDED
...
/// The arming typed-phrase for the (stricter) MUTATE-DOWNLOAD tier ( ⑲). A
/// distinct gesture so a download is NEVER armed by the egress / mutate phrase
/// . The model holds no `ApprovalPrompt` and types no phrase, so it cannot arm.
pub const DOWNLOAD_ARM_PHRASE: &str = "arm-download-bounded-revocable";
/// The arming typed-phrase for the (composite) BOLD-SESSION tier ( ⑳). A
/// DISTINCT gesture: one owner phrase explicitly arms BOTH egress AND mutate-local for
/// a bounded, revocable session (bold-within-bounds). It does NOT arm download
/// or custody (uninhabited). The model holds no `ApprovalPrompt` and types no
...
/// / TTL bound for on-chain transactions. NEVER armed by any other phrase and NEVER
...
/// The session bounds an armed grant is boxed by (session / time / rate).
...
/// Owner-path (ENDGAME): arm a MUTATE-LOCAL grant from a typed-phrase
/// ceremony completed THIS turn. This is the SINGLE home for the mutate ceremony +
/// arm (the e0c allowlist), used by BOTH the synchronous single-shot local
...
/// into a temp file; / ⑲). STRICTER than egress + type-distinct: it cannot
...
/// Owner-path (ENDGAME / ⑲): arm a DOWNLOAD grant from a typed-phrase ceremony
/// completed THIS turn. The SINGLE home for the download ceremony + arm (the e0c
...
/// An owner-armed COMPOSITE bold session grant (ENDGAME / ⑳). One
...
/// `GrantTier::Custody`) and DOWNLOAD is ABSENT — a bold session can never
/// arm either.
///
/// Unforgeable: PRIVATE fields, no struct literal; [`arm_bold_session`] (consuming a
/// `BoldSession` ceremony) is the ONLY ctor — a forge is a COMPILE error:
/// ```compile_fail
...
/// session is bounded + revocable as one unit. Is preserved: a `BoldSession`
...
/// Owner-path (ENDGAME / ⑳): arm a composite bold session from a typed-phrase
/// ceremony completed THIS turn. The SINGLE home for the bold ceremony + arm (the e0c
/// allowlist home, `grant.rs`), used by the owner-armed `daemon bold` verb. `None`
...
/// forge is a COMPILE error. It gates the NEW
...
/// ceremony completed THIS turn. The SINGLE home for the custody ceremony + arm (the e0c
...
mod tests {
    ...
    fn a_bold_ceremony_is_tier_distinct_from_every_single_tier() {
        // a BoldSession ceremony cannot arm any plain single-tier grant.
