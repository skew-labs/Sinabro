//! `zerog_finetune` — 0G Compute fine-tune PREPARE for a sinabro expert.
//!
...
//! internal executor and is minted as an ERC-7857 iNFT (`zerog_inft`). It PREPARES the
//! fine-tune; it never performs the (paid) training run.
//!
//! ## what this targets (pinned to the package, not the docs)
...
/// Build the OWNER-runbook lines for the fine-tune (PURE — only strings). The agent
