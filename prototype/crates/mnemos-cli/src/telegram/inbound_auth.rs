//! `telegram::inbound_auth` — inbound approval AUTH + the narrow, unforgeable
//! grant mint. The security-critical core of Telegram full-duplex.
//!
...
//!
//! 1. **Sender pin.** The reply authorizes nothing unless its `chat.id`
...
//! 2. **Action binding.** The reply must NAME a pending action by its
...
//! 3. **Replay ledger.** The approval is recorded ONCE in the (now
...
//! 4. **Narrow mint.** On approval, a single-shot, action-bound,
...
//!    Broad arming stays the LOCAL typed-phrase ceremony (`EGRESS_ARM_PHRASE`),
//!    which the network cannot reach — this is deliberately narrow.
//!
//! CUSTODY is unreachable: the mint is egress-tier only; there is no
//! `GrantTier::Custody`, and this module references no mutate/custody constructor.
//! The model cannot self-approve: it cannot satisfy the sender pin (it holds
//! no `TELEGRAM_CHAT_ID`), and `agent_loop.rs` references no symbol here.
//!
//! This is an owner-path grant caller. The no-self-mint PROPERTY is preserved
//! (a grant is minted only via the owner-arm ceremony — here, gated by the
//! owner-pinned `chat.id`), not relaxed.
...
/// broad egress; this is deliberately narrow. A short verb is sufficient HERE: the
/// unforgeable gate is the sender pin + the action-hash binding + the grant
...
/// non-`Approved` variant mints NOTHING). Each maps to a specific threat. No
...
pub enum InboundAuthOutcome {
    /// The sender's `chat.id` is not the owner's pin — dropped, nothing
    /// recorded, nothing minted.
    SenderNotOwner,
    /// The reply verb was not `approve` / `deny` — ignored (not an approval reply).
    UnrecognizedReply,
    /// The reply names no known pending action — refused, nothing minted.
    NoSuchPendingAction,
    /// The approval had no binding hash or was already recorded (replay) —
    ...
    /// The owner APPROVED an EGRESS action — a single-shot, action-bound,
    /// fast-expiring egress grant is minted. It authorizes ONLY
    /// this action.
    Approved {
    ...
    /// The owner APPROVED a MUTATE-LOCAL action (a
    ...
    /// `EgressGrant` could NEVER authorize this). It authorizes ONLY this
    /// action.
    ApprovedMutate {
    ...
    /// Fail-closed: the ceremony/arm refused (should not occur for a valid
    /// approve + non-zero action hash) — nothing minted.
    MintFailed,
}

/// The capability TIER a remote approval of `action` mints:
/// a tool side effect (an agent-proposed exec / file-apply) is a MUTATE-LOCAL
/// action; every other class is EGRESS. CUSTODY is never representable (there is
/// no `GrantTier::Custody`), so a phone reply can never mint funds/chain
...
/// REMOTE-CONTROL chat path (ENDGAME / ⑱). PURE classification, fail-closed in
/// this exact order: owner-pin → secret-withhold → approval-reply
...
pub enum InboundDisposition<'a> {
    /// the sender's `chat.id` is not the owner pin — dropped before ANY work
    /// (no secret scan, no turn, no reply, nothing recorded).
    NotOwner,
    /// the inbound text is secret-shaped — WITHHELD before the agent ever
    /// sees it (never fed to a turn, never echoed back). Uses the SAME scanners the
    /// [`redact`](crate::provider::redaction::redact) choke uses.
    WithheldSecret,
    ...
    /// agent turn whose redacted answer is replied (RC5).
    ChatPrompt(&'a str),
}

/// Classify an UNTRUSTED inbound update for the Telegram remote-control CHAT path —
/// PURE and fail-closed, in this exact order ( → → split). Takes the
...
pub fn classify_inbound(update: &InboundUpdate, owner_chat_id: i64) -> InboundDisposition<'_> {
    // GATE 1 — owner pin FIRST: a non-owner sender is dropped before any
    // parse / scan / turn / reply (cheap rejection of a flood; reuse).
    if update.sender_chat_id() != owner_chat_id {
        return InboundDisposition::NotOwner;
    }
    let text = update.text();
    // GATE 2 — secret withhold: a secret-shaped inbound is WITHHELD before
    // the agent sees it — the SAME scanners the `redact` choke uses
...
/// egress grant — the four fail-closed gates (.T4), in order. PURE: takes the
...
/// what makes replay-refusal real across polls. It is mutated ONLY on a
...
pub fn authenticate_and_mint(
    ...
    // GATE 1 — sender pin: a non-owner sender authorizes nothing, records
    ...
    // GATE 2 — action binding: the reply must name a KNOWN pending action by
    ...
    // GATE 3 — replay ledger: record the approval ONCE, bound by
    ...
    // GATE 4 — narrow mint: the SAME unforgeable ceremony/grant
    ...
    // NEVER authorize a mutate and vice-versa. It can authorize ONLY this
    ...
    match tier_for(target.action) {
        ...
        // / a DOWNLOAD is owner-armed DISPATCH only, NEVER an inbound
        ...
        // structurally impossible while the closed-enum match stays exhaustive.
        GrantTier::MutateDownload => InboundAuthOutcome::MintFailed,
        // / ⑳ a BOLD SESSION is owner-armed DISPATCH only (`daemon bold`),
        ...
        // exhaustive. A bold session is NOT a single-tier grant anyway (it has no
        ...
        // stays exhaustive. Custody is local-owner-ceremony only.
...
mod tests {
    ...
    // --- /RC2 — classify_inbound: the pure chat-path router ( ⑱). -----

    /// A secret-shaped owner text — the canonical `suiprivkey` fixture the choke
    /// scanners catch (same family as the redaction-gate tests).
    const SECRET_TEXT: &str = "key = \"suiprivkey1qexamplenotreal\"";

    // A non-owner message is dropped before ANY work (no turn, no reply),
    ...
    // A secret-shaped owner text is WITHHELD before the agent sees it
    ...
    // A spoofed sender mints NOTHING and records NOTHING, even with a
    ...
    // An approval for an unknown action mints nothing; approving X never
    // touches Y.
    #[test]
    ...
    // A replayed approval is refused and fires the mint exactly once.
    #[test]
    ...
    // An inbound mint is single-action + egress-only + fast-expiring; it
    ...
    // / (ENDGAME) — a tool-side-effect (MUTATE) approval mints a
