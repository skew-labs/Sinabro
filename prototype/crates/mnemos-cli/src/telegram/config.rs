//! Operational Telegram config view.
//!
...
//! is never loaded, cloned, `Debug`-printed, or networked (secret-zero). A
...
/// (mirrors the `platform_telegram` convention; the full key is never shown).
#[must_use]
