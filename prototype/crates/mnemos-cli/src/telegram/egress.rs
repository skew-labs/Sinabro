...
impl TelegramHost {
    ...
    /// egress `sendMessage` target and the inbound sender-pin (a reply is
...
/// never passed redaction is UNREPRESENTABLE. A struct literal outside
...
impl RedactedTelegramSend {
    ...
    /// from a [`RedactionReceipt`] — the redaction choke. The body is represented ONLY
...
// ---- The REAL Bot-API sendMessage codec -------
//
// `send_over_tls` above is the proof-of-egress skeleton (it posts the
...
mod tests {
    ...
    /// Allowlist-excludes-chain (telegram leg). The telegram allowlist
