//! `telegram::inbound` — the bounded Bot-API INBOUND transport (getUpdates
//! long-poll RECEIVE + defensive PARSE).
//!
...
//! anything. Authorization (sender-pin + action-binding + the unforgeable
//! mint) and the wire to the pending action live in separate modules. The
//! module's threat model enumerates the transport invariants noted below.
//!
...
//! types, whose security invariants (bounded fields, monotone offset)
//! are testable in the default build.
//!
//! Defensive parse: the parser extracts ONLY three fields — `update_id`
...
//!
//! Monotone offset: [`UpdateOffset`] advances to `max(update_id)+1` over a
...
//! anti-replay, complementing the `approval_sync` ledger).
//!
...
//! `api.telegram.org`; ONE env-name source). The bot token is a
...
/// input). A hostile oversized text cannot grow this module's memory or
/// reach a prompt: it is truncated at a UTF-8 boundary at construction.
pub const TELEGRAM_INBOUND_MAX_TEXT_BYTES: usize = 256;

/// Max updates consumed per long-poll cycle — a bounded batch (no unbounded
/// buffer). The getUpdates request also sets `limit` to this, so the server returns
...
/// whose text exceeds the cap is UNREPRESENTABLE. The text is still
...
impl InboundUpdate {
    ...
    /// oversized text is truncated, never retained whole). The ONLY
    ...
    /// The Telegram `update_id` — used ONLY to advance the monotone offset;
    /// it is not authority.
    #[must_use]
    ...
    /// The sender's `chat.id`. The AUTH gate compares this against the
    /// `TELEGRAM_CHAT_ID` pin; a non-owner sender is dropped before any mint.
    /// It is a plain integer, never a secret value.
    #[must_use]
...
/// consumed batch; a replayed or lower `update_id` can NEVER rewind it, so
...
impl UpdateOffset {
    ...
    /// equal `update_id` is a no-op (the cursor never rewinds).
...
/// malformed update can never jam the poll).
#[repr(u8)]
...
/// extracted; every other field is ignored. Fail-closed on a missing field.
...
/// a malformed or non-owner update can never jam the poll.
#[cfg(feature = "telegram-inbound")]
#[must_use]
...
/// host is re-checked against the single Telegram allowlist and the bot-token
...
impl InboundTransport {
    ...
    /// updates and the ADVANCED offset (monotone — never rewinds). Reached
    /// ONLY behind the feature gate; the host is re-checked against the Telegram
    /// allowlist and the bot-token reference must be present. The token rides
...
// NOT need serde_json — bounded text + monotone offset. ---------------
#[cfg(test)]
mod tests {
    use super::*;
