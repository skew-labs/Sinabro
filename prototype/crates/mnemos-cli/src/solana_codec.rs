//! `solana_codec` — MNEMOS × SKEW K-2 (ONCHAIN PIVOT C-2): the PURE, byte-exact Solana
//! transaction codec. NO `solana-sdk` (seam-lock D-K2-1: "no heavy Solana crates"): the legacy
...
//! byte-correctness oracle — a wrong byte ⇒ the program rejects, IV-K2-9).
//!
...
//! - the byte-exact Skew `PROTOCOL_PERP` instruction builders (recon-cited 2026-06-30):
//!   `open_risk_account` (0x0060) → `deposit_margin` (0x0061) → `submit_perp_order` (0x0071) — the
//!   perp trade lifecycle "perp부터 1번까지" (seam-lock D-K2-2). Each is a u16-LE discriminator ‖
//!   borsh-LE descriptor + the handler's exact account-meta order + the cited PDA seeds.
//!
//! ## Sources (read 2026-06-30; cited, never guessed — [[no-formula-guessing]])
...
/// devnet is REFUSED against any other cluster (IV-K2-7 / IV-K2-11).
...
/// `amount` MUST equal the custody-authorized amount (IV-K2-3, asserted by the caller).
#[must_use]
...
/// LE). Maps 1:1 onto the K-1 oracle's `escrow_wcc_affine_corner` inputs (the oracle re-derives the
...
/// family as the K-3 account-disc derivation (`skew_read::SkewAccountKind`, `account:<Name>`).
#[must_use]
...
// ============================================================================
// WAVE B part-2b — the fixed-forward OTC lifecycle (lock · mark · settle · form).
...
// Sources read 2026-06-30 (lib.rs:424/441/495/594; the handler `#[derive(Accounts)]`; pda.rs SEED_LABEL_*).
// Byte-locked + golden-tested; the live devnet simulate is the ultimate byte-correctness oracle (IV-K2-9).
// ============================================================================

/// Append a borsh `Vec<u8>` — `u32 LE length` ‖ raw bytes. The OTC lifecycle descriptors carry FF-05
...
/// `Vec<u8>` carry FF-05 borsh policy/snapshot structs (caller-supplied; in FixedLock mode
...
pub struct LockCollateralDescriptor {
    ...
    /// The borsh-encoded FF-05 collateral params struct (caller-supplied).
    pub collateral_params_bytes: Vec<u8>,
    /// The borsh-encoded FF-05 collateral snapshot (`[]` for FixedLock — a zero-byte unit struct).
    ...
    /// The borsh-encoded FF-05 VmPolicy struct (caller-supplied).
...
/// Vec<u8> is the FF-05 VmPolicy struct (caller-supplied). Fields in declaration order.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// multi-sig / 2-agent / quote-authority owner go-live (FF-08b). escrow=0 (creates PDAs only). Data =
...
// ============================================================================
// WAVE D — the SECONDARY market (list / quote / accept / atomic-transfer / cancel an existing OTC
...
/// WCL + price (the K-1 oracle bounds it). Data = `[0x68,0x00]` ‖ descriptor(88) = 90 B. 14 accounts
...
// ============================================================================
// WAVE C — the batch-auction books (open / close / settle / claim_fill / settle_batch_contract). All
...
// ============================================================================
// WAVE E — the KEEPER band (permissionless deterministic-liveness ops). All 2-byte u16-LE discs
...
// ============================================================================
// WAVE C-listings + WAVE E-piecewise — the VARIABLE-leg permissionless listing /
// piecewise instructions. All 2-byte u16-LE discs (lib.rs:645/663/684/702
// `#[instruction(discriminator=[lo,hi])]` — the disc authority, read 2026-06-30).
...
// resolved from source, NOT guessed — the two recon agents had conflicted on the
...
/// ⚠ The two recon agents had conflicted (u32×3 vs i128×3); SOURCE says all `i128`.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
// ---------------------------------------------------------------------------
// WAVE G-unwired — the 5 catalog-only-on-chain ix Sinabro now ASSEMBLES byte-exact.
...
/// the factory ADDS proof+clamp+bond; NO token CPI (the bond escrow is §9-deferred) ⇒ escrow=0.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// dispute_resolved bool(1) ‖ current_unix_timestamp i64`. Closes the FF-12a lifecycle (`init`s
...
mod tests {
    ...
    // ---- WAVE C-listings — the VARIABLE-leg permissionless listing codecs. ----
    ...
    // ---- WAVE E-piecewise — the bilateral piecewise contract lifecycle codecs. ----
    ...
    // ---- WAVE G-unwired — the 5 newly-assembled ix golden byte layouts ------------------
