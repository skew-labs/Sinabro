//! `recognition_elicit` — the recognition-ELICITATION engine (the customer-anchor
//! front-end for recognition-elicitation UX: label/compare/triad + active-query). The
//! DETERMINISTIC, ZERO-LLM-TOKEN, zero-drift surface that elicits a customer's TACIT standard
//! as ~10 typed RECOGNITION anchors, choosing each question by an ACTIVE-QUERY criterion — so a
//! downstream synthesis step (deferred) can induce a deterministic checker from a handful of
//! recognitions instead of a hand-written rule. **The customer never writes a rule**:
...
//!   "coverage achieved / when have we asked enough" signal (the "O(10) anchors").
...
//! SET (+ the customer's named axes) that is INPUT CAPITAL for synthesis (deferred).
//! Discovering features / synthesizing the checker / certifying the
//! false-accept budget are the fenced follow-ons; the residual tacit quality stays the fenced
//! Bucket-B residue, never sold as a deterministic guarantee. custody/funds HARD-LOCKED.
...
/// One RECOGNITION the customer supplies — never a rule, only a judgment.
/// The three modalities are label / compare / triad.
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// overflow-proof in i128 for any realistic dimension. `pub(crate)` so the synthesis
...
/// The elicited ANCHOR SET — the owned, content-addressed capital the engine produces:
