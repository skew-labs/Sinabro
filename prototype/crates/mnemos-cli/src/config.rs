//! Config precedence + feature / learning schema.
//!
...
/// Config layer, lowest to highest precedence.
#[repr(u8)]
...
/// Sponsor mode view; mirrors the C `GasSponsorMode` discriminants.
#[repr(u8)]
...
/// Learning mode (default [`LearningMode::Off`]).
#[repr(u8)]
...
/// Data egress mode (default [`DataEgressMode::None`]).
#[repr(u8)]
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
...
/// The non-disableable safety-kernel feature names.
...
///
/// `Serialize` is additive (the deserialize path / `deny_unknown_fields`
...
pub struct RawCliConfig {
    ...
    /// Optional user web3 RPC endpoint (READ-intent). A deploy-time
    ...
    /// owner-armed V2 step. No chain host is allowlisted by this field
    /// and no chain WRITE is ever representable here.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub web3_rpc_endpoint: Option<String>,
    /// The owner-configured MULTI-CHAIN READ registry. Each entry is
    ...
    /// Walrus self-host (BYO) PUBLISHER endpoint — a deploy-time seam.
    ...
    /// key — our app holds no key, never signs, never pays (custody intact). https-only +
    /// SSRF-walled at use.
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// The owner's remote box for the owner-armed
    ...
    /// READ diagnostic runs there; custody stays HARD-LOCKED.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub remote_ssh_host: Option<String>,
    /// Owner-added DOWNLOAD allowlist hosts. These EXTEND the
    ...
    /// Owner-configured local stdio MCP
...
/// ONE owner-configured local stdio MCP
...
/// / funds field — custody stays HARD-LOCKED.
...
fn validate_config(cfg: &RawCliConfig) -> CliResult<()> {
    ...
    // Fail-closed: each MCP server must name a non-empty command and
...
/// The file name of the persisted local config under `$HOME/.mnemos`
...
/// Render the canonical `config.toml`
...
/// allowlist + token validity + safety-kernel disable refusal), then
...
/// The returned text is guaranteed validated, secret-free, and
/// `parse_layer`-round-trippable. The raw config values never enter an
...
/// // The persisted text round-trips through the READ path.
/// assert!(parse_layer(&text).is_ok());
/// // A secret-shaped value is refused at serialize time — never written.
...
pub fn serialize_config(cfg: &RawCliConfig) -> CliResult<String> {
    ...
    // The SAME `looks_like_secret` gate `parse_layer` runs on READ, now on
...
/// The deploy-time user-RPC READ seam posture. A deployer plugs in
...
/// * NO chain client is built and NO chain host is allowlisted;
...
/// Web3 RPC reader — resolve the effective owner-configured RPC endpoint
...
/// Resolve the owner-configured MULTI-CHAIN READ registry across config layers
...
/// Presence-only posture of the Walrus self-host
...
impl WalrusSelfHostSeam {
    ...
    /// memory-only bearer secret, no Sui key / no funds, and that WRITES stay
...
/// Resolve the Walrus self-host presence seam across config layers (precedence
...
/// Resolve the effective self-host PUBLISHER endpoint VALUE across config layers
...
/// Resolve the effective self-host AGGREGATOR endpoint VALUE (the READ/GET side)
...
/// Resolve the effective owner-configured remote SSH host across config layers
...
/// Resolve the owner-extended DOWNLOAD allowlist hosts across
...
/// Resolve the owner-configured READ-tier
...
mod tests {
    ...
    // The deploy-time user-RPC READ seam.
    ...
    fn download_allowlist_parses_resolves_union_and_round_trips() {
        // The owner-extension allowlist is a known field that parses,
        ...
        // the additive field round-trips through the serialize/READ path.
    ...
    // ---- config persist (serialize_config crux) -------------------

    #[test]
    fn serialize_config_round_trips_through_parse_layer() {
        // a serialized config re-parses cleanly through the READ path.
        let cfg = RawCliConfig {
    ...
    fn serialize_config_refuses_a_secret_shaped_value() {
        // the SAME inline-secret screen `parse_layer` uses on READ — a
    ...
    fn serialize_config_refuses_disabling_a_safety_kernel_feature() {
        // validate-before-write — a config that disables a kernel feature
    ...
    fn serialize_config_refuses_an_unknown_profile() {
        // an out-of-allowlist profile is refused — the persist path can
