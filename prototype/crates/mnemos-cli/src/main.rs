//! `sinabro` / `mnemos` binary entry (atom #402 F.0.1).
//!
//! Dispatch (atom #586 · 5-mode): `--version` / `--help` / `doctor` are handled
...
//! routes to [`sinabro::dispatch::run`] (G-WP-07 #561-#568), which
//! classifies the command through [`sinabro::command::CommandEnvelope`] and
//! renders the handler status / approval-locked surface (no side effect runs in
//! Phase 0). The binary writes to
...
/// Gather the doctor-visible secret-scan surfaces (E5-2): the user-controlled
...
fn run_doctor(out: &mut impl Write, invoked: &str) -> io::Result<()> {
    // E5-2 (PD-1): the security verdicts are MEASURED, never hardcoded `true`.
...
fn main() -> ExitCode {
    ...
    let result: io::Result<ExitCode> = match args.first().map(String::as_str) {
        ...
        // atom #586 — 5-mode dispatch. The default (no args) and `repl` enter the
        ...
        // approval-gated and disabled in Phase 0 (no side effect runs). In a non-TTY
