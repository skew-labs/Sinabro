//! `oracle_inft` — capitalize a CERTIFIED oracle as an ERC-7857 iNFT (the Oracle
//! Bootstrap's ownership step). Earlier work mints a
//! *pattern* ([`crate::zerog_chain::W2D_PATTERN_HASH`]) or a fine-tuned *expert* (a LoRA
//! adapter rootHash) as an iNFT; this module mints a third thing: **a certified ORACLE itself** — "an
//! oracle is a verifiable, owned, tradeable asset". It COMPOSES the LOCKED
...
//!
//! ## the certification is TYPED — [`OracleCert`]
...
//! * **`Conformal { k, n }`** — the recognition oracle: a STATISTICAL bound, certified
//!   iff [`crate::conformal::certify_far_default`] (FAR ≤ α*_safe @ `1−δ`, `k` held-out
//!   false-accepts in `n` negatives). The "O(10) recognition-anchors per oracle-mint" unit.
//! * **`DeterministicSound`** — the reconcile / metamorphic oracles: SOUND BY
...
//!
//! ## ★ THE MINT GATE — certified-only, fail-CLOSED
...
//!
//! ## the commitment (a deterministic LOCAL content-commitment)
...
//! Storage upload run OUTSIDE the binary. `CustodyCapability` stays uninhabited; names no
//! custody symbol.
//!
...
/// The recognition oracle kind (the conformal-certified, anchor-derived oracle).
pub const RECOGNITION_ORACLE_KIND: &str = "recognition";
/// The finance reconciliation oracle kind (deterministic-forever, sound by construction).
pub const RECONCILE_ORACLE_KIND: &str = "reconcile";
/// The summarization metamorphic oracle kind (deterministic-forever sound rejector).
...
pub enum OracleCert {
    /// A conformal α-budget cert (recognition): FAR ≤ α*_safe @ `1−δ`, `k` held-out
    ...
    /// A DETERMINISTIC-FOREVER oracle (reconcile / metamorphic): SOUND BY CONSTRUCTION
...
/// (certified-only). The `Some` commitment binds the `kind` + the oracle
/// `identity` bytes + the typed `cert` — so re-deriving it from the same inputs is byte-identical
/// (drift-0). The model never reaches the judge; only the deterministic cert does.
#[must_use]
...
/// a *certified* oracle's provenance, NOT per-user correctness (the honest LOCK).
#[must_use]
...
mod tests {
    ...
    fn an_uncertified_oracle_is_unmintable() {
        // conformal: 0/9 does NOT certify (the n≈10 boundary: 0.73^9=0.0589>0.05).
