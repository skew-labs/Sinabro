//! `recognition_synth` — O-3b: SYNTHESIZE a deterministic checker from the O-3 recognition
//! anchors, then FALSIFY + CERTIFY it (master plan §6.2 Pillar 2 steps 3-5; the back-end to
//! [`crate::recognition_elicit`]'s front-end). This closes the Oracle-Bootstrap loop in a
//! TACIT domain: ~10 recognitions → an executable predicate → a 3-way verdict → (O-3b-C) the
//! ladder. **The customer never writes a rule** — the rule is INDUCED from their labels.
//!
//! ## The synthesis: an axis-aligned bounding-box conjunctive classifier (§6.2 step 3 — exact)
...
//! This is precisely the §6.2 "entails the positives, EXCLUDES the negatives; generalizes from
...
//! Bucket-B residue (§6.7) made concrete, never faked.
//!
//! ## The runtime verdict: 3-way ACCEPT / REJECT / ESCALATE (§6.2 step 5; fail-closed)
...
//!   a human — exactly the §6.2-★ "fail-closed-reject / provisional-accept = no-reward-hacking".
//!
//! ## The certification: the EXACT conformal false-accept-budget gate (§6.5-4; O-3c)
...
//! certifies `FAR ≤ α*_safe` at 95% confidence from `(k, n)` — so the §6.5 "n≈10" labor is
...
//! ([`crate::conformal`], O-3c) certifies the held-out false-accept rate `FAR ≤ α*_safe` at 95% —
...
//! quality stays the fenced Bucket-B residue (§6.7).
...
/// SYNTHESIZE the deterministic checker from a recognition pool + its recognitions (§6.2 step 3).
#[must_use]
...
/// The held-out certification report (§6.5-4): the leave-one-out tallies + the `certified` gate.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl CertifyReport {
    /// Whether the checker is CERTIFIED for accumulation (the §6.5-4 gate). Only a certified
...
/// CERTIFY the checker by LEAVE-ONE-OUT over the labeled anchors (§6.5-4; the held-out
...
/// has both a positive (a box) and a negative (something to exclude). The quantitative conformal
/// α-VALUE is the deferred O-3c.
#[must_use]
pub fn certify_leave_one_out(pool: &ElicitPool, recognitions: &[Recognition]) -> CertifyReport {
    ...
    // O-3c: the QUANTITATIVE conformal gate REPLACES the qualitative zero-false-accept stand-in —
    ...
    // §6.5 "n≈10" labor is now enforced, float-free).
...
mod tests {
    ...
    /// THE CERTIFY GATE (O-3c conformal): a cleanly separable set with ENOUGH negatives certifies
    /// (0 held-out false-accepts over n=10 negatives ⇒ the exact Clopper-Pearson bound `FAR ≤
    /// α*_safe` @95% holds — the §6.5 "n≈10" labor). Two negatives would NOT suffice (see below).
    #[test]
    ...
    /// O-3c TIGHTENING: the SAME separable shape with only TWO negatives is NOT conformally
    /// certified — the held-out evidence is too thin to bound `FAR ≤ α*_safe` (the §6.5 "n≈10").
    #[test]
