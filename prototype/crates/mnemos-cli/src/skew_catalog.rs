//! `skew_catalog` — MNEMOS × SKEW K-0: the SINGLE SOURCE OF TRUTH for the Skew Solana
//! derivatives-factory capability surface that Sinabro must be AWARE of (owner 2026-06-30:
//! "시나브로 자체가 모든 기능을 인지하고 있어야 해 — skew에서 지원하는").
//!
//! ## PURE knowledge — money 0, no key, no network, no cross-repo build dependency
//! This module is **static data**: Sinabro's *knowledge* of Skew, hand-encoded from the
//! 2026-06-30 four-repo recon (program `skew_otc` = [`SKEW_PROGRAM_ID_DEVNET`], DEVNET). It dials
//! no RPC, holds no key, moves no funds. Later K-slices wire this catalog into the autonomous loop
//! / consult awareness and a `skew capabilities` readout — they all READ this registry, so there is
//! one truth and no prose drift (the E11-6 lesson). See `ops/design/MNEMOS_SKEW_INTEGRATION_PLAN.md`
//! §5.
//!
//! ## Honest grounding
...
/// Capability grouping (mirrors the integration plan §5).
...
pub fn render_catalog() -> String {
    ...
    // WAVE H — the agent KNOWS the now-EXECUTABLE surface (the K-2X full-surface wiring). PROSE-only;
