//! A② oracle test-loop — `sui move test` / `cargo test` as a sandboxed READ
//! capability (CURSOR PARITY TIER-2; design:
//! `ops/evidence/stage_g/agent_loop/CURSOR_PARITY_REFRAME_DESIGN.md` §3 A②).
//!
//! ## Thesis (verify-oracle ethos, AXIS-2 / P-HALL)
//!
...
//! PROPOSE-EXECs a fix (E10) and re-runs the oracle. The agent reasons with the
...
//! * REDACTION (SI-2): the test output passes the `redact()` wall; secret-shaped ⇒
//!   WITHHELD.
...
/// presence probe; `None` ⇒ absent ⇒ honest-degrade). Mirrors the A① / A⑤ resolver.
#[must_use]
...
/// SI-2 redaction gate (the SAME canonical `redact()` wall the loop / web fetch / lsp
