//! Find-in-files — a regex `search` across the workspace source as a
//! sandbox-free READ capability.
//!
...
//! * REDACTION: every matching line passes the SAME per-line `redact` wall
...
/// runs NO subprocess, and touches NO network. Custody/funds untouched.
#[must_use]
    ...
    'walk: while let Some((dir, depth)) = stack.pop() {
        ...
        for dirent in read_dir.flatten() {
            ...
            for (idx, line) in text.lines().enumerate() {
                ...
                // a secret-shaped matching line renders as a withheld marker
...
/// per-line redaction gate (the SAME canonical `redact` wall the file-read /
/// test-run / git tools use): `true` ⇒ the line is secret-shaped ⇒ WITHHELD. Exposed
/// `pub(crate)` so the codebase index redacts indexed chunks through the SAME
...
mod tests {
    ...
    fn a_secret_shaped_matching_line_is_withheld_in_the_walk_output() {
        ...
        // withheld marker, never its raw bytes (enforced inside the walk).
        let (policy, root) = crate_src_policy();
