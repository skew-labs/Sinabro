//! OS-enforced skill sandbox — kernel-confined bounded execution.
//!
...
//! No extra crate is required (std-only plus the OS `sandbox-exec` binary).
//!
//! ## What this module owns
//!
//! * **The tier → profile map** ([`seatbelt_profile_for`]) — a PURE TOTAL
//!   function over the closed [`SandboxTier`] enum (a skill never supplies
...
//!   DENIED, NEVER executed unsandboxed.
//!
//! ## What it REUSES (no second spawn discipline — no drift)
//!
//! The bounded child itself is [`crate::exec_local::run_argv_command_with`]: the
//! proven env-scrub (secret-shaped keys never cross), wall-clock timeout + kill + reap
//! (no zombie), per-stream byte caps, pinned cwd, and the
//! argv-only no-shell contract. This layer only PREPENDS the
//! `sandbox-exec -p <profile> --` wrapper as leading argv elements, so the same
//! walls bound a kernel-confined child. Custody is untouched: no
...
/// The canonical macOS Seatbelt wrapper binary. An absolute path (no
...
pub enum SandboxRunDeny {
    /// fail-CLOSED: no kernel sandbox primitive is available on this host
...
/// is `false` BY CONSTRUCTION ⇒ [`run_in_sandbox`] fail-closes.
#[must_use]
...
///
/// Gate order (the threat model's): fail-closed admission → pre-spawn
/// argv walls (reused) → pure tier→profile → the
/// `sandbox-exec`-wrapped child through the SAME env-scrub / timeout / byte-cap /
/// cwd-pin discipline. The returned [`ExecOutcome`]'s `exit_code`
/// is the inner child's code on success; the RENDER (dispatch) redacts the
/// captured streams before display.
pub fn run_in_sandbox(
    ...
    // no kernel sandbox ⇒ DENY (never an unsandboxed fallback).
    if !seatbelt_available() {
        return Err(SandboxRunDeny::SandboxUnavailable);
    }
    // Pre-spawn walls on the SKILL line (reuse the exec_local bounds).
    ...
    // the profile is derived from the tier alone (a closed enum), never
...
mod tests {
    ...
    /// file-write is kernel-gated by tier. `touch` (a real fs write,
    ...
    /// (the load-bearing egress proof): under a non-Networked tier the
