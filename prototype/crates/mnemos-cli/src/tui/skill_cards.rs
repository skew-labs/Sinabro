//! Cockpit skill card list projecting the [`SkillCardView`].
//!
...
//! never loaded here — they load only on inspect, so a 100-card
//! list renders fast.
//!
//! The hard law: popularity can never hide a security
...
//! type — imported, not re-minted (no-reinvent). The
...
/// A lightweight skill listing card. All heavy fields are hashes; the
...
/// A bounded list of skill cards with security-first ranking.
