...
//!
//! Reuses the [`StageFEvidenceRef`] / [`StageFTraceLink`] evidence seam
//! (earlier evidence refs project into it) and the [`RenderTruth`] enum. Pure
...
/// What the inspector is focused on.
#[repr(u8)]
...
/// The evidential tier of a hint. Prompt-only and stale tiers are never
...
/// An evidence-backed hint. Every field the inspector promises is present
/// so a verdict is never bare.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct EvidenceHint {
    /// Evidential tier.
    pub tier: HintTier,
    /// Source atom that produced the evidence.
...
/// The inspector view for a selected target. Always carries an evidence
