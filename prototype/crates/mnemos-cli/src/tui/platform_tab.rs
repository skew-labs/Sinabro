//! `sinabro tui` shared platform status tab.
//!
//! A read-only cockpit projection over the *canonical* notification / platform
//! sync truth that the [`crate::commands::platform_telegram`]
...
//! full trace, or touches the network (refresh p95 ≤ 100ms cached). No false
//! green: an unbound channel is `Red`, a paused
