//! Provider health dashboard tab (atom #434 F.3.7).
//!
...
//!
//! The render law (`G-F-UI-TRUTH`): a degraded or offline provider is never
...
//! projection only — it makes **no provider call** (`G-F-ADAPTIVE-ROUTER`
//! speed law), so the dashboard refreshes within budget regardless of provider
//! liveness.
//!
//! Reuse (no reinvention): [`ProviderView`] / [`ProviderKind`] / [`ModelRole`]
//! from [`crate::commands::provider`] (the F provider commands), the
...
impl ProviderHealthRow {
    /// Build a health row from a reused [`ProviderView`] (the F provider command
