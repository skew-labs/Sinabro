//! TUI cockpit screen model + the shared render-truth enum.
//!
...
//!
//! Submodules: [`status_bar`], [`job_rail`], [`tabs`], [`trace_pane`],
//! [`skill_cards`], [`skill_use_modal`], [`approval_modal`], [`inspector`].
...
/// The three-valued (plus unknown) render truth every cockpit surface
...
/// The cockpit shell state machine. Holds no business state: only the lifecycle
/// phase and the last known terminal size,
...
impl CockpitShell {
    ...
    /// the panic-free teardown this surface requires; calling it twice is safe.
