//! §4.3 cockpit skill use / install status modal (atom #423 F.2.6) projecting
//! the §4.4 [`SkillUseView`].
//!
...
//!
//! `G-F-NO-COMMERCE`: there is no checkout / buy / pay / refund surface here.
...
/// §4.4 — the use/install status projection for one skill.
...
/// §4.3 — the lifecycle status of a skill use/install.
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// §4.3 — the skill use/install modal. Pure projection; performs no install and
/// opens no checkout.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
