//! §4.3 cockpit approval modal (atom #424 F.2.7) — the *TUI approval state*.
//!
...
pub enum DenyReason {
    ...
    /// The command is forbidden in Stage F (e.g. train execution).
    ForbiddenInStageF = 2,
...
/// §4.3 — the approval modal: the displayed approval state plus the typed
/// decision gate.
...
impl ApprovalModal {
    ...
    /// forbidden-in-Stage-F ⇒ deny; missing mandatory field ⇒ deny; otherwise
