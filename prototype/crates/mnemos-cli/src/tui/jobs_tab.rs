//! §4.3 cockpit jobs / task / session dashboard (atom #467 · F.8.0 train/eval/
//! measure/gas job dashboard).
//!
...
//! (Stage F) surfaces train jobs as dashboard/doctor only — never execution.
//!
...
//! touches the network (atom #467 criterion: refresh p95 ≤ 250ms). No false
//! green (G-F-UI-TRUTH): a failed job is `Red`, a job awaiting approval or a
//! stale subsystem is `Yellow`, never a false `Green`.

use crate::sha256_32;
use crate::tui::RenderTruth;
use crate::tui::job_rail::{JobKind, JobRailItem, JobState};

/// A task-inbox action a user can take on a tracked job (atom #467 광기 spec).
#[repr(u8)]
...
/// §4.3 — the unified task / session / job dashboard projection (atom #467).
...
pub struct JobsDashboard {
    ...
    /// Train jobs present — Stage F surfaces these as dashboard/doctor only
    /// (never execution); the count is shown so the lock stays visible.
...
mod tests {
    ...
    fn train_jobs_surface_locked_count_dashboard_only() {
        ...
        // Stage F: train jobs are dashboard/doctor only; the count is visible.
