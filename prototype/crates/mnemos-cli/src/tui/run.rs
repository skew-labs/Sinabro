...
const fn decode_key(b: u8) -> CockpitEvent {
    match b {
        ...
        // #647 — the live designed-impossibility summary (UPPERCASE 'T' = Ten).
...
/// `tab_bar` / `footer_line` would leak escapes into the Plain frame.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
enum RowPaint {
...
/// #582 gas-drain dashboard: the seven drain-gate invariants, all holding.
...
/// #647 — the live designed-impossibility summary. These are
...
/// The terminal harness enters the alt-screen + hides the cursor and
...
mod tests {
    ...
    fn plain_render_is_colorless_no_escape_sequences() {
        ...
        // (rich-but-not-cringe): the no-color path stays colorless +
    ...
    fn ten_live_lines_section10_designed_impossibilities_structural() {
        // #647 — the section-10 designed-impossibilities are structural: a loop is bounded
    ...
    fn ten_live_view_renders_via_uppercase_t_key() {
        // #647 — UPPERCASE 'T' switches the center to the live section-10 surface (the
    ...
    fn steady_state_redraw_does_not_reallocate_the_frame_buffer() {
        // Zero-alloc proxy: after warmup, a redraw with no preceding
    ...
    fn keystroke_and_render_p95_within_budget() {
        // Keystroke (decode + handle) and render p95 within the budget.
