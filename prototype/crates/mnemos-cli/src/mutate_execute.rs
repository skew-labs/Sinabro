//! The single gated EXECUTE chokepoint for agent-proposed side
//! effects.
//!
...
//! `TOOL: exec` stays denied. This module owns the SINGLE function that
//! turns an owner-AUTHORIZED proposal into a real run:
//! [`execute_authorized_mutate`]. Its signature REQUIRES a [`MutateCapability`]
//! witness — and a `MutateCapability` exists ONLY from a valid owner-armed
...
//!   byte-cap, cwd-pinned. NEVER the un-sandboxed `run_local_command`.
//! * edit → [`apply_proposal`]: lane-A re-confine + `read_sha` staleness lock +
//!   atomic temp+fsync+rename + re-read verify.
//!
//! Custody is unreachable: this module references no
...
//! one). Redaction of the captured exec stream is the DISPATCH render
//! layer's job — the same `render_exec_stream` choke the proven
...
pub enum AuthorizedMutate<'a> {
    ...
    /// Used by the OWNER-LITERAL paths (`registry exec`, `tool exec-apply`, bold) —
    /// the owner reviewed the exact command, so a local WRITE is permitted.
    Exec(&'a ExecProposal),
    /// Run the proposed command at the NARROWER `ReadOnly` tier (network AND write
    /// kernel-DENIED; pure compute). Used by the KIND-AUTO paths (`registry exec-auto`,
    /// bold absorption) where the owner did NOT type the exact command — the derived
    /// `<interpreter> <path>` command runs with the smallest blast radius (stdout only).
    ExecReadOnly(&'a ExecProposal),
...
/// result (the DISPATCH layer redacts captured streams before render).
#[derive(Debug)]
...
/// chokepoint. The [`MutateCapability`] witness is taken BY VALUE: its
...
/// live `(now, used)`), not by move semantics — the capability is `Copy`
/// (shared shape with `EgressCapability`).
///
/// Custody is unreachable: no wallet/chain/funds path. The exec executor
/// is the kernel sandbox; the edit executor is the atomic verified
/// apply.
///
/// Compile-time: the chokepoint CANNOT be called without a real
...
pub fn execute_authorized_mutate(
    ...
    // The capability is presented (and dropped here). Its existence is the
    // gate; the GRANT it came from bounds how many actions may fire.
    ...
    match action {
        ...
        // The kind-auto path — the owner did not type the exact command, so
        // it runs at ReadOnly (network AND write kernel-DENIED). Same chokepoint, narrower
        // tier; a verified-but-hostile artifact cannot egress OR write a local file.
        AuthorizedMutate::ExecReadOnly(proposal) => MutateExecOutcome::Exec(
            run_in_sandbox_default(SandboxTier::ReadOnly, &proposal.command),
        ),
...
/// Owner-path: authorize a SYNCHRONOUS single-shot local
...
/// and the `from_grant` derive lives in
/// `commands::authority::local_mutate_capability`.
#[must_use]
...
mod tests {
    ...
    /// SandboxUnavailable on a non-macOS host — NEVER unsandboxed).
    #[test]
    ...
    /// re-confine + verify) and writes the new bytes.
    #[test]
