//! Evidence replay dry-run command.
//!
...
//! denied ([`ReplayReject::LiveSideEffectDenied`]). This
//! module performs no live action and carries no secret.
//!
//! Reuse (no reinvention): the determinism check is the
...
impl EvidenceReplayDryRun {
    ...
    pub fn replay(
        ...
        // Reuse the determinism check: the re-run hash must equal the
