...
//! lock (agent-core VS-1; a tool-not-IDE shape).
//!
//! THE FIRST ARBITRARY-PATH FILESYSTEM WRITE IN THE CORE. The split of
//! authority is the whole design: the MODEL only PROPOSES (a typed,
...
//! proposed content at mint time ( an unreviewable diff must not
...
/// Pending-proposal cap: a save beyond this is a typed deny — the
/// pending set stays a bounded, reviewable, actionable list.
pub const MAX_PENDING_PROPOSALS: usize = 32;

/// Bounded diff render lines (explicit truncation marker beyond).
...
pub struct FileEditProposal {
    ...
    /// `sha256` of the target's bytes AS VERIFIED-READ by the loop.
...
// ===========================================================================
// 2. PROPOSE — answer-block extraction + mint walls (W7a/W8)
// ===========================================================================

/// A verified file read the loop performed (the executor's OWN record —
/// 's truth source; never model-claimed). Recorded only when the read
...
pub enum ProposeDeny {
    ...
    Malformed,
    /// The claimed target was never verified-read this run.
    TargetNotRead,
    ...
    TargetExists,
    /// A CREATE proposal's parent did not confine to an allowed root (E-NEW).
    TargetNotConfined,
    /// The proposed content exceeds [`MAX_FILE_BYTES`].
    ContentTooLarge,
    /// The target name matches the secret-container denylist (belt).
    DeniedName,
    /// The proposed content is secret-shaped ( refused outright).
    SecretShaped,
    /// The pending store already holds [`MAX_PENDING_PROPOSALS`].
    StoreFull,
...
/// verified-read records. `content_is_secret_shaped` is the caller's
/// canonical redaction verdict over the proposed content — passed
...
pub fn mint_proposal(
    ...
    // The binding read: match the claimed target against the
    ...
    // Belt (upstream): a denylisted name was never readable, but the
    ...
    // Bounded content (refuse, never truncate).
    if proposed.content.len() as u64 > MAX_FILE_BYTES {
        return Err(ProposeDeny::ContentTooLarge);
    }
    // Secret-shaped content is refused outright (an unreviewable
...
/// [`mint_proposal`]. The edit path binds to a prior verified read; a file that
...
/// ([`FileReadPolicy::confine_new`]), the content is bounded + non-secret.
...
pub fn mint_new_file_proposal(
    ...
    // Confine the parent to an allowed root + denylist the resolved name (at create).
    let resolved = policy
    ...
    // Secret-shaped content refused outright.
    if content_is_secret_shaped {
        return Err(ProposeDeny::SecretShaped);
    }
    Ok(FileEditProposal {
...
// ===========================================================================
// 3. The sealed, content-addressed, bounded proposal store
// ===========================================================================
...
/// Typed apply denials (.W5 walls; data-free except hashes the caller
...
pub enum ApplyDeny {
    ...
    AmbiguousId,
    /// The target failed the lane-A read wall stack at apply time;
...
/// the memory store; a DIFFERENT magic/extension/subdir so a proposal
...
impl ProposalStore {
    ...
    pub fn save(&self, proposal: &FileEditProposal) -> Result<String, ProposeDeny> {
        ...
        if !path.exists() {
            // The pending cap counts DISTINCT pending artifacts.
...
// ===========================================================================
// 4. APPLY — staleness + confinement + atomic replace
// ===========================================================================
...
/// lane-A read walls on the CURRENT target (apply-time cwd policy) →
/// staleness hash equality → atomic mode-preserving replace
/// → re-read verify. Returns the receipt; every failure is typed
...
pub fn apply_proposal(
    ...
    // Re-confinement + the staleness read in one gated pass.
    let current = policy
    ...
    // Atomic replace: sibling temp (same dir = same fs), original
...
/// Apply a NEW-FILE proposal (E-NEW): re-confine the parent (at apply time) →
...
fn apply_new_file(
    ...
    let target = policy
        ...
        .map_err(ApplyDeny::TargetWall)?;
    // belt (the mint already capped; re-check at the write boundary).
...
pub enum OwnerSaveDeny {
    /// The target failed the lane-A read wall stack — only an owner-granted root is
    ...
    BaseShaMalformed,
    /// The target's CURRENT hash differs from the editor's read baseline (staleness).
    Stale,
    /// The atomic write failed (temp/perms/fsync/rename io class).
    WriteFailed,
...
/// target (only an owner-granted root is writable, exactly as it is readable) → staleness
/// equality (refuse a file changed since the editor read it) → atomic mode-preserving
/// replace → re-read verify — but the content comes DIRECTLY from the owner, NOT a model
...
/// write; no chain/wallet/custody on this path; untouched).
pub fn owner_save_file(
    ...
    // Re-confinement + the staleness read in one gated pass (the lane-A read walls).
    let current = policy.read(path).map_err(OwnerSaveDeny::TargetWall)?;
    // The drift-0 staleness law: refuse a target changed since the editor read it.
    if hex32(&current.sha256_32) != base_sha_hex {
        return Err(OwnerSaveDeny::Stale);
    }
    // Atomic replace: sibling temp (same dir = same fs), original mode preserved
...
/// Same temp+fsync+rename discipline as the store.
...
mod tests {
    ...
    /// Mint binds the EXECUTOR's read record (as-typed match), and a
    ...
    fn mint_walls_hold() {
        ...
        // Never-read target ⇒ TargetNotRead.
        ...
        // Secret-shaped verdict ⇒ refused outright.
        assert_eq!(
    ...
    /// Save→load round-trip; ciphertext at rest (no plaintext
    ...
    /// The pending cap denies the 33rd DISTINCT proposal, while an
    ...
    /// The staleness lock: ANY current-byte drift after the bound
    ...
    /// Apply-time confinement: outside-root and missing targets are
    ...
    /// The staleness lock: a file changed since the editor read it refuses the
