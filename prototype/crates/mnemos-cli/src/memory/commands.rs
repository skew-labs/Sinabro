...
//! delegate to the canonical portability views over the local / owned
//! memory roots. A deleted tombstone can never be resurrected by import,
//! compaction, or replay, and the archive location is never the truth.
//!
//! Secret custody: the surface holds no secret / wallet
...
//! / replay semantics are the canonical `TombstonePolicy` /
