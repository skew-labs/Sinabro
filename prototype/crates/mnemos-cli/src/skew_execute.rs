//! `skew_execute` — MNEMOS × SKEW K-2 (ONCHAIN PIVOT C-2): the SIGNING + chain-WRITE chokepoint.
//!
...
//!
//! 1. **oracle gate** (IV-K2-1) — the plan's verdict is `AffordableInBounds`, escrow == the request.
//! 2. **custody gate** (IV-K2-2) — the `ChainTxCapability` witness is required BY VALUE (mintable
//!    ONLY from a within-bounds owner-armed `CustodyGrant`); UNREACHABLE without it.
//! 3. **amount-binding** (IV-K2-3) — the on-chain value the tx commits == the authorized amount ==
//!    the oracle escrow == the request amount (three-way bind; the signed tx can never move more).
//! 4. **assemble + simulate** (IV-K2-8, D2/D3) — fetch a recent blockhash, compile the legacy
//!    message, simulate the UNSIGNED tx on devnet; a sim error ⇒ fail-closed (the live program is
//!    the authority).
//! 5. **D14 genesis pin** (IV-K2-7) — before broadcast, assert the RPC genesis == the devnet hash.
//! 6. **sign + D13** (IV-K2-6) — sign the message with the isolated key; assert the broadcast tx
...
//! no loop tool (IV-K2-12). The chain WRITE socket is compiled ONLY under `chain-write`; the gate
...
/// priority fee is a documented follow-on (the Skew FE's `LIVE_BUTTONS_CU_PLAN.md`). Tunable.
...
/// IV-K2-13). `redirect(none)` + `no_proxy()` + timeout + byte cap.
#[cfg(feature = "chain-write")]
#[derive(Debug)]
...
/// content is withheld wholesale (IV-K2-13). Bounded to 240 chars. Used by the live `chain-write`
...
pub struct SkewTxPlan {
    ...
    /// and the oracle escrow (IV-K2-3). `0` for `open_risk_account` (moves no settlement value).
    pub authorized_amount_atoms: u64,
    /// A short, secret-free action label (e.g. `open_risk_account`) for the receipt.
    pub action_label: &'static str,
}

/// The fixed chain/protocol the K-2 custody bound is matched against (the §6 seam: replace the EVM
/// demo allowlist). The owner arms a `CustodyGrant` whose allowlist must contain these.
...
/// is bound to the authorized amount (IV-K2-3). The depositor ATA must be funded (broadcast = a
...
/// three-way amount-binding `escrow == request == on-chain` (IV-K2-3) holds trivially — rather than
...
/// (`escrow_minor`); the on-chain reservation == that escrow (IV-K2-3). The model PROPOSES the
...
/// == that WCL (amount-binding, IV-K2-3 — the descriptor's WccParams MUST match the `trade`'s WCC
...
/// payment_amount (IV-K2-3). `template_id` is the product_template seed (the descriptor omits it).
pub fn plan_pay_vm(
    signer: &Pubkey,
    settlement_mint: &Pubkey,
    template_id: &[u8; 32],
...
/// request == lock_amount` (IV-K2-3). The on-chain program enforces `lock_amount >= required_initial`,
...
/// == request == WCL + price` (IV-K2-3). The model PROPOSES the WCC params + price; the oracle DECIDES.
#[allow(clippy::too_many_arguments)]
...
// ---------------------------------------------------------------------------
// WAVE C-listings — the VARIABLE-leg PERMISSIONLESS template registrations. A
...
// ---------------------------------------------------------------------------
// WAVE E-piecewise — the bilateral piecewise contract lifecycle (form / settle).
...
// program pulls (amount-binding, IV-K2-3). 2-signer ⇒ ASSEMBLE+SIM only (the chokepoint
...
/// (amount-binding, IV-K2-3). The model PROPOSES the legs; the oracle re-derives + DECIDES.
#[allow(clippy::too_many_arguments)]
...
// ---------------------------------------------------------------------------
// WAVE G-unwired — the 5 newly-assembled ix planners (all escrow=0 EXCEPT form_funding_swap,
...
/// Plan `factory_list_perp_market` (0x81) — PERMISSIONLESS perp listing under the INC-A1 UDSI gate +
...
/// vault (amount-binding, IV-K2-3); the `/10_000` FLOOR-slope payoff has its own dedicated path. The
...
pub enum SkewExecDenied {
    /// The oracle verdict is not `AffordableInBounds` (IV-K2-1).
    OracleNotAffordable,
    /// The oracle escrow / request amount / on-chain amount do not all agree (IV-K2-3).
    AmountBindingMismatch,
    /// The owner-configured endpoint failed SSRF hygiene (IV-K2-13).
    EndpointRejected,
    /// The chain-write transport is not compiled (default build) — honest-degrade.
    TransportNotCompiled,
    /// A recent blockhash could not be fetched.
    BlockhashUnavailable,
    /// The devnet simulation reported a program error (D2/D3 — IV-K2-8). Carries a redacted summary.
    SimulateFailed(String),
    /// The RPC genesis hash != the devnet pin (IV-K2-7).
    GenesisMismatch,
    /// The signed tx message != the assembled/simulated message (IV-K2-6).
    D13Mismatch,
...
/// THE single K-2 chokepoint. Requires the [`ChainTxCapability`] witness BY VALUE (IV-K2-2:
...
pub fn execute_skew_chain_tx(
    ...
    // GATE 1 — oracle: the plan must be AffordableInBounds, escrow == request amount (IV-K2-1).
    ...
    // GATE 3 — amount-binding: oracle escrow == request amount == on-chain amount (IV-K2-3).
    ...
    // SSRF wall on the owner-configured endpoint BEFORE any dial (IV-K2-13).
    ...
    // go-live (FF-08b). Single-signer txs (open/deposit/withdraw/perp/submit_order/pay/lock/settle/mark)
    ...
    // GATE 5 — D14 devnet genesis pin BEFORE broadcast (IV-K2-7), in EVERY broadcast mode. (The live
