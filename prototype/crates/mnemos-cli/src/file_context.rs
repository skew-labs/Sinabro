...
//! read-only file access (agent-core lane A; owner "파일 컨텍스트"
//! 2026-06-11). Threat model:
//! `ops/evidence/stage_g/agent_loop/FILE_CONTEXT_THREAT_MODEL.md`.
//!
...
//! there is no write / delete / exec path here (those are roadmap §3.B/§3.C
//! with their own threat models). The model can read a file; it can never
//! write or run one (IV-F7).
//!
...
//!    RESOLVED path defeats both `../` traversal and symlink escape (IV-F1).
//! 3. **denylist** — known secret CONTAINERS (dotfiles, `*.pem`/`*.key`,
//!    `id_rsa*`, `.env*`, `.ssh`/`.git`/`.aws` components, …) are refused
//!    before being opened (IV-F2); defense-in-depth ON TOP of redaction.
//! 4. **size cap** — `> MAX_FILE_BYTES` ⇒ [`FileReadDeny::FileTooLarge`]
//!    (refused, never partially read; IV-F3).
//! 5. **read + hash** — `{bytes, sha256(bytes)}` (IV-F4, content-addressed).
//! 6. **UTF-8** — binary ⇒ metadata only, bytes never rendered (IV-F5).
//!
//! Frontier redaction (IV-F6) is applied by the CALLER (the agent loop /
...
/// Maximum readable file size in bytes (IV-F3). A larger file is refused,
/// never truncated — a partial read of a config/secret is worse than no read.
pub const MAX_FILE_BYTES: u64 = 256 * 1024;

/// Maximum rendered/injected content lines (IV-F3); overflow gets an
/// explicit truncation marker, never a silent cut.
pub const MAX_FILE_RENDER_LINES: usize = 200;

/// Environment variable that OWNER-WIDENS the allowlist (IV-F1, the
/// documented "owner explicitly widens" path, TM R-F1). A `:`-separated list
...
/// Environment variable that sets a SINGLE explicit workspace root (E14-B1), read by
...
/// Workspace-root markers (E14-B1): the nearest ancestor of cwd containing one of
...
/// Secret-container filename SUFFIXES (lowercased final component; IV-F2).
...
/// Secret-container filename PREFIXES (lowercased final component; IV-F2).
...
/// Path COMPONENT names that deny the whole path (IV-F2): a file anywhere
...
pub enum FileReadDeny {
    ...
    NotFound,
    /// The canonical path is not under any allowed root (IV-F1).
    OutsideAllowedRoots,
    /// A path component / filename matched the secret-container denylist
    /// (IV-F2); carries the matched token for an honest, secret-free reason.
    DeniedName(&'static str),
    /// The file exceeds [`MAX_FILE_BYTES`] (IV-F3).
    FileTooLarge,
...
/// A successful bounded read (IV-F4): the canonical path, the bytes, their
/// content hash, and the UTF-8 view (or `None` for binary — IV-F5).
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct FileReadResult {
    ...
    /// (IV-F5 — binary is never rendered/injected, only its metadata).
...
/// are stored canonicalised so the prefix check (IV-F1) is over resolved
/// paths on both sides.
...
impl FileReadPolicy {
    ...
    /// project, not just the launch dir (E14-B1; the owner's "agent can't read a
    /// sibling file" bug). `SINABRO_PROJECT_ROOT` (a single explicit root) overrides
    ...
    /// Whether a CANONICAL path is confined to an allowed root (IV-F1).
    #[must_use]
    ...
    pub fn read(&self, requested: &Path) -> Result<FileReadResult, FileReadDeny> {
        ...
        //    bytes hashed are the bytes read, IV-F4/T7).
    ...
    /// directories), require it WITHIN an allowed root (defeats `..` + symlink, IV-F1), then
    /// re-apply the secret-container denylist over the resolved path (IV-F4 — never CREATE a
...
/// entry (the project root); fall back to `start` if none is found (E14-B1). The
...
/// The resolved workspace root the agent treats as the project root (E14-B1):
...
/// constitution file (CURSOR_PARITY B⑮) — a stat-only locate, never a policy read.
#[must_use]
...
/// the denylist (IV-F2). PURE over the path string — unit-testable without
...
pub fn denied_token(path: &Path) -> Option<&'static str> {
    ...
    // (classify-fail = deny; IV-F2). `.` / `..` cannot be a final component
...
mod tests {
    ...
    /// IV-F2 — every secret-container class denies, by suffix / prefix /
    ...
    /// IV-F1/F4/F5 — a normal file inside the root reads with a correct hash
    ...
    /// IV-F1 — `..` traversal AND symlink escape both reject (the prefix
    ...
    /// IV-F2 — a denylisted file INSIDE the root is still refused (the
    ...
    /// IV-F3 — an over-cap file is refused, not truncated.
    #[test]
    ...
    fn workspace_root_of_walks_up_to_a_marker_and_returns_an_ancestor() {
        // E14-B1: a nested dir under a base that has a `.git` marker resolves to base.
