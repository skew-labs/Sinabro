//! D-1 (AGENT-NATIVE GITHUB, owner-directed 2026-07-01 "우리 에이전트만의 깃허브도
//! 만들자 … ai만의 언어로 소통하고 파일 배포하는 창구"): the PURE, content-addressed
//! artifact-registry CORE — a git-object analog for the things an agent produces.
//!
...
//! NO network, NO clock, NO artifact EXECUTION here. The gated Walrus publish/fetch (D-3),
//! the AI-native wire protocol (D-2), the agent loop tool (D-4), and the sandboxed
//! fetch-then-propose (D-6) build ON TOP — each its own gated + threat-modeled slice.
//! NO funds / wallet / chain-write (PD-6): a content address is public, moves no money.
...
pub enum ArtifactKind {
    ...
    /// Source code / a patch.
    Code,
    /// A certified oracle (the O-5 iNFT analog).
    Oracle,
}
...
pub struct AgentArtifact {
    ...
    pub author: String,
    /// A bounded single-line description (plaintext; redacted before any publish, D-3).
    pub summary: String,
...
/// byte codec — deterministic + fail-closed — so it can be content-addressed and (D-3)
/// published to Walrus.
...
// ── D-2 (AI-NATIVE PROTOCOL): the deterministic `AgentMessage` envelope — the compact,
...
mod tests {
    ...
    // ── D-2: the AI-native AgentMessage protocol codec ──
    #[test]
