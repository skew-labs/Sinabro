//! `zerog_attestation` — 0G Compute TEE attestation verify (W2-B), via a Node sidecar.
//!
...
//! (`prototype/sidecar/zerog-attestation/verify.js`) — exactly as W2-C drives the Go
//! storage client.
//!
//! ## funds-safe posture (PD-6) — keyless, no funds, no on-chain write
...
//! [provider]`) and runs through the SAME bounded `exec_local` runner as W2-C — no shell,
