//! `skew_history` — the PURE, byte-locked time-series WINDOW codec + the
...
//! AEAD-encrypted memory (local storage-of-record + a 2-tier Walrus ciphertext publish).
...
//! funding step; it can only READ the rendered series. This applies the "LLM off the hot path" +
//! "deterministic-no-LLM-judge oracle" discipline to analytics.
