//! B⑫ MCP client — a sandboxed stdio MCP server as a READ capability (CURSOR
//! PARITY keystone-3; design:
//! `ops/evidence/stage_g/agent_loop/CURSOR_PARITY_REFRAME_DESIGN.md` §3 B⑫ + §6 B⑫).
//!
//! ## Thesis (the capability-type model is the ideal MCP host)
//!
//! Each configured MCP server is a CAPABILITY-TYPED connection. v1 (owner-locked
//! §6 B⑫ triangle 2026-06-16) = stdio LOCAL servers only (NO egress — the child's
...
//! * REDACTION (SI-2). The outbound tool ARG AND the inbound tool RESULT both pass
...
//! `Drop`) REUSES the proven A① [`crate::lsp`] idiom — only the framing differs.
...
/// verb). Gate order (the §6 B⑫ triangle): (1) WALL — the server must be an
...
/// stays closed, PD-1). custody/funds untouched (the child's network is
...
pub fn render_mcp_call(
    ...
    // 2. REDACT the OUTBOUND arg — a secret-shaped argument is WITHHELD (the SI-2
...
/// SI-2 redaction gate: `true` ⇒ the text carries no secret-shaped fragment and may
...
impl McpServer {
    ...
    fn spawn_sandboxed(
        ...
        // start; both are on the allowlist) — keys never cross (IV-E3).
