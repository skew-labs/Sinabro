//! All-command trace sidecar writer.
//!
...
//! - Self-evolution candidate manifests are display / read-only and
...
/// The execution class of a traced command.
#[repr(u8)]
...
pub enum TraceReject {
    ...
    Untraced,
    /// Applying a self-evolution candidate is forbidden (no apply path exists;
    /// manifests are display / read-only).
...
impl TraceWriter {
    ...
    /// Self-evolution lock. There is no apply path: the success type is
