//! `skew_strategy` — the strict TYPED STRATEGY DSL + the deterministic
...
//! EXECUTES the DSL over the REAL history series, emits candidate [`crate::skew_oracle::SkewTrade`]
//! legs, and gates EACH with the trade oracle ([`crate::skew_oracle::oracle_verdict`] — escrow ≤
...
//! ([`crate::conformal::certify_far_default`], the exact Clopper-Pearson FAR bound) CERTIFIES a strategy
//! iff its out-of-bounds proposal RATE is provably bounded (`k` oracle-denied fires in `n` total fires;
//! `k=0, n≥10 ⇒ certified`). Only a CERTIFIED strategy ACCUMULATEs into the certified corpus (the
//! dispatch glue runs it through `autonomy_evolve::strategy_candidate` →
//! `autonomy_evolve::select_evolution_writes` — the two-derivation hallucination gate; a hallucinated
//! "win" NEVER writes).
//!
//! ## PURE (no Solana / serde-json / net / clock / float / RNG / key / chain-write dependency)
//! Every signal analyzer + the rule evaluation is pure integer math over the slot-sorted history window;
...
//! grant, NO custody, NO socket; the live sub-budget is the EXISTING owner-armed execution path.
//!
//! ## Honest scope
...
//! `0 ≤ k ≤ n`). shadow money 0; the live sub-budget is the owner go-live.
...
/// The strategy archetype label — the grammar is general,
...
/// Which time-series a signal reads.
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum SeriesSel {
...
/// A deterministic integer signal feature computed over the series (no float, no clock, no LLM).
#[derive(Clone, Copy, Debug, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
/// NO un-gated action in the grammar (no "exec" / "sign" / "address" field exists-1/-4).
...
/// Parse + validate a proposed strategy DSL from TOML — the serde-fail-closed wall (-1). A
...
pub struct RuleShadow {
    ...
    pub fires: u32,
    /// Of those, how many cleared the oracle (in-bounds wins).
    pub in_bounds: u32,
...
/// The conformal certification verdict for a shadow track record (the exact FAR bound — drift-0).
...
/// evaluate the condition, and on each FIRE gate the (fixed) trade leg with the oracle
...
pub fn shadow_rule(
    ...
    for i in 0..samples.len() {
        if consumed >= remaining_budget {
            break; // the whole-strategy fire cap is honored (bounded; -7)
        }
...
/// Shadow-evaluate a whole strategy over the accumulated history (money 0). Aggregates the per-rule
...
/// CERTIFY a strategy from its shadow track record (the exact Clopper-Pearson FAR bound — reuse
/// [`certify_far_default`]). `k` = oracle-denied fires (out-of-bounds false accepts), `n` =
...
/// money 0; certified = in-bounds rate provably bounded, NOT profitable; the live sub-budget is the
/// owner-armed execution path.
#[must_use]
...
"#;
    ...
    // ---- -5: the conformal cert is the certify gate -----------------------------------

    #[test]
    fn certification_needs_enough_in_bounds_fires() {
        // a strategy that fires only a FEW times can't certify (n < 10 — the conformal boundary).
        let dsl = StrategyDsl {
    ...
    fn trade_template_maps_to_every_skew_trade_class() {
        // every payoff class is representable and maps to a SkewTrade.
