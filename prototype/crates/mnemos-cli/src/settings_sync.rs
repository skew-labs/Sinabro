//! [6] Settings-sync — portable config across machines via an AEAD blob on
//! Walrus. The cross-Cursor "Settings Sync" reframed onto sinabro's physics: the
//! config is serialized + secret-screened (reuse `serialize_config`), sealed
//! with the LOCAL AEAD key (the substrate), PUT to the Walrus testnet
...
//!   (`PublishPayloadClass::EncryptedUserMemory`, the policy). The 32-byte key
...
//!   tag fails.
//! * [`validate_and_normalize`] re-runs the secret-screen + validation on BOTH
...
//! * custody/funds/wallet/chain-write are HARD-LOCKED: the config schema
//!   carries no wallet/funds field, and a Walrus testnet PUT needs no wallet/funds.
//!
//! Cross-machine key: like all Walrus memory, the seal uses the per-machine
...
/// Validate + normalize a config TOML through the pipeline: parse it
