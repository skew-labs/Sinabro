//! Local owner-initiated command execution — bounded, env-scrubbed (P3-1).
//! Threat model: `ops/evidence/stage_g/agent_loop/CODE_EXEC_THREAT_MODEL.md`
//! (IV-E1..E8). THE FIRST process-spawn surface in the core; the MODEL has
...
/// Default wall-clock timeout for one owner command (IV-E4).
...
/// The ONLY environment variables a child inherits (IV-E3). Everything else
...
pub struct ExecOutcome {
    /// The exact argv that ran (echoed verbatim in the receipt — IV-E2).
    pub argv: Vec<String>,
...
/// [`run_local_command_with`]. The argv-only contract (IV-E2) holds: the argv is
...
/// NARROW the child's view, never widen it, so IV-E3 is never weakened). The CODE oracle
...
pub fn run_argv_command_with_env(
    ...
    command
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    // IV-E3: the child sees ONLY the allowlist (a parent var that is unset
            ...
            match child.try_wait() {
                ...
                Ok(None) => {
                    if Instant::now() >= deadline {
                        timed_out = true;
                        // IV-E4: kill THEN wait — the child is always
...
/// them (bounded memory, honest totals — IV-E4). A missing pipe drains to
/// the empty capture.
...
mod tests {
    ...
    /// IV-E2/IV-E4 — a plain command runs, exits 0, captures stdout exactly,
    ...
    /// IV-E3 — the cardinal leak wall: a child dumping its environment sees
    ...
    /// IV-E4 — the timeout kills + reaps a hanging child and says so.
    #[test]
    #[cfg(unix)]
    ...
    /// IV-E4 — the retained cap holds while the TOTAL stays honest and the
