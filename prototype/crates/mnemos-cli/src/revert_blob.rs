...
//! lane-A write confinement + the staleness lock (refuses if the
...
//! is "NEVER chain-write" (`CustodyCapability` stays uninhabited). The
...
//!
//! # store discipline (reused verbatim from the proposal store)
//! AEAD-sealed with the [`MemoryCipher`], a DISTINCT magic/extension/subdir
...
pub struct RevertBlob {
    ...
    /// restore refuses unless the target STILL hashes to this — so a
...
pub enum RevertDeny {
    ...
    NoRevertPoint,
    /// The target changed since the apply (the owner-save staleness lock) —
...
/// (a refused rewind — e.g. The target moved since the apply — keeps it).
/// NEVER touches funds/wallet/chain (owner-save is local-file-only).
fn restore_seq(
