...
//!
//! (local-first autonomy): an AUTONOMOUS turn defaults to the LOCAL
//! executor. The external frontier is reached ONLY as an explicit owner-armed
//! escalation — it requires a VALID [`EgressCapability`], which exists ONLY
//! from a valid owner-armed grant ([`crate::commands::grant::EgressGrant`]).
//! The model has NO constructor for an `EgressCapability` (compile-fail
//! proven), so it CANNOT self-route to the frontier (reinforced here at
...
//!
//! Deferred: a smart confidence / blast-radius router is NOT
//! built here — this is the simplest honest default (local-first + explicit
//! escalation). The autonomous RUNTIME that calls this selector with
//! [`ConsultCaller::Autonomous`] is the daemon; the LIVE consumer is the
//! owner-interactive dispatch arm (the local-route decision), and the autonomous
//! branch is the typed seam the daemon consumes.
//!
//! Reuse (no reinvention): [`EgressCapability`]
//! from [`crate::commands::authority`]. This is the ORTHOGONAL
...
/// Who is driving the consult. An autonomous turn defaults to local.
...
pub enum ConsultCaller {
    ...
    Owner,
    /// The autonomous runtime (the daemon). Defaults to the local executor
...
/// The selected consult executor — the single typed routing truth.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// Select the consult executor — the single typed routing truth.
///
...
/// - AUTONOMOUS (the daemon): the default ⇒ local. An explicit frontier
///   escalation fires the frontier ONLY with a valid owner-armed [`EgressCapability`];
///   without one it fails closed ([`ConsultRoute::FrontierDeniedNoGrant`]) —
///   never silent local, never silent frontier. The model cannot mint an
///   `EgressCapability`, so it cannot self-route to the frontier.
#[must_use]
pub fn select_consult_route(
    ...
    match caller {
        ...
        ConsultCaller::Autonomous => match phrase {
            // the autonomy default is local — free, private, zero egress.
...
mod tests {
    use super::*;
    // The capability MINT stays inside
    ...
    // authority.rs (one constructor site; no security relaxation).
    ...
    // ---- AUTONOMOUS: local-first, frontier is owner-armed only ----

    #[test]
    fn autonomous_defaults_to_local() {
        // no phrase and the local phrase both default to the local executor.
        assert_eq!(
    ...
    /// THE SECURITY PROOF (at the route layer + L5 no-silent-fallback): an
    ...
    /// `EgressCapability`, so this branch is the only one a model could
