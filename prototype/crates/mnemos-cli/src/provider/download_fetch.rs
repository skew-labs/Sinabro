//! Gated download — the agent's owner-armed bounded GET into a temp file.
//!
...
//!   — the write CANNOT escape the temp dir). GET-only ⇒ no chain WRITE.
...
/// from. EXACT host match (lowercased). Owner-extended via the `config.rs` seam.
...
pub enum DownloadDenied {
    ...
    /// (deny-by-default; owner-extensible).
    HostNotAllowlisted,
...
impl DownloadDenied {
    /// A stable, secret-free class label (for renders).
    #[must_use]
...
/// The SSRF wall + allowlist narrowing (DL2) — PURE, no network. Admit `raw`
...
pub fn classify_download_url(
    ...
    // 1. The EXISTING SSRF wall FIRST: https-only, IP-literal / localhost /
    ...
    // 2. The allowlist narrowing: even an SSRF-safe host must be allowlisted.
...
/// Build the temp-file path for a download — a DIRECT child of
...
/// The bounded, metadata-only result of a permitted download. It carries NO
...
/// blocking client built with the /DL6 paranoia set: `redirect(none)` +
...
impl DownloadTransport {
    ...
    /// ), `redirect(none)`, byte- + time-bounded. A 3xx / non-2xx is a
    /// typed deny; an over-cap body is refused (nothing left on disk). The bytes are
    /// UNTRUSTED and NEVER executed (DL8) — only metadata is returned.
    pub fn fetch_to_temp(&self, safe: &SafeUrl) -> Result<DownloadReceipt, DownloadDenied> {
        ...
        // Bounded read: read at most max_bytes+1; MORE than max_bytes ⇒ refuse
        ...
        // The file name is OURS (host + content sha), separator-free; the
        // bytes are written but NEVER executed.
...
/// The SHARED gated-download pipeline (.) — the one place the `daemon
/// fetch` verb runs a download. It REQUIRES a `&FetchCapability` witness (the owner-arm
/// proof at the type level — this fn is UNREACHABLE without one). Order:
///
...
/// 3. Metadata-only render — host / status / bytes / temp_path / sha, NEVER the
...
pub fn render_download_fetch(
    ...
    // 1. SSRF wall + allowlist (DL2). Deny ⇒ typed render (reason LEADS the line).
    ...
    // 3. Metadata-only render: host / status / bytes / temp_path / sha — NEVER
