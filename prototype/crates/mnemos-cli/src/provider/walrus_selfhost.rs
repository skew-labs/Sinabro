//! S2 — self-host (BYO) Walrus **mainnet** transport (WALRUS_MAINNET_SELFHOST).
//!
//! The deploy-time model (kickoff `WALRUS_MAINNET_SELFHOST_KICKOFF.md`, owner-locked
//! 2026-06-21): each user runs their OWN `walrus publisher` (their wallet pays) or a
//! hosted one, and plugs its **https** URL + a bearer **token** into the config. Our
//! app then stores its two-tier encrypted memory there.
//!
...
//! `web3_rpc` + the c-walrus testnet publisher), and **PD-6 custody stays HARD-LOCKED**
...
//!   denied (kept byte-in-sync with the c-walrus testnet admit by the `e24` verifier).
...
//! ceremony in `dispatch.rs` (S3). This module mints the transport primitives only.
...
/// NEVER a Sui private key — our app holds no key and never signs (PD-6).
...
/// Testable in the default build (no feature needed) — the `e24` non-vacuity leans on it.
///
/// # Errors
///
...
impl WalrusSelfHostTransport {
    ...
    pub fn put_blob(
        ...
        // Content secret-screen (mirrors c-walrus `PublisherPutRequest::new`, atom #8):
        // ONLY ciphertext (EncryptedUserMemory) or a synthetic fixture may leave the
        // process; every PLAINTEXT class is denied (secret-zero). The `e24` verifier
        // (CHECK-E) pins these two admit arms == the c-walrus testnet admit arms, so the
...
mod tests {
    ...
    /// S6 — LIVE self-host TESTNET round-trip (the NO-RISK proof of the mainnet code path).
    ...
    /// mainnet ceremony.
    ///
    /// `#[ignore]`d (gate G-WALRUS-NET): it performs real network I/O, so it never runs in
    /// the default suite. Run after owner approval:
