//! `provider::lora_manifest` — K-6: the corpus→adapter MANIFEST + the send-time
...
//! on the wire (`send_local_text_with`). K-6 adds the two PURE pieces the dynamic-LoRA
//! switch was missing:
//!
...
//!    The key is GENERAL (owner seam-lock Q1 = "both / general manifest"): any domain
//!    label — a Skew strategy archetype (`market_making`, `hedge`), an executor expert
//!    kind (`sui_move`, `audit`), or a user domain — is a valid [`AdapterKey`]. The
//!    Skew archetypes populate it first because they are the only certified corpus
...
//!    paid train are the owner GPU go-live (plan §13).
//!
...
//! never reaches a key. `CustodyCapability` stays uninhabited (PD-6).

use crate::provider::executor_route::ExecutorRoutingTable;
use std::collections::BTreeSet;

/// A validated GENERAL domain key for an adapter binding (owner seam-lock Q1 = "both /
/// general manifest"): a Skew strategy archetype (`market_making`, `hedge`), an executor
...
/// rides the wire (the base answers) — the core K-6 honesty guarantee.
#[must_use]
...
/// table (requested adapter → wire model, served/degraded). HONEST (PD-1): a degraded
...
mod tests {
    ...
    /// THE NO-SERVER HONESTY PROOF (the K-6 DoD): with NO co-resident server up (the
    ...
    // ---- the honest render (CLI + GUI share it; PD-1) ----
