...
//! New here is ONLY the loopback client wrapper (loopback invariants).
//!
//! Loopback is STRUCTURAL, not policy: the only target type is
...
//! endpoint surfaces a typed 401 card). The local server's
//! response is UNTRUSTED network-input regardless of locality:
...
/// request/response SHA-256 receipts (replay parity with the frontier
...
impl LocalChatTransport {
    ...
    /// the loopback hardening set: `no_proxy()` (proxy env must never route a
    ...
    /// (v1). The response bytes are UNTRUSTED: non-200 ⇒
    ...
    /// executor router's per-sub-task adapter selection (the seam,
...
/// hermetic, deterministic, zero egress — the established real-fs /
/// real-process test pattern extended to a real loopback socket.
#[cfg(test)]
...
mod tests {
    ...
    /// Redirect-exfil closed: a malicious local server answering 302
    ...
    /// (local bytes are untrusted network input).
    #[test]
    ...
    /// Structural loopback: the transport's only target type is
    ...
    /// `send_local_text_with` places an EXPLICIT model id in the
