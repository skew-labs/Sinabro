//! Operational provider-body redaction gate + receipt.
//!
//! The secret scanners ([`crate::secrets::scan_inline_secret`],
//! [`crate::repl::history::classify`]), the tombstone reuse pattern
//! (a deleted-id slice, following `model_compress::assemble_context`), and the
//! [`RedactionClass`] taxonomy are composed here into one
...
//!
//! Secret custody (secret-zero; no PII retained): this gate is pure (no I/O); it
...
pub struct RedactionRequest<'a> {
    ...
    /// The tombstoned (deleted) memory ids (delete/replay truth).
...
///
/// # The unforgeable receipt (the single egress choke)
///
...
/// that never passed redaction" state is UNREPRESENTABLE, not
...
/// build, so production keeps the invariant that the ONLY constructor is
/// [`redact`].
#[cfg(test)]
impl RedactionReceipt {
