//! ⑭ Web fetch — the agent's LIVE web READ (E11-1). Threat model:
//! `ops/evidence/stage_g/agent_loop/WEB_FETCH_THREAT_MODEL.md` (IV-WF1..IV-WF10).
//!
...
//!   `.send()`: an UNAUTHENTICATED `GET` (D-WF1 secret-zero — no Authorization /
//!   cookie / key / owner secret), `redirect(none)` (a 302 → internal-host exfil
//!   is impossible, D-WF3), `no_proxy()`, a per-call timeout and a response-body
//!   byte cap (D-WF4). GET-only is the structural chain-WRITE wall: a JSON-RPC
//!   mutation needs a POST, which this transport cannot issue, and no wallet key
//!   exists (`CustodyCapability` uninhabited, PD-6).
//!
...
//! and the loop grammar stays closed (IV-WF7). The fetched body is redacted by
//! the CALLER before it enters loop context (IV-WF5) and a web answer is
//! surfaced only through the existing `WebSourcePolicy` (source-linked,
//! quote-limited, advisory-only — IV-WF6).
...
/// The SSRF wall (IV-WF1, IV-WF10) — PURE, no network. Admit `raw` ONLY if it is
...
pub fn classify_url(raw: &str) -> Result<SafeUrl, WebFetchDenied> {
    ...
    // Embedded credentials (`user:pass@host`) — refused (IV-WF10 obfuscation).
...
/// The default per-fetch timeout (ms) and response-body byte cap (IV-WF4).
...
/// pooled blocking client built with the IV-WF2/WF3/WF4 paranoia set:
...
impl WebFetchTransport {
    ...
    /// (IV-WF3); an over-cap body is refused (IV-WF4). The bytes are UNTRUSTED —
    /// the caller redacts before use (IV-WF5).
...
// ===========================================================================
// E11-1b — the SHARED GLUE (loop tool + dispatch verb both call it)
// ===========================================================================
...
/// [`WebFetchDenied::TransportNotCompiled`] (IV-WF7) and the loop stays pure.
...
/// a deny / withhold never consumes K, IV-WF8), and a stable class label (for
...
/// The SHARED web-fetch pipeline (IV-WF1..IV-WF6) — the one place the loop tool
...
///    (IV-WF5), exactly as the file-read tool withholds a secret-shaped file.
...
///    code execution (IV-WF6). A non-advisory verdict is a typed deny.
#[must_use]
pub fn render_web_fetch(
    ...
    // 1. SSRF wall (IV-WF1, IV-WF10). Deny ⇒ K not consumed. The reason LEADS the
    ...
    // 3. Redact the UNTRUSTED body BEFORE it surfaces (IV-WF5). The whole body
    ...
    //    (IV-WF6). A non-advisory verdict is a typed deny (K not consumed).
...
mod tests {
    ...
    // -- E11-1b glue: the SHARED pipeline (loop tool + dispatch verb) ---------
    ...
    fn glue_policy_disabled_denies_advisory_non_vacuous() {
        ...
        // benign fetched body (the advisory gate is not vacuous, IV-WF6).
        let port = MockPort {
            response: Ok(benign_response()),
        };
