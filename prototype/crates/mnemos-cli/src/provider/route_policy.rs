...
//! (atom #492 · G.1.1).
//!
//! Stage F minted the [`FallbackDiff`] type (a provider/model fallback carrying a
//! visible reason + approval flag, with `is_silent` / `is_permitted`). Stage G
//! adds the *operational policy engine* that decides, per route state, whether a
//! fallback may apply and classifies a local→external move as an explicit,
//! user-visible egress event — never a silent upgrade (`G-G-OPERATIONAL-ENTRY`,
//! `G-G-COST-BUDGET`; consistency-lock "No Silent Fallback"). A silent fallback,
...
/// The no-silent-fallback visibility of a proposed fallback (#609, G.8.15). A
...
/// Classify a proposed fallback for the no-silent-fallback route trace (#609): a
...
mod tests {
    ...
    // ---- #609 no-silent-fallback live (visible approved route event) ----
    ...
    fn escalation_consult_surfaces_as_visible_fallback() {
        // integration (#605 escalation + #609 route_policy): an escalation that opens
