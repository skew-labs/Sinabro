...
//! Linux/GPU prod target.
//!
//! The PROD counterpart to the MLX/ollama dev adapter, behind the IDENTICAL
...
//!
//! KV-cache mode is ROUTE-VISIBLE (no hidden compression, no silent fallback):
...
//! become a stable serving path. This adapter makes NO speed claim — TTFT/TPOT
//! measurement is a separate, data-gated concern.
//!
//! The autonomous default makes ZERO live calls: the loopback transport is a deferred
//! seam (identical posture to the MLX adapter). A freshly constructed [`VllmEndpoint`]
//! has no runtime attached and every serving call fails closed
...
//! socket). BASE model only ([`AdapterSwapPoint::Empty`]; no training here);
...
/// adapter — uniform route identity + evidence.
#[derive(Clone, Debug)]
...
impl VllmEndpoint {
    ...
    /// attached: the autonomous default fails serving closed
    ...
    /// path NEVER calls this — it exists so the load → stream contract is
    ...
    pub fn kv_status(&self) -> KvCacheModeStatus {
        // prefill/decode split candidacy is deferred (no over-claim here)
...
mod tests {
    ...
    fn quantized_kv_mode_is_never_a_false_green_and_flags_canary() {
        ...
        // a false green — they cannot silently become a stable serving path.
