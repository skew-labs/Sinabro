//! Operational adaptive model-escalation heuristic.
//!
//! [`ConsultTrigger`] (the typed reasons a frontier consult may be
//! requested) and [`consult_token_cap`] (per-state token caps; FAST/NORMAL/
//! LOCKDOWN/USER_FULL = `(0,0)`) define the consult vocabulary. This module adds the
//! *escalation decision*: routine
...
/// The typed-trigger egress consult policy. A provider egress consult
