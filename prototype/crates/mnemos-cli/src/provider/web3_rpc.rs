//! Web3 RPC reader — the agent's AUTONOMOUS multi-chain CHAIN READ (reframed by ONCHAIN
//! PIVOT C-1).
//!
...
//! never a URL). A chain WRITE stays UNREPRESENTABLE (the method enum has no write
...
//!   at propose-time and `CustodyCapability` uninhabited).
...
//!   argument (the `chain_env` "no arbitrary endpoint" invariant).
...
//!   "no unredacted outbound byte" property holds via the redact choke; the
...
//! before it surfaces (a secret-shaped result ⇒ WITHHELD). CUSTODY is
//! untouched: no wallet/sign/funds symbol exists here, the method allowlist blocks a
//! chain WRITE, and `CustodyCapability` is uninhabited.
...
/// The model cannot type it; only the owner-input loop supplies it.
pub const WEB3_READ_ARM_PHRASE: &str = "arm-web3-rpc-read-bounded-revocable";

/// The default per-call timeout (ms) and the response-body byte cap. An RPC
...
/// so a chain WRITE cannot be issued through this transport (structural).
...
impl Web3RpcMethod {
    ...
    /// any write method name — yields `None`, never a guessed method).
    #[must_use]
...
pub enum Web3Denied {
    ...
    UnknownMethod,
    /// The outbound `params` were secret-shaped — WITHHELD before any send.
    SecretShapedParams,
    ...
    OverCap,
    /// The response was secret-shaped — WITHHELD before it surfaced.
    SecretShapedResult,
...
/// SSRF hygiene on the OWNER-CONFIGURED endpoint — PURE, no network. Admit
...
/// response body (the caller redacts it before it surfaces).
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// secret-shaped byte). Used for BOTH the outbound params and the inbound
/// result — the same wall the file-read / web-fetch tools use.
...
/// supplies a chain NAME, never a URL (the SSRF posture).
#[derive(Clone, Debug, PartialEq, Eq)]
...
/// The SHARED multi-chain web3-read pipeline (.; ONCHAIN PIVOT C-1) — the one place
...
/// bound; the agent supplies a chain NAME, never a URL). Order:
///
...
///    secret-shaped param ⇒ WITHHELD. The method is a fixed read-only literal.
/// 5. `port.call` — the secret-zero POST (`None` ⇒ `TransportNotCompiled`).
/// 6. `redact(response)` + metadata render — a secret-shaped result ⇒ WITHHELD; the
...
pub fn render_web3_read(
    ...
    // 1. The requested chain must be owner-configured (the bound; no arbitrary URL).
    ...
    // 3. The OUTBOUND params pass redact BEFORE any send. A
    ...
    // 5. Redact the UNTRUSTED result BEFORE it surfaces. A secret-shaped
...
mod tests {
    ...
    fn no_chain_write_method_is_representable() {
        ...
        // cannot be issued through this path; structural).
        for write in [
