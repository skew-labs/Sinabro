...
//! (G-WP-09 atom #595 G.8.1). Apple-Silicon dev target.
//!
...
//! is wired in a later stage. Secret-zero (`G-F-SECRET-ZERO`): the endpoint holds a
...
//! carries [`AdapterSwapPoint::Empty`] (`G-G-NO-TRAINING-IN-G`); Stage H fills the
//! adapter seam.
//!
...
/// Whether a loopback MLX/ollama runtime is reachable. Phase-0 autonomous attaches
...
impl MlxEndpoint {
    ...
    /// the Phase-0 autonomous default fails serving closed
    ...
    /// autonomous Phase-0 path NEVER calls this — it exists so the load → stream
...
mod tests {
    ...
    fn loads_base_then_streams() {
        ...
        if let Ok(r) = receipt {
            // BASE only (adapter empty) — no fine-tune in Stage G
    ...
    fn missing_runtime_is_a_clean_error_not_a_panic() {
        // Phase-0 default: no loopback runtime attached → fail closed (no panic, no
