//! `provider::executor_route` — the task-aware EXECUTOR router (P1-1, the
//! orchestrator spine; owner brief 2026-06-13, plan
...
//! local-vs-frontier decision). The dynamic-LoRA reality is EXTERNAL (R1/R2
//! reconcile): the model server hot-switches adapters behind the OpenAI-compatible
...
//!
//! No real adapter exists on disk (R3) — the table is seeded with STUB model-ids
...
//! consumed by `LocalChatTransport::send_local_text_with` (the R1 seam; that
//! module is feature-gated, so this is a plain reference not an intra-doc link).
//! Mode tagging + the owner config that fills this table land at
//! P1-5; the orchestrator that consumes this router lands at P1-2.
...
/// The kind->target lookup (OPEN data; owner config fills it at P1-5). Holds an
...
impl ExecutorRoutingTable {
    ...
    /// The explicit `(kind, target)` bindings, in order (P1-5: the config seam
...
/// default; deploy/owner config overrides per worker, P1-5).
...
/// sequential; no real adapter exists on disk, R3). Owner config (P1-5) replaces
...
/// the router consumes (`kind`), and the local executor implements (`goal`).
/// LOCKED at plan-time ([[cross-language-schema-lock]]); `kind` is the routing
...
mod tests {
    ...
    /// THE DRIFT-0 ROUTING PROOF (P1-1 DoD): the SAME router, queried with two
