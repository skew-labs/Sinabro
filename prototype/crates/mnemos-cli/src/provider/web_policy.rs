//! Operational web-research source policy.
//!
//! [`WebResearchRecord`] (source-url / fetch / citation hashes +
//! rights decision + browser-credential redaction proof) and
//! [`web_answer_allowed`] (a source-less or rights-denied answer is denied)
//! provide the base primitives. This module adds the *operational policy*: web
//! research is **opt-in** (default off), source-linked, quote-limited, redacted,
//! and **never proof of code execution**; a high-stakes / security claim
//! additionally requires local verification before the web answer may be
//! surfaced as advisory.
//!
//! Reuses [`WebResearchRecord`] / [`web_answer_allowed`] from
//! [`crate::commands::tool`].
