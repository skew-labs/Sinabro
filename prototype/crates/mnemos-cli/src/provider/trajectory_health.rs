//! Operational trajectory-health guard.
//!
//! [`TrajectoryHealth`] (a `u16` bitset of [`TrajectorySignal`]
//! flags, with `record` / `should_halt`) backs the *operational guard* this
//! module adds:
//! it maps a set of unhealthy signals to a concrete route action — continue,
//! slow, audit, or lockdown — and the route state that action implies.
//!
//! Secret-custody invariant (secret-zero): this guard reads **only** the
...
impl GuardAction {
    /// Stable, render-ready label (consumed by the agent-loop receipt).
    #[inline]
    #[must_use]
