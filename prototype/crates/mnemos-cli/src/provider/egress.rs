...
//!
//! This is the FIRST real network capability in the project. It lifts the default
...
//! safety kernel, as the designed path — never a disabled-path workaround. Funds
...
//!
//! Triple-gated and disabled by default — autonomous operation makes ZERO live
...
//!
//! Secret custody (secret-zero): the API key is held as a
...
pub enum ProviderHost {
    ...
    Gemini = 3,
    /// OpenRouter — the default LLM backend (`RuntimeLlmBackend { OpenRouter=1 }`,
    /// OpenAI-compatible, OpenRouter→DeepSeek default). OpenAI-compatible Chat
    /// Completions codec (live in v1).
    OpenRouter = 4,
...
/// host — is rejected (funds-host unreachable).
#[must_use]
...
/// request is advisory + private-memory-free — making the redaction guarantee
/// physical: the egress payload cannot carry raw private content.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
/// ceremony (the cockpit approval modal).
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
...
impl EgressApproval {
    /// A denied (not-granted) approval — the autonomous default.
    #[must_use]
...
impl ProviderTransport {
    ...
    /// reference is present. By default this cannot pass — `live_dispatch_allowed`
    ...
    /// with the feature enabled, [`preflight`](Self::preflight) must pass, which
    /// by default it cannot. No byte leaves unless every gate passes.
    pub fn send(
        &self,
        consult: &RedactedConsult,
        approval: EgressApproval,
...
//
// The runtime LLM backend defaults to OpenRouter (`RuntimeLlmBackend { OpenRouter=1 }`),
// an OpenAI-compatible trait (OpenRouter → DeepSeek by default). The live consult goes to
...
pub struct LiveConsultOutcome {
    ...
    /// neither. Cache-savings visibility, never a charge input.
...
/// `local_chat` transport — ONE parse, no second codec to drift.
#[cfg(any(
...
/// loopback `local_chat` transport: mlx/ollama/vLLM speak this SAME wire.
#[cfg(any(
    feature = "provider-egress",
    feature = "local-mlx",
    feature = "local-vllm"
...
pub(crate) fn openai_chat_body(
    ...
    // (not as a bare deepseek). The agentic tool-call loop is step 2.
    serde_json::json!({
        "model": model,
...
/// with the loopback `local_chat` transport — same wire, same parse.
#[cfg(any(
    feature = "provider-egress",
    feature = "local-mlx",
    feature = "local-vllm"
...
pub(crate) fn parse_openai_chat_response(bytes: &[u8]) -> Option<ParsedAnswer> {
    ...
    let output_tokens = usage
        ...
        .and_then(serde_json::Value::as_u64)
        .unwrap_or(0);
    // Cached prompt tokens (cache-savings visibility): the
...
/// `std::io::Read`: no `stream` feature, no async
/// runtime). Each non-empty `delta.content` piece is handed to `on_delta`
...
/// (a LOOPBACK server's bytes too — shared with `local_chat`).
#[cfg(any(
    feature = "provider-egress",
    feature = "local-mlx",
    feature = "local-vllm"
...
// not written; the live fire is the OWNER's V2 step.
...
mod live_codec_tests {
    ...
    /// Cached prompt tokens parse from BOTH provider shapes (the
...
/// The same-message egress approval ceremony. Before a provider
...
impl EgressConsultApproval {
    ...
    /// caps) — the hash recorded in the route trace so the approval is
...
mod tests {
    ...
    fn default_request_denies_live_dispatch() {
        ...
        // with a granted approval (the baseline invariant).
        let c = consult(false);
        assert!(c.is_some());
        if let Some(c) = c {
    ...
    fn funds_host_unreachable() {
        // no funds / chain / mainnet / local host is allowlisted.
        for h in [
    ...
    /// The allowlist excludes chain hosts (provider leg). A comprehensive
    ...
    fn redacted_consult_rejects_stored_body() {
        ...
        assert!(req.is_some());
        if let Some(req) = req {
            // A forged receipt cannot be struct-literal'd outside
    ...
    // ---- same-message egress approval ceremony -----------------------
...
// Streaming SSE codec — pure, reader-generic tests over an in-memory cursor (the
// incremental-over-a-real-socket property was proven by a real-socket probe; here we pin
...
mod stream_codec_tests {
    ...
    // The permanent twin of the real-socket probe: consume_openai_sse over a REAL TCP
