//! S1/S2 stream splitter (`StreamKind`, `S1GroundTruthRecord`,
//! `S2NarrativeRecord`).
//!
...
//! reuses the file-kind partition from `atom_record`: only the
...
/// Reward eligibility for a stream record. Only S1 may ever be
...
/// An S2 narrative record. Its reward is fixed
