//! `mnemos-l-dataset` — Stage E AtomDiet dataset builder (schema, manifest, and
//! sidecar parsers).
//!
//! # What this crate is
//!
...
//! A-D sidecars, and never emits a positive reward. It reads the per-atom
//! sidecar evidence directories read-only and produces a derived, hash-pinned
//! dataset as its output.
//!
//! # Invariants
//!
...
//! * **No canonical reinvention.** The trace/handoff types below are the
//!   canonical Stage E registry definitions; signal/reward/didactic/export types
//!   live in other modules of this crate.
#![deny(missing_docs)]
...
/// Stage E per-artifact trace stamp.
///
/// Unlike the prior stages' trace links (which *compose* the prior stage's link),
...
pub struct StageETraceLink {
    ...
    /// Stage E atom number the artifact belongs to.
...
/// drifted anchor halts the stage.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
pub struct StageEHandoffDigest {
    /// `sha256` of the Stage A atom plan.
    pub atom_plan_a_hash_32: [u8; 32],
    /// `sha256` of the Stage B atom plan.
    pub stage_b_plan_hash_32: [u8; 32],
    /// `sha256` of the Stage C atom plan.
    pub stage_c_plan_hash_32: [u8; 32],
    /// `sha256` of the Stage D atom plan.
    pub stage_d_plan_hash_32: [u8; 32],
    /// `sha256` of the Stage D definition-of-done closure.
    pub stage_d_dod_hash_32: [u8; 32],
    /// `sha256` of the 21-file sidecar contract.
