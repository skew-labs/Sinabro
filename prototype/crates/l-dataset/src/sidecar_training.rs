...
//! summary.
//!
//! Sidecar-provided labels are *suggestions* until a later stage recomputes
//! eligibility. Self-reported reward is parsed for provenance but **quarantined**
//! — its `reward_eligible` is always `false` here; eligibility is granted only
//! later, after an S1 reverify.
...
pub struct RewardLabelView {
    ...
    /// Always `true` here: self-reported reward is quarantined.
    pub quarantined: bool,
    /// Always `false` here: eligibility is recomputed later via S1 reverify.
    pub reward_eligible: bool,
}

/// Parse `reward_labels.json`. The self-reported score is read for provenance
/// only; the label is quarantined and never reward-eligible here.
