...
//! from the diff (via the shared code-diff parser); miri/fuzz/property verdicts
//! are read from explicit gate statuses, never from prose.
...
pub struct RustDeepSignal {
    ...
    /// The diff added an `unsafe` block/fn (structural detection).
    pub has_unsafe: bool,
