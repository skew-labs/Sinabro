//! criterion / perf / allocation signal (atom #354 · E.1.3).
//!
//! Performance reward requires a *baseline* (a criterion/bench gate), an
//! *environment lock* (#339 reuse: rust + deps descriptors must be real, not the
//! `"none"` sentinel), and a non-failing perf review axis (#342 reuse). A missing
