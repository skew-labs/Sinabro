//! Rust fmt/clippy/test basic signal (`RustSignal`).
//!
...
//! explicit `G-MIRI` gate only (the deep miri/fuzz/property surface is a
//! separate signal); `criterion_hash_32` is a perf anchor filled elsewhere and
//! is the `"none"` sentinel here.
...
pub struct RustSignal {
    ...
    pub tests_pass: bool,
    /// `G-MIRI` was an explicit pass (the deep miri surface is a separate signal).
    pub miri_pass: bool,
    /// Perf anchor — `sha256("none")` until a perf collector fills it.
