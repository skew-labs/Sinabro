//! Read-only signal-collector orchestration.
//!
//! This module turns the validated 21-file sidecar into typed
...
//! S1/S2 records is handled by `stream_split`; the scalar layered reward is
//! computed in a later stage. A collector never grants a positive reward.
...
/// The fixed set of signal collectors, in deterministic run order.
/// The order is part of the contract: two runs over the same corpus emit signals
/// in the same sequence.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
pub enum CollectorKind {
    /// Rust fmt/clippy/test basic signal.
    RustBasic = 1,
    /// Rust miri/fuzz/property deep signal.
    RustDeep = 2,
    ...
    Gas = 7,
    /// Sui effect-shape / Gas Station signal.
    GasStation = 8,
    /// Walrus BCS/blob-id signal.
    WalrusBlob = 9,
    ...
    Memory = 13,
    /// audit-log to repo pairing.
    AuditRepoPair = 14,
}

impl CollectorKind {
    /// All collectors in deterministic run order.
