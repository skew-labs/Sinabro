//! MURPHY failure-attempt tree schema (atom #386 · E.3.0, §4.5 `MurphyNode`,
//! `MurphyTree`; §4.2 `FailureKind` first-consumer definition).
//!
//! # Madness
//!
...
//!
//! The §4.2 `FailureKind` taxonomy is defined here verbatim from the canonical
//! registry. It was registered for Stage E Cluster 1 but never implemented (no
//! prior atom consumed it); MURPHY is its first consumer, so the type lands at
//! its first use rather than being reinvented downstream.
use crate::diet_kind::AtomDietKey;
use crate::error::{DietError, DietResult};
use std::collections::BTreeMap;

/// The verified failure surface a repair step hit (§4.2 `FailureKind`). Defined
/// here verbatim from the canonical registry as MURPHY's first consumer so that
/// MURPHY nodes and repair traces ([`super::build::RepairStep`]) share one
/// failure taxonomy.
...
/// One node in a MURPHY failure-attempt tree (§4.5 `MurphyNode`).
///
...
/// A MURPHY failure-attempt tree summary (§4.5 `MurphyTree`).
///
