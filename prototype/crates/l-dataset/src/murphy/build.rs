//! RepairTrace → MURPHY tree builder (atom #387 · E.3.1).
//!
//! Defines the §4.2 `RepairStep` / `RepairTrace` registry types (first
//! consumer, verbatim) and turns a repair trace into a [`MurphyNode`] chain plus
//! a [`MurphyTree`] summary.
//!
//! # Madness
//!
...
/// One step of a repair trace (§4.2 `RepairStep`).
...
/// A full repair trace for one source atom (§4.2 `RepairTrace`).
