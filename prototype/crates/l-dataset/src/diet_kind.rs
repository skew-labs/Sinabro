...
//! exactly 21 discriminants (`1..=21`) in the canonical contract order. An unknown
...
/// Which build stage a source atom came from.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
pub enum DietSourceStage {
    /// Stage A / Phase 0 (`phase_0/`).
    Phase0 = 1,
    /// Stage B (`stage_b/`).
    StageB = 2,
    /// Stage C (`stage_c/`).
...
impl DietSourceStage {
    ...
    /// Map a stage subdirectory name to a source stage.
...
/// One of the 21 closed sidecar file kinds (canonical contract order).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
...
impl DietFileKind {
    /// All 21 kinds in canonical contract order.
...
/// A source atom identity: `(source stage, atom number)`.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
