//! Failure-cause governance + didactic signals (atom #388 · E.3.2, §4.5
//! `FailureCause`, `OperationalMemoryClass`, `DidacticSignalClass`,
//! `DidacticConceptMap`, `DidacticSignal`).
//!
//! Research basis: master §5.2 (RewardHacking / RLVR verifier-gaming warnings;
...
//! [`DidacticConceptMap`] + [`DidacticSignal`] so Stage G can schedule deliberate
//! practice instead of random replay. A low-confidence concept label becomes a
...
/// The governed cause of a failure (§4.5 `FailureCause`).
...
/// The operational-memory class a signal belongs to (§4.5 `OperationalMemoryClass`).
...
/// The teachable class of a didactic signal (§4.5 `DidacticSignalClass`).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
...
/// A concept map for one failure/success (§4.5 `DidacticConceptMap`).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
...
/// A didactic signal for Stage G scheduling (§4.5 `DidacticSignal`).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
...
/// step. This is the failure-cause governance taxonomy (distinct from the §4.1
/// trajectory parser registry, which this WorkPackage does not own).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
...
/// Map a raw failure to its base cause (§4.5). Trajectory pathologies are layered
