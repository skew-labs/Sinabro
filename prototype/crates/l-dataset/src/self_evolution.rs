...
//! (`ContextQualitySignal` / `HarnessQualitySignal` /
//! `SelfEvolutionCandidateManifest`).
//!
//! # Design
//!
//! This stage never *runs* self-evolution and never promotes a candidate. It only
//! normalizes the upstream trajectory and measurement-only telemetry into
//! **schema-only** evidence that a later stage may evaluate **inside a
//! sandbox**. These records are not reward and not runtime policy — they only
//! record "what to experiment with later".
//!
//! Three fail-closed invariants make the schema safe to mint at this stage:
//!
...
//! * **Authority cannot widen.** A [`SelfEvolutionCandidateManifest`] minted by
//!   this stage has every promotion guard `true` and `authority_expansion_denied`
...
/// The kind of self-evolution candidate.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
#[repr(u8)]
...
/// A self-evolution candidate manifest.
///
/// Schema-only evidence describing a *possible* future experiment. Here,
...
pub struct SelfEvolutionCandidateManifest {
    ...
    /// authority. Always `true` here.
    pub authority_expansion_denied: bool,
}

impl SelfEvolutionCandidateManifest {
    /// Mint a schema-only, promotion-locked self-evolution candidate. Every
    /// promotion guard is forced `true` and `authority_expansion_denied` is
    /// `true`, regardless of caller intent.
    ///
    ...
    /// the only shape a candidate may take.
