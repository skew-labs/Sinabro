...
//! monotone schema version and deterministic digest (atom #334 · E.0.3, §4.1).
//!
...
/// (§4.1). Only `Complete` is ever reward-shaped; the others block reward.
...
impl DietCompleteness {
    ...
    /// and privacy checks (later WorkPackages) must also pass.
    pub const fn reward_blocked(self) -> bool {
        !matches!(self, Self::Complete)
    }
}

/// A content-addressed reference to one sidecar file (§4.1).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
...
/// The per-atom manifest (§4.1): a versioned, trace-stamped set of file refs
