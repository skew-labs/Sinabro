//! Audit finding parser + §4.4 `SecuritySignal` aggregate (atom #372 · E.2.1).
//!
//! A normalized audit finding carries a severity (§E.2.0 ladder) and a status
...
//! negative context, never as a positive sample. The §4.4 [`SecuritySignal`] is
...
/// The §4.4 per-atom security aggregate.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
...
/// Aggregate a set of findings into the §4.4 [`SecuritySignal`], folding in the
