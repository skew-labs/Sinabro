//! Split / held-out leakage guard (atom #395 · E.3.9, §4.3 `SplitAssignment`,
//! `TrainingSplit`).
//!
//! # Madness
//!
...
//!
//! Reuses the §E.1.18 dedup composite ([`crate::dedup::DedupKey::leakage_group`])
//! and the §4.5 MURPHY node-set digest as group keys.
use crate::dedup::DedupKey;
use crate::diet_kind::AtomDietKey;
use crate::error::{DietError, DietResult};
use std::collections::BTreeMap;

/// Which split a sample is assigned to (§4.3 `TrainingSplit`).
...
/// A split assignment for one sample (§4.3 `SplitAssignment`).
#[derive(Clone, Copy, Debug, Eq, PartialEq, Hash)]
